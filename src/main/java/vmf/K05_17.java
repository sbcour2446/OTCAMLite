 ...
 * @author hansen
 */
public class K05_17 extends VmfMessage {
    ...
    private ArrayList<OverlayInfo> infoList = new ArrayList<>();
    static String lastMsgKey = "";
    boolean _isDuplicate = false;
    String body;
    public long reportTime;
    public int reportUrn;
    long defaultDuration = 86400000L * 100L; // 100 days as a default
    Calendar dtg = Calendar.getInstance(TimeZone.getTimeZone("UTC"));
    Calendar revDtg = Calendar.getInstance(TimeZone.getTimeZone("UTC"));
    int newUpdateReplace;

    class OverlayInfo {

        String id, staffComments;
        int iconFill, iconSize, symbolDimension, entityType, entitySubType, iconStatus, addDeleteUpdateEntity, identity;
    ...
    // 
    public K05_17(Header header, VmfDataBuffer data, Calendar collectTime, String collector) {
        this.header = header;
        this.collectTime = collectTime;
        this.collector = collector;
        String key = header.getKey();
        _isDuplicate = key.equals(lastMsgKey);
        if (!header.isAck()) {
            lastMsgKey = key;
        }
        parsedOk = header.isAck() || isDuplicate() ? true : parse(data);
    }

    final public boolean isDuplicate() {
        return _isDuplicate;
    }
    ...
    final boolean parse(VmfDataBuffer data) {
        ...
        if (data.getGpi()) {
            ...
            int day_of_month = data.getInt(5);
            int hour = data.getInt(5);
            int minute = data.getInt(6);
            int second = data.getFpiInt(6);

            dtg.set(year, month - 1, day_of_month, hour, minute, second == VmfDataBuffer.NO_INT ? 0 : second);
            dtg.set(Calendar.MILLISECOND, 0);
            reportTime = dtg.getTimeInMillis();
            addBodyLine("dtg", Utility.formatTime(dtg.getTimeInMillis()));
        }//G1
        ...
        int newUpdateReplace = data.getInt(2);
...
//            int second = data.getFpiInt(6);
            ...
            revDtg.set(year, month - 1, day_of_month, hour, minute, 0);
            revDtg.set(Calendar.MILLISECOND, 0);
            addBodyLine("revDtg", Utility.formatTime(revDtg.getTimeInMillis()));
...
//todo                Overlay.endGroup(groupId, revDtg.getTimeInMillis());
            }
        }//G2
                ...
                if (data.getGpi()) {
                    ...
                    do {
                        ...
                        do {
                            ...
                            double latitude = (double)data.getInt(25) * LAT_CONVERSION_25BIT;
                            if (latitude > 90) {
                                latitude -= 90.0;
                            }

                            double longitude = (double)data.getInt(26) * LON_CONVERSION_26BIT;
                            if (longitude > 180.0) {
                                longitude -= 360.0;
                            }
...
//        System.out.printf("%7d, %s, %d\n", reportUrn, Main.SDF_LONG.format(dtg.getTime()), newUpdateReplace);
...
//                Overlay overlay = Overlay.getOverlay(info.id);
//                if (overlay != null) {
//                    overlay.setEndTime(revDtg.getTimeInMillis());
//                }
//            }
////            if (newUpdateReplace != 2) {
////                System.out.println("  REV " + Main.SDF_LONG.format(revDtg.getTime()));
////            }
//            Overlay overlay = Overlay.getOverlay(info.id);
//            if (overlay == null) {
//                overlay = Overlay.createOverlay(info.id, reportUrn, false);
//            } else {
////              System.out.println("revision");
//            }
//            Overlay.OverlayData overlayData = overlay.new OverlayData();
...
//            overlayData.positions = info.positions;
//            overlayData.textFields = info.text;
