...
public class WWIO {
    ...
    /**
     * The maximum number of characters allowed in a file path. Covers Windows,
     * Linux and OS X.
     */
    ...
    /**
     * Replaces any illegal filename characters in a specified string with an
     * underscore, "_".
     *
     ...
     * {@link java.io.File}. Returns null if the source cannot be converted to a
     * file. The source type may be one of the following:
     * <ul><li>{@link java.net.URL}</li> <li>{@link
     * java.net.URI}</li> <li>{@link java.io.File}</li> <li>{@link String}
     * containing a valid URL description, a valid URI description, or a valid
    ...
    /**
     * Converts a specified URI as to a path in the local file system. If the
     * URI cannot be converted to a file path for any reason, this returns null.
     *
    ...
    /**
     * Converts a specified URL as to a path in the local file system. If the
     * URL cannot be converted to a file path for any reason, this returns null.
     *
     ...
     * {@link java.nio.MappedByteBuffer} according to the specified mode.
     * <p>
     * If the mode is {@link java.nio.channels.FileChannel.MapMode#READ_ONLY},
     * the file is mapped in read-only mode, and any attempt to modify the
     * contents of the returned MappedByteBuffer causes a
     * {@link ReadOnlyBufferException}.
     * <p>
     * If the mode is {@link java.nio.channels.FileChannel.MapMode#READ_WRITE},
     * the file is mapped in read-write mode. Changing the contents of the
     * returned MappedByteBuffer to be eventually propagated to the file. The
     * specified file must be avialable for both reading and writing.
     * <p>
     * If the mode is {@link java.nio.channels.FileChannel.MapMode#PRIVATE}, the
     * file is mapped in copy-on-write mode. Changing the contents of the
     * returned MappedByteBuffer causes private copies of portions of the buffer
     * to be created. The specified file must be avialable for both reading and
     * writing.
     *
     * @param file the file to map.
     * @param mode the mapping mode, one of {@link java.nio.channels.FileChannel.MapMode#READ_ONLY}, {@link
     *             java.nio.channels.FileChannel.MapMode#READ_WRITE}, or
     * {@link java.nio.channels.FileChannel.MapMode#PRIVATE}.
     *
    ...
    /**
     * Maps the specified File's bytes directly into memory as a
     * {@link java.nio.MappedByteBuffer}. The file is mapped in read-only mode;
     * any attempt to modify the contents of the returned MappedByteBuffer
     * causes a {@link
    ...
    /**
     * Reads all the bytes from the specified {@link URL}, returning the bytes
     * as a non-direct {@link ByteBuffer} with the current JVM byte order.
     * Non-direct buffers are backed by JVM heap memory.
     *
     * @param url the URL to read.
     *
     * @return the bytes from the specified URL, with the current JVM byte
     * order.
     *
     * @throws IllegalArgumentException if the URL is null.
    ...
    /**
     * Reads all the bytes from the specified {@link URL}, returning the bytes
     * as a {@link ByteBuffer} with the current JVM byte order. This returns a
     * direct ByteBuffer if allocateDirect is true, and returns a non-direct
     * ByteBuffer otherwise. Direct buffers are backed by native memory, and may
     * resite outside of the normal garbage-collected heap. Non-direct buffers
     * are backed by JVM heap memory.
     *
     * @param url the URL to read.
     * @param allocateDirect true to allocate and return a direct buffer, false
     * to allocate and return a non-direct buffer.
     *
     * @return the bytes from the specified URL, with the current JVM byte
     * order.
     *
    ...
    /**
     * Reads all the bytes from the specified <code>{@link URL}</code>,
     * returning the bytes as a String. The bytes are interpreted according to
     * the specified encoding, or UTF-8 if no encoding is specified.
     *
     * @param url the URL to read.
     * @param encoding the encoding do use. If <code>null</code> is specified
     * then UTF-8 is used.
     *
     * @return the string representation of the bytes at the <code>URL</code>
     * decoded according to the specified encoding.
     *
    ...
    /**
     * Reads all the bytes from the specified {@link File}, returning the bytes
     * as a non-direct {@link ByteBuffer} with the current JVM byte order.
     * Non-direct buffers are backed by JVM heap memory.
     *
     * @param file the file to read.
     *
     * @return the bytes from the specified file, with the current JVM byte
     * order.
     *
     ...
     * direct ByteBuffer if allocateDirect is true, and returns a non-direct
     * ByteBuffer otherwise. Direct buffers are backed by native memory, and may
     * reside outside of the normal garbage-collected heap. Non-direct buffers
     * are backed by JVM heap memory.
     *
     * @param file the file to read.
     * @param allocateDirect true to allocate and return a direct buffer, false
    ...
    /**
     * Reads all the available bytes from the specified {@link InputStream},
     * returning the bytes as a non-direct {@link
     * ByteBuffer} with the current JVM byte order. Non-direct buffers are
     * backed by JVM heap memory.
     *
     * @param inputStream the stream to read.
     *
     * @return the bytes from the specified stream, with the current JVM byte
     * order.
     *
    ...
    /**
     * Reads all the available bytes from the specified {@link InputStream},
     * returning the bytes as a {@link ByteBuffer} with the current JVM byte
     * order. This returns a direct ByteBuffer if allocateDirect is true, and
     * returns a non-direct ByteBuffer otherwise. Direct buffers are backed by
     * native memory, and may reside outside of the normal garbage-collected
     ...
     * {@link java.io.InputStream}, returning the bytes as a String. The bytes
     * are interpreted according to the specified encoding, or UTF-8 if no
     * encoding is specified.
     *
     * @param stream the stream to read.
     * @param encoding the encoding do use. If null is specified then UTF-8 is
     * used.
     *
     * @return the string representation of the bytes in the stream decoded
    ...
    /**
     * Reads all the available bytes from the specified
     * {@link java.nio.channels.ReadableByteChannel}, returning the bytes as a
     * {@link ByteBuffer} with the current JVM byte order. This returns a direct
     * ByteBuffer if allocateDirect is true, and returns a non-direct ByteBuffer
     * otherwise. Direct buffers are backed by native memory, and may reside
     * outside of the normal garbage-collected heap. Non-direct buffers are
     * backed by JVM heap memory.
     *
     ...
     * to allocate and return a non-direct buffer.
     *
     * @return the bytes from the specified channel, with the current JVM byte
     * order.
     *
     ...
     * {@link java.nio.channels.ReadableByteChannel} up to the number of bytes
     * remaining in the buffer. Bytes read from the specified channel are copied
     * to the specified {@link
     * ByteBuffer}. Upon returning the specified buffer's limit is set to the
     * number of bytes read, and its position is set to zero.
     *
     ...
     * Reads all the available bytes from the specified
     * {@link java.nio.channels.ReadableByteChannel}, returning the bytes as a
     * String. The bytes are interpreted according to the specified encoding, or
     ...
     * used.
     *
     * @return the string representation of the bytes in the channel decoded
     * according to the specified encoding.
     *
     * @throws IllegalArgumentException if the <code>stream</code> is null.
     * @throws IOException if an I/O error occurs.
     ...
     * {@link java.io.Reader}, returning a the accumulated content as a String .
     *
     * @param reader the character stream to read.
     *
     * @return the string representing the accumulated content from the
     * character stream.
     *
     ...
     * This is the last file name in the path, or null if the path does not
     * contain any file names.
     *
     * @param filePath a file path String.
     *
     * @return the last name in the specified path, or null if the path does not
     * contain a name.
     *
    ...
    /**
     * Returns the file path's parent directory path, or null if the file path
     * does not have a parent.
     *
     * @param filePath a file path String.
     *
     * @return the file path's parent directory, or null if the path does not
     * have a parent.
     *
     * @throws IllegalArgumentException if the file path is null.
     */
    ...
    /**
     * Ensure that all directories leading the element at the end of a file path
     * exist. Create any nonexistent directories in the path. A directory is not
     * creared for the last element in the path; it's assumed to be a file name
     * and is ignored.
     *
     * @param path the path whose directories are vefified to exist or be
     * created. The last element of the path is ignored.
     *
     ...
     *
     * @return a file reference to the new directory, of null if a directory
    ...
    /**
     * Indicates whether a {@link File} contains content of a specified mime
     * type.
     * <p>
     * Only the filename suffix is consulted to determine the file's content
     * type.
     *
     * @param file the file to test.
     * @param mimeTypes the mime types to test for.
     *
     * @return true if the file contains a specified content type, false if the
     * file does not contain a specified content type, the specified file is
     * null, or no content types are specified.
     */
    ...
    /**
     * Returns the mime type string corresponding to the specified file suffix
     * string.
     *
    ...
    /**
     * Returns the data type constant corresponding to the specified mime type
     * string. Supported mime types are as mapped to data types as follows:
    ...
    /**
     * Creates an {@link InputStream} for the contents of a {@link String}. The
     * method creates a copy of the string's contents and passes a steam
     ...
     * @param encoding the character encoding of the string. UTF-8 is used if
     * null.
     *
     * @return an {@link InputStream} for the string's contents.
     *
    ...
    /**
     * Creates an {@link InputStream} for the contents of a {@link ByteBuffer}.
     * The method creates a copy of the buffer's contents and passes a steam
     * reference to that copy.
     *
    ...
    /**
     * Returns a new {@link java.io.BufferedInputStream} which wraps the
     * specified InputStream. If the specified InputStream is already a
     * BufferedInputStream, this returns the original InputStream cast to a
     * BufferedInputStream.
     *
    ...
    /**
     * Opens a file located via an absolute path or a path relative to the
     * classpath.
     *
     * @param fileName the path of the file to open, either absolute or relative
     * to the classpath.
     * @param c the class that will be used to find a path relative to the
     * classpath.
     *
     * @return an {@link InputStream} to the open file
     *
     * @throws IllegalArgumentException if the file name is null.
     * @throws WWRuntimeException if an exception occurs or the file can't be
     * found. The causing exception is available via this exception's
     ...
     * used.
     *
     * @return the string representation of the bytes in the buffer decoded
     * according to the specified encoding.
     *
     ...
     *
     * @param buffer the byte buffer to convert.
     * @param length the maximum number of characters to read from the buffer.
     ...
     * according to the specified encoding.
     *
     * @throws IllegalArgumentException if the buffer is null or the length is
     * less than 1.
     * @throws java.nio.charset.IllegalCharsetNameException if the specified
     * encoding name is illegal.
     * @throws java.nio.charset.UnsupportedCharsetException if no support for
     ...
     * used.
     *
     * @return the ByteBuffer representation of the string decoded according to
     ...
     * <ul> <li>{@link Reader}</li> <li>{@link
     * InputStream}</li> <li>{@link File}</li> <li>{@link URL}</li>
     ...
     *
     * @throws java.io.IOException if i/o or other errors occur trying to create
     * the reader.
     */
    ...
    /**
     * Open an {@link java.io.InputStream} from a general source. The source
     * type may be one of the following: <ul>
     * <li>{@link java.io.InputStream}</li> <li>{@link java.net.URL}</li>
     * <li>absolute {@link java.net.URI}</li>
     * <li>{@link java.io.File}</li> <li>{@link String} containing a valid URL
     ...
     *
     * @throws IllegalArgumentException if the source is null, an empty string,
    ...
    /**
     * Returns the specified input source's abstract path, or null if the input
     * source has no path. The input source may be one of the following: <ul>
     * <li>{@link String}</li> <li>{@link java.io.File}</li>
    ...
    /**
     * Creates a URL from an object. If the object does not already convert
     * directly to a URL, a URL with a specified protocol is created.
     *
     * @param path the object from which to create a URL, typically a string.
     * @param defaultProtocol if non-null, a protocol to use if the specified
     * path does not yet include a protocol.
     *
     * @return a URL for the specified object, or null if a URL could not be
     * created.
     *
     ...
     *
     * @return a URI for the specified object, or null if a URI could not be
     * created.
     *
     * @see #makeURL(String)
     * @see #makeURL(Object)
     ...
     * directory denoted by the specified file, that satisfy the specified
     * filter. If the filter is null, then all files and directories are
     * accepted. This returns null if the specified file is not a directory.
     *
     * @param file the directory who's contents to list.
     * @param filter a file filter.
     *
     * @return an array of file names denoting the files and directories in the
     * directory denoted by the specified file, or null if the specified file is
     * not a directory.
     *
     * @throws IllegalArgumentException if the file is null.
     */
    ...
    /**
     * Returns an array of relative file paths naming the files and directories
     * in the directory tree rooted by the specified file, that satisfy the
     * specified filter. The returned paths are relative to the specified file.
     * If the filter is null, then all files and directories are accepted. This
     * returns null if the specified file is not a directory.
     *
     * @param file the directory tree who's contents to list.
     * @param filter a file filter.
     *
     * @return an array of relative file paths naming the files and directories
     * in the directory tree rooted by the specified file, or null if the
     ...
     * Returns an array of relative file paths naming the files and directories
     * in the directory tree rooted by the specified file, that satisfy the
     * specified filter. If the parameter <code>recurseAfterMatch</code> is
     * false, then this ignores any directory branches beneath a matched file.
     * This has the effect of listing the top matches in the directory tree. The
     * returned paths are relative to the specified file. If the filter is null,
     * then all files and directories are accepted. This returns null if the
     * specified file is not a directory.
     *
     * @param file the directory tree who's contents to list.
     * @param filter a file filter.
     * @param recurseAfterMatch true to list the contents of directory branches
     * beneath a match; false to ignore branches beneath a match.
     *
     * @return an array of relative file paths naming the files and directories
     * in the directory tree rooted by the specified file, or null if the
     * specified file is not a directory.
     ...
     * @throws java.io.IOException is an exception occurs while skipping the
     * bytes.
     */
    ...
    /**
     * Determines whether a jar URL is a reference to a local jar file or an
     * entry in a local jar file. See {@link
     * java.net.JarURLConnection} for a description of jar URLs.
     *
     * @param jarUrl the jar URL, in the form jar:[url]!{entry}. (Omit [, ], {
     * and } in the actual URL
     *
