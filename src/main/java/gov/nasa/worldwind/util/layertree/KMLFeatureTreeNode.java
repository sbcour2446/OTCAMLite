 ...
 * <p>
 * The node's selection state is synchronized with its KML feature's visibility
 * state. <code>{@link
 * #isSelected()}</code> returns whether the node's feature is visible. Calling <code>{@link
 * #setSelected(boolean)}</code> specifies both the the node's selection state,
 * and whether its feature should be enabled for rendering and selection.
 *
 * @author dcollins
...
public class KMLFeatureTreeNode extends BasicTreeNode {
    ...
    /**
     * Indicates the KML feature this node represents. Initialized during
     * construction.
     */
    protected KMLAbstractFeature feature;

    /**
     * Creates a new <code>KMLFeatureTreeNode</code> from the specified
     * <code>feature</code>. The node's name is set to the feature's name.
     *
     * @param feature the KML feature this node represents.
     *
     * @throws IllegalArgumentException if the <code>feature</code> is
     * <code>null</code>.
     */
    ...
    /**
     * Creates a new <code>KMLFeatureTreeNode</code> from the specified
     * <code>feature</code>. This maps the feature type to a node type as
     * follows: <ul> <li>KML container to
     * <code>KMLContainerTreeNode</code>.</li> <li>KML network link to
     * <code>KMLNetworkLink</code>.</li> <li>All other KML features to
     * <code>KMLFeatureTreeNode</code>.</li> </ul>
     *
     * @param feature the KML feature to create a new
     * <code>KMLFeatureTreeNode</code> for.
     *
     ...
     *
     * @return <code>true</code> if the KML feature is enabled for rendering,
    ...
    /**
     * Specifies whether this node's feature is enabled for rendering and
     * selection. This sets both the node's selection state and its KML
     * feature's visible state.
     *
     * @param selected <code>true</code> to enable the KML feature, otherwise
     * <code>false</code>.
     */
    @Override
    ...
    /**
     * Expands paths in the specified <code>tree</code> corresponding to open
     * KML container elements. This assumes that the <code>tree</code>'s model
     * contains this node.
     * <p>
     * This node's path is expanded if the feature's <code>open</code> property
     * is <code>true</code>.
     * <p>
     * This calls <code>expandOpenContainers</code> on all children which are
     * instances of <code>KMLFeatureTreeNode</code>.
     *
    ...
    /**
     * Indicates whether the tree path for this node must expanded. This return
     * <code>true</code> if the KML feature's <code>open</code> property is
     * <code>true</code>, and <code>false</code> otherwise.
     *
     * @return <code>true</code> if the tree path for this node must be
    ...
    /**
     * Makes this node's description text from its KML feature. This uses the
     * feature's <code>KMLSnippet</code> if present, or the feature's
     * <code>description</code> if there is no snippet.
     *
    ...
    /**
     * Remove HTML tags and extra whitespace from a string. Runs of whitespace
     * will be collapsed to a single space.
     *
     * @param input Text to strip of HTML tags and extra whitespace.
     *
     * @return The input string with HTML tags removed, and runs of whitespace
     * collapsed to a single space. Returns {@code null} if {@code input} is
     * {@code null}.
     */
