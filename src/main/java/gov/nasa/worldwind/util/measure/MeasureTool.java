...
/**
 * A utility class to interactively draw shapes and measure distance and area
 * across the terrain. When armed, the class monitors mouse events to allow the
 * definition of a measure shape that can be one of {@link #SHAPE_LINE}, {@link
 * #SHAPE_PATH}, {@link #SHAPE_POLYGON}, {@link #SHAPE_CIRCLE}, {@link #SHAPE_ELLIPSE}, {@link #SHAPE_SQUARE}
 * or {@link
 * #SHAPE_QUAD}.
 * <p>
 * In order to allow user interaction with the measuring shape, a controller
 * must be set by calling {@link #setController(MeasureToolController)} with a
 * new instance of a <code>MeasureToolController</code>.</p>
 * <p>
 * The interaction sequence for drawing a shape and measuring is as follows:
 * </p>  <ul> <li>Set the measure shape.</li>
 * <li>Arm the <code>MeasureTool</code> object by calling its
 * {@link #setArmed(boolean)} method with an argument of true.</li> <li>Click on
 * the terrain to add points.</li> <li>Disarm the <code>MeasureTool</code>
 * object by calling its {@link #setArmed(boolean)} method with an argument of
 * false. </li> <li>Read the measured length or area by calling the
 * <code>MeasureTool</code> {@link #getLength()} or {@link #getArea()} method.
 * Note that the length and area can be queried at any time during or after the
 * process.</li> </ul><p>
 * While entering points or after the measure tool has been disarmed, dragging
 * the control points allow to change the initial points positions and alter the
 * measure shape.</p>
 * <p>
 * While the <code>MeasureTool</code> is armed, pressing and immediately
 * releasing mouse button one while also pressing the control key (Ctl) removes
 * the last point entered. Once the <code>MeasureTool</code> is disarmed, a
 * measure shape of type SHAPE_POLYGON can be moved by dragging a control point
 * while pressing the alt/option key.</p>
 * <p>
 * Arming and disarming the <code>MeasureTool</code> does not change the
 * contents or attributes of the measure tool's layer. Note that the measure
 * tool will NOT disarm itself after the second point of a line or a regular
 * shape has been entered - the MeasureToolController has that
 * responsibility.</p>
 * <p>
 ...
 * using {@link #setPositions(java.util.ArrayList)}. If the provided list
 * contains two positions, the measure shape will be set to {@link #SHAPE_LINE}.
 * If more then two positions are provided, the measure shape will be set to
 * {@link #SHAPE_PATH} if the last position differs from the first (open path),
 * or {@link
 * #SHAPE_POLYGON} if the path is closed.</p>
 * <p>
 * The application can also set the measure shape to a predefined regular shape
 * by calling
 * {@link #setMeasureShapeType(String, Position, double, double, Angle)},
 * providing a shape type (one of
 * {@link #SHAPE_CIRCLE}, {@link #SHAPE_ELLIPSE}, {@link #SHAPE_SQUARE} or
 * {@link #SHAPE_QUAD}), a center position, a width, a height (in meters) and a
 * heading angle.</p>
 * <p>
 * Finally, the application can use an existing <code>Path</code> or
 * <code>SurfaceShape</code> by using {@link #setMeasureShape(Path)} or {@link
 * #setMeasureShape(SurfaceShape)}. The surface shape can be one of
 * <code>SurfacePolyline</code>, <code>SurfacePolygon</code>,
 * <code>SurfaceQuad</code>, <code>SurfaceSquare</code>,
 * <code>SurfaceEllipse</code> or <code>SurfaceCircle</code>.
 * <p>
 * <b>Measuring</b></p>
 * <p>
 * The application can read the measured length or area by calling the
 * <code>MeasureTool</code> {@link #getLength()} or {@link #getArea()} method.
 * These methods will return -1 when no value is available.</p>
 * <p>
 ...
 * <p>
 * The measurements are displayed in units specified in the measure tool's
 * {@link UnitsFormat} object. Access to the units format is via the method {@link
 * #getUnitsFormat()}.
 * <p>
 * <b>Events</b></p>
 * <p>
 * The <code>MeasureTool</code> will send events on several occasions: when the
 * position list has changed -
 * {@link #EVENT_POSITION_ADD}, {@link #EVENT_POSITION_REMOVE} or {@link
 * #EVENT_POSITION_REPLACE}, when metrics has changed
 * {@link #EVENT_METRIC_CHANGED} or when the tool is armed or disarmed
 * {@link #EVENT_ARMED}.</p>
 * <p>
 * Events will also be fired at the start and end of a rubber band operation
 * during shape creation: {@link #EVENT_RUBBERBAND_START} and
 * {@link #EVENT_RUBBERBAND_STOP}.</p>
 * <p>
 * See {@link
 * gov.nasa.worldwindx.examples.MeasureToolPanel} for some events usage.</p>
 * <p>
 * Several instances of this class can be used simultaneously. However, each
 * instance should be disposed of after usage by calling the {@link #dispose()}
 * method.</p>
 *
...
public class MeasureTool extends AVListImpl implements Disposable {
    ...
    /**
     * Construct a new measure tool drawing events from the specified
     * <code>WorldWindow</code> and using the given
     * <code>RenderableLayer</code>.
     *
     * @param wwd the <code>WorldWindow</code> to draw events from.
     * @param applicationLayer the <code>RenderableLayer</code> to use. May be
     * null. If specified, the caller is responsible for adding the layer to the
     * model and enabling it.
     */
    ...
    /**
     * Return the {@link UnitsFormat} instance governing the measurement value
    ...
    /**
     * Set the measure tool's @{link UnitsFormat} instance that governs
     * measurement value display units and format.
     *
     ...
     * monitors user input and builds the shape in response to user actions.
     * When disarmed, the controller ignores all user input.
     *
    ...
    /**
     * Set the measure shape to an arbitrary list of positions. If the provided
     * list contains two positions, the measure shape will be set to
     * {@link #SHAPE_LINE}. If more then two positions are provided, the measure
     * shape will be set to {@link #SHAPE_PATH} if the last position differs
     * from the first (open path), or {@link #SHAPE_POLYGON} if the path is
     * closed.
     *
     ...
     *
     * @return the list of control points associated with the current measure
     * shape.
     */
     ...
     * {@link #SHAPE_CIRCLE}, {@link #SHAPE_ELLIPSE}, {@link #SHAPE_SQUARE} or
     * {@link #SHAPE_QUAD}. This will reset the measure tool and clear the
     * current measure shape.
     *
     ...
     * @param orientation the shape orientation or azimuth angle - clockwise
     * from north.
     */
    ...
    /**
     * Set the measure shape to an existing <code>SurfaceShape</code>. Can be
     * one of <code>SurfacePolygon</code>, <code>SurfaceQuad</code>,
     * <code>SurfaceSquare</code>, <code>SurfaceEllipse</code> or
    ...
    // *** Editing shapes ***
    /**
     * Add a control point to the current measure shape at the current
     * WorldWindow position.
     *
     * @return The position of the new control point, or null if the control
    ...
    /**
     * Update the current measure shape according to a given control point
     * position.
     *
    ...
    /**
     * Update the current measure shape according to a given control point
     * position and shape edition mode.
     *
     * @param point one of the shape control points.
     ...
     * azimuth <code>Angle</code> for a given distance <code>Angle</code>.
     *
