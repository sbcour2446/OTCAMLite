...
/**
 * Utility class to compute approximations of projected and surface (terrain
 * following) area on a globe.
 *
 * <p>
 * To properly compute surface area the measurer must be provided with a list of
 * positions that describe a closed path - one which last position is equal to
 * the first.</p>
 *
 * <p>
 * Segments which are longer then the current maxSegmentLength will be
 * subdivided along lines following the current pathType -
 * {@link gov.nasa.worldwind.render.Polyline#LINEAR}, {@link gov.nasa.worldwind.render.Polyline#RHUMB_LINE}
 * or {@link gov.nasa.worldwind.render.Polyline#GREAT_CIRCLE}.</p>
 *
 * <p>
 * Projected or non terrain following area is computed in a sinusoidal
 * projection which is equivalent or equal area. Surface or terrain following
 * area is approximated by sampling the path bounding sector with square cells
 * along a grid. Cells which center is inside the path have their area estimated
 * and summed according to the overall slope at the cell south-west corner.</p>
 *
...
public class AreaMeasurer extends LengthMeasurer implements MeasurableArea {
    ...
    /**
     * Get the sampling grid maximum number of rows or columns for terrain
     * following surface area approximation.
     *
    ...
    /**
     * Set the sampling grid maximum number of rows or columns for terrain
     * following surface area approximation.
     *
     ...
     *
     * <p>
     * If the measurer is set to follow terrain, the computed area will account
     * for terrain deformations. Otherwise the area is that of the path once
     * projected at sea level - elevation zero.</p>
     *
     * @param globe the globe to draw terrain information from.
     * @return the current shape surface area or -1 if the position list does
     * not describe a closed path or is too short.
