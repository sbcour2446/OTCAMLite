 ...
 * <p>
 * The measurer must be provided a list of at least two positions to be able to
 * compute a distance.</p>
 * <p>
 * Segments which are longer then the current maxSegmentLength will be
 * subdivided along lines following the current pathType - AVKey.LINEAR,
 * AVKey.RHUMB_LINE or AVKey.GREAT_CIRCLE.</p>
 * <p>
 * If the measurer is set to follow terrain, the computed length will account
 * for terrain deformations as if someone was walking along that path. Otherwise
 * the length is the sum of the cartesian distance between the positions.</p>
 * <p>
 * When following terrain the measurer will sample terrain elevations at regular
 * intervals along the path. The minimum number of samples used for the whole
 * length can be set with setLengthTerrainSamplingSteps(). However, the minimum
...
public class LengthMeasurer implements MeasurableLength {
     ...
     *
     * @param followTerrain set to true if measurements should account for
    ...
    /**
     * Get the type of path used when subdividing long segments, one of
     * Polyline.GREAT_CIRCLE, which draws segments as a great circle,
     * Polyline.LINEAR, which determines the intermediate positions between
     * segments by interpolating the segment endpoints, or Polyline.RHUMB_LINE,
     * which draws segments as a line of constant heading.
     *
    ...
    /**
     * Get the type of path used when subdividing long segments, one of
     * AVKey.GREAT_CIRCLE, which draws segments as a great circle, AVKey.LINEAR,
     * which determines the intermediate positions between segments by
     * interpolating the segment endpoints, or AVKey.RHUMB_LINE, which draws
     * segments as a line of constant heading.
     *
     * @return The path type
     */
    ...
    /**
     * Sets the type of path used when subdividing long segments, one of
     * AVKey.GREAT_CIRCLE, which draws segments as a great circle, AVKey.LINEAR,
     * which determines the intermediate positions between segments by
     * interpolating the segment endpoints, or AVKey.RHUMB_LINE, which draws
     * segments as a line of constant heading.
    ...
    /**
     * Get the maximum length a segment can have before being subdivided along a
     * line following the current pathType.
     *
    ...
    /**
     * Set the maximum length a segment can have before being subdivided along a
     * line following the current pathType.
     *
     * @param length the maximum length a segment can have before being
     * subdivided.
     */
    ...
    /**
     * Returns true if the current position list describe a closed path - one
     * which last position is equal to the first.
     *
    ...
    /**
     * Get the number of terrain elevation samples used along the path to
     * approximate it's terrain following length.
     *
    ...
    /**
     * Set the number of terrain elevation samples to be used along the path to
     * approximate it's terrain following length.
     *
     ...
     * <p>
     * If the measurer is set to follow terrain, the computed length will
     * account for terrain deformations as if someone was walking along that
     * path. Otherwise the length is the sum of the cartesian distance between
     * each positions.</p>
     *
    ...
    /**
     * Subdivide a list of positions so that no segment is longer then the
     * provided maxLength.
     * <p>
     * If needed, new intermediate positions will be created along lines that
     * follow the given PathType - one of AVKey.LINEAR, AVKey.RHUMB_LINE or
     * AVKey.GREAT_CIRCLE. All position elevations will be either at the terrain
     * surface if followTerrain is true, or interpolated according to the
     * original elevations.</p>
     *
     ...
     * @param followTerrain true if the positions should be on the terrain
     * surface.
     * @param avkeyPathType the type of path to use in between two positions.
     *
     * @return a list of positions with no segment longer then maxLength and
     * elevations following terrain or not.
     */
    ...
    /**
     * Subdivide a list of positions so that no segment is longer then the
     * provided maxLength. Only the positions between start and start + count -
     * 1 will be processed.
     * <p>
     * If needed, new intermediate positions will be created along lines that
     * follow the given pathType - one of AVKey.LINEAR, AVKey.RHUMB_LINE or
     ...
     * @param count how many positions from the original list have to be
