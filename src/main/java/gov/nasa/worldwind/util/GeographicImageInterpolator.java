 ...
 * {@link gov.nasa.worldwind.util.ImageInterpolator} to correctly map from
 * geographic coordinates to image coordinates. Unlike its superclass, which
 * works in Cartesian coordinates, GeographicImageInterpolator handles the
 * singularities of geographic coordinates. For example,
 * GeographicImageInterpolator can map images which cross the international
 * dateline.
 *
 * @author dcollins
 * @version $Id: GeographicImageInterpolator.java 1171 2013-02-11 21:45:02Z
 * dcollins $
 */
public class GeographicImageInterpolator extends ImageInterpolator {
     ...
     * {@link gov.nasa.worldwind.util.ImageInterpolator.Cell} to correctly
     * handle image cells which have geographic coordinates. Unlike its
     * superclass, which works in Cartesian coordinates, GeographicCell handles
     * the singularities of geographic coordinates, such as image cells which
     * cross the international dateline.
     */
    protected static class GeographicCell extends Cell {

        /**
         * Denotes if the pixels in this geographic image cell crosses the
         ...
         * {@link gov.nasa.worldwind.util.GeographicImageInterpolator.GeographicCell}.
         *
         * @param m0 the cell's left image coordinate.
         * @param m1 the cell's right image coordinate.
         * @param n0 the cell's bottom image coordinate.
        ...
        /**
         * Returns true if this image cell crosses the international dateline,
         * and false otherwise.
         *
         * @return true if this cell crosses the international dateline; false
         * otherwise.
         */
        ...
        /**
         * Overridden to correctly compute intersection for cells which cross
         * the international dateline. If the specified cell does not cross the
         * dateline, this invokes the superclass' functionality.
         *
         ...
         *
         * @return true if the (x, y) point intersects this cell; false
         * otherwise.
         */
        @Override
        ...
        /**
         * Overridden to correctly compute the extremes for leaf cells which
         * cross the international dateline. If this cell does not cross the
         * dateline, this invokes the superclass' functionality.
         *
         * @param dim the image's dimensions.
         * @param xs the x-coordinates of the image's pixels. Must contain at
         * least <code>gridSize.width *
         *            gridSize.height</code> elements.
         * @param ys the y-coordinates of the image's pixels. Must contain at
         * least <code>gridSize.width *
         *            gridSize.height</code> elements.
         */
        @Override
        ...
        /**
         * Overridden to correctly compute the extremes for parent cells who's
         * children cross the international dateline. If the this cell does not
         * cross the dateline, this invokes the superclass' functionality.
         */
        @Override
        ...
        /**
         * Returns true if a line segment from the first pixel in this cell to
         * any other pixel in this cell crosses the international dateline, and
         * false otherwise.
         *
         * @param dim the image's dimensions.
         * @param longitudes the longitude coordinates of the image's pixels in
         * degrees. Must contain at least <code>dim.width * dim.height</code>
         * elements.
         *
         * @return true if this image cell's crosses the international dateline;
         * false otherwise.
         */
        ...
        /**
         * Returns true if any of this image cell's children cross the
         * international dateline, and false otherwise. Returns false if this
         * image cell has no children.
         *
         * @return true if any children cross the international dateline; false
         * otherwise.
         */
    ...
    /**
     * Creates a new GeographicImageInterpolator, initializing this
     * interpolator's internal image cell tree with root cell dimensions of
     * <code>(gridSize.width * gridSize.height)</code> and with the specified
     * <code>depth</code>.
     *
     ...
     * @param cellSize the size of a leaf cell in this interpolator's image cell
     * tree, in pixels.
     *
     * @throws IllegalStateException if any of the the gridSize, x-coordinates,
     * or y-coordinates are null, if either the x-coordinates or y-coordinates
     * contain less than <code>gridSize.width *
     *                               gridSize.height</code> elements, if the depth is less than zero, or if
     * the cell size is less than one.
     */
    ...
    /**
     * Returns the sector containing the image's geographic coordinates. This
     * returns a sector which spans the longitude range [-180, 180] if the image
    ...
    /**
     * Overridden to correctly compute bilinear interpolation coordinates for
     * image cells which cross the international dateline. If the specified cell
     * does not cross the dateline, this invokes the superclass' functionality.
     * This returns null if the specified (x, y) point does not intersect the
     * cell.
     *
     ...
     *
     * @return the bilinear coordinates of the specified (x, y) point in the
     * specified cell, or null if the point does not intersect the cell.
     */
    @Override
