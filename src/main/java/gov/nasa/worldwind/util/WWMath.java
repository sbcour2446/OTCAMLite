...
public class WWMath {
     ...
     * the given value.
     *
     * @param reference the reference value. The power of 2 returned is greater
     ...
     * the given value.
     *
     * @param reference the reference value. The power of 2 returned is less
     * than or equal to this value.
     *
     ...
     * the value b^i, where b is the base and i is the element number (0, 1,
     * etc.).
     */
    ...
    /**
     * Returns a number between 0.0 and 1.0 indicating whether a specified
     * floating point value is before, between or after the specified min and
     * max. Returns a linear interpolation of min and max when the value is
     * between the two.
     * <p>
     * The returned number is undefined if min &gt; max. Otherwise, the returned
     * number is equivalent to the following:
     * <ul> <li>0.0 - If value &lt; min</li> <li>1.0 - If value &gt; max</li>
     * <li>Linear interpolation of min and max - If min &lt;= value &lt;=
     * max</li> </ul>
     *
     ...
     * Returns a number between 0.0 and 1.0 indicating whether a specified
     * floating point value is before, between or after the specified min and
     * max. Returns a smooth interpolation of min and max when the value is
     * between the two.
     * <p>
     * This method's smooth interpolation is similar to the interpolation
     * performed by {@link #stepValue(double, double,
     * double)}, except that the first derivative of the returned number
     * approaches zero as the value approaches the minimum or maximum. This
     * causes the returned number to ease-in and ease-out as the value travels
     * between the minimum and maximum.
     * <p>
     * The returned number is undefined if min &gt; max. Otherwise, the returned
     * number is equivalent to the following:
     * <ul> <li>0.0 - If value &lt; min</li> <li>1.0 - If value &gt; max</li>
     * <li>Smooth interpolation of min and max - If min &lt;= value &lt;=
     * max</li> </ul>
     *
     * @param value the value to compare to the minimum and maximum.
     * @param min the minimum value.
     ...
     * function {@link #mix(double,
     * double, double)}, when <code>a</code> is <code>0.5</code> this returns
     * <code>(x + y)/2</code>. But unlike the linear function, the hermite
     * function's slope gradually increases when <code>a</code> is near 0, then
     * gradually decreases when <code>a</code> is near 1. This is a useful
     * property where a more gradual transition from <code>x</code> to
     * <code>y</code> is desired.
     *
     * @param a the interpolation factor.
     * @param x the first value.
     * @param y the second value.
    ...
    /**
     * Returns the distance in model coordinates from the
     * {@link gov.nasa.worldwind.View} eye point to the specified
     * {@link gov.nasa.worldwind.geom.Extent}. If the View eye point is inside
     * the extent, this returns 0.
     *
     * @param dc the {@link gov.nasa.worldwind.render.DrawContext} which the
     * View eye point is obtained from.
     * @param extent the extent to compute the distance from.
     *
     * @return the distance from the View eye point to the extent, in model
     * coordinates.
     *
     * @throws IllegalArgumentException if either the DrawContext or the extent
     * is null.
     */
    ...
    /**
     * Returns the size in window coordinates of the specified
     * {@link gov.nasa.worldwind.geom.Extent} from the current
     * {@link gov.nasa.worldwind.View}. The returned size is an estimate of the
     * Extent's diameter in window coordinates.
     *
     ...
     *
     * @return size of the specified Extent from the specified View, in window
     * coordinates (screen pixels).
     *
     * @throws IllegalArgumentException if either the DrawContext or the extent
     * is null.
     */
    ...
    /**
     * Computes the area in square pixels of a sphere after it is projected into
     * the specified <code>view's</code> viewport. The returned value is the
     * screen area that the sphere covers in the infinite plane defined by the
     * <code>view's</code> viewport. This area is not limited to the size of the
     * <code>view's</code> viewport, and portions of the sphere are not clipped
     * by the <code>view's</code> frustum.
     * <p>
     * This returns zero if the specified <code>radius</code> is zero.
     *
     * @param view the <code>View</code> for which to compute a projected screen
     * area.
     ...
     *
     * @throws IllegalArgumentException if the <code>view</code> is
     * <code>null</code>, if <code>center</code> is <code>null</code>, or if
     * <code>radius</code> is less than zero.
     */
     ...
     * The normal vector is computed from the first three non-colinear points in
     * the buffer.
     *
     * @param coords the coordinates. This method returns null if this argument
     * is null.
     * @param stride the number of floats between successive points. 0 indicates
     * that the points are arranged one immediately after the other.
     *
     * @return the computed unit-length normal vector, or null if a normal
     * vector could not be computed.
     */
    ...
    /**
     * Computes a unit-length normal vector for an array of coordinates. The
     * normal vector is computed from the first three non-colinear points in the
     * array.
     *
     * @param coords the coordinates. This method returns null if this argument
     * is null.
     ...
     * that the points are arranged one immediately after the other.
     *
     * @return an array of three points, or null if three non-colinear points
     * could not be found.
     */
     ...
     *
     * @throws IllegalArgumentException if any of the specified vertices are
     * null.
     */
    ...
    /**
     * Returns the winding order of the polygon described by the specified
     * locations, with respect to an axis perpendicular to the (lat, lon)
     * coordinates, and pointing in the direction of "positive elevation".
     *
     * @param locations the locations defining the geographic polygon.
     *
     * @return {@link AVKey#CLOCKWISE} if the polygon has clockwise winding
     * order, and {@link AVKey#COUNTER_CLOCKWISE} otherwise.
     */
    ...
    /**
     * Returns the winding order of the 2D polygon described by the specified
     * (x, y) points (z and w coordinates are ignored), with respect to the
     * positive z axis.
     *
     * @param points the (x, y) points which define the 2D polygon.
     *
     * @return AVKey.CLOCKWISE if the polygon has clockwise winding order about
     * the positive z axis, and AVKey.COUNTER_CLOCKWISE otherwise.
     */
    ...
    /**
     * Returns an array of normalized vectors defining the three principal axes
     * of the x-, y-, and z-coordinates from the specified points Iterable,
     * sorted from the most prominent axis to the least prominent. This returns
     * null if the points Iterable is empty, or if all of the points are null.
     * The returned array contains three normalized orthogonal vectors defining
     * a coordinate system which best fits the distribution of the points
     * Iterable about its arithmetic mean.
     *
     * @param points the Iterable of points for which to compute the principal
     * axes.
     *
     * @return the normalized principal axes of the points Iterable, sorted from
     * the most prominent axis to the least prominent.
     *
    ...
    /**
     * Returns an array of normalized vectors defining the three principal axes
     * of the x-, y-, and z-coordinates from the specified buffer of points,
     * sorted from the most prominent axis to the least prominent. This returns
     * null if the buffer is empty. The returned array contains three normalized
     * orthogonal vectors defining a coordinate system which best fits the
     * distribution of the points about its arithmetic mean.
     * <p>
     * The buffer must contain XYZ coordinate tuples which are either tightly
     * packed or offset by the specified stride. The stride specifies the number
     * of buffer elements between the first coordinate of consecutive tuples.
     * For example, a stride of 3 specifies that each tuple is tightly packed as
     * XYZXYZXYZ, whereas a stride of 5 specifies that there are two elements
     * between each tuple as XYZabXYZab (the elements "a" and "b" are ignored).
     * The stride must be at least 3. If the buffer's length is not evenly
     * divisible into stride-sized tuples, this ignores the remaining elements
     * that follow the last complete tuple.
     *
     * @param coordinates the buffer containing the point coordinates for which
     * to compute the principal axes.
     * @param stride the number of elements between the first coordinate of
     * consecutive points. If stride is 3, this interprets the buffer has having
     * tightly packed XYZ coordinate tuples.
     *
     * @return the normalized principal axes of the points, sorted from the most
     * prominent axis to the least prominent.
     *
     * @throws IllegalArgumentException if the buffer is null, or if the stride
     * is less than three.
     */
    ...
    /**
     * Returns whether the geographic polygon described by the specified
     * locations defines a closed loop. If the iterable holds fewer than two
     * points, this always returns false. Therefore a polygon consisting of a
     * single point and the empty polygon are not considered closed loops.
     *
    ...
    /**
     * Returns whether the 2D polygon described by the specified (x, y) points
     * defines a closed loop (z and w coordinates are ignored). If the iterable
     * holds fewer than two points, this always returns false. Therefore a
     * polygon consisting of a single point and the empty polygon are not
     * considered closed loops.
     *
     * @param points the (x, y) points which define the 2D polygon.
     *
     * @return true if the polygon defines a closed loop, and false otherwise.
    ...
    /**
     * Determines whether a {@link LatLon} location is located inside a given
     * polygon.
     *
     * @param location the location
     * @param locations the list of positions describing the polygon. Last one
...
//                result = !result;
    ...
    /**
     * Computes the center, axis, and radius of the circle that circumscribes
     * the specified points. If the points are oriented in a clockwise winding
     * order, the circle's axis will point toward the viewer. Otherwise the axis
     * will point away from the viewer. Values are returned in the first element
     * of centerOut, axisOut, and radiusOut. The caller must provide a
     * preallocted arrays of length one or greater for each of these values.
     *
     ...
     *
     * @throws IllegalArgumentException if <code>p0</code>, <code>p1</code>, or
     * <code>p2</code> is null
     */
    ...
    /**
     * Intersect a line with a convex polytope and return the intersection
     * points.
     * <p>
     * See "3-D Computer Graphics" by Samuel R. Buss, 2005, Section X.1.4.
     *
     * @param line the line to intersect with the polytope.
     * @param planes the planes defining the polytope. Each plane's normal must
     * point away from the the polytope, i.e. each plane's positive halfspace is
     * outside the polytope. (Note: This is the opposite convention from that of
     * a view frustum.)
     *
     * @return the points of intersection, or null if the line does not
     * intersect the polytope. Two points are returned if the line both enters
     * and exits the polytope. One point is retured if the line origin is within
     * the polytope.
     *
     * @throws IllegalArgumentException if the line is null or ill-formed, the
     * planes array is null or there are fewer than three planes.
     */
     ...
     * the interior of a vertex grid as a triangle strip. The returned buffer
     * may be used as the source <code>buffer</code> in a call to {@link
     * com.jogamp.opengl.GL2#glDrawElements(int, int, int, java.nio.Buffer)},
     * where <code>mode</code> is {@link
     * com.jogamp.opengl.GL#GL_TRIANGLE_STRIP}, <code>count</code> is the number
     * of elements remaining in the buffer, and <code>type</code> is
     * {@link com.jogamp.opengl.GL#GL_UNSIGNED_INT}.
     * <p>
     ...
     *
     * @return an index buffer that tessellate's the grid interior as a triangle
     * strip.
     *
    ...
    /**
     * Computes an index buffer in the system native byte order that tessellates
     * the outline of a vertex grid as a line strip. The returned buffer may be
     * used as the source <code>buffer</code> in a call to {@link
     * com.jogamp.opengl.GL2#glDrawElements(int, int, int, java.nio.Buffer)},
     * where <code>mode</code> is {@link
     * com.jogamp.opengl.GL#GL_LINE_STRIP}, <code>count</code> is the number of
     * elements remaining in the buffer, and <code>type</code> is
     * {@link com.jogamp.opengl.GL#GL_UNSIGNED_INT}.
     * <p>
     * For details the drawing OpenGL primitives, see
     * <a href="http://www.glprogramming.com/red/chapter02.html#name14">http://www.glprogramming.com/red/chapter02.html#name14</a>.
     ...
     * @param height the patch height, in vertices.
     *
     * @return an index buffer that tessellates the grid outline as a line
     * strip.
     *
     * @throws IllegalArgumentException if either the width or height are less
     * than or equal to zero.
    ...
    /**
     * Computes per-vertex normals of an indexed triangle strip, storing the
     * normal coordinates in the specified normal buffer. If the normal buffer
     * is null, this creates a new buffer in the system native byte order with
     * the capacity to store the same number of vertices as the vertex buffer.
     * The vertex buffer is assumed to contain tightly packed 3-coordinate
     * tuples. The 3-coordinate normal for each vertex is stored in the normal
     * buffer at the same position each vertex appears in the vertex buffer.
     * <p>
     * For details the drawing OpenGL primitives, see
     * <a href="http://www.glprogramming.com/red/chapter02.html#name14">http://www.glprogramming.com/red/chapter02.html#name14</a>.
     *
     * @param indices indices into the vertex buffer defining a triangle strip.
     * @param vertices buffer of vertex coordinate tuples used to compute the
     * normal coordinates.
     * @param normals buffer of normal coordinate tuples that receives the
     * normal coordinates, or null to create a new buffer to hold the normal
     * coordinates.
     *
     * @return buffer of normal coordinate tuples.
     *
     * @throws IllegalArgumentException if either the index buffer or the vertex
     * buffer is null.
     */
    ...
    /**
     * Computes a triangle normal given the starting position of three tuples in
     * the specified vertex buffer, and stores adds the result to three tuples
     * in the specified normal buffer with the same positions.
     *
    ...
    /**
     * Normalizes the 3-coordinate tuple starting at the buffer's position, then
     * advances the buffer's position to the end of the tuple.
     *
    ...
    /**
     * Computes a line between two integer-coordinate points using Bresenham's
     * algorithm.
     *
     * @param x0 the x coordinate of the first point.
     * @param y0 the y coordinate of the first point, relative to an upper-left
     * origin.
     * @param x1 the x coordinate of the second point.
     * @param y1 the y coordinate of the second point, relative to an upper-left
     * origin.
     *
     ...
     *
     * @param controlPositions List of positions along the control line. Must be
     * greater than 1.
     ...
     * @param distance Distance from the center line to the left and right
     * lines.
     * @param globe Globe used to compute positions.
     *
     * @throws IllegalArgumentException if any of the lists are null, the number
     * of control positions is less than 2, or the globe is null.
     */
     ...
     * @param prev Previous point on the line. May be null if {@code next} is
     * non-null.
     * @param next Next point on the line. May be null if {@code prev} is
     * non-null.
     ...
     * @param globe Globe used to compute positions.
     * @param previousOffset Offset vector from a previous call to this method.
     * May be null.
     *
     * @return Offset vector that should be passed back to this method on the
     * next call for a list of positions. (Used to generate parallel points when
     * a position list contains sequential co-located positions.)
     *
     * @throws IllegalArgumentException if the necessary point, previous or next
     * references are null, either the left or right position list is null, or
     * the globe is null.
     */
