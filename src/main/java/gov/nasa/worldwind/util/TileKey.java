...
public class TileKey implements Comparable<TileKey> {
     ...
     * @throws IllegalArgumentException if <code>level</code>, <code>row</code>
     * or <code>column</code> is negative or if <code>cacheName</code> is null
     * or empty
     */
...
//        }
    ...
    /**
     * Compare two tile keys. Keys are ordered based on level, row, and column
     * (in that order).
     *
     * @param key Key to compare with.
     *
     * @return 0 if the keys are equal. 1 if this key &gt; {@code key}. -1 if
     * this key &lt; {@code key}.
     *
