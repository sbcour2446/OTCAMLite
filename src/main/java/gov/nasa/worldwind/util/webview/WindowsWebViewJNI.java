...
/**
 * JNI bindings for the Windows WebView library. This library provides functions
 * for creating and destroying native WebViews, sending user input to a WebView,
 * and adding listeners to a WebView.
 * <h3>Message loops</h3>
 * <p>
 * WebViews created by this library must be managed by a message loop in native
 * code. This class provides methods for creating and running a native message
 * loop using a Java thread. Each WebView must be associated with one message
 * loop. Each message loop can handle any number of WebViews.
 * <p>
 * To create a WebView message loop: <ol> <li> Create a new Java thread to run
 * the message loop.</li> <li> Call {@link
 * #newMessageLoop()} from the message loop thread.</li> <li> Call
 * {@link #runMessageLoop(long)} from the message loop thread. This enters a
 * blocking loop in native code. It will not return until
 * {@link #releaseMessageLoop(long)} is called by another thread.</li> </ol>
 * <p>
...
public class WindowsWebViewJNI {
    ...
    /**
     * Initialize the native library. This method must be called before any of
    ...
    /**
     * Run a native message loop. This method call does not return until the
     * message loop is terminated.
     *
     * @param messageLoop Identifier for the message loop. This value is
     * returned by {@link #newMessageLoop()}.
     */
    ...
    /**
     * Set a WebViewWindow to be active or inactive. The window only handles
     * simulated input when it is active.
     *
     * @param webViewWindowPtr window to set active or inactive.
     * @param active {@code true} if the window is being activated.
     * {@code false} if the window is being deactivated.
     */
    ...
    /**
     * Create a new notification adapter to bridge changes in the native WebView
     * to PropertyChangeEvents.
     *
     * @param listener listener that will receive PropertyChangeEvents caused by
     * changes in the native WebView
     *
     * @return identifier for the new notification adapter, or zero if creation
     * fails. The notification adapter must be freed by
     * {@code releaseComObject}.
     *
    ...
    /**
     * Set the HTML content of a WebView, with a {@link WebResourceResolver} to
     * resolve local references.
     *
     * @param webViewWindowPtr WebView window to set content of
     * @param htmlString new HTML content
     * @param resolver WebResourceResolver that will resolve local references in
     * the HTML content.
     */
     ...
     * @param colorString Color expressed as a string. Color strings must follow
     * the format defined by the HTML specification.
     */
    ...
    /**
     * Send an input event to a WebView window. The AWT InputEvent will
     * translated into native Windows input messages.
     *
     ...
     *
     * @return The time (in milliseconds since the system started) at which the
     * WebView rendered content last changed.
     */
    public static native long getUpdateTime(long webViewWindowPtr);

    /**
     * Add an observer that will be notified when the rendered contents of the
     * WebView change (due to animation, user input, etc)
     *
     ...
     *
     * @param webViewWindowPtr pointer to native WebView from which to remove
     * observer
     ...
     * @param webViewWindowPtr pointer to native WebView.
     *
     * @return The URL of the current page, or null if the WebView content has
     * been loaded from a String.
     */
     ...
     * Has no effect if there is no previous page.
     *
     * @param webViewWindowPtr pointer to native WebView.
     */
    ...
    /**
     * Navigate a WebView window to the next page in the navigation history. Has
     * no effect if there is no next page.
     *
