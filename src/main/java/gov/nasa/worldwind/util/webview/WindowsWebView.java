...
/**
 * {@link WebView} implementation for Windows. This implementation uses the
 * Window's native web browser control and the MSHTML library to render a web
 * page and create an OpenGL texture from the web browser window.
 * <h2>Limits on the number of WebViews that can be created</h2> WindowsWebView
 * creates a hidden native window. Creating the native window can fail if the
 * process runs out of Windows user object handles. Other GUI elements in an
 * application also consume these handles, so it is difficult to put a firm
 * limit on how many WebViews can be created. An application that creates only
 * WebViews and no other windows can create about 1500 WebViews before running
 * out of handles. See
 * <a href="http://msdn.microsoft.com/en-us/library/ms725486%28v=vs.85%29.aspx">MSDN</a>
 * for more information on User Objects and operating system limits.
 *
 * @author pabercrombie
...
public class WindowsWebView extends AbstractWebView {
    ...
    /**
     * Thread to run web view message loop. At most one message loop thread is
     * running at any time, and it is shared by all WebView instances.
     */
    ...
    /**
     * The address of the native WindowsWebView object. Initialized during
     * construction.
     */
    protected long webViewWindowPtr;
    /**
     * The address of the native NotificationAdapter object. Initialized during
     * construction.
     */
    protected long observerPtr;

    /**
     * Count of the number of active (non-disposed) WebView instances. The
     * WebView UI thread is started when a WebView is created (if it's not
     * already running), and terminated when there are no active instances.
     */
     ...
     *
     * @throws UnsupportedOperationException if this class is instantiated on a
     * non-Windows operating system.
     * @throws WWRuntimeException if creating the native web browser window
     * fails for any reason. For example, because the process has run out of
     * User Object handles (see documentation <a href="#limits">above</a>).
     */
    ...
    /**
     * This method is called by the constructor if an exception is thrown
     * creating the WebView. It gives the WebView a change to cleanup static
     * state that may have been set during the failed WebView construction.
     */
    ...
    /**
     * Ensure that the message loop thread is running. This method simply
     * returns if the thread is already running. It creates a new thread if the
     * message thread is not running. This method does not return until the
     * message loop is initialized and ready for use.
     */
    ...
    /**
     * Terminate the message loop thread if there are no active (non-disposed)
     * WebView instances. Has no effect if there are active instances.
     */
    ...
    /**
     * Converts the specified mouse event's screen point from WebView
     * coordinates to Windows coordinates, and returns a new event who's screen
     * point is in Windows coordinates, with the origin at the upper left corner
