...
public class ImageUtil {
    ...
    /**
     * Draws the specified <code>image</code> onto the <code>canvas</code>,
     * scaling or stretching the image to fit the canvas. This will apply a
     * bilinear filter to the image if any scaling or stretching is necessary.
     *
     * @param image the BufferedImage to draw, potentially scaling or stretching
     * to fit the <code>canvas</code>.
     * @param canvas the BufferedImage to receive the scaled or stretched
     * <code>image</code>.
     *
     * @throws IllegalArgumentException if either <code>image</code> or
     * <code>canvas</code> is null.
     */
    ...
    /**
     * Rasterizes the image into the canvas, given a transform that maps canvas
     * coordinates to image coordinates.
     *
     * @param image the source image.
     * @param canvas the image to receive the transformed source image.
     * @param canvasToImageTransform <code>Matrix</code> that maps a canvas
     * coordinates to image coordinates.
     *
     * @throws IllegalArgumentException if any of <code>image</code>,
     * <code>canvas</code>, or <code>canvasToImageTransform</code> are null.
     */
    ...
    /**
     * Convenience method for transforming a georeferenced source image into a
     * geographically aligned destination image. The source image is
     * georeferenced by either three four control points. Each control point
     * maps a location in the source image to a geographic location. This is
     * equivalent to calling {#transformConstrainedImage3} or
     ...
     * @param geoPoints three or four geographic locations corresponding to each
     * source control point.
     * @param destImage the destination image to receive the transformed source
     * imnage.
     *
     * @return bounding sector for the geographically aligned destination image.
     *
     * @throws IllegalArgumentException if any of <code>sourceImage</code>,
     * <code>destImage</code>, <code>imagePoints</code> or
     * <code>geoPoints</code> is null, or if either <code>imagePoints</code> or
    ...
    /**
     * Transforms a georeferenced source image into a geographically aligned
     * destination image. The source image is georeferenced by four control
     * points. Each control point maps a location in the source image to a
     * geographic location.
     *
     * @param sourceImage the source image to transform.
     * @param imagePoints four control points in the source image.
     * @param geoPoints four geographic locations corresponding to each source
     * control point.
     * @param destImage the destination image to receive the transformed source
     * imnage.
     *
    ...
    /**
     * Transforms a georeferenced source image into a geographically aligned
     * destination image. The source image is georeferenced by three control
     * points. Each control point maps a location in the source image to a
     * geographic location.
     *
     * @param sourceImage the source image to transform.
     * @param imagePoints three control points in the source image.
     * @param geoPoints three geographic locations corresponding to each source
     * control point.
     * @param destImage the destination image to receive the transformed source
     * imnage.
     *
    ...
    /**
     * Transforms a georeferenced source image into a geographically aligned
     * destination image. The source image is georeferenced by a world file,
     * which defines an affine transform from source coordinates to geographic
     * coordinates.
     *
     * @param sourceImage the source image to transform.
     * @param worldFileParams world file parameters which define an affine
     * transform.
     * @param destImage the destination image to receive the transformed source
     * imnage.
     *
     ...
     *
     * @throws IllegalArgumentException if any of <code>sourceImage</code>,
     * <code>destImage</code> or <code>worldFileParams</code> is null.
     */
    ...
    /**
     * Computes which three control points out of four provide the best estimate
     * an image's geographic location. The result is placed in the output
     * parameters <code>outImagePoints</code> and <code>outGeoPoints</code>,
     * both of which must be non-null and at least length 3.
     *
     * @param imagePoints four control points in the image.
     * @param geoPoints four geographic locations corresponding to the four
     * <code>imagePoints</code>.
     * @param outImagePoints three control points that best estimate the image's
     * location.
     * @param outGeoPoints three geographic locations corresponding to the three
     * <code>outImagePoints</code>.
     *
     * @throws IllegalArgumentException if any of <code>imagePoints</code>,
     * <code>geoPoints</code>, <code>outImagePoints</code> or
     * <code>outGeoPoints</code> is null, or if <code>imagePoints</code> or
     * <code>geoPoints</code> have length less than 4, or if
     * <code>outImagePoints</code> or <code>outGeoPoints</code> have length less
     * than 3.
     */
    ...
    /**
     * Returns the geographic corners of an image with the specified dimensions,
     * and a transform that maps image coordinates to geographic coordinates.
     *
     * @param imageWidth width of the image grid.
     * @param imageHeight height of the image grid.
     * @param imageToGeographic Matrix that maps image coordinates to geographic
     * coordinates.
     *
     * @return List of the image's corner locations in geographic coordinates.
     *
     * @throws IllegalArgumentException if either <code>imageWidth</code> or
     * <code>imageHeight</code> are less than 1, or if
     * <code>imageToGeographic</code> is null.
     */
    ...
    /**
     * Merge an image into another image. This method is typically used to
     * assemble a composite, seamless image from several individual images. The
     * receiving image, called here the canvas because it's analogous to the
     * Photoshop notion of a canvas, merges the incoming image according to the
     ...
     * @param aspectRatio the aspect ratio, width/height, of the assembled
     * image. If the aspect ratio is greater than or equal to one, the assembled
     * image uses the full width of the canvas; the height used is proportional
     * to the inverse of the aspect ratio. If the aspect ratio is less than one,
     * the full height of the canvas is used; the width used is proportional to
     * the aspect ratio.
     * <p>
     * The aspect ratio is typically used to maintain consistent width and
     * height units while assembling multiple images into a canvas of a
     * different aspect ratio than the canvas sector, such as drawing a
     * non-square region into a 1024x1024 canvas. An aspect ratio of 1 causes
     * the incoming images to be stretched as necessary in one dimension to
     * match the aspect ratio of the canvas sector.
     * @param image the image to merge into the canvas.
     * @param canvas the canvas into which the images are merged. The canvas is
     * not changed if the specified image and canvas sectors are disjoint.
     *
     * @throws IllegalArgumentException if the any of the reference arguments
     * are null or the aspect ratio is less than or equal to zero.
     */
    ...
    /**
     * Builds a sequence of mipmaps for the specified image. The number of
     * mipmap levels created will be equal to <code>maxLevel + 1</code>,
     * including level 0. The level 0 image will be a reference to the original
     * image, not a copy. Each mipmap level will be created with the specified
     * BufferedImage type <code>mipmapImageType</code>. Each level will have
     * dimensions equal to 1/2 the previous level's dimensions, rounding down,
     * to a minimum width or height of 1.
     *
     * @param image the BufferedImage to build mipmaps for.
     * @param mipmapImageType the BufferedImage type to use when creating each
     * mipmap image.
     * @param maxLevel the maximum mip level to create. Specifying zero will
     * return an array containing the original image.
     *
     * @return array of mipmap levels, starting at level 0 and stopping at
     * maxLevel. This array will have length maxLevel + 1.
     *
     * @throws IllegalArgumentException if <code>image</code> is null, or if
     * <code>maxLevel</code> is less than zero.
    ...
    /**
     * Builds a sequence of mipmaps for the specified image. This is equivalent
     * to invoking <code>buildMipmaps(BufferedImage, int, int)</code>, with
     * <code>mipmapImageType</code> equal to
     * <code>getMipmapType(image.getType())</code>, and <code>maxLevel</code>
     * equal to
     ...
     * <code>width</code> and <code>height</code>. The maximum desired level is
     * the number of levels required to reduce the original image dimensions to
     * a 1x1 image.
     *
     * @param width the level 0 image width.
     * @param height the level 0 image height.
     *
     * @return maximum mip level for the specified <code>width</code> and
     * <code>height</code>.
     *
    ...
    /**
     * Returns a copy of the specified image such that the new dimensions are
     * powers of two. The new image dimensions will be equal to or greater than
     * the original image. The flag <code>scaleToFit</code> determines whether
     * the original image should be drawn into the new image with no special
     * scaling, or whether the original image should be scaled to fit exactly in
     * the new image.
     * <p>
     * If the original image dimensions are already powers of two, this method
     * will simply return the original image.
     *
     * @param image the BufferedImage to convert to a power of two image.
     * @param scaleToFit true if <code>image</code> should be scaled to fit the
     * new image dimensions; false otherwise.s
     *
    ...
    /**
     * Returns a copy of the specified image with any fully-transparent borders
     * removed. The resultant image is cropped to fit its contents.
     *
    ...
    /**
     * Returns the size in bytes of the specified image. This takes into account
     * only the image's backing DataBuffers, and not the numerous supporting
     * classes a BufferedImage references.
     *
     ...
     * @param interpolation_mode the interpolation mode if the image is
     * reprojected.
     *
     * @return AVList
     *
     ...
     * @param imageIndex image index (could be a band; GeoTiff file could
     * contain overview images, bands, etc)
     * @param values AVList
     *
    ...
    /**
     * Opens a Geotiff image image. Reprojects the image if it is in UTM
     * projection.
     *
     * @param image BufferedImage
     * @param reader GeotiffReader
     * @param imageIndex image index (GeoTiff file could contain overview
     * images, bands, etc)
     * @param interpolation_mode the interpolation mode if the image is
     * reprojected.
     *
     ...
     *
     * @param values AVList: contains the bufferedimage and the values from the
     * world file. Stores resulting image in values
    ...
    /**
     * Performs bilinear interpolation of 32-bit colors over a convex
     * quadrilateral.
     *
     * @param x horizontal coordinate of the interpolation point relative to the
     * lower left corner of the quadrilateral. The value should generally be in
     * the range [0, 1].
     * @param y vertical coordinate of the interpolation point relative to the
     * lower left corner of the quadrilateral. The value should generally be in
     * the range [0, 1].
    ...
    /**
     * Reprojects an image into an aligned image, one with edges of constant
     * latitude and longitude.
     *
     * @param sourceImage the image to reproject, typically a non-aligned image
     * @param latitudes an array identifying the latitude of each pixels if the
     * source image. There must be an entry in the array for all pixels. The
     * values are taken to be in row-major order relative to the image -- the
     * horizontal component varies fastest.
     * @param longitudes an array identifying the longitude of each pixels if
     * the source image. There must be an entry in the array for all pixels. The
     * values are taken to be in row-major order relative to the image -- the
     * horizontal component varies fastest.
     *
     * @return a new image containing the original image but reprojected to
     * align to the bounding sector. Pixels in the new image that have no
     * correspondence with the source image are transparent.
     *
     * @throws InterruptedException if any thread has interrupted the current
     * thread while alignImage is running. The
     ...
     * values are taken to be in row-major order relative to the image -- the
     * horizontal component varies fastest.
     * @param sector the sector to align the image to. If null, this computes
     * the aligned image's sector.
     * @param dimension the the aligned image's dimensions. If null, this
     * computes the aligned image's dimension.
     *
     * @return a new image containing the original image but reprojected to
     * align to the sector. Pixels in the new image that have no correspondence
     * with the source image are transparent.
     *
     * @throws InterruptedException if any thread has interrupted the current
     * thread while alignImage is running. The
     * <i>interrupted status</i> of the current thread is cleared when this
...
//        return gc.createCompatibleImage(width, height, transparency);
    ...
    /**
     * Converts a non-imagery data raster (elevations) to visually representable
     * image raster. Calculates min and max values, and normalizes pixel value
     * from 0 - 65,535 and creates a GRAY color image raster with pixel data
     * type of unsigned short.
     *
     * @param raster non-imagery data raster (elevations) instance of data
     * raster derived from BufferWrapperRaster
     *
     * @return BufferedImage visual representation of the non-imagery data
     * raster
     */
