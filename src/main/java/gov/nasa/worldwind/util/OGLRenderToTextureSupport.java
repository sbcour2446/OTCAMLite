...
/**
 * OGLRenderToTextureSupport encapsulates the pattern of rendering GL commands
 * to a destination texture. Currently only the color pixel values are written
 * to the destination texture, but other values (depth, stencil) should be
 * possible with modification or extension.
 * <p>
 * OGLRenderToTextureSupport is compatible with GL version 1.1 or greater, but
 * it attempts to use more recent features whenever possible. Different GL
 * feature sets result in different approaches to rendering to texture,
 * therefore the caller cannot depend on the mechanism by which
 * OGLRenderToTextureSupport will write pixel values to the destination texture.
 * For this reason, OGLRenderToTextureSupport must be used when the contents of
 * the windowing system buffer (likely the back framebuffer) can be freely
 * modified by OGLRenderToTextureSupport. The WorldWind pre-render stage is a
 * good example of when it is appropriate to use OGLRenderToTextureSupport. Fore
 * more information on the pre-render stage, see
 * {@link gov.nasa.worldwind.render.PreRenderable} and
 * {@link gov.nasa.worldwind.layers.Layer#preRender(gov.nasa.worldwind.render.DrawContext)}.
 * <b>Note:</b> In order to achieve consistent results across all platforms, it
 * is essential to clear the texture's contents before rendering anything into
 * the texture. Do this by invoking {@link
 * #clear(gov.nasa.worldwind.render.DrawContext, java.awt.Color)} immediately
 * after any call to {@link
 ...
 * passed in as an argument to the containing method.<br> Texture texture =
 * TextureIO.newTexture(new TextureData(...);<br> <br> // Setup the drawing
 * rectangle to match the texture dimensions, and originate from the texture's
 * lower left corner.<br> OGLRenderToTextureSupport rttSupport = new
 * OGLRenderToTextureSupport();<br>
 * rttSupport.beginRendering(dc, 0, 0, texture.getWidth(),
 * texture.getHeight());<br> try<br> {<br> // Bind the texture as the
 * destination for color pixel writes.<br> rttSupport.setColorTarget(dc,
 * texture);<br> // Clear the texture contents with transparent black.<br>
 * rttSupport.clear(dc, new Color(0, 0, 0, 0));<br> // Invoke desired GL
 * rendering commands.<br> }<br> finally<br> {<br>
 * rttSupport.endRendering(dc);<br> }<br> </code>
 *
 * @author dcollins
 * @version $Id: OGLRenderToTextureSupport.java 1676 2013-10-21 18:32:30Z
 * dcollins $
 */
public class OGLRenderToTextureSupport {
    ...
    /**
     * Specifies if framebuffer objects should be used if they are available in
    ...
    /**
     * Returns the texture currently set as the color buffer target, or null if
     * no texture is currently bound as the color buffer target.
     *
     * @return the Texture currently set as the color buffer target, or null if
     * none exists.
     */
    ...
    /**
     * Sets the specified texture as the color buffer target. This texture
     * receives the output of all GL commands affecting the color buffer.
     * Binding a null texture specifies that no texture should receive color
     * values. If the current color target texture is the same reference as the
     * specified texture, this does nothing. Otherwise this flushes any buffered
     * pixel values to the current color target, and assigns the specified
     * texture as the new color target.
     * <p>
     * If {@link #isEnableFramebufferObject()} is false, the supported texture
     * formats for the color target are limited only by the OpenGL
     * implementation's supported formats. If
     * {@link #isEnableFramebufferObject()} is true and the DrawContext supports
     * OpenGL framebuffer objects, the supported texture formats for the color
     * target are as follows: <ul> <li>RGB</li> <li>RGBA</li> <li>FLOAT_R_NV (on
     * NVidia hardware)</li> <li>FLOAT_RG_NV (on NVidia hardware)</li>
     * <li>FLOAT_RGB_NV (on NVidia hardware)</li> <li>FLOAT_RGBA_NV (on NVidia
     * hardware)</li> </ul>
     *
     * @param dc the current DrawContext.
     * @param texture the Texture to use as the destination for GL commands
     * affecting the color buffer. A null value is permitted.
     *
    ...
    /**
     * Clears the current texture target's pixels with the specified RGBA clear
     * color. If the current color texture target is null, this does nothing.
     *
     * @param dc the current DrawContext.
     * @param color the RGBA clear color to write to the current color texture
     * target.
     *
     * @throws IllegalArgumentException if either the DrawContext or the color
     * is null.
     */
    ...
    /**
     * Configures the GL attached to the specified DrawContext for rendering a
     * 2D model to a texture. The modelview matrix is set to the identity, the
     * projection matrix is set to an orthographic projection aligned with the
     * specified draw rectangle (x, y, width, height), the viewport and scissor
     * boxes are set to the specified draw rectangle, and the depth test and
     * depth write flags are disabled. Because the viewport and scissor boxes
     * are set to the draw rectangle, only the texels intersecting the specified
     * drawing rectangle (x, y, width, height) are affected by GL commands. Once
     * rendering is complete, this should always be followed with a call to {@link
    ...
    /**
     * Flushes any buffered pixel values to the appropriate texure targets, then
     * restores the GL state to its previous configuration before
     * {@link #beginRendering(gov.nasa.worldwind.render.DrawContext, int, int, int, int)}
     * was called. Finally, all texture targets associated with this
     * OGLRenderToTextureSupport are unbound.
     *
     * @param dc the current DrawContext.
     *
     * @throws IllegalArgumentException if the DrawContext is null.
