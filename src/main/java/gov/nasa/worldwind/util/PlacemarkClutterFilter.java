...
/**
 * Provides a clutter filter that rearranges {@link PointPlacemark} labels to
 * avoid overlap. When placemarks overlap, only their label and a line to their
 * position are drawn. The placemark's icon is not drawn for overlapping
 * placemarks.
 *
 * @author tag
...
public class PlacemarkClutterFilter implements ClutterFilter {
     ...
     *
     * @return the intersected region if the input region intersects one or more
     * other regions in the filter, otherwise false.
     */
    ...
    /**
     * Draws the decluttered shape representation. For shapes that are not
     * {@code PointPlacemark}s, the first non- placemark shape is drawn in
     * addition to the placemark representation. This causes this filter to
     * produce the same results for non-placemark shapes as does
