...
/**
 * Traverses an implicit tree and makes decisions at tree nodes visited in
 * depth-first order.
 * <p>
 * Users provide an object at construction that implements the
 * {@link gov.nasa.worldwind.util.DecisionTree.Controller} interface. This
 * controller provides methods to determine node inclusion, to terminate
 * traversal, and to create descendant nodes. It is carried to each node during
 * traversal.
 * <p>
 * Users also provide a user-defined context object to carry state and other
 * information to each node. The context can hold information to assist in
 * decisions or to retains objects or information gathered during traversal.
 * <p>
 * At the start of traversal a user-defined object is specified as the object
 * defining the decision. It's the object against which decisions are made at
 * each node.
 * <p>
 * See the source code of {@link gov.nasa.worldwind.util.SectorVisibilityTree}
 * for example usage.
 *
 * @author tag
...
public class DecisionTree<T, C> // T = type being managed. C = traversal context
{
     ...
     *
     * @param <T> the data type of objects associated with tree nodes -- the
     * objects implicitly contained in the tree.
    ...
    public interface Controller<T, C> {
         ...
         *
         * @param o the user-defined object specified at traversal start and
         * examined at each tree nodes.
         ...
         * @param context the traversal context.
         *
         * @return true if the node is terminal, otherwise false. Traversal
         * continues to descendants if false is returned. otherwise traversal of
         * the node's branch of the tree stops.
         */
        public boolean isTerminal(T o, C context);

        /**
         * Create a cell's descendant nodes. Called in order to continue
         * traversal down a branch of the tree from the current node. The
         * returned nodes are visited in the order returned.
         *
         * @param o the user-defined object specified at traversal start and
         * examined at each tree nodes.
         * @param context the traversal context.
     ...
     *
     * @param controller a user-defined object implementing the
     * <code>Controller</code> interface providing the traversal control
     * methods.
     */
