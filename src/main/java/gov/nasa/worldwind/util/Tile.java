...
/**
 * Large images and most imagery and elevation-data sets are subdivided in order
 * to display visible portions quickly and without excessive memory usage. Each
 * subdivision is called a tile, and a collections of adjacent tiles
 * corresponding to a common spatial resolution is typically maintained in a
 * {@link Level}. A collection of levels of progressive resolutions are
 * maintained in a {@link LevelSet}. The <code>Tile</code> class represents a
 * single tile of a subdivided image or elevation raster.
 * <p>
 * Individual tiles are identified by the level, row and column of the tile
 * within its containing level set.
 *
 * @author tag
...
public class Tile implements Comparable<Tile>, Cacheable {
    ...
    /**
     * Constructs a tile for a given sector, level, row and column of the tile's
     * containing tile set.
     *
     ...
     * indicated level.
     *
     * @throws IllegalArgumentException if <code>sector</code> or
     * <code>level</code> is null.
     */
...
//        }
    ...
    /**
     * Constructs a tile for a given sector, level, row and column of the tile's
     * containing tile set. If the cache name is non-null, it overrides the
     * level's cache name and is returned by {@link #getCacheName()}. Otherwise,
     * the level's cache name is used.
     *
     * @param sector the sector corresponding with the tile.
     * @param level the tile's level within a containing level set.
     * @param row the row index (0 origin) of the tile within the indicated
     ...
     * @param column the column index (0 origin) of the tile within the
     * indicated level.
     * @param cacheName optional cache name to override the Level's cache name.
     * May be null.
     *
     * @throws IllegalArgumentException if <code>sector</code> or
     * <code>level</code> is null.
    ...
    /**
     * Constructs a texture tile for a given sector and level, and with a
    ...
    /**
     * Constructs a texture tile for a given sector with a default level, row
     * and column.
     *
     * @param sector the sector to create the tile for.
     */
    ...
    /**
     * Returns the tile's cache name. If a non-null cache name was specified at
     * construction, that name is returned. Otherwise this returns the level's
     * cache name.
     *
    ...
    /**
     * Computes the row index of a latitude in the global tile grid
     * corresponding to a specified grid interval.
     *
     ...
     *
     * @throws IllegalArgumentException if <code>delta</code> is null or
     * non-positive, or <code>latitude</code> is null, greater than positive 90
     * degrees, or less than negative 90 degrees
     */
    ...
    /**
     * Computes the column index of a longitude in the global tile grid
     * corresponding to a specified grid interval.
     *
     * @param delta the grid interval
     ...
     *
     * @throws IllegalArgumentException if <code>delta</code> is null or
     * non-positive, or <code>longitude</code> is null, greater than positive
     * 180 degrees, or less than negative 180 degrees
     */
    ...
    /**
     * Determines the minimum latitude of a row in the global tile grid
     * corresponding to a specified grid interval.
     *
     ...
     * @param origin the origin of the grid
     *
     * @return the minimum latitude of the tile corresponding to the specified
     * row
     *
     * @throws IllegalArgumentException if the grid interval
     * (<code>delta</code>) is null or zero or the row index is negative.
     */
    ...
    /**
     * Determines the minimum longitude of a column in the global tile grid
     * corresponding to a specified grid interval.
     *
     ...
     * @param origin the origin of the grid
     *
     * @return the minimum longitude of the tile corresponding to the specified
     * column
     *
     * @throws IllegalArgumentException if the grid interval
     * (<code>delta</code>) is null or zero or the column index is negative.
     */
