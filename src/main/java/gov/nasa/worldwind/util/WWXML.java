...
/**
 * A collection of static methods use for opening, reading and otherwise working
 * with XML files.
 *
 * @author tag
...
public class WWXML {
     ...
     *
     * @param isNamespaceAware true if the builder is to be namespace aware,
     * otherwise false.
     *
    ...
    /**
     * Open and XML document from a general source. The source type may be one
     * of the following: <ul> <li>a {@link
     * URL}</li> <li>an {@link InputStream}</li> <li>a {@link File}</li> <li>a
     * {@link String} containing a valid URL description or a file or resource
     * name available on the classpath.</li> </ul>
     *
     * @param docSource the source of the XML document.
     *
     * @return the source document as a {@link Document}, or null if the source
     * object is a string that does not identify a URL, a file or a resource
    ...
    /**
     * Opens an XML file given the file's location in the file system or on the
     * classpath.
     *
     * @param filePath the path to the file. Must be an absolute path or a path
     ...
     * @throws WWRuntimeException if an exception or error occurs while opening
     * and parsing the file. The causing exception is included in this
     * exception's {@link Throwable#initCause(Throwable)} .
     */
     ...
     * @throws WWRuntimeException if an exception or error occurs while opening
     * and parsing the url. The causing exception is included in this
     * exception's {@link Throwable#initCause(Throwable)} .
     */
     ...
     * @throws WWRuntimeException if an exception or error occurs while parsing
     * the stream. The causing exception is included in this exception's
     ...
     * @param filePath the path to the file. Must be an absolute path in the
     * file system.
     *
     * @throws IllegalArgumentException if either the document or file path is
     * null.
     * @throws WWRuntimeException if an exception or error occurs while writing
     * the document. The causing exception is included in this exception's
     * {@link Throwable#initCause(Throwable)}
     */
     ...
     *
     * @throws IllegalArgumentException if either the document or input stream
     * is null.
     * @throws WWRuntimeException if an exception or error occurs while writing
     * the document. The causing exception is included in this exception's
     * {@link Throwable#initCause(Throwable)}
    ...
    /**
     * Opens an XML event stream given an input stream, and a namespace-aware
     * processing mode.
     *
     * @param inputStream an XML document as an input stream.
     * @param isNamespaceAware true to enable namespace-aware processing and
     * false to disable it.
     *
    ...
    /**
     * Opens an XML event stream given the file's location in the file system or
     * on the classpath.
     *
     * @param filePath the path to the file. Must be an absolute path or a path
     * relative to a location in the classpath.
     ...
     * false to disable it.
     *
     * @return an XMLEventReader for the file, or null if the specified cannot
     * be found.
     *
     * @throws IllegalArgumentException if the file path is null.
     * @throws WWRuntimeException if an exception or error occurs while opening
    ...
    /**
     * Open a namespace-aware XML event stream from a general source. The source
     * type may be one of the following: <ul>
     * <li>{@link URL}</li> <li>{@link InputStream}</li> <li>{@link File}</li>
     * <li>{@link String} containing a valid URL description or a file or
     * resource name available on the classpath.</li> </ul>
     *
     * @param docSource the source of the XML document.
     *
     * @return the source document as a {@link javax.xml.stream.XMLEventReader},
     * or null if the source object is a string that does not identify a URL, a
     * file or a resource available on the classpath.
     */
    ...
    /**
     * Open an XML event stream from a general source. The source type may be
     * one of the following: <ul> <li>{@link
     * URL}</li> <li>{@link InputStream}</li> <li>{@link File}</li>
     * <li>{@link String} containing a valid URL description or a file or
     * resource name available on the classpath.</li> </ul>
     *
     * @param docSource the source of the XML document.
     ...
     * {@link javax.xml.stream.XMLStreamException} generated in the process.
     *
     * @param eventReader the event reader to close. If null, this method does
     * nothing.
     * @param name the name of the event reader to place in the log message if
    ...
    /**
     * Opens an XML stream writer to a general output destination. The source
     * type may be one of the following: <ul>
     * <li>{@link java.io.OutputStream}</li> <li>{@link java.io.Writer}</li>
     * </ul> This returns <code>null</code> if the <code>output</code> is not
     * one of the recognized types.
     *
     * @param output the output destination for the XML document stream.
     *
     * @return the <code>XMLStreamWriter</code> that writes to the specified
     * <code>output</code>, or <code>null</code> if the output type is not
     * recognized.
     *
     ...
     * open the <code>XMLStreamWriter</code>.
     */
    ...
    /**
     * Returns the next StartElement event from the specified XML event stream,
     * or null if the stream has no remaining StartElement events.
     *
    ...
    /**
     * Returns a string containing the text from all character nodes under the
     * current element appended into a single continuous string. After this
     * method returns the specified eventReader is positioned at the end element
     * of the eventReader's current element.
     *
     * This returns the empty string if there are no character nodes under the
     * current element, or if the element contains only whitespace.
     *
     * @param eventReader the stream to poll for XML events.
     *
     * @return A string containing the text from character nodes under the
     * current element.
     *
     * @throws IllegalArgumentException if the event reader is null.
     */
    ...
    /**
     * Returns the element node's unqualified name. If the element is qualified
     * with a namespace, this returns the local part of the qualified name.
     * Otherwise, this returns the element's unqualified tag name.
     *
    ...
    /**
     * Returns the element event's unqualified name. If the element is qualified
     * with a namespace, this returns the local part of the qualified name.
     * Otherwise, this returns the element's unqualified tag name.
     *
     * @param event the XML StartElement event who's unqualified name is
     * returned.
     *
     ...
     *
     * @return the text of an element matching the XPath expression, or null if
     * no match is found.
     *
     * @throws IllegalArgumentException if the context or XPath expression are
     * null.
     */
     ...
     * @param context the context from which to start the XPath search.
     * @param path the XPath expression.
     * @param xpath an {@link XPath} object to use for the search. This allows
     * the caller to re-use XPath objects when performing multiple searches. May
     * be null.
     *
     * @return the text of an element matching the XPath expression, or null if
     * no match is found.
     ...
     * be null.
     *
     * @return an array containing the text of each element matching the XPath
     * expression.
     *
     * @throws IllegalArgumentException if the context or XPath expression are
     * null.
    ...
    /**
     * Returns the text of all unique elements identified by an XPath
     * expression.
     *
     * @param context the context from which to start the XPath search.
     * @param path the XPath expression.
     ...
     *
     * @return an array containing the text of each element matching the XPath
     * expression and containing unique text. If multiple elements contain the
     * same text only the first one found is returned. Returns null if no
     ...
     * be null.
     *
     * @return the element matching the XPath expression, or null if no element
     * matches.
     *
     * @throws IllegalArgumentException if the context or XPath expression are
     * null.
    ...
    /**
     * Returns the unique elements identified by an XPath expression and a
     * sub-expression.
     *
     * @param context the context from which to start the XPath search.
     * @param path the XPath expression.
     * @param uniqueTag an XPath expression to match with the elements matched
     * with the above expression.
     * @param xpath an {@link XPath} object to use for the search. This allows
     * the caller to re-use XPath objects when performing multiple searches. May
     * be null.
     *
     * @return an array containing the unique elements matching the XPath
     * expression and subexpression. If multiple elements have the same content
     * only the first one found is returned. Returns null if no matching element
     * is found.
     *
     * @throws IllegalArgumentException if either the context, XPath expression
     * or XPath sub-expression are null.
     */
    ...
    /**
     * Returns the {@link Double} value of an element identified by an XPath
     * expression.
     *
     * @param context the context from which to start the XPath search.
     * @param path the XPath expression.
     ...
     * be null.
     *
     * @return the value of an element matching the XPath expression, or null if
     * no match is found or the match does not contain a {@link Double}.
     *
     * @throws IllegalArgumentException if the context or XPath expression are
     * null.
     */
    ...
    /**
     * Returns the {@link Integer} value of an element identified by an XPath
     * expression.
     *
     * @param context the context from which to start the XPath search.
     * @param path the XPath expression.
     ...
     *
     * @return the value of an element matching the XPath expression, or null if
     * no match is found or the match does not contain a {@link Integer}.
     *
     * @throws IllegalArgumentException if the context or XPath expression are
     * null.
     */
    ...
    /**
     * Returns the {@link Long} value of an element identified by an XPath
     * expression.
     *
     * @param context the context from which to start the XPath search.
     * @param path the XPath expression.
    ...
    /**
     * Returns the {@link Boolean} value of an element identified by an XPath
     * expression.
     *
     * @param context the context from which to start the XPath search.
     * @param path the XPath expression.
     ...
     *
     * @return the value of an element matching the XPath expression, or null if
     * no match is found or the match does not contain a {@link Boolean}.
     *
     * @throws IllegalArgumentException if the context or XPath expression are
     * null.
     */
    ...
    /**
     * Returns the {@link gov.nasa.worldwind.geom.LatLon} value of an element
     * identified by an XPath expression.
     *
     * @param context the context from which to start the XPath search.
     * @param path the XPath expression. If null, indicates that the context is
     * the LatLon element itself. If non-null, the context is searched for a
     * LatLon element using the expression.
     * @param xpath an {@link XPath} object to use for the search. This allows
     * the caller to re-use XPath objects when performing multiple searches. May
     * be null.
     *
     * @return the value of an element matching the XPath expression, or null if
     * no match is found or the match does not contain a
     * {@link gov.nasa.worldwind.geom.LatLon}.
     *
     * @throws IllegalArgumentException if the context is null.
     */
    ...
    /**
     * Returns the {@link Color} value of an element identified by an XPath
     * expression.
     *
     * @param context the context from which to start the XPath search.
     * @param path the XPath expression. If null, indicates that the context is
     * the Color element itself. If non-null, the context is searched for a
     * Color element using the expression.
     * @param xpath an {@link XPath} object to use for the search. This allows
     * the caller to re-use XPath objects when performing multiple searches. May
     * be null.
     *
     * @return the value of an element matching the XPath expression, or null if
     * no match is found or the match does not contain a {@link Color}.
     *
     * @throws IllegalArgumentException if the context is null.
     */
    ...
    /**
     * Returns the {@link gov.nasa.worldwind.geom.Sector} value of an element
     * identified by an XPath expression.
     *
     * @param context the context from which to start the XPath search.
     * @param path the XPath expression. If null, indicates that the context is
     * the Sector element itself. If non-null, the context is searched for a
     * Sector element using the expression.
     * @param xpath an {@link XPath} object to use for the search. This allows
     * the caller to re-use XPath objects when performing multiple searches. May
     * be null.
     *
     * @return the value of an element matching the XPath expression, or null if
     * no match is found or the match does not contain a
     * {@link gov.nasa.worldwind.geom.Sector}.
     *
     * @throws IllegalArgumentException if the context is null.
     */
    ...
    /**
     * Returns the {@link gov.nasa.worldwind.util.LevelSet.SectorResolution}
     * value of an element identified by an XPath expression.
     *
     * @param context the context from which to start the XPath search.
     * @param path the XPath expression. If null, indicates that the context is
     * the SectorResolution element itself. If non-null, the context is searched
     * for a SectorResolution element using the expression.
     * @param xpath an {@link XPath} object to use for the search. This allows
     * the caller to re-use XPath objects when performing multiple searches. May
     * be null.
     *
     * @return the value of an element matching the XPath expression, or null if
     * no match is found or the match does not contain a
     * {@link gov.nasa.worldwind.util.LevelSet.SectorResolution}.
     *
     * @throws IllegalArgumentException if the context is null.
     */
    ...
    /**
     * Returns the time in milliseconds of an element identified by an XPath
     * expression.
     *
     * @param context the context from which to start the XPath search.
     * @param path the XPath expression. If null, indicates that the context is
     * the Time element itself. If non-null, the context is searched for a Time
    ...
    /**
     * Returns the date and time in milliseconds of an element identified by an
     * XPath expression.
     *
     * @param context the context from which to start the XPath search.
     * @param path the XPath expression. If null, indicates that the context is
     * the element itself. If non-null, the context is searched for an element
     * matching the expression.
     * @param pattern the format pattern of the date. See
     * {@link java.text.DateFormat} for the pattern symbols. The element content
     * must either match the pattern or be directly convertible to a long.
     * @param xpath an {@link XPath} object to use for the search. This allows
     * the caller to re-use XPath objects when performing multiple searches. May
     * be null.
     *
    ...
    /**
     * Returns the {@link gov.nasa.worldwind.render.ScreenCredit} value of an
     * element identified by an XPath expression.
     *
     * @param context the context from which to start the XPath search.
     * @param path the XPath expression. If null, indicates that the context is
     * the ScreenCredit element itself. If non-null, the context is searched for
     * a ScreenCredit element using the expression.
     * @param xpath an {@link XPath} object to use for the search. This allows
     * the caller to re-use XPath objects when performing multiple searches. May
     * be null.
     *
     * @return the value of an element matching the XPath expression, or null if
     * no match is found or the match does not contain a ScreenCredit.
     *
     * @throws IllegalArgumentException if the context is null.
     */
    ...
    /**
     * Sets the specified document's root element to a new element node with the
     * specified name. If the document already has a root element, this replaces
     * the existing root node with the new element node.
     *
     ...
     *
     * @throws IllegalArgumentException if the document is null, if the name is
    ...
    /**
     * Append a new element with a specified name to a context element. If the
     * element name is null or empty, this returns the context element, and does
     * not make any modifications to the context.
     *
     * @param context the context on which to append a new element.
     * @param name the new element name to append.
     *
     * @return the new element appended to the context, or the context if the
     * element name is null or empty.
     *
     * @throws IllegalArgumentException if the context is null.
     */
    ...
    /**
     * Append a heirarcy of new elements with a path to a context element. The
     * path is treated as a list of descendant elements which created and
     * appended to the context. Each new element in the path is delimited by the
     * "/" character. For example, the path "A/B/C" appends A to context, B to
     * A, and C to B. If the path is null or empty, this returns the context
     * element, and does not make any modifications to the context. If any
     * element name in the path is empty, that element is skipped.
     *
     * @param context the context on which to append new elements.
     * @param path the element path to append.
     *
     * @return the new element appended to the context, or the context if the
     * element path is null or empty.
     *
     * @throws IllegalArgumentException if the context is null.
     */
     ...
     * ending with a text element with a specified value. Elements are added to
     * the context as in
     * {@link #appendElementPath(org.w3c.dom.Element, String)}, except that a
    ...
    /**
     * For each non-null string in a specified array, appends a heirarcy of new
     * elements with a path to a context element, ending with a text element
     * with a specified value. Elements are added to the context as in {@link
    ...
    /**
     * Append a heirarcy of new elements with a path to a context element,
     * ending with an element formatted as a LatLon. Elements are added to the
     * context as in {@link #appendElementPath(org.w3c.dom.Element, String)},
    ...
    /**
     * Append a heirarcy of new elements with a path to a context element,
     * ending with an element formatted as a Sector. Elements are added to the
     * context as in {@link #appendElementPath(org.w3c.dom.Element, String)},
     * except that a terminating text element is appended to the last element.
     *
     * @param context the context on which to append new elements.
    ...
    /**
     * Append a heirarcy of new elements with a path to a context element,
     * ending with an element formatted as a LevelSet.SectorResolutionLimit.
     * Elements are added to the context as in {@link
     * #appendElementPath(org.w3c.dom.Element, String)}, except that a
     * terminating text element is appended to the last element.
     *
     * @param context the context on which to append new elements.
     * @param path the element path to append.
     * @param sectorResolution the LevelSet.SectorResolutionLimit value to
     * create.
     *
     * @return the new element appended to the context, or the context if the
     * element path is null or empty.
    ...
    /**
     * Append a heirarcy of new elements with a path to a context element,
     * ending with an element formatted as a time in milliseconds. Elements are
     * added to the context as in
     * {@link #appendElementPath(org.w3c.dom.Element, String)}, except that a
     * terminating text element is appended to the last element.
     *
     * @param context the context on which to append new elements.
    ...
    /**
     * Append a heirarcy of new elements with a path to a context element,
     * ending with an element formatted as a ScreenCredit. Elements are added to
     * the context as in {@link WWXML#appendElementPath(org.w3c.dom.Element,
     * String)}, except that a terminating text element is appended to the last
     * element.
     *
     * @param context the context on which to append new elements.
     * @param path the element path to append.
    ...
    /**
     * Sets the element's attribute with the specified name to the specified
     * String value. If the element already has an attribute with this name, its
     * value is repaced with the specified value.
     *
     * @param context the element on which to set the attribute.
     * @param name the attribute's name.
     * @param value the attribute's value.
     *
     * @throws IllegalArgumentException if the context is null, if the name is
     * null, or if the name is empty.
     */
    ...
    /**
     * Sets the element's attribute with the specified name to the specified
     * double value, converted to a String. If the element already has an
    ...
    /**
     * Sets the element's attribute with the specified name to the specified
     * integer value, converted to a String. If the element already has an
     * attribute with this name, its value is repaced with the specified value.
     *
     * @param context the element on which to set the attribute.
     * @param name the attribute's name.
    ...
    /**
     * Sets the element's attribute with the specified name to the specified
     * long integer value, converted to a String. If the element already has an
     * attribute with this name, its value is repaced with the specified value.
     *
     * @param context the element on which to set the attribute.
     * @param name the attribute's name.
    ...
    /**
     * Sets the element's attribute with the specified name to the specified
     * boolean value, converted to a String. If the element already has an
     * attribute with this name, its value is repaced with the specified value.
     *
     * @param context the element on which to set the attribute.
     * @param name the attribute's name.
    ...
    /**
     * Checks a parameter list for a specified key and if not present attempts
     * to find a value for the key from an element matched by an XPath
     * expression. If found, the key and value are added to the parameter list.
     *
     * @param context the context from which to start the XPath search.
     * @param params the parameter list.
     * @param paramKey the key used to identify the paramater in the parameter
     * list.
     * @param paramName the Xpath expression identifying the parameter value
     * within the specified context.
     * @param xpath an {@link XPath} object to use for the search. This allows
     * the caller to re-use XPath objects when performing multiple searches. May
     * be null.
     *
     * @throws IllegalArgumentException if either the context, parameter list,
     * parameter key or parameter name are null.
     */
     ...
     *
     * @throws IllegalArgumentException if either the context, parameter list,
     * parameter key, pattern or parameter name are null.
     */
    ...
    /**
     * Checks a parameter list for a specified key and if present attempts to
     * append new elements represeting the parameter to a specified context.
     *
     * @param context the context on which to append new elements.
     * @param params the parameter list.
     * @param paramKey the key used to identify the paramater in the parameter
     ...
     * @param path the element path to append.
     *
     * @throws IllegalArgumentException if either the parameter list parameter
     * key, or context are null.
     */
    ...
    /**
     * Returns the byte order constant for a specified string. This performs a
     * mapping between text and an AVKey constant: <table>
     * <caption style="font-weight: bold;">Mapping</caption><tr><th>Text</th><th>Constant</th></tr> <tr><td>LittleEndian</td><td>{@link
     * AVKey#LITTLE_ENDIAN}</td></tr>
     * <tr><td>BigEndian</td><td>{@link AVKey#BIG_ENDIAN}</td></tr> </table>
     *
     * @param s the string to parse as a byte order.
     *
     * @return a byte order constant, or null if the string text is not
     * recognized.
     *
    ...
    /**
     * Returns the string text for a specified byte order constant. This
     * performs a mapping between text and an AVKey constant: <table>
     * <caption style="font-weight: bold;">Mapping</caption><tr><th>Text</th><th>Constant</th></tr> <tr><td>LittleEndian</td><td>{@link
     * AVKey#LITTLE_ENDIAN}</td></tr>
     * <tr><td>BigEndian</td><td>{@link AVKey#BIG_ENDIAN}</td></tr> </table>
     *
     ...
     *
     * @return a string representing the byte order constant, or null if the
     * byte order constant is not recognized.
     *
    ...
    /**
     * Returns the data type constant for a specified string. This performs a
     * mapping between text and an AVKey constant: <table>
     * <caption style="font-weight: bold;">Mapping</caption><tr><th>Text</th><th>Constant</th></tr>
     ...
     *
     * @return a data type constant, or null if the string text is not
     * recognized.
     *
     * @throws IllegalArgumentException if the string is null.
     */
    ...
    /**
     * Returns the string text for a specified data type constant. This performs
     * a mapping between text and an AVKey constant: <table>
     * <caption style="font-weight: bold;">Mapping</caption><tr><th>Text</th><th>Constant</th></tr>
     * <tr><td>Float32</td><td>{@link AVKey#FLOAT32}</td></tr>
     * <tr><td>Int32</td><td>{@link AVKey#INT32}</td></tr>
     * <tr><td>Int16</td><td>{@link AVKey#INT16}</td></tr>
     ...
     *
     * @return a string representing the data type constant, or null if the data
     * type constant is not recognized.
     *
    ...
    /**
     * Copy any <code>Property</code> elements in an XML document to an
     * attribute-value list.
     *
     * @param element the XML element potentially containing
     * <code>Property</code> elements.
     * @param params an attribute-value list to copy the properties to.
     *
     * @return if an attribute-value list is specified, the reference to that
     * list now containing any properties copied from the XML element. If an
     * attribute-value list is not specified and properties exist in the XML
     * element, a new attribute-value list containing those properties.
    ...
    /**
     * Uses reflection to invoke property methods on an object, with the
     * properties specified in an XML document. For each element named
     * "Property" in the document, the corresponding <i>set</i> method is called
     * on the specified object, if such a method exists.
     *
     * @param parent the object on which to set the properties.
     * @param domElement the XML document containing the properties.
     *
     * @throws IllegalArgumentException if the specified object or XML document
     * element is null.
