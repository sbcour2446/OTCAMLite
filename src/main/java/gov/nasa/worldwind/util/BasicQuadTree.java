...
/**
 * Implements a quadtree backed by a bit-set index. A bit-set provides a
 * minimal-memory index. Each bit identifies one cell in the quadtree.
 * <p>
 * This class provides methods to add and remove items from the quadtree, and to
 * determine the items intersecting specified regions.
 * <p>
 * Items can be added with an associated name, and can be retrieved and removed
 * by name.
 *
 * @author tag
...
public class BasicQuadTree<T> extends BitSetQuadTreeFilter implements Iterable<T> {
    ...
    /**
     * Constructs a quadtree of a specified level and spanning a specified
     * region.
     * <p>
     * The number of levels in the quadtree must be specified to the
     * constructor. The more levels there are the more discriminating searches
     * will be, but at the cost of some performance because more cells are
     * searched. For the Earth, a level count of 8 provides leaf cells about 75
     * km along their meridian edges (edges of constant Earth, a level count of
     * 8 provides leaf cells about 75 km along their meridian edges (edges of
     * constant longitude). Additional levels successfully halve the distance,
     * fewer levels double that distance.
     *
     * @param numLevels the number of levels in the quadtree. The more levels
     * there are the more discriminating searches will be, but at the cost of
     * some performance.
     * @param sector the region the tree spans.
     * @param itemMap a {@link Map} to hold the items added to the quadtree. May
     * be null, in which case a new map is created.
     *
     ...
     * @param itemMap a {@link Map} to hold the items added to the quadtree. May
     * be null, in which case a new map is created.
     * @param allowDuplicates Indicates whether the collection held by this
     * quadtree may contain duplicate entries. Specifying <code>true</code>,
     * which is the default, may cause an individual item to be associated with
     * multiple quadtree regions if the item's coordinates fall on a region
     * boundary. In this case that item will be returned multiple times from an
     * iterator created by this class. Specifying <code>false</code> prevents
     * this.
     *
     * @throws IllegalArgumentException if <code>numLevels</code> is less than
     * 1.
    ...
    /**
     * Add a named item to the quadtree. Any item duplicates are duplicated in
     * the tree. Any name duplicates replace the current name association; the
     * name then refers to the item added.
     *
     ...
     *
     * @return the named item, or null if the item is not in the tree or the
    ...
    /**
     * Returns an iterator over the items in the tree. There is no specific
     * iteration order and the iterator may return duplicate entries.
     * <p>
     * <em>Note</em> The {@link java.util.Iterator#remove()} operation is not
     * supported.
     *
    ...
    synchronized public Iterator<T> iterator() {
        return new Iterator<T>() {
            ...
            /**
             * This operation is not supported and will produce a
    ...
    /**
     * Finds and returns the items within a tree cell containing a specified
     * location.
     *
     * @param location the location of interest.
     * @param outItems a {@link Set} in which to place the items. If null, a new
     * set is created.
     *
     * @return the set of intersecting items. The same set passed as the
     * <code>outItems</code> argument is returned, or a new set if that argument
     * is null.
     *
    ...
    /**
     * Finds and returns the items within tree cells containing specified
     * locations.
     *
    ...
    /**
     * Finds and returns the items intersecting a specified collection of
     * {@link gov.nasa.worldwind.terrain.SectorGeometry}. This method is a
     * convenience for finding the items intersecting the current visible
     * regions.
     *
    ...
    /**
     * Adds the items identified by a list of bit IDs to the set returned by the
     * get methods.
     *
     * @param bitIds the bit numbers of the cells containing the items to
     * return.
     * @param outItems a {@link Set} in which to place the items. If null, a new
     * set is created.
     *
     * @return the set of items. The value passed as the <code>outItems</code>
     * is returned.
     */
