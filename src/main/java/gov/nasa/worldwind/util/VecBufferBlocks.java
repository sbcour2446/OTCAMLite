...
/**
 * VecBufferBlocks provides storage and retrieval of a set of potentially random
 * VecBuffer blocks in a single backing ByteBuffer. Its is assumeed that the
 * backing ByteBuffer already contains the necessary data, and the caller
 * defines which regions in the ByteBuffer define each individual VecBuffer.
 * VecBuffers are defined by calling {@link
 * #addBlock(int, int)}, where the positions define the byte range containing a
 * VecBuffer's data.
 *
 * @author dcollins
...
public class VecBufferBlocks extends CompoundVecBuffer {
    ...
    /**
     * Constructs a VecBufferBlocks with the specified number of coordinates per
     * logical vector, primitive data type, backing ByteBuffer and the specified
     * initial capacity. This interprets the ByteBuffer according to the
     * specified primitive data type and number of coordinates per logical
     * vector, and does not modify the ByteBuffer in any way. This assumes the
     * buffer's position and limit are not changed by the caller for the
     ...
     * of sub-buffers.
     *
     * @throws IllegalArgumentException if the coordsPerVec is less than 1, if
     * the dataType is null, if the buffer is null, or if the capacity is less
     * than 1.
     */
    ...
    /**
     * Constructs a VecBufferBlocks with the specified number of coordinates per
     * logical vector, primitive data type, and backing ByteBuffer. This
     * interprets the ByteBuffer according to the specified primitive data type
     * and number of coordinates per logical vector, and does not modify the
     * ByteBuffer in any way. This assumes the buffer's position and limit are
     * not changed by the caller for the lifetime of this instance.
     *
     * @param coordsPerVec the number of coordinates per logical vector.
     * @param dataType the primitive data type.
     * @param buffer the backing ByteBuffer.
     *
     * @throws IllegalArgumentException if the coordsPerVec is less than 1, if
     * the dataType is null, or if the buffer is null.
     */
    ...
    /**
     * Returns the primitive data type used to interpret the elements of this
     * VecBufferBlocks' backing ByteBuffer.
     *
    ...
    /**
     * Adds a range of bytes that define a new sub-buffer within this
     * VecBufferBlocks. This contents of this buffer's backing ByteBuffer is not
     * changed, nor are its position and limit. The specified range must define
     * a sequence of bytes representing logical vector elements according to
     * this buffer's number of coordinates per logical vector, and this buffer's
     * primitive data type.
     *
     ...
     *
     * @throws IllegalArgumentException if either the position are less than
     * zero, if either position is greater than the backing buffer's capacity,
     * or if the begin position is greater than the end position.
     */
