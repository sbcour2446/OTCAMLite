...
/**
 * A collection of 32-bit integer primitives that contains no duplicate
 * elements. IntSet provides the minimal operations for working with on a set of
 * integers: add, remove, contains, and clear. Additionally, IntSet provides
 * methods for determining the number of integers in the set, and for retrieving
...
public class IntSet {
    ...
    /**
     * Creates an empty IntSet with the default number of buckets and initial
     * bucket capacity.
     */
    public IntSet() {
    ...
    /**
     * Creates an empty IntSet with the specified number of buckets and initial
     * bucket capacity. Both the number of buckets and the initial bucket
     * capacity must be greater than zero. For optimal performance with more
     * than a few unique values, the number of buckets should be configured to a
     * large value such as 128. The bucket initial capacity does not
     * significantly affect performance, as each bucket eventually grows to fit
     * its entries.
     *
     ...
     *
     * @throws IllegalArgumentException if either numBuckets or
     * bucketInitialCapacity is less than 1.
     */
    ...
    /**
     * Adds the specified value to this set. If this set does not contain the
     * value, it is added to this set and this returns true. Otherwise this does
    ...
    /**
     * Removes the specified value from this set. If this set does not contain
     * the value, this does nothing and returns false. Otherwise this removes
    ...
    /**
     * Removes all of the values from this set. This set is empty after this
     * call returns.
     */
    public void clear() {
    ...
    /**
     * Returns the values in this set as a 32-bit integer array. The values are
     * stored in the specified array if it has enough room. If the array is
     * <code>null</code> or is not large enough, this allocates and returns a
     * new array with length equal to this set's size.
     *
     * @param array the array into which the values are stored.
     *
     * @return the array of values in this set, or a new array if the specified
     * array is <code>null</code> or not large enough.
     */
