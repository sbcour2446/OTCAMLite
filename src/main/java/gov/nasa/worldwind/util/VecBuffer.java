...
/**
 * VecBuffer provides an logical interface on {@link BufferWrapper} to interpret
 * its contents as a series of vector tuples (rather than individual primitive
 * types). The number of coordinates in each logical vector is specified by the
 * property <code>coordsPerElem</code>. For example, if a VecBuffer is composed
 * of (x, y, z) tuples then coordsPerElem would be 3.
 *
 * @author dcollins
...
public class VecBuffer {
    ...
    /**
     * Constructs a new VecBuffer with the specified vector size, and backing
     * BufferWrapper.
     *
     * @param coordsPerVec the number of coordinates per logical vector.
     * @param buffer the backing BufferWrapper.
     *
     * @throws IllegalArgumentException if coordsPerElem is 0 or negative, or if
     * the buffer is null.
     */
    ...
    /**
     * Returns the empty VecBuffer. The returned VecBuffer has no backing
     * buffer, and is immutable.
     *
     * @param coordsPerVec the number of coordinates per logical vector.
     *
    ...
    /**
     * Returns the vector element at the specified position. The position is a
     * logical vector position, position n corresponds to the buffer's nth
     * vector. If the specified array length is smaller than the logical vector
     * size, only the specified portion of the vector element is returned.
     *
     ...
     *
     * @throws IllegalArgumentException if the position is out of range, or if
     * the array is null.
     */
    ...
    /**
     * Sets the vector element at the specified position. The position is a
     * logical vector position, position n corresponds to the buffer's nth
     * vector. If the specified array length is smaller than the logical vector
     * size, only the specified portion of the vector element is set.
     *
     * @param position the logical vector position.
    ...
    /**
     * Sets the vector elements starting at the specified position, and ending
     * at the specified position + count. The position is a logical vector
     * position, position n corresponds to the buffer's nth vector. The array
     * must have sufficient length to represent count separate logical vectors
     * (each with size equal to coordsPerVec) tightly packed into the array,
     * starting at index 0.
     *
     ...
     *
     * @throws IllegalArgumentException if the position is out of range, if the
     * array is null, or if the array has insufficient length.
     */
    ...
    /**
     * Returns a new VecBuffer which is a subsequence of this buffer. The new
     * buffer starts with the vector at the specified position, and has the
     * specified length. The two buffers share the same backing store, so
     * changes to this buffer are reflected in the new buffer, and visa versa.
     *
    ...
    /**
     * Sets a subsequence of this buffer with the contents of the specified
     * buffer. The subsequence to set starts with the vector at the specified
     * position, and has size equal to the specified buffer's size. The
     * specified buffer must have the same logical vector size as this buffer
     ...
     *
     * @throws IllegalArgumentException if the position is out of range, if the
     * buffer is null or incompatible, or if this buffer has insufficient length
     * to store the sub-buffer at the specified position.
     */
     ...
     * Sets a subsequence of this buffer with the contents of the specified
     * buffer. The subsequence to set starts with the vector at the specified
     * position, and has size equal to the specified size. The specified buffer
     * must have the same logical vector size as this buffer (coordsPerVec must
     * be equivalent).
     *
     * @param position the starting vector position to set.
     * @param buffer the input buffer.
     * @param offset the vector position to start copying values from the
     * specified buffer.
     * @param size the number of vectors to read copy form the specified buffer.
     *
     * @throws IllegalArgumentException if the position is out of range, if the
     * buffer is null or incompatible, if this buffer has insufficient length to
     * store the sub-buffer at the specified position, or if the specified
     * offset and size define a range outside of the specified buffer.
     */
    ...
    /**
     * Returns the vector element at the specified position, as a {@link Vec4}.
     * This buffer's logical vector size must be either 2, 3 or 4.
     *
     * @param position the logical vector position.
     *
    ...
    /**
     * Sets the vector element at the specified position, as a Vec4. This
     * buffer's logical vector size must be either 2, 3 or 4.
     *
     * @param position the logical vector position.
     * @param vec the vector to set.
     *
     * @throws IllegalArgumentException if the position is out of range, if the
     * vector is null, or if this buffer cannot store a Vec4.
     */
     ...
     * {@link LatLon}. This buffer's logical vector size must be at least 2.
     *
     * @param position the logical vector position.
     *
    ...
    /**
     * Sets the vector element at the specified position, as a geographic
     * LatLon. This buffer's logical vector size must be at least 2.
     *
     * @param position the logical vector position.
     * @param ll the geographic location to set.
     *
     * @throws IllegalArgumentException if the position is out of range, if the
     * LatLon is null, or if this buffer cannot store a LatLon.
     */
    ...
    /**
     * Returns the vector element at the specified position, as a geographic
     * {@link Position}. This buffer's logical vector size must be at least 2.
     *
     * @param position the logical vector position.
     *
     ...
     *
     * @throws IllegalArgumentException if the position is out of range, or if
     * this buffer cannot store a Position.
     */
    ...
    /**
     * Sets the vector element at the specified position, as a geographic
     * Position. This buffer's logical vector size must be at least 2.
     *
     * @param position the logical vector position.
     * @param p the geographic Position to set.
     *
     * @throws IllegalArgumentException if the position is out of range, if the
     * Position is null, or if this buffer cannot store a Position.
     */
    ...
    /**
     * Returns a copy of this VecBuffer with the specified new size. The new
     * size must be greater than or equal to this VecBuffer's size. If the new
     * size is greater than this buffer's size, this returns a new buffer which
     * is partially filled with the contents of this buffer. The returned
     * VecBuffer has the same number of coordinates per tuple and the same
     * backing buffer type, but its contents are independent from this
     * VecBuffer.
     *
    ...
    /**
     * Returns an iterator over this buffer's logical vectors, as double[]
     * coordinate arrays. The array returned from each call to Iterator.next()
     * will be newly allocated, and will have length equal to coordsPerVec.
     *
    ...
    /**
     * Returns an iterator over this buffer's logical vectors, as double[]
     * coordinate arrays. The array returned from a call to Iterator.next() will
     * be newly allocated, and will have length equal to coordsPerVec or
     * minCoordsPerVec, whichever is larger. If minCoordsPerVec is larger than
     * coordsPerVec, then the elements in the returned array will after index
     * "coordsPerVec - 1" will be undefined.
     *
     * @param minCoordsPerVec the minimum number of coordinates returned in each
     * double[] array.
     *
     * @return iterator over this buffer's vectors, as double[] arrays.
     */
    ...
    /**
     * Returns a reverse iterator over this buffer's logical vectors, as
     * double[] coordinate arrays. The array returned from a call to
     * Iterator.next() will be newly allocated, and will have length equal to
     * coordsPerVec or minCoordsPerVec, whichever is larger. If minCoordsPerVec
     * is larger than coordsPerVec, then the elements in the returned array will
    ...
    /**
     * Sets a subsequence of this buffer with the contents of the specified
     * Iterable. The subsequence to set starts with the vector at the specified
     * position, and has size equal to the number of elements in the Iterable or
     * the number of remaining vectors in the buffer, whichever is less.
     *
     * @param position the starting vector position to set.
    ...
    /**
     * Returns an iterator over this buffer's logical vectors, as Vec4
     * references.
     *
    ...
    /**
     * Returns a reverse iterator over this buffer's logical vectors, as Vec4
     * references.
     *
     ...
     * Iterable. The subsequence to set starts with the vector at the specified
     * position, and has size equal to the number of elements in the Iterable or
     * the number of remaining vectors in the buffer, whichever is less. This
     * buffer's logical vector size must be either 2, 3 or 4.
     *
     * @param position the starting vector position to set.
     * @param iterable iterator over the elements to set.
     *
     * @throws IllegalArgumentException if the position is out of range, if the
     * iterable is null, or if this buffer cannot store a Vec4.
     */
    ...
    /**
     * Returns an iterator over this buffer's logical vectors, as LatLon
     * locations.
     *
    ...
    /**
     * Returns a reverse iterator over this buffer's logical vectors, as LatLon
     * locations.
     *
     ...
     * position, and has size equal to the number of elements in the Iterable or
     * the number of remaining vectors in the buffer, whichever is less. This
     * buffer's logical vector size must be at least 2.
     *
     * @param position the starting vector position to set.
     * @param iterable iterator over the elements to set.
     *
     * @throws IllegalArgumentException if the position is out of range, if the
     * iterable is null, or if this buffer cannot store a LatLon.
     */
    ...
    /**
     * Returns an iterator over this buffer's logical vectors, as geographic
     * Positions.
     *
     ...
     *
     * @return reverse iterator over this buffer's vectors, as geographic
     * Positions.
     */
    ...
    /**
     * Binds this buffer as the source of color values to use when rendering
     * OpenGL primitives. The color type is equal to buffer's underlying
     * BufferWrapper GL type, the stride is 0, and the vertex data itself is
     * this buffer's backing NIO {@link Buffer}. This buffer's vector size must
     * be 3, or 4.
     *
     * @param dc the current {@link DrawContext}.
     *
     * @throws IllegalArgumentException if the DrawContext is null, or if this
     * buffer is not compatible as a color buffer.
     */
    ...
    /**
     * Binds this buffer as the source of normal coordinates to use when
     * rendering OpenGL primitives. The normal type is equal to buffer's
     * underlying BufferWrapper GL type, the stride is 0, and the vertex data
     * itself is this buffer's backing NIO {@link Buffer}. This buffer's vector
     * size must be 3.
     *
     * @param dc the current {@link DrawContext}.
     *
     * @throws IllegalArgumentException if the DrawContext is null, or if this
     * buffer is not compatible as a normal buffer.
     */
    ...
    /**
     * Binds this buffer as the source of vertex coordinates to use when
     * rendering OpenGL primitives. The vertex size is equal to coordsPerVertex,
     * the vertex type is equal to buffer's underlying BufferWrapper GL type,
     * the stride is 0, and the normal data itself is this buffer's backing NIO
     * Buffer. This buffer's vector size must be 2, 3, or 4.
     *
     * @param dc the current DrawContext.
     *
     * @throws IllegalArgumentException if the DrawContext is null, or if this
     * buffer is not compatible as a vertex buffer.
     */
    ...
    /**
     * Binds this buffer as the source of texture coordinates to use when
     * rendering OpenGL primitives. The texture coordinate size is equal to
     * coordsPerVertex, the texture coordinate type is equal to buffer's
     * underlying BufferWrapper GL type, the stride is 0, and the texture
     * coordinate data itself is this buffer's backing NIO Buffer. This buffer's
     * vector size must be 1, 2, 3, or 4.
     *
     * @param dc the current DrawContext.
     *
     * @throws IllegalArgumentException if the DrawContext is null, or if this
    ...
    /**
     * Renders <code>getSize()</code> elements from the currently bounds OpenGL
     * coordinate buffers, beginning with element 0. The specified drawMode
