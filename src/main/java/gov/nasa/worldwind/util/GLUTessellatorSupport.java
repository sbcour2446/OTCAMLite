 ...
 * {@link com.jogamp.opengl.glu.GLUtessellator} to tessellate complex polygons
 * into triangles.
 * <p>
 * The standard pattern for using GLUTessellatorSupport to prepare a
 * GLUtessellator is as follows: <code>
 * GLUTessellatorSupport glts = new GLUTessellatorSupport();<br>
 * GLUtessellatorCallback cb = ...; // Reference to an implementation of
 * GLUtessellatorCallback.<br> Vec4 normal = new Vec4(0, 0, 1); // The polygon's
 * normal. This example shows an appropriate normal for tessellating x-y
 * coordinates.<br> <br><br> glts.beginTessellation(cb, new Vec4(0, 0, 1));<br>
 * try<br> {<br> GLUtessellator tess = glts.getGLUtessellator();<br> }<br>
 * finally<br> {<br>
...
public class GLUTessellatorSupport {
     ...
     * {@link com.jogamp.opengl.glu.GLUtessellator} instance. This returns a
     * valid GLUtessellator instance if called between {@link #beginTessellation(com.jogamp.opengl.glu.GLUtessellatorCallback,
     * gov.nasa.worldwind.geom.Vec4)} and {@link #endTessellation()}. This
     * returns null if called from outside a beginTessellation/endTessellation
     * block.
     *
     * @return the internal GLUtessellator instance, or null if called from
     * outside a beginTessellation/endTessellation block.
     */
     ...
     * This initializes the internal GLUtessellator to a new instance by
     * invoking {@link com.jogamp.opengl.glu.GLU#gluNewTess()}, and configures
     * the tessellator with the specified callback and normal with calls to {@link com.jogamp.opengl.glu.GLU#gluTessCallback(com.jogamp.opengl.glu.GLUtessellator,
    ...
    /**
     * Frees any GLU resources used by this GLUTessellatorSupport, and
     * invalidates this instance's internal GLU tessellator.
     */
    ...
    /**
     * Creates a new {@link com.jogamp.opengl.glu.GLUtessellatorCallback} that
     * draws tessellated polygons as OpenGL primitives by calling glBegin,
     * glEnd, and glVertex.
     *
     * @param gl the GL context to draw into.
     *
     * @return a new GLUtessellatorCallback for drawing tessellated polygons as
     * OpenGL primtives.
     *
    ...
    /**
     * Converts the specified GLU tessellator error number to a string
     * description. This returns "unknown" if the error number is not
     * recognized.
     *
    ...
    /**
     * Provides the callback class used to capture the shapes determined by the
     * tessellator.
     */
...
//            System.out.println("COMBINE CALLED");
    ...
    /**
     * Provides a container for associating a tessellator's vertex with its
     * index and application-specified edge flag.
     */
    ...
    /**
     * Provides the callback class used to capture triangle and line primitive
     * indices determined by the tessellator.
     */
    ...
    /**
     * Recursively forwards boundary tessellation results from one GLU
     * tessellator to another. The GLU tessellator this callback forwards to may
     * be configured in any way the caller chooses.
     * <p>
     * RecursiveCallback must be used as the GLUtessellatorCallback for the
     * begin, end, vertex, and combine callbacks for a GLU tessellator
     * configured to generate line loops. A GLU tessellator can be configured
     * generate line loops by calling gluTessProperty(GLU_TESS_BOUNDARY_ONLY,
     * GL_TRUE). Additionally, the caller specified vertex data passed to
     * gluTessVertex must be a double array containing three elements - the x, y
     * and z coordinates associated with the vertex.
     */
    public static class RecursiveCallback extends GLUtessellatorCallbackAdapter {

        /**
         * The GLU tessellator that receives the tessellation results sent to
         * this callback.
         */
        protected GLUtessellator tess;

        /**
         * Creates a new RecursiveCallback with the GLU tessellator that
         * receives boundary tessellation results.
         *
         * @param tessellator the GLU tessellator that receives the tessellation
         * results sent to this callback. This tessellator may be configured in
         * any way the caller chooses, but should be prepared to receive contour
         * input from this callback.
         *
         * @throws java.lang.IllegalArgumentException if the tessellator is
         * null.
         */
        ...
        /**
         * Called by the GLU tessellator to indicate the beginning of a new line
         * loop. This recursively begins a new contour with the GLU tessellator
         * specified during construction by calling
        ...
        /**
         * Called by the GLU tessellator to indicate the next vertex of the
         * current contour. The vertex data must be a double array containing
         * three elements - the x, y and z coordinates associated with the
         * vertex. This recursively indicates the next contour vertex with the
         * GLU tessellator specified during construction by calling
         * gluTessVertex(tessellator, (double[]) vertexData, 0, vertexData).
         *
         * @param vertexData the caller specified vertex data. Must be a double
         * array containing three elements - the x, y and z coordinates
        ...
        /**
         * Called by the GLU tessellator to indicate the end of the current line
         * loop. This recursively ends the current contour with the GLU
         * tessellator specified during construction by calling
        ...
        /**
         * Called by the GLU tessellator to indicate that up to four vertices
         * must be merged into a new vertex. The new vertex is a linear
         * combination of the original vertices. This assigns the first element
         * of outData to coords, the coordinates of the new vertex.
         *
         * @param coords A three element array containing the x, y and z
         * coordinates of the new vertex.
         * @param vertexData The caller specified vertex data of the original
         * vertices.
         * @param weight The coefficients of the linear combination. These
         * weights sum to 1.
         * @param outData A one element array that must contain the caller
         * specified data associated with the new vertex after this method
         * returns.
         */
        @Override
        ...
        /**
         * Called by the GLU tessellator when the tessellation algorithm
         * encounters an error. This logs a severe message describing the error.
         *
         * @param errno a GLU enumeration indicating the error.
         */
        @Override
