...
/**
 * HotSpot is an interface for forwarding select, keyboard, and mouse events to
 * picked objects in the {@link
 * gov.nasa.worldwind.WorldWindow}. When the HotSpot is active it receives input
 * events that occur in the WorldWindow.
 *
 * @author dcollins
...
public interface HotSpot extends SelectListener, KeyListener, MouseListener, MouseMotionListener, MouseWheelListener {
    ...
    /**
     * Called when this HotSpot is activated or deactivated. The HotSpot only
     * receives input events when it is active.
     *
     * @param active {@code true} if this HotSpot is being activated.
     * {@code false} if this HotSpot is being deactivated.
     */
    ...
    /**
     * Returns the AWT {@link java.awt.Cursor} representation to display when
     * the HotSpot is active, or {@code null} to use the default Cursor.
     *
     * @return The Cursor representation associated with the HotSpot. May return
     * {@code null}.
     */
    Cursor getCursor();
}
