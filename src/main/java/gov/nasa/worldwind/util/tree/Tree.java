 ...
 * How the tree is drawn is determined by the {@link TreeLayout}.
 *
 * @author pabercrombie
...
public interface Tree extends WWObject, OrderedRenderable {
    ...
    /**
     * Make a node in the tree visible in the rendered tree. For example, scroll
     * the tree viewport so that a path is visible.
     *
    ...
    /**
     * Expand a collapsed path, or collapse an expanded path. Has no effect on
     * leaf nodes.
     *
     * @param path Path to operate on. If the node defined by {@code path} is
     * expanded, it will be collapsed. If it is collapsed it will be expanded.
     */
     ...
     *
     * @return true if the path is expanded, false if collapsed. Always returns
     ...
     *
     * @return true if the node is expanded, false if collapsed. Always returns
     * false for leaf nodes.
     */
