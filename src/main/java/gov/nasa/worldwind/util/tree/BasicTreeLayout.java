...
public class BasicTreeLayout extends WWObjectImpl implements TreeLayout, Scrollable, PreRenderable {
     ...
     * {@link #scrollToNode(gov.nasa.worldwind.render.DrawContext)} during
     * rendering.
     */
    ...
    /**
     * Indicates that a triangle must be drawn to indicate if a group node is
     * expanded or collapsed.
     */
    protected boolean drawNodeStateSymbol = true;
    /**
     * Indicates that a checkbox must be drawn for each node to indicate if the
     * node is selected or not.
     */
    protected boolean drawSelectedSymbol = true;

    /**
     * Indicates whether or not the description text will be wrapped to fit the
     * frame.
     */
    protected boolean wrapText = true;
    /**
     * Maximum number of lines of wrapped description text to draw. If the
     * description exceeds this it will be cut off at this number of lines, with
     * a trailing "...".
     */
    ...
    /**
     * Cache of node layouts. This list is populated when the tree layout is
     * computed.
     */
    protected java.util.List<NodeLayout> treeNodes = new ArrayList<NodeLayout>();

    /**
     * A little extra space is added to the tree dimensions to give the tree a
     * little bit of separation from the scrollable frame. This value determines
     ...
     * @param y Y coordinate of the upper left corner of the tree frame,
     ...
     *
     * @param tree Tree to create layout for.
     * @param screenLocation The location of the upper left corner of the tree
     * frame. The offset is interpreted relative to the lower left corner of the
     * screen.
     */
    ...
    /**
     * Indicates whether or not the layout wraps the node description to
     * multiple lines. Note that the node title is never wrapped, only the
     * description.
     *
    ...
    /**
     * Specifies whether or not the layout wraps the node description to
     * multiple lines. Note that the node title is never wrapped, only the
     * description.
     *
     * @param wrapText {@code true} if the description text must be wrapped to
     * fit the frame.
     */
    ...
    /**
     * Get the size of the symbol that indicates that a node is expanded or
     * collapsed.
     *
    ...
    /**
     * Set the renderer to renderer node descriptions (additional text rendered
     * under the node title).
     *
     * @param showDescription True if the description should be rendered. False
     * if only the icon and title should be rendered.
     */
     ...
     * The default symbol is a checkbox.
     *
     * @return True if the node selected symbol (a checkbox by default) will be
     * drawn.
     */
    ...
    /**
     * Set whether or not the renderer will draw a symbol to indicate that the
     * node is selected. The default symbol is a checkbox.
     *
     * @param drawSelectedSymbol True if the node selected symbol (a checkbox by
     * default) will be drawn.
     */
    ...
    /**
     * Will the renderer draw a symbol to indicate that the node is expanded or
     * collapsed (applies only to non-leaf nodes). The default symbol is a
     * triangle pointing to the right, for collapsed nodes, or down for expanded
     * nodes.
     *
     * @return True if the node state symbol (default is a triangle pointing
     * either to the right or down) will be drawn.
     */
    ...
    /**
     * Specifies the maximum number of lines of text wrapped description text to
     * draw. If the description exceeds this number of lines it will be cut off
     * with a trailing "...".
     *
    ...
    /**
     * Indicates the maximum number of lines of text wrapped description text to
     * draw. If the description exceeds this number of lines it will be cut off
     * with a trailing "...".
     *
     * @param maxLines Maximum number of lines of description text that will be
     * drawn.
     */
    ...
    /**
     * Set whether or not the renderer will draw a symbol to indicate that the
     * node is expanded or collapsed (applies only to non-leaf nodes). The
     * default symbol is a triangle pointing to the right, for collapsed nodes,
     * or down for expanded nodes.
     *
     * @param drawNodeStateSymbol True if the node state symbol (default is a
     * triangle pointing either to the right or down) will be drawn.
     */
    ...
    /**
     * Get the size of the entire tree, including the part that is not visible
     * in the scroll pane.
     *
     * @param dc Draw context.
     * @param frameSize Size of the frame the tree will be rendered into. May be
     * {@code null}.
     *
    ...
    /**
     * Compute the size of a tree. This method invokes itself recursively to
     * calculate the size of the tree, taking into account which nodes are
     * expanded and which are not. This computed size will be stored in the
     * {@code size} parameter.
     *
     * @param tree Tree that contains the root node.
     * @param root Root node of the subtree to find the size of. This does not
     * need to be the root node of the tree.
     * @param dc Draw context.
     * @param frameSize Size of the frame into which the tree will render.
     * @param size Size object to modify. This method will change the width and
     * height fields of {@code size} to hold the new size of the tree.
     * @param x Horizontal coordinate of the start of this node. This parameter
     * must be zero. This method calls itself recursively and changes the
     * {@code x} parameter to reflect the indentation level of different levels
     * of the tree.
     * @param level Level of this node. Tree root node is level 1, children of
     * the root are level 2, etc.
     */
    ...
    /**
     * Determine if a node needs to be displayed. This method examines only one
     * node at a time. It does not take into account that the node's parent may
     * be in the collapsed state, in which the children are not rendered.
     *
     * @param node Node to test.
     * @param level Level of the node in the tree. The root node is level 1, its
     * children are level 2, etc.
     *
    ...
    /**
     * Scroll the frame to make a the node set in {@link #scrollToNode} node
     * visible. Does nothing if {@link
     ...
     *
     * @param frameSize Size of the frame that holds the tree. Size may be null
     * if the frame size is not known.
     *
     * @return {@code true} if the size needs to be recomputed, otherwise
     * {@code false}.
     */
     ...
     *
     * @param dc Current draw context.
     * @param drawPoint Point in GL coordinates (origin bottom left corner of
     * the screen) that locates the bottom left corner of the tree.
     * @param nodes Nodes to draw.
     * @param clipBounds Pixels outside of this rectangle will be discarded. Any
     * nodes that do not intersect this rectangle will not be drawn.
     */
...
//            gl.glEnd();
     ...
     * @param scrollBounds bounds of the area currently visible in the scroll
     * frame.
     *
    ...
    /**
     * Draw the description text for tree nodes. The description text is drawn
     * under the main line of text.
     *
     * @param dc Current draw context.
     * @param nodes List of visible nodes.
     */
    ...
    /**
     * Draw check boxes. Each box includes a check mark is the node is selected,
     * or is filled with a gradient if the node is partially selected.
     *
     * @param dc Current draw context.
     * @param nodes List of visible nodes.
     */
    ...
    /**
     * Determine the tree layout. This method determines which nodes are
     * visible, and where they will be drawn.
     *
     * @param root Root node of the subtree to render.
     * @param dc Draw context.
     * @param frameSize Size of the frame into which the tree will render.
     * @param location Location at which to draw the node. The location
     * specifies the upper left corner of the subtree.
     * @param level The level of this node in the tree. The root node is at
     * level 1, its child nodes are at level 2, etc.
     * @param nodes List to collect nodes that are currently visible. This
     * method adds nodes to this list.
     */
     ...
     * @param location Point in OpenGL screen coordinates (origin lower left
     * corner) that defines the upper left corner of the subtree.
     * @param level Level of this subtree in the tree. The root node is level 1,
     * its children are level 2, etc.
     *
     ...
     *
     * @return Highlight attributes if the tree is highlighted. Otherwise, the
     * normal attributes.
     */
     ...
     * @param dc Current draw context.
     * @param frameSize Size of the frame into which the tree is rendered.
     * @param x Offset in pixels from the left side of the screen to the left
     * most part of the node.
    ...
    /**
     * Get the wrapped description text for a node. The wrapped text will be
     * cached in the {@link #layoutCache}.
     *
     * @param dc Current draw context.
     ...
     *
     * @return The wrapped text as a String. The string will contain newline
     * characters to delimit the lines of wrapped text.
     */
    ...
    /**
     * Invalidate the computed wrapped text, forcing the text wrap to be
     * recomputed.
     */
    ...
    /**
     * Create a pickable object to represent a toggle control in the tree. The
     * toggle control will expand or collapse a node in response to user input.
     *
     * @param tree Tree that contains the node.
     * @param node The node to expand or collapse.
     *
     * @return A {@link TreeHotSpot} that will be added as a pickable object to
     * the screen area occupied by the toggle control.
     */
    ...
    /**
     * Create a pickable object to represent selection control in the tree. The
     * selection control will select or deselect a node in response to user
     * input. The returned <code>HotSpot</code> calls <code>{@link
     ...
     * com.jogamp.opengl.util.awt.TextRenderer.getBounds for information on how
     * this rectangle should be interpreted.
     */
    ...
    /**
     * Get the bounds of a multi-line text string. Each newline character in the
    ...
    /**
     * Toggles the selection state of the specified <code>node</code>. In order
     * to provide an intuitive tree selection model to the application, this
     * changes the selection state of the <code>node</code>'s ancestors and
     * descendants as follows:
     * <ul> <li>The branch beneath the node it also set to the node's new
     * selection state. Toggling an interior node's selection state causes that
     * entire branch to toggle.</li> <li>The node's ancestors are set to match
     * the node's new selection state. If the new state is <code>false</code>,
     * this stops at the first ancestor with another branch that has a selected
     * node. When an interior or leaf node is toggled, the path to that node is
     * also toggled, except when doing so would clear a selected path to another
     * interior or leaf node.</li> </ul>
     * <p>
     *
     * @param node the <code>TreeNode</code> who's selection state should be
     * toggled.
     */
     ...
     * <code>node</code>.
     *
     * @param node the <code>TreeNode</code> who descendants selection should be
     * set.
     * @param selected <code>true</code> to mark the descendants and selected,
     ...
     * @param node Node to get text for.
     *
     * @return Description text for {@code node}. May return null if there is no
     * description.
     */
    ...
    protected static class NodeLayout {
        ...
        /**
         * Node bounds relative to the bottom left corner of the viewport. This
         * field is set by {@link
        ...
        /**
         * Wrapped version of the node description text. Computed once and then
         * cached here.
         */
        ...
        /**
         * Point at which the next component should be drawn. Nodes are drawn
         * left to right, and the draw point is updated as parts of the node are
         * rendered. For example, the toggle triangle is drawn first at the draw
         * point, and then the draw point is moved to the right by the width of
         * the triangle, so the next component will draw at the correct point.
         * The draw point is reset to the lower left corner of the node bounds
         ...
         *
         * @param treePoint location of the lower left corner of the tree,
         * measured in GL coordinates (origin lower left corner of the screen).
         */
