...
/**
 * A frame that can scroll its contents. The frame can be interactively resized
 * by dragging the border, and be moved by dragging the frame or title bar. The
 * frame can be minimized. The frame displays scroll bars if the size of the
 * content exceeds the size of the frame, and optionally displays a title bar
 * with a text string and an icon.
 * <p>
 * The frame renders its contents into a texture, and then draws the texture
 * when the frame is rendered. This provides good performance for content that
 * is expensive to draw, and changes infrequently. If the frame is sized so
 * large that the visible portion of the contents cannot be rendered into a
 * single texture, then the contents will be drawn directly when the frame is
 * rendered.
 *
 * @author pabercrombie
...
public class ScrollFrame extends DragControl implements PreRenderable, Renderable {
    ...
    /**
     * Default delay (in milliseconds) between frame when the frame is
     * animating.
     */
    ...
    /**
     * The full frame title. This title may be displayed in a truncated form if
     * the frame is too small to accommodate the full title.
     */
    ...
    /**
     * Indicates whether or not to draw a title bar in the frame. Default is
     * true.
     */
    protected boolean drawTitleBar = true;
    /**
     * Indicates whether or not the user can resize the frame by dragging the
     * edge. Default is true.
     */
    protected boolean enableResize = true;
    /**
     * Indicates whether or not the user can move the frame by dragging with the
     * mouse. Default is true.
     */
    ...
    /**
     * The maximum size of the frame. This is a constraint applied to the
     * frame's size.
     */
    ...
    /**
     * Texture for the icon displayed in the frame title bar. Loaded from
     * {@link #iconImageSource}.
     */
    ...
    /**
     * Indicates whether the contents should be rendered into a texture and
     * cached, or rendered directly. The frame renders into a texture if the
     * frame size can be accommodated by a single texture. If the size is too
     * large to render using a single texture, the contents are drawn directly.
     */
    protected boolean renderToTexture;

    /**
     * Cache key used to locate the rendering texture in the DrawContext's
     * texture cache.
     */
    protected final Object textureCacheKey = new Object();

    /**
     * Map that associates logical tiles in the scrollable content with
     * allocated tiles in the texture used to render the content.
     */
    ...
    /**
     * Dimension of the texture used to render the scrollable content. Must be a
     * power of two.
     */
    protected int textureDimension;
    /**
     * Dimension of a sub-tile in the rendering texture. Also the dimension of
     * logical tiles in the scrollable content that are rendered into the
     * texture tiles.
     */
    ...
    // title font changes.
    /**
     * Frame title that is actually drawn in the title bar. If the frame is too
     * narrow to display the entire title, the title will be truncated. This
     * title must be regenerated if the frame size or the title font change.
     */
    ...
    /**
     * Indicates that the frame must be regenerated because the size or
     * attributes have changed.
     */
    protected boolean mustRecomputeFrameGeometry = true;
    /**
     * Indicates the location of the upper left corner of the frame, in AWT
     * coordinates (origin at the upper left corner of the screen.
     */
    ...
    /**
     * Total size of the frame content. This size may exceed the size of the
     * frame, in which case scroll bars will be displayed.
     */
     ...
     *
     * @param x x coordinate of the upper left corner of the frame, in AWT
     * screen coordinates (origin upper left corner of the screen).
     * @param y y coordinate of the upper left corner of the frame, in AWT
     * screen coordinates (origin upper left corner of the screen).
     */
     ...
     *
     * @param screenLocation initial location of the upper left corner of the
     * frame.
     */
    ...
    /**
     * Indicates if the frame is minimized. If the frame is not minimized, it is
     * either maximized or animating toward a maximized state.
     *
    ...
    /**
     * Sets the frame to its minimized or maximized state. If the frame is not
     * already in the desired state it will animate to the desired state.
     *
     * @param minimized {@code true} if the frame must be minimized.
     * {@code false} if the frame must not be minimized.
     */
     ...
     * Indicates whether or not the frame is highlighted.
     *
     * @return {@code true} if the frame is highlighted, otherwise
     * {@code false}.
     */
    ...
    /**
     * Get the size of tree frame. The size determines the size of the frame
    ...
    /**
     * Set the size of the frame. The size determines the size of the frame when
    ...
    /**
     * Indicates the size of the minimized tree frame. This size is used when
     * the tree is minimized or animating.
     *
     * @return the size of the minimized frame. {@code null} indicates that
     * there is no minimized size, in which case the normal maximized frame size
     * is used in the minimized state.
     */
    ...
    /**
     * Specifies the size of the minimized tree frame. This size is used when
     * the tree is minimized or animating. An animation can use this field to
     * manipulate the size of the frame as the animation runs without
     * interfering with the frame's normal maximized size.
     *
     * @param size the size of the minimized frame. Set {@code null} to use the
     * same size in maximized and minimized states.
     */
    ...
    /**
     * Indicates the maximum size of frame. This size is applied as a constraint
     * to the tree's normal size (indicated by {@link #getSize()}).
     *
    ...
    /**
     * Specifies the maximum size of the frame. This size is applied as a
     * constraint to the tree's normal size (indicated by {@link #getSize()}).
     *
     * @param size the maximum size of the minimized frame. Set {@code null} for
     * no maximum.
     */
     ...
     * The frame size may change due to a window resize, or an animation.
     *
     * @return The size of the frame on screen, in pixels. This method will
     * return null until the frame has been rendered at least once.
     */
     ...
     *
     * @param enable {@code true} to allow the user to resize the frame by
     * dragging the border.
     */
     ...
     *
     * @return {@code true} if the user can allowed to move the frame by
     ...
     * Specifies whether the user can move the frame by dragging the title bar.
     *
     * @param enable {@code true} if the user is allowed to move the frame by
     * dragging.
     */
    ...
    /**
     * Set the image source of the frame icon. This icon is drawn in the upper
     * right hand corner of the tree frame.
     *
     * @param imageSource New image source. May be a String, URL, or
     * BufferedImage.
     */
     ...
     *
     * @return The bounds of the tree frame on screen, in screen coordinates
    ...
    /**
     * Get the location of the upper left corner of the frame, measured from the
     * upper left corner of the screen.
     *
     * @return The location of the upper left corner of the frame. This method
     * will return null until the has been rendered.
     */
    ...
    /**
     * Indicates the frame attributes used to draw the frame when it is not
     * highlighted.
     *
    ...
    /**
     * Specifies the frame attributes used to draw the frame when it is not
     * highlighted.
     *
    ...
    /**
     * Indicates the frame attributes used to draw the frame when it is
     * highlighted.
     *
    ...
    /**
     * Specifies the frame attributes used to draw the frame when it is
     * highlighted.
     *
    ...
    //**************************************************************//
    /**
     * Build the list of ContentTile that represents the logical tiles in the
     * frame contents.
     *
     ...
     *
     * @return {@code true} if the tile needs to be updated. Always returns
     * {@code false} if the tile is not visible in the current frame bounds.
     */
     ...
     *
     * @return TextureTile allocated for the given ContentTile, or {@code null}
     * if no TextureTile has been allocated.
     */
    ...
    /**
     * Allocate a texture tile for a ContentTile. There are a limited number of
     * texture tiles. If no tiles are free, the least recently used texture tile
     * will be reallocated.
     *
    ...
    /**
     * Draws the current list of ScrollableTiles into the texture tiles. The
     * tiles are updated when necessary.
     *
     * @param dc the draw context the tile relates to.
     * @param tile the tile to update. A new texture tile will be allocated for
     * the tile, if the tile does not have a texture.
     * @param tileBounds bounds of the tile being updated, within the larger
     * texture.
     */
    ...
    /**
     * Compute the dimension of a texture large enough to represent the amount
     * of the contents visible in the frame.
     *
     * @param frameSize Size of the frame content area.
     * @param contentSize Size of the frame content.
     *
     * @return Dimension of a texture large enough to render the full frame
     * content area. This method always returns a power of two dimension.
     */
     ...
     *
     * @return {@code true} If the frame intersects the frustum, otherwise
     * {@code false}.
     */
    ...
    /**
     * Increment the active animation. Has no effect if there is no active
     * animation. This method must be called once per frame.
     *
     * @param dc Current draw context.
     */
     ...
     *
     * @return Bounds of the desired tile, relative to the lower left corner of
     * {#link contentBounds}.
     */
    ...
    /**
     * Compute the bounds of the content frame, and the extents of the scroll
     * range. Calling this method updates the frame and scroll model to match
     * the size of the scrollable content. Bounds are only computed once per
     * frame. Multiple calls in the same frame will not change the computed
     * bounds.
     *
     * @param dc Current draw context.
     */
    ...
    /**
     * Compute the size of the frame rectangle required to accommodate a given
     * content size without displaying scroll bars.
     *
     * @param contentSize Size of the frame content.
     *
    ...
    /**
     * Determines if the frame size is relative to the size of the scrollable
     * content (as opposed to an absolute pixel value, or a fraction of the
     * viewport).
     *
     * @param size Size to test.
     *
     * @return {@code true} if the absolute size of {@code size} depends on the
     ...
     *
     * @return {@code true} if the vertical scrollbar should be displayed,
     ...
     * @param contentSize The total size, in pixels, of the scrollable content.
     *
     * @return {@code true} if the horizontal scrollbar should be displayed,
     * otherwise {@code false}.
     */
     ...
     *
     * @return {@code true} if an animation is in progress, otherwise
     * {@code false}.
     */
    ...
    /**
     * Compute the content bounds, taking into account the frame size and the
     * presence of scroll bars.
     *
     * @param showVerticalScrollBar True if the frame will have a vertical
     * scroll bar. A vertical scroll bar will make the content frame narrower.
     * @param showHorizontalScrollBar True if the frame will have a horizontal
     * scroll bar. A horizontal scroll bar will make the content frame shorter.
     *
    ...
    /**
     * Updates the frame's screen-coordinate geometry in {@link #vertexBuffer}
     * according to the current screen bounds.
     */
    ...
    /**
     * Get the smallest dimension that the frame can draw itself. This user is
     * not allowed to resize the frame to be smaller than this dimension.
     *
     ...
     *
     * @return {@code true} if the frame should draw minimized, otherwise
     * {@code false}.
     */
    ...
    /**
     * Draw visible scroll bars for the frame. Scroll bars that are not visible
     * will not be drawn.
     *
     * @param dc Current draw context.
     */
    ...
    /**
     * Draw an icon in the upper left corner of the title bar. This method takes
     * a point relative to lower left corner of the title bar. This point is
     * modified to indicate how much horizontal space is consumed by the icon.
     *
     * @param dc Draw context
     * @param drawPoint Point at which to draw the icon. This point is relative
     * to the lower left corner of the title bar. This point will be modified to
     * indicate how much horizontal space was consumed by drawing the icon.
     * After drawing the icon, the x value with point to the first available
     ...
     *
     * @param dc Draw context
     * @param drawPoint Point at which to draw text. This point is relative to
     * the lower left corner of the title bar.
     */
     ...
     *
     * @return {@code true} if the shortened title needs to be regenerated,
     * otherwise {@code false}.
     */
    ...
    /**
     * Generate a shortened version of the frame title that will fit in the
     * frame title area. If the frame is wide enough to accommodate the full
     * title, then the short title will be the same as the full title. The
     * method sets the fields
     * {@link #shortTitle}, {@link #shortFrameTitleFont}, {@link #frameTitleWidth}.
     *
     * @param dc Current draw context.
     * @param frameTitle Full frame title.
     * @param width Width in pixels of the frame title area.
     * @param cutOff String to append to title to indicate that text has been
     * cut off due to a small frame. For example, if the cut off string is
     * "...", the string "Frame Title" might be shortened to "Frame T...".
     */
    ...
    /**
     * Update the attributes of the scroll bars to match the frame's highlight
     * state.
     */
    ...
    /**
     * Indicates the size that applies to the frame for this frame, either the
     * maximumed or the minimized size.
     *
    ...
    /**
     * Get the texture loaded for the icon image source. If the texture has not
     * been loaded, this method will attempt to load it in the background.
     *
     * @return The icon texture, or no image source has been set, or if the icon
    ...
    /**
     * Create and initialize the texture from the image source. If the image is
     * not in memory this method will request that it be loaded and return null.
     *
     ...
     *
     * @param direction Determines which scroll bar to get. Either
     * {@link AVKey#VERTICAL} or {@link AVKey#HORIZONTAL}.
     *
    ...
    /**
     * Handle a mouse wheel event on Mac OS X. This method correctly handles the
     * Magic Mouse and Magic Trackpad devices, which support horizontal
     * scrolling.
     *
     * @param e Mouse event that triggered the scroll.
     */
     ...
     *
     * @return The scroll unit that will be applied when the mouse wheel is
     * moved.
     */
    ...
    /**
     * A tile in the frame content. This class represents one tile in the frame
     * contents, and the time at which that tile was last drawn to texture tile.
     */
    class ContentTile {
    ...
    /**
     * A region of the backing texture used to render one tile of the scrollable
     * content.
     */
    class TextureTile implements Comparable<TextureTile> {
        ...
        /**
         * The last time that this tile was accessed. Used to implement an LRU
         * tile replacement scheme.
         */
        long lastUsed;
        ...
        /**
         * Compare two TextureTiles by the time that the tiles were last
         * accessed.
         *
         * @param that Tile to compare with.
         *
         * @return -1 if this tile was accessed less recently than that tile, 0
         * if the access times are the same, or 1 if this tile was accessed more
         * recently.
         */
