...
public class SessionCacheUtils {
    ...
    /**
     * Retrieves the contents of a specified {@link java.net.URL}. If
     * successful, this places the URL contents in a specified session cache
     * with a specified key. This either marks the resource as available or
     * missing, depending on whether the retrieval succeeds or fails. Finally,
     * this optionally notifies the caller that the retrieval has succeeded by
     * firing a property change event. If either the property listener or
     * property name are null, that functionality is disabled.
     *
     * @param url the URL contents to retrieve.
     * @param cache the cache which receives the retrieved data.
     * @param cacheKey the cache key which identifies where the retrieved data
     * is placed in the session cache.
     ...
     * @param propertyListener the property change listener which is fired when
     * the retrieved data is available.
     * @param propertyName the property name to fire when retrieved data is
     * available.
     *
     * @throws IllegalArgumentException if any of the url, retrieval service,
     * cache, or cache key are null.
     */
    ...
    /**
     * Checks a session cache for a specified key, and if present attempts to
     * interpret the cache entry as a {@link
     * WMSCapabilities} document. If the key does not exist in the cache, or the
     * cache entry cannot be interpreted as a Capabilities document, this
     * returns null. If the entry exists, but must be converted to a
     * Capabilities document, this overrides the previous cache entry with the
     * the newly converted Capabilities.
     *
     * @param cache the session cache.
     * @param cacheKey the key to identify the object in the session cache.
     * @param name the name to use in logging messages.
     *
     * @return the Capabilities document in the session cache, or null if either
     * the key does not match an entry in the cache, or that entry cannot be
     * interpreted as a Capabilities document.
     *
     * @throws IllegalArgumentException if either the cache or cache key are
     * null.
     */
     ...
     * Checks a session cache for a specified key, and if present attempts to
     * interpret the cache entry as a {@link
     * WMSCapabilities} document. If the key does not map to a Capabilities
     * document for any reason, this attempts to asynchronously retrieve the
     * Capabilities from a specified URL, and returns null.
     *
     * @param url the URL contents to retrieve.
     * @param cache the session cache.
     * @param cacheKey the key to identify the object in the session cache.
     ...
     * available.
     *
     * @return the Capabilities document in the session cache, or null if the
     * document is not in the cache.
     *
     * @throws IllegalArgumentException if either the url, retrieval service,
     * cache or cache key are null.
     */
