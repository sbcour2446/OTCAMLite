...
/**
 * Provides services and resources used by XML event parsers during event
 * reading and parsing.
 *
 * @author tag
...
public interface XMLEventParserContext extends AVList {
    ...
    /**
     * Identifies the name of the parser handling unrecognized elements. Can be
     * used to explicitly specify the context's parser-table entry for
     ...
     *
     * @return the new parser, or null if no parser has been registered for the
     * specified event's element name.
     */
     ...
     *
     * @param eventName indicates the element name for which a parser is
     * created.
     *
     * @return the new parser, or null if no parser has been registered for the
    ...
    /**
     * Determines whether an event is the corresponding end element for a
     * specified start event.
     * <p>
     * Note: Only the event's element name and type are compared. The method
     * returns true if the start and end events are the corresponding event
     * types for an element of the same name.
     *
     * @param event the event of interest.
     * @param startElement the start event associated with the potential end
     * event.
     *
     * @return true if the event is the corresponding end event to the specified
     * start event, otherwise false.
     */
     ...
     * @param event the event of interest.
     *
     * @return the event's characters, or null if the event is not a character
     * event.
     */
    ...
    /**
     * Registers a parser for a specified element name. A parser of the same
     * type and namespace is returned when {@link
     * #getParser(javax.xml.stream.events.XMLEvent)} is called for the same
     * element name.
     *
    ...
    /**
     * Indicates whether the event stream associated with this context contains
     * another event.
     *
    ...
    /**
     * Specifies the context's default namespace URI. Must be called prior to
     * initiating the parser table if this context's parsers will be qualified
     ...
     *
     * @return true if both names have the same namespace (or no namespace) and
     * local name, or if either name has no namespace but the namespace of the
     * other is the context's default namespace.
     */
    boolean isSameName(QName qa, QName qb);

    /**
     * Create a parser for a specified event's element name, if a parser for
     * that name is registered with the context.
     *
     * @param event the event whose element name identifies the parser to
     * create.
     * @param defaultParser a parser to return if no parser is registered for
     * the specified name. May be null.
     *
     * @return a new parser, or the specified default parser if no parser has
     * been registered for the element name.
     */
    ...
    /**
     * Returns the table associating objects with their <i>id</i> attribute as
     * specified in the object's KML file.
     *
     ...
     *
     * @param id the object id. If null, this method returns without creating a
     * mapping.
    ...
    /**
     * Resolves references to elements in the same KML file. Certain KML
     * elements such as <i>styleUrl</i> may contain references to other elements
     * within the same KML file. (A leading "#" indicates a reference to an
     * element within the same file.) This method searches the elements in the
     * file for these references and adds the object for the element they refer
     * to the referring object's field table.
     *
     * @param referenceName the element name of the elements whose references
     * this method resolves. An example is
     * <i>styleUrl</i>. Resolution is performed for only elements of this name.
     * @param fieldName the key used to identify the resolved object in a
     * parser's field table. After this method resolves references, the
     * referenced object can be obtained by calling the parsers {@link
     *                      gov.nasa.worldwind.util.xml.AbstractXMLEventParser#getField(javax.xml.namespace.QName)}
     * method with the <code>fieldName</code> specified here as the name
     * argument.
     * @param parser the parser whose references to resolve.
     *
     * @deprecated Reference resolution is handled by parsers specific to a
     * certain document type. For example, {@link
     *             gov.nasa.worldwind.ogc.kml.KMLRoot} handles resolution of references in
     * KML files.
     */
    @Deprecated
    ...
    /**
     * Specify the object to receive notifications, which are sent when
     * exceptions occur during parsing and when unrecognized element types are
     * encountered. See
     * {@link gov.nasa.worldwind.util.xml.XMLParserNotification} for more
     * information.
     * <p>
     * The parser context may have only one notification listener. That listener
    ...
    /**
     * Indicates whether the specified namespace URI is the default namespace
     * URI used by this parser context.
     *
     * @param namespaceURI the namespace URI to check.
     *
     * @return true if the specified namespace is the default namespace,
     * otherwise false.
     */
    boolean isDefaultNamespace(String namespaceURI);

    /**
     * Returns a parser to handle unrecognized elements. The default
     * unrecognized event parser is {@link
    ...
    /**
     * Add string list parsers for a list of element types and qualified for a
     * specified namespace.
     *
    ...
    /**
     * Add double parsers for a list of element types and qualified for a
     * specified namespace.
     *
     * @param namespace the namespace URI.
    ...
    /**
     * Add integer parsers for a list of element types and qualified for a
     * specified namespace.
     *
     * @param namespace the namespace URI.
    ...
    /**
     * Add boolean parsers for a list of element types and qualified for a
     * specified namespace.
     *
     * @param namespace the namespace URI.
    ...
    /**
     * Add boolean integer parsers for a list of element types and qualified for
