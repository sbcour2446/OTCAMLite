...
/**
 * Provides a global registry of XML parsers. Enables registration of parsers
 * for specific mime types and namespace specialization. Parsers should
 * generally be drawn from this class in order to ensure that parsers configured
 * by World Wind and the application are used.
 *
 * @author tag
 * @version $Id: XMLEventParserContextFactory.java 1171 2013-02-11 21:45:02Z
 * dcollins $
 */
public class XMLEventParserContextFactory {
    ...
    protected static class ParserTableEntry {
        ...
        /**
         * A prototype parser able to construct a copy of itself. The copy
         * typically shares the prototype's parser table and may also share
         * other internal fields.
         */
        XMLEventParserContext prototypeParser;

        /**
         * Construct an instance for a specified list of mime types and a
         * specified prototype parser.
         *
         * @param mimeTypes the list of mime types for which to use the
         * specified prototype parser context.
         * @param prototypeContext the prototype parser context to use for the
         * specified mime types. This parser context's class must provide a copy
         * constructor, a constructor that takes an instance of its class as its
         * only argument.
         *
         * @throws IllegalArgumentException if the mime type list is null or
         * empty or the prototype context is null or has no copy constructor.
         */
    ...
    /**
     * The list of registered parser contexts. A copy-on-write list is used to
     * avoid the need to explicitly synchronize access.
     */
...
//        parsers.add(new ParserTableEntry(mimeTypes, new ColladaParserContext(XMLConstants.NULL_NS_URI)));
    ...
    /**
     * Appends a specified prototype parser context to the list of those already
     * registered.
     *
     * @param mimeTypes the list of mime types for which to use the specified
     * prototype parser context.
     * @param prototypeContext the prototype parser context to use for the
     * specified mime types. This parser context's class must provide a copy
     * constructor, a constructor that takes an instance of its class as its
     * only argument.
     *
     * @throws IllegalArgumentException if the mime type list is null or empty
     * or the prototype context is null or has no copy constructor.
     */
    ...
    /**
     * Prepends a specified prototype parser context to the list of those
     * already registered. Because the new entry is prepended to the list of
     * registered parsers, it will be the first match for the specified mime
     * types.
     *
     * @param mimeTypes the list of mime types for which to use the specified
     * prototype parser context.
    ...
    /**
     * Constructs and returns a parser context for a specified mime type and
     * namespace. The list of registered parser contexts is searched from first
     * to last. A parser context is constructed from the first entry matching
     * the specified mime type and namespace. Null is returned if no match is
     * found.
     * <p>
     * Note that the empty namespace, {@link XMLConstants#NULL_NS_URI} does not
     * match any other namespace. In order for a parser context with the empty
     * namespace to be returned, one with the empty namespace must be
     * registered.
     *
     * @param mimeType the mime type for which to construct a parser.
     * @param defaultNamespace a namespace qualifying the parser context to
     * return. May be null, in which case a parser context for the specified
     * mime type and an empty namespace, {@link
     *                         XMLConstants#NULL_NS_URI}, is searched for.
     *
     * @return a new parser context constructed from the prototype context
     * registered for the specified mime type and having the specified default
     * namespace.
     *
     ...
     *
     * @param prototype the prototype parser context. This parser context's
     * class must provide a copy constructor, a constructor that takes an
     * instance of the class as its only argument.
     *
     * @return the new parser context.
     *
     * @throws Exception if an exception occurs while attempting to construct
     * the new context.
     */
