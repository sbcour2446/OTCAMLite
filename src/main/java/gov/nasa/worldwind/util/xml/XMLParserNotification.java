...
/**
 * This class identifies the type and data content of notifications from parsers
 * and parser contexts. Notifications are sent to inform of important
 * occurrences that occur during parsing, such as exceptions and unrecognized
 * element types.
 *
 * @author tag
 * @version $Id: XMLParserNotification.java 1171 2013-02-11 21:45:02Z dcollins $
 * @see
 * gov.nasa.worldwind.util.xml.XMLEventParserContext#setNotificationListener(XMLParserNotificationListener)
...
public class XMLParserNotification extends PropertyChangeEvent {
    ...
    /**
     * A notification type indicating that a parser encounter an element it did
     * not recognize.
     */
     ...
     * @param notificationType the notification type, such as {@link #EXCEPTION}
     * or {@link #UNRECOGNIZED}.
     * @param event if an event is associated with the notification, that event.
     * May be null.
     * @param msg a message from the notification source suitable for logging.
     * @param oldValue any old value associated with the notification. Not
     * typically used.
     * @param newValue any new value associated with the notification. if this
     * is an exception notification, the exception that occurred is passed via
     ...
     *
     * @return the associated exception, or null if this is not an exception
     * notification.
     */
    ...
    /**
     * Respecifies the notification source. Used to forward the notification so
     * that it appears to be from the new source.
     *
