...
/**
 * SessionCache inspects the results of a retrieval for success or failure. If
 * the retrieval succeeded, this places the retrieved data in a specified
 * session cache with a specified cache key, and marks the resource as
 * available. If the retrieval failed, this logs the cause and marks the
 * resource as missing. Finally, this optionally fires a property change event
 * signalling that the retrieval is complete.
 *
 * @author dcollins
 * @version $Id: SessionCacheRetrievalPostProcessor.java 3086 2015-05-13
 * 20:27:38Z dcollins $
 */
public class SessionCacheRetrievalPostProcessor implements RetrievalPostProcessor {
    ...
    /**
     * Constructs a SessionCachePostProcessor with a specified cache and cache
     * key, and an optional property listener and property name.
     *
     ...
     * @param propertyListener property listener to notify when the data is
     * available. Can be null.
     * @param propertyName property name to use for the property event when the
     * data is available. Can be null.
     */
    ...
    /**
     * Returns this post processor's name, or null if this post processor
     * doesn't have a name.
     *
    ...
    /**
     * Sets this post processor's name to a specified String value, or null to
     * specify that the post processor does not have a name.
     *
    ...
    /**
     * Returns the cache key which identifies where the retrieved data is placed
    ...
    /**
     * Returns the absent resource list that is updated when the retrieval
     * completes.
     *
    ...
    /**
     * Returns the property change listener which is fired when the retrieved
     * data is available. A null value indicates that no property event is
     * fired.
     *
     * @return property change listener to fire when retrieved data is
     * available.
     */
    ...
    /**
     * Returns the property name to use in the property change event fired when
     * retrieved data is available. A null value indicates that no property
     * event is fired.
     *
    ...
    /**
     * Inspect the results of a retrieval for success or failure. If the
     * retrieval succeeded, this places the retrieved data in the session cache
     * with the specified cache key and marks the resource as available. If the
     * retrieval failed, this logs the cause and marks the resource as missing.
     * Finally, this optionally fires a property change event signalling that
     * the retrieval is complete.
     *
     * @param retriever the Retriever which has either succeeded or failed in
     * fetching the data.
     *
     ...
     * Otherwise, this returns the superclass' toString().
     *
