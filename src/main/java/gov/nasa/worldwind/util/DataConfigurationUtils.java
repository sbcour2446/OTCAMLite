...
/**
 * A collection of static methods useful for opening, reading, and otherwise
 * working with WorldWind data configuration documents.
 *
 * @author dcollins
...
public class DataConfigurationUtils {
    ...
    /**
     * Returns true if the specified {@link org.w3c.dom.Element} is a data
     * configuration document. This recognizes the following data configuration
     * documents: <ul> <li>Layer Configuration Documents</li> <li>Elevation
     * Model Configuration Documents</li> <li>Installed DataDescriptor
     * Documents</li> <li>WorldWind .NET LayerSet Documents</li> </ul>
     *
     * @param domElement the document in question.
     *
     * @return true if the document is a data configuration document; false
     * otherwise.
     *
    ...
    /**
     * Returns the specified data configuration document transformed to a
     * standard Layer or ElevationModel configuration document. This returns the
     * original document if the document is already in a standard form, or if
     * the document is not one of the recognized types. Installed DataDescriptor
     * documents are transformed to standard Layer or ElevationModel
     * configuration documents, depending on the document contents. WorldWind
     * .NET LayerSet documents are transformed to standard Layer configuration
     * documents. This returns null if the document's root element is null.
     *
     * @param doc the document to transform.
     *
     * @return the specified document transformed to a standard data
     * configuration document, the original document if it's already in a
     * standard form or is unrecognized, or null if the document's root element
     * is null.
     *
     * @throws IllegalArgumentException if the document is null.
     */
    ...
    /**
     * Returns the specified data configuration document's display name as a
     * string, or null if the document is not one of the recognized types. This
     * determines the display name for each type of data configuration document
     * as follows: <table>
     * <caption style="font-weight: bold;">Mapping</caption><tr><th>Document
     * Type</th><th>Path to Display Name</th></tr> <tr><td>Layer
     * Configuration</td><td>./DisplayName</td></tr> <tr><td>Elevation Model
     ...
     * <tr><td>WorldWind .NET LayerSet</td><td>./QuadTileSet/Name</td></tr>
     * <tr><td>Other</td><td>null</td></tr>
     * </table>
     *
     * @param domElement the data configuration document who's display name is
     * returned.
     *
     * @return a String representing the data configuration document's display
     * name, or null if the document is not recognized.
     *
     * @throws IllegalArgumentException if the document is null.
     */
    ...
    /**
     * Returns the specified data configuration document's type as a string, or
     * null if the document is not one of the recognized types. This maps data
     * configuration documents to a type string as follows:
     * <table><caption style="font-weight: bold;">Mapping</caption>
     * <tr><th>Document Type</th><th>Type String</th></tr> <tr><td>Layer
     * Configuration</td><td>"Layer"</td></tr> <tr><td>Elevation Model
     * Configuration</td><td>"Elevation Model"</td></tr> <tr><td>Installed
     * DataDescriptor</td><td>"Layer" or "ElevationModel"</td></tr>
     * <tr><td>WorldWind .NET LayerSet</td><td>"Layer"</td></tr>
     * <tr><td>Other</td><td>null</td></tr> </table>
     *
     * @param domElement the data configuration document to determine a type
     * for.
     *
     * @return a String representing the data configuration document's type, or
     * null if the document is not recognized.
     *
     * @throws IllegalArgumentException if the document is null.
     */
    ...
    /**
     * Returns a file store path name for the specified parameters list. This
     * returns null if the parameter list does not contain enough information to
     * construct a path name.
     *
     * @param params the parameter list to extract a configuration filename
     * from.
     * @param suffix the file suffix to append on the path name, or null to
     * append no suffix.
     *
     * @return a file store path name with the specified suffix, or null if a
     * path name cannot be constructed.
     *
    ...
    /**
     * Convenience method for computing a data configuration file's cache name
     * in a FileStore, given the file's cache path. This writes the computed
     * cache name to the specified parameter list under the key {@link
     * gov.nasa.worldwind.avlist.AVKey#DATA_CACHE_NAME}. If the parameter
     * already exists, it's left unchanged.
     * <p>
     * A data configuration file's cache name is its parent directory in the
     * cache. The cache name therefore points to the directory containing both
     * the configuration file and any cached data associated with it.
     * Determining the cache name at run time - instead of hard wiring it in the
     * data configuration file - enables cache data to be moved to an arbitrary
     * location within the cache.
     *
     * @param dataConfigCachePath the data configuration file's cache path.
     * @param params the output key-value pairs which receive the
     * DATA_CACHE_NAME parameter. A null reference is permitted.
     *
     * @return a reference to params, or a new AVList if params is null.
     *
     * @throws IllegalArgumentException if the data config file's cache path is
     * null or has length zero.
     */
    ...
    /**
     * Returns true if a configuration file name exists in the store which has
     * not expired. This returns false if a configuration file does not exist,
     * or it has expired. This invokes {@link #findExistingDataConfigFile(gov.nasa.worldwind.cache.FileStore,
     * String)} to determine the URL of any existing file names. If an existing
     * file has expired, and removeIfExpired is true, this removes the existing
     * file.
     *
     * @param fileStore the file store in which to look.
     * @param fileName the file name to look for. If a file with this name does
     * not exist in the store, this looks at the file's siblings for a match.
     * @param removeIfExpired true to remove the existing file, if it exists and
     * is expired; false otherwise.
     * @param expiryTime the time in milliseconds, before which a file is
     * considered to be expired.
     *
     * @return whether a configuration file already exists which has not
     * expired.
     *
     * @throws IllegalArgumentException if either the file store or file name
     * are null.
     */
    ...
    /**
     * Returns the URL of an existing data configuration file under the
     * specified file store, or null if no configuration file exists. This first
     * looks for a configuration file with the specified name. If that does not
     * exist, this checks the siblings of the specified file for a configuration
     * file match.
     *
     * @param fileStore the file store in which to look.
     * @param fileName the file name to look for. If a file with this name does
     * not exist in the store, this looks at the file's siblings for a match.
     *
     * @return the URL of an existing configuration file in the store, or null
     * if none exists.
     *
     * @throws IllegalArgumentException if either the file store or file name
     * are null.
     ...
     * {@link java.util.concurrent.ScheduledExecutorService} which can be used
     * by World Wind components to schedule periodic resource checks. The
     * returned ExecutorService is backed by a single daemon thread with minimum
     * priority.
     *
     * @param threadName the String name for the ExecutorService's thread, may
     * be <code>null</code>.
     *
     * @return a new ScheduledExecutorService appropriate for scheduling
    ...
    //**************************************************************//
    /**
     * Appends WMS layer parameters as elements to a specified context. This
     * appends elements for the following parameters: <table>
     ...
     * AVKey#DATASET_NAME}</td><td>AVKey.LAYER_NAMES</td><td>String</td></tr>
     * </table>
     *
     * @param params the key-value pairs which define the WMS layer
     * configuration parameters.
     * @param context the XML document root on which to append WMS layer
     * configuration elements.
     *
     * @return a reference to context.
     *
     * @throws IllegalArgumentException if either the parameters or the context
     * are null.
     */
    ...
    /**
     * Appends WCS layer parameters as elements to a specified context. This
     * appends elements for the following parameters:
     * <table><caption style="font-weight: bold;">Mapping</caption>
     ...
     * AVKey#DATASET_NAME}</td><td>AVKey.COVERAGE_IDENTIFIERS</td><td>String</td></tr>
     * </table>
     *
     * @param params the key-value pairs which define the WMS layer
     * configuration parameters.
    ...
    /**
     * Parses WMS layer parameters from the XML configuration document starting
     * at domElement. This writes output as key-value pairs to params. If a
     * parameter from the XML document already exists in params, that parameter
     * is ignored. Supported key and parameter names are: <table>
     * <caption style="font-weight: bold;">Mapping</caption><tr><th>Parameter</th><th>Element
     ...
     * </table>
     *
     * @param domElement the XML document root to parse for WMS layer
     * parameters.
     * @param params the output key-value pairs which receive the WMS layer
     * parameters. A null reference is permitted.
     *
     * @return a reference to params, or a new AVList if params is null.
     *
     * @throws IllegalArgumentException if the document is null.
    ...
    /**
     * Convenience method to get the OGC GetCapabilities URL from a specified
     * parameter list. If all the necessary parameters are available, this
     * returns the GetCapabilities URL. Otherwise this returns null.
     *
     * @param params parameter list to get the GetCapabilities parameters from.
     *
     * @return a OGC GetCapabilities URL, or null if the necessary parameters
     * are not available.
     *
     * @throws IllegalArgumentException if the parameter list is null.
     */
    ...
    /**
     * Convenience method to get the OGC {@link AVKey#LAYER_NAMES} parameter
     * from a specified parameter list. If the parameter is available as a
     * String, this returns all the OGC layer names found in that String.
    ...
    //**************************************************************//
    /**
     * Appends LevelSet configuration parameters as elements to the specified
     * context. This appends elements for the following parameters: <table>
     ...
     * parameters.
     * @param context the XML document root on which to append LevelSet
     * configuration elements.
     *
     * @return a reference to context.
     *
     ...
     * This writes output as key-value pairs to params. If a parameter from the
     * XML document already exists in params, that parameter is ignored.
     * Supported key and parameter names are: <table>
     ...
     *
     * @param domElement the XML document root to parse for LevelSet
     * configuration parameters.
     * @param params the output key-value pairs which receive the LevelSet
    ...
    /**
     * Gathers LevelSet configuration parameters from a specified LevelSet
     * reference. This writes output as key-value pairs params. If a parameter
     ...
     * <tr><td>{@link
     * gov.nasa.worldwind.avlist.AVKey#DATASET_NAME}</td><td>First Level's
     * dataset</td><td>String</td></tr>
     * <tr><td>{@link gov.nasa.worldwind.avlist.AVKey#DATA_CACHE_NAME}</td><td>First
     * Level's cacheName</td><td>String</td></tr>
     * <tr><td>{@link gov.nasa.worldwind.avlist.AVKey#SERVICE}</td><td>First
     * Level's service</td><td>String</td></tr>
     * <tr><td>{@link gov.nasa.worldwind.avlist.AVKey#EXPIRY_TIME}</td><td>First
     ...
     * <tr><td>{@link gov.nasa.worldwind.avlist.AVKey#NUM_EMPTY_LEVELS}</td><td>1
     * + index of first non-empty Level</td><td>Integer</td></tr>
     * <tr><td>{@link gov.nasa.worldwind.avlist.AVKey#INACTIVE_LEVELS}</td><td>Comma
     ...
     * gov.nasa.worldwind.geom.LatLon}</td></tr>
     * <tr><td>{@link gov.nasa.worldwind.avlist.AVKey#TILE_WIDTH}</td><td>First
     * Level's tileWidth<td><td>Integer</td></tr>
     * <tr><td>{@link gov.nasa.worldwind.avlist.AVKey#TILE_HEIGHT}</td><td>First
     ...
     * </table>
     *
     * @param levelSet the LevelSet reference to gather configuration parameters
     * from.
     * @param params the output key-value pairs which receive the LevelSet
     * configuration parameters. A null reference is permitted.
     *
    ...
    //**************************************************************//
    /**
     * Returns true if a specified DOM document is a DataDescriptor
     * configuration document, and false otherwise.
     *
     * @param domElement the DOM document in question.
     *
     * @return true if the document is a DataDescriptor configuration document;
     * false otherwise.
     *
    ...
    /**
     * Transforms a DataDescriptor configuration document to a standard layer or
     * elevation model configuration document, depending on the contents of the
     * {@link org.w3c.dom.Element}.
     *
     * @param domElement DataDescriptor document to transform.
     *
     * @return standard Layer or ElevationModel document, or null if the
     * DataDescriptor cannot be transformed to a standard document.
     *
     * @throws IllegalArgumentException if the document is null.
     */
    ...
    //**************************************************************//
    /**
     * Returns true if a specified document is a WorldWind .NET LayerSet
     * configuration document, and false otherwise.
     *
     * @param domElement the document in question.
     *
     * @return true if the document is a LayerSet configuration document; false
     * otherwise.
     *
     * @throws IllegalArgumentException if document is null.
     */
    ...
    /**
     * Returns true if a specified XML event is the root of a WorldWind .NET
     * LayerSet configuration document, and false otherwise.
     *
     * @param event the XML event in question.
     *
     * @return true if the event is a LayerSet configuration document element;
     * false otherwise.
     *
    ...
    /**
     * Parses WorldWind .NET LayerSet configuration parameters from the
     * specified document. This writes output as key-value pairs to params. If a
     * parameter from the LayerSet document already exists in params, that
     ...
     * <tr><td>{@link gov.nasa.worldwind.avlist.AVKey#SERVICE_NAME}</td><td>"Offline"
     * (string constant)<td></td><td>String</td></tr>
     ...
     * <tr><td>{@link gov.nasa.worldwind.avlist.AVKey#NUM_EMPTY_LEVELS}</td><td>0
     * (integer constant)<td></td><td>Integer</td></tr>
     * <tr><td>{@link gov.nasa.worldwind.avlist.AVKey#SECTOR}</td><td>QuadTileSet/BoundingBox<td></td><td>Sector</td></tr>
     * <tr><td>{@link gov.nasa.worldwind.avlist.AVKey#TILE_ORIGIN}</td><td>(-90,
     * -180) (geographic location constant)<td></td><td>LatLon</td></tr>
     ...
     * <tr><td>{@link gov.nasa.worldwind.avlist.AVKey#NETWORK_RETRIEVAL_ENABLED}</td><td>false
     ...
     * <tr><td>{@link gov.nasa.worldwind.avlist.AVKey#USE_MIP_MAPS}</td><td>true
     * (boolean constant)<td></td><td>Boolean</td></tr>
     * <tr><td>{@link gov.nasa.worldwind.avlist.AVKey#USE_TRANSPARENT_TEXTURES}</td><td>true
     * (boolean constant)<td></td><td>Boolean</td></tr> </table>
     *
     * @param domElement the XML document root to parse for LayerSet
     * configuration parameters.
     * @param params the output key-value pairs which receive the LayerSet
     * configuration parameters. A null reference is permitted.
     *
     * @return a reference to params, or a new AVList if params is null.
     *
    ...
    /**
     * Transforms a WorldWind .NET LayerSet configuration document to a standard
     * layer configuration document.
     *
     * @param domElement LayerSet document to transform.
     *
     * @return standard Layer document, or null if the LayerSet document cannot
     * be transformed to a standard document.
     *
     * @throws IllegalArgumentException if the document is null.
     */
