...
public class GDALUtils {
    ...
    protected static void initialize() {
        try {
            ...
            if (gdalJNI.isAvailable() && gdalconstJNI.isAvailable()) {
                ...
                /**
                 * "VERSION_NUM": Returns GDAL_VERSION_NUM formatted as a
                 * string. ie. "1170" "RELEASE_DATE": Returns GDAL_RELEASE_DATE
                 * formatted as a string. "20020416" "RELEASE_NAME": Returns the
                 * GDAL_RELEASE_NAME. i.e. "1.1.7" "--version": Returns full
                 * version , ie. "GDAL 1.1.7, released 2002/04/16"
                 */
     ...
     *
     * @param source the location of the local file, expressed as either a
     * String path, a File, or a file URL.
     * @param isSilentMode specifies a silent mode of reading file (usually
     * needed for canRead() and readMetadata())
     *
    ...
    /**
     * Opens image or elevation file, returns as a BufferedImage (even for
     * elevations)
     *
    ...
    /**
     * Attempts to detect if there are any black|white|gray voids (also called
     * black skirts) i n the image raster caused by inaccurate clipping. The
     * algorithm checks each corner of the image and if it detects
     * black|white|gray pixel, uses a scanline version of flood fill algorithm
     * to make the area transparent. See more
     * <code>http://en.wikipedia.org/wiki/Flood_fill#Scanline_fill</code>
     *
     * @param sourceImage a source image raster
     *
     * @return BufferedImage with voids (if detected) filled with a transparent
     * pixel values
     */
     ...
     *
     * @throws IllegalArgumentException if sector is null, or raster size is
     * zero
     */
...
//      GeoTransform[3] -= 0.5 * GeoTransform[5];
     ...
     * @param params AVList to hold retrieved metadata, if null, a new instance
     * will be created and returned as a return value
     * @param quickReadingMode if quick reading mode is enabled GDAL will not
     * spend much time on heavy calculations, like for example calculating
     * Min/Max for entire elevation raster
     *
     * @return AVList with retrieved metadata
     *
     * @throws IllegalArgumentException when the passed dataset is null pr
     * emtpy, or any of the dimension is 0
     * @throws gov.nasa.worldwind.exception.WWRuntimeException if GDAL is not
     * available, or a dataset contains no bands
     * <p>
     ...
     * <p>
     * AVKey.SECTOR - in case of Geographic CS, contains a regular Geographic
     * Sector defined by lat/lon coordinates of corners in case of Projected CS,
     * contains a bounding box of the area
     * <p>
     * AVKey.COORDINATE_SYSTEM_NAME
     * <p>
     * AVKey.PIXEL_WIDTH (Double) pixel size, UTM images usually specify 1 (1
     * meter); if missing and Geographic Coordinate System is specified will be
     * calculated as LongitudeDelta/WIDTH
     * <p>
     * AVKey.PIXEL_HEIGHT (Double) pixel size, UTM images usually specify 1 (1
     * meter); if missing and Geographic Coordinate System is specified will be
     * calculated as LatitudeDelta/HEIGHT
     * <p>
     * AVKey.ORIGIN (LatLon) specifies coordinate of the image's origin (one of
     * the corners, or center) If missing, upper left corner will be set as
     * origin
     * <p>
     * AVKey.DATE_TIME (0 terminated String, length == 20) if missing, current
     ...
     * <p>
     * AVKey.IMAGE_COLOR_FORMAT required if AVKey.PIXEL_FORMAT is AVKey.IMAGE
     * (valid values: AVKey.COLOR and AVKey.MONOCHROME)
     * <p>
     * AVKey.DATA_TYPE required ( valid values: AVKey.INT16, and AVKey.FLOAT32 )
     * <p>
     * AVKey.VERSION optional, if missing a default will be used "NASA
     * WorldWind"
     * <p>
     * AVKey.DISPLAY_NAME, (String) optional, specifies a name of the
     * document/image
     * <p>
     * AVKey.DESCRIPTION (String) optional, for any kind of descriptions
     * <p>
     * AVKey.MISSING_DATA_SIGNAL optional, set the AVKey.MISSING_DATA_SIGNAL
     * ONLY if you know for sure that the specified value actually represents
     * void (NODATA) areas. Elevation data usually has "-32767" (like DTED), or
     * "-32768" like SRTM, but some has "0" (mostly images) and "-9999" like
     * NED. Note! Setting "-9999" is very ambiguos because -9999 for elevation
     * is valid value;
     * <p>
     * AVKey.MISSING_DATA_REPLACEMENT (String type forced by spec) Most images
     * have "NODATA" as "0", elevations have as "-9999", or "-32768" (sometimes
     * "-32767")
     * <p>
     * AVKey.COORDINATE_SYSTEM required, valid values
     * AVKey.COORDINATE_SYSTEM_GEOGRAPHIC or AVKey.COORDINATE_SYSTEM_PROJECTED
     * <p>
     * AVKey.COORDINATE_SYSTEM_NAME Optional, A name of the Coordinates System
     * as a String
     * <p>
     * AVKey.PROJECTION_EPSG_CODE Required; Integer; EPSG code or Projection
     * Code If CS is Geodetic and EPSG code is not specified, a default WGS84
     * (4326) will be used
     * <p>
    ...
    /**
     * The "composeDataRaster" method creates a ByteBufferRaster from an
     * elevation (or non-image) Dataset.
     *
     * @param ds The GDAL dataset with data raster (expected only elevation
     * raster); f or imagery rasters use composeImageDataRaster() method
     * @param params , The AVList with properties (usually used to force
     * projection info or sector)
     *
     * @return ByteBufferRaster as DataRaster
     *
     * @throws IllegalArgumentException if raster parameters (height, width,
     * sector, etc) are invalid
     * @throws WWRuntimeException when invalid raster detected (like attempt to
     * use the method for imagery raster)
     */
