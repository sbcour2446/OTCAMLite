...
/**
 * A HotSpot for resizing a frame or window. This class handles the resize input
 * events, but does does not actually draw the resize controls. The HotSpot is
 * defined by a direction, for example, {@link AVKey#NORTH} indicates that the
 * HotSpot resizes the frame vertically from the north edge (the user clicks the
 * top edge of the frame and drags vertically).
 * <p>
 * An instance of this class should be added to the picked object when the edge
 * or corner of the frame is picked.
 * <p>
 * Subclasses must the implement {#getSize}, {#setSize}, {#getScreenPoint}, and
 * {#setScreenPoint} to manipulate the frame that they want to resize.
 *
 * @author pabercrombie
...
public abstract class AbstractResizeHotSpot extends AbstractHotSpot {
     ...
     * For example, if the upper left corner is being dragged, the window should
     * move to keep that corner under the cursor.
     */
    ...
    /**
     * Set the resize direction based on which point on the frame was picked (if
     * a point on the left of the frame is picked, the resize direction is west,
     * if a point on the top edge is picked the resize direction is north, etc).
     *
     ...
     * #endDrag} as appropriate. Subclasses may override this method if they
     * need to handle events other than drag events.
     *
    ...
    /**
     * Called when a drag action ends. This implementation sets
     * {@link #dragRefPoint} to null.
     */
     ...
     *
     * @param active {@code true} if the HotSpot is being activated,
     * {@code false} if it is being deactivated.
     */
    @Override
    ...
    /**
     * Is a frame size valid? This method is called before attempting to resize
     * the frame. If this method returns false, the resize operation is not
     * attempted. This implementation ensures that the proposed frame size is
     * greater than or equal to the minimum frame size.
     *
    ...
    /**
     * Get the minimum size of the frame. The user is not allowed to resize the
     * frame to be smaller than this size. This implementation returns 0, 0.
     *
     ...
     *
     * @return Screen point measured from upper left corner of the screen (AWT
     * coordinates).
     */
     ...
     *
     * @param newPoint New screen point measured from upper left corner of the
     * screen (AWT coordinates).
     */
