...
/**
 * Determines the visible sectors at a specifed resolution within the draw
 * context's current visible sector.
 *
 * @author Tom Gaskins
...
public class SectorVisibilityTree {
     ...
     * @param sectorSize the granularity of sector visibility, in degrees. All
     * visible sectors of this size are found. The value must be in the range, 1
     * second &lt;= sectorSize &lt;= 180 degrees.
     *
     * @return the list of visible sectors. The list will be empty if no sectors
     * are visible.
     *
    ...
    /**
     * Determines the visible sectors at a specified resolution within a
     * specified sector.
     *
     * @param dc the current draw context
     * @param sectorSize the granularity of sector visibility, in degrees. All
     * visible sectors of this size are found. The value must be in the range, 1
     * second &lt;= sectorSize &lt;= 180 degrees.
     * @param searchSector the overall sector for which to determine visibility.
     * May be null, in which case the current visible sector of the draw context
     * is used.
     *
     * @return the list of visible sectors. The list will be empty if no sectors
     * are visible.
     *
     * @throws IllegalArgumentException if the draw context is null, the sector
     * size is less than or equal to zero, or the search sector list is null.
     */
    ...
    /**
     * Determines the visible sectors at a specified resolution within a
     * collection of sectors. This method can be used to recursively determine
     * visible sectors: the output of one invocation can be passed as an
     * argument to the next invocation.
     *
     * @param dc the current draw context
     * @param sectorSize the granularity of sector visibility, in degrees. All
     * visible sectors of this size are The value must be in the range, 1 second
     * &lt;= sectorSize &lt;= 180 degrees. found.
