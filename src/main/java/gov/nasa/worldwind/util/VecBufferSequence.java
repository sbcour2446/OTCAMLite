...
/**
 * VecBufferSequence provides storage and retrieval of a sequence of logical
 * VecBuffers in a single VecBuffer that expands when more capacity is needed.
 * VecBuffers added to a VecBufferSequence by calling
 * {@link #append(VecBuffer)}. This copies the specified VecBuffer's data to the
 * VecBuffer backing the VecBufferSequence, and expands the backing VecBuffer if
 * necessary.
 *
 * @author dcollins
...
public class VecBufferSequence extends CompoundVecBuffer {
    ...
    /**
     * Constructs a PackedCompoundVecBuffer with the specified backing VecBuffer
     * and the specified initial capacity.
     *
     * @param buffer the backing VecBuffer.
     * @param capacity the PackedCompoundVecBuffer's initial capacity, in number
     * of sub-buffers.
     *
     * @throws IllegalArgumentException if the buffer is null, or if the
    ...
    /**
     * Returns an empty VecBufferSequence. The returned VecBufferSequence has a
    ...
    /**
     * Returns the VecBuffer that stores this PackedCompoundVecBuffer's
     * sub-buffers.
     *
    ...
    /**
     * Appends the contents of the specified sub-buffer to the end of this
     * PackedCompoundVecBuffer, incrementing the number of sub-buffers by one.
     * The backing buffer grows to accomodate the sub-buffer if it does not
     * already have enough capacity to hold it.
     *
     ...
     * itself is this buffer's backing NIO {@link java.nio.Buffer}. This
     * buffer's vector size must be 3.
     *
    ...
    /**
     * Renders <code>getTotalBufferSize()</code> elements from the currently
     * bounds OpenGL coordinate buffers, beginning with element 0. The specified
     * drawMode indicates which type of OpenGL primitives to render.
     *
     * @param dc the current DrawContext.
     ...
     * This behaves exactly like {@link
     * #drawArrays(gov.nasa.worldwind.render.DrawContext, int)}, except that
     * each sub-buffer is rendered independently. The specified drawMode
