...
/**
 * CompoundVecBuffer defines an interface for storing and retrieving a
 * collection of variable length {@link
 * gov.nasa.worldwind.util.VecBuffer} objects. Each VecBuffer is retrieved via
 * an index. The range of valid indices in a CompoundVecBuffer is [0, size() -
 * 1], inclusive. Implementations of CompoundVecBuffer define how each VecBuffer
 * is stored and retrieved according to its index.
 * <p>
 * To retrieve a single VecBuffer given an index, invoke
 * {@link #subBuffer(int)}. To retrieve a VecBuffer's size, in number of logical
 * tuples, invoke {@link #subBufferSize(int)}.
 * <p>
 * To create a new view of this CompoundVecBuffer from one or many VecBuffers,
 * invoke one of the <code>slice</code> methods: <ul>
 * <li>{@link #slice(int, int)} creates a view of this CompoundVecbufer given a
 * contiguous sequence of VecBuffer indices.</li>
 * <li>{@link #slice(int[], int, int)} creates a view of this CompoundVecBuffer
 * given an array of VecBuffer indices.</li> </ul>
 *
 * @author dcollins
...
public abstract class CompoundVecBuffer {
     ...
     *
     * @param capacity the CompoundVecBuffer's initial capacity, in number of
     * sub-buffers.
     *
    ...
    /**
     * Returns an empty CompoundVecBuffer. The returned CompoundVecBuffer has a
     * size of zero and contains no sub-buffers.
     *
    ...
    /**
     * Returns the size in logical vectors of the VecBuffer with the specified
     * index.
     *
     ...
     * {@link gov.nasa.worldwind.util.VecBuffer}.
     *
    ...
    /**
     * Returns a new logical view of this CompoundVecBuffer. The returned buffer
     * has length <code>endIndex - beginIndex
     * + 1</code> and references this buffer's contents starting at
     * <code>beginIndex</code>, and ending at <code>endIndex</code>. The
     * returned buffer shares this buffers's backing data. Changes to this
     * buffer are reflected in the returned buffer, and vice versa.
     *
     * @param beginIndex the index of the first sub-buffer to include in the
     * subset.
     * @param endIndex the index of the last sub-buffer to include in the
     * subset.
     *
     * @return a new CompoundVecBuffer representing a subset of this
     * CompoundVecBuffer.
     *
     * @throws IllegalArgumentException if beginIndex is out of range, if
     * endIndex is out of range, or if beginIndex &gt; endIndex.
     */
    ...
    /**
     * Returns a new logical view of this CompoundVecBuffer. The returned
     * buffer's length is equal to the specified <code>length</code>, and
     * contains this buffer's contents for each index in <code>indices</code>.
     * The returned buffer shares this buffers's backing data. Changes to this
     * buffer are reflected in the returned buffer, and vice versa.
     *
     ...
     * CompoundVecBuffer.
     *
     * @throws IllegalArgumentException if the array of indices is null, if the
     * offset or length are invalid, or if any of the indices is out of range.
     */
    ...
    /**
     * Returns a new logical view of this CompoundVecBuffer. The returned
     * buffer's length is equal to the length of <code>indices</code>, and
     * contains this buffer's contents for each index in <code>indices</code>.
     * The returned buffer shares this buffers's backing data. Changes to this
     * buffer are reflected in the returned buffer, and vice versa.
     *
     ...
     * CompoundVecBuffer.
     *
     * @throws IllegalArgumentException if the array of indices is null, or if
     * any of the indices is out of range.
     */
    ...
    /**
     * Sets the number sub-buffers to zero. This does not free any memory
     * associated with this CompoundVecBuffer.
     */
    public void clear() {
        this.count = 0;
    }
