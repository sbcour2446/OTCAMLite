 ...
 * {@link gov.nasa.worldwind.SceneController}'s bulk rendering of
 * SurfaceObjects. The SceneControllers bulk renders all SurfaceObjects added to
 * the {@link
 * gov.nasa.worldwind.render.DrawContext}'s ordered surface renderable queue
 * during the preRendering pass. While building the composite representation the
 * SceneController invokes {@link #render(DrawContext)} in ordered rendering
 * mode. To avoid overloading the purpose of the render method,
 * AbstractSurfaceObject does not add itself to the DrawContext's ordered
 * surface renderable queue during rendering.
 * <p>
 * Subclasses that do not wish to participate in this composite representation
 * can override this behavior as follows:
 * <ol> <li>Override {@link #makeOrderedPreRenderable(DrawContext)}; do not add
 * this object to the draw context's ordered renderable queue. Perform any
 * preRender processing necessary for the subclass to pick and render
 * itself.</li>
 * <li>Override {@link #pickOrderedRenderable(DrawContext, PickSupport)}; draw
 * the custom pick representation.</li>
 * <li>Override {@link #makeOrderedRenderable(DrawContext)}; add this object to
 * the draw context's ordered renderable queue. AbstractSurfaceObject does not
 * add itself to this queue during rendering. render() is called from the
 * SceneController while building the composite representation because
 * AbstractSurfaceObject adds itself to the ordered surface renderable queue
 * during preRendering.</li> <li>Override
 * {@link #drawOrderedRenderable(DrawContext)}; draw the custom representation.
 * Unlike AbstractSurfaceObject subclasses should assume the modelview and
 * projection matrices are consistent with the current
 * {@link gov.nasa.worldwind.View}.</li> </ol>
 *
 * @author dcollins
...
public abstract class AbstractSurfaceObject extends WWObjectImpl implements SurfaceObject {
    ...
    /**
     * Support class used to build surface tiles used to draw the pick
     * representation.
     */
    ...
    /**
     * Creates a new AbstractSurfaceObject, assigning it a unique ID and
     * initializing its last modified time to the current system time.
     */
     ...
     * This method is synchronized to ensure that two threads calling
     * simultaneously receive different values. Since this method is called from
     * AbstractSurfaceObject's constructor, this is critical to ensure that
     * AbstractSurfaceObject can be safely constructed on separate threads.
     *
     ...
     *
     * @return <code>true</code> if the shape draws its outline;
    ...
    /**
     * Specifies if the SurfaceObject should draw its bounding sector. If
     * <code>true</code>, the SurfaceObject draws an outline of its bounding
     * sector in green on top of its shape. The default value is
     * <code>false</code>.
     *
     * @param draw <code>true</code> to draw the shape's outline;
     * <code>false</code> otherwise.
     */
    ...
    /**
     * Returns an integer number uniquely identifying the surface object. This
     * number is unique relative to other instances of SurfaceObject, but is not
    ...
    /**
     * Sets the SurfaceObject's modified time to the current system time. This
     * causes cached representations of this SurfaceObject to be refreshed.
     */
    ...
    //**************************************************************//
    /**
     * Computes the surface object's extent. Uses the sector list returned by
     * {@link #getSectors(DrawContext)} to compute the extent bounding this
     * object's sectors on the draw context's surface. This returns null if the
     * surface object has no sectors.
     *
     * @param dc the draw context the extent relates to.
     *
     * @return the surface object's extent. Returns null if the surface object
     * has no sectors.
     */
    ...
    /**
     * Computes an extent bounding the the specified sectors on the specified
     * Globe's surface. If the list contains a single sector this returns a box
     * created by calling {@link gov.nasa.worldwind.geom.Sector#computeBoundingBox(gov.nasa.worldwind.globes.Globe,
     * double, gov.nasa.worldwind.geom.Sector)}. If the list contains more than
     * one sector this returns a {@link
     * gov.nasa.worldwind.geom.Box} containing the corners of the boxes bounding
     * each sector. This returns null if the sector list is empty.
     *
    ...
    /**
     * Test if this SurfaceObject intersects the specified draw context's
     ...
     *
     * @return true if this SurfaceObject intersects the draw context's frustum;
     * false otherwise.
     */
    ...
    /**
     * Test if this SurfaceObject intersects the specified draw context's pick
     * frustums.
     *
     * @param dc the draw context the SurfaceObject is related to.
     *
     * @return true if this SurfaceObject intersects any of the draw context's
     * pick frustums; false otherwise.
     */
    ...
    /**
     * Test if this SurfaceObject intersects the specified draw context's
     * visible sector. This returns false if the draw context's visible sector
     * is null.
     *
     * @param dc draw context the SurfaceObject is related to.
     *
     * @return true if this SurfaceObject intersects the draw context's visible
     ...
     * {@link gov.nasa.worldwind.render.OrderedRenderable} and adds it to the
     * DrawContext's ordered surface renderable list. Additionally, this
     * prepares the SurfaceObject's pickable representation if the
     * SurfaceObject's containing layer is enabled for picking and the
     * SurfaceObject intersects one of the DrawContext's picking frustums.
     * <p>
     * During ordered preRendering, the
     * {@link gov.nasa.worldwind.SceneController} builds a composite
     * representation of this SurfaceObject and any other SurfaceObject on the
     * DrawContext's ordered surface renderable list. The SceneController causes
     * each SurfaceObject's to draw itself into the composite representation by
     * calling its {@link #render(DrawContext)} method in ordered rendering
     * mode.
     *
     ...
     * Prepares the SurfaceObject as an
     * {@link gov.nasa.worldwind.render.OrderedRenderable} and adds it to the
     * DrawContext's ordered surface renderable list. We ignore this call during
     * rendering mode to suppress calls to {@link #render(DrawContext)} during
     * ordered rendering mode. The SceneController already invokes render during
     * ordered picking mode to build a composite representation of the
     * SurfaceObjects.
     * <p>
     * During ordered picking, the {@link gov.nasa.worldwind.SceneController}
     * invokes the SurfaceObject's {@link
    ...
    /**
     * Causes the SurfaceObject to draw itself in a unique pick color, and add
     * itself as a pickable object to the specified pickSupport.
     *
    ...
    /**
     * Create a {@link gov.nasa.worldwind.pick.PickedObject} for this surface
     * object. The PickedObject created by this method will be added to the pick
     * list to represent the current surface object.
     *
    ...
    /**
     * Causes adjacent SurfaceObjects in the DrawContext's ordered surface
     * renderable list to draw themselves in in a unique pick color, and adds
     * themselves as pickable objects to the specified pickSupport. Adjacent
     * SurfaceObjects are removed from the DrawContext's list and processed
     * until one is encountered that has a different containing layer or is not
     * enabled for batch picking.
     *
     * @param dc the current DrawContext.
     * @param pickSupport the PickSupport to add the SurfaceObject to.
     */
    ...
    /**
     * Causes the SurfaceObject to render itself. SurfaceObjects are drawn in
     * geographic coordinates into offscreen surface tiles. This attempts to get
     * a {@link gov.nasa.worldwind.util.SurfaceTileDrawContext} from the
     * DrawContext's AVList by querying the key
     * {@link gov.nasa.worldwind.avlist.AVKey#SURFACE_TILE_DRAW_CONTEXT}. If the
     * DrawContext has a SurfaceTileDrawContext attached under that key, this
     * calls {@link
     * #drawGeographic(DrawContext, gov.nasa.worldwind.util.SurfaceTileDrawContext)}
     * with the SurfaceTileDrawContext. Otherwise this logs a warning and
     * returns.
     *
     * @param dc the current DrawContext.
     */
    ...
    /**
     * Causes the SurfaceObject to render itself to the specified region in
     * geographic coordinates. The specified viewport denotes the geographic
     * region and its corresponding screen viewport.
     *
     * @param dc the current draw context.
     * @param sdc the context containing a geographic region and screen viewport
     * corresponding to a surface tile.
     */
    ...
    //**************************************************************//
    /**
     * Builds this AbstractSurfaceObject's pickable representation. This method
     * is called during the preRender phase, and is therefore free to modify the
     * framebuffer contents to create the pickable representation.
     *
    ...
    /**
     * Causes this SurfaceObject to draw a representation of itself suitable for
     * use during picking.
     *
     * @param dc the current DrawContext.
     */
    ...
    /**
     * Returns a {@link SurfaceObjectTileBuilder} appropriate for building and
     * drawing the surface object's pickable representation. The returned
     * SurfaceObjectTileBuilder's is configured to create textures with the
     * GL_ALPHA8 format, and to use GL_NEAREST filtering. This reduces a surface
     * object's pick texture resources by a factor of 4, and ensures that linear
     * texture filtering and mip-mapping is disabled while drawing the pick
     * tiles.
     *
     * @return a SurfaceObjectTileBuilder used for building and drawing the
     * surface object's pickable representation.
     */
    ...
    //**************************************************************//
    /**
     * Causes this SurfaceObject to render its bounding sectors to the specified
     * region in geographic coordinates. The specified viewport denotes the
     * geographic region and its corresponding screen viewport.
     * <p>
    ...
    //**************************************************************//
    /**
     * Represents a surface object's current state. StateKey uniquely identifies
     * a surface object's current state as follows: <ul> <li>The StateKey class
     * distinguishes the key from other object types.</li> <li>The object's
     * unique ID distinguishes one surface object instances from another.</li>
     * <li>The object's modified time distinguishes an object's internal state
     * from any of its previous states.</li> </ul> Using the unique ID to
     * distinguish between objects ensures that the StateKey does not store
     * dangling references to the surface object itself. Should the StateKey
     * live longer than the surface object that created it, the StateKey does
     * not prevent the object from being garbage collected.
     */
    protected static class SurfaceObjectStateKey implements Cacheable {

        /**
         * The SurfaceObject's unique ID. This is unique to all instances of
         * SurfaceObject.
         */
        protected final long uniqueId;
        /**
        ...
        /**
         * Constructs a new SurfaceObjectStateKey with the specified unique ID
         * and modified time.
         *
