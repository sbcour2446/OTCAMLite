...
/**
 * Defines the dimensions of an image, label or other screen-space item relative
 * to a container (for example, the viewport). A size contains a width, a
 * height, a width size mode, a height size mode, and for each of these a
 * "units" string indicating the coordinate units.
 * <p>
 * The possible size modes are: <ul> <li> {@link #NATIVE_DIMENSION} - Maintain
 * the native dimensions.</li> <li> {@link
 * #MAINTAIN_ASPECT_RATIO} - Maintain the aspect ratio of the image when one
 * dimension is specified and the other is not.</li> <li>
 * {@link #EXPLICIT_DIMENSION} - Use an explicit dimension. This dimension may
 * be either an absolute pixel value, or a fraction of the container.</li></ul>
 * <p>
 * Recognized units are {@link AVKey#PIXELS}, which indicates pixel units
 * relative to the lower left corner of the image, or {@link AVKey#FRACTION},
 * which indicates the units are fractions of the image width and height.
 * <p>
 * Examples:
 * <pre>
...
public class Size {
    ...
    /**
     * Size mode to indicate that the size parameter indicates an explicit
     * dimension measured either in pixels, or as a fraction of the container.
     */
    public static final String EXPLICIT_DIMENSION = "gov.nasa.worldwind.render.Size.ExplicitDimension";

    /**
     * Size mode for width. May be one of
     * {@link #NATIVE_DIMENSION}, {@link #MAINTAIN_ASPECT_RATIO}, or {@link
     * #EXPLICIT_DIMENSION}.
     */
    ...
    /**
     * Size mode for height. May be one of
     * {@link #NATIVE_DIMENSION}, {@link #MAINTAIN_ASPECT_RATIO}, or {@link
     * #EXPLICIT_DIMENSION}.
     */
     ...
     *
     * @param widthMode Width mode, one of
     * {@link #NATIVE_DIMENSION}, {@link #MAINTAIN_ASPECT_RATIO}, or {@link
     *                    #EXPLICIT_DIMENSION}.
     * @param widthParam The width (applies only to {@link #EXPLICIT_DIMENSION}
     * mode).
     * @param widthUnits Units of {@code width}. Either {@link AVKey#PIXELS} or
     * {@link AVKey#PIXELS}.
     * @param heightMode height mode, one of
     * {@link #NATIVE_DIMENSION}, {@link #MAINTAIN_ASPECT_RATIO}, or {@link
     *                    #EXPLICIT_DIMENSION}.
     * @param heightParam The height (applies only to
     * {@link #EXPLICIT_DIMENSION} mode).
     * @param heightUnits Units of {@code height}. Either {@link AVKey#PIXELS}
     * or {@link AVKey#PIXELS}.
     *
     ...
     *
     * @param widthFraction the size's width as a fraction of the containing
     * rectangle.
     * @param heightFraction the size's height as a fraction of the containing
     * rectangle.
     *
     ...
     * Set the width.
     *
     * @param mode Width mode, one of
     * {@link #NATIVE_DIMENSION}, {@link #MAINTAIN_ASPECT_RATIO}, or {@link
     *              #EXPLICIT_DIMENSION}.
     * @param width The width (applies only to {@link #EXPLICIT_DIMENSION}
     * mode).
     * @param units Units of {@code width}. Either {@link AVKey#PIXELS} or
     * {@link AVKey#PIXELS}.
     *
     ...
     * {@link #NATIVE_DIMENSION}, {@link #MAINTAIN_ASPECT_RATIO}, or {@link
     *               #EXPLICIT_DIMENSION}.
     * @param height The width (applies only to {@link #EXPLICIT_DIMENSION}
     * mode).
     * @param units Units of {@code width}. Either {@link AVKey#PIXELS} or
     * {@link AVKey#FRACTION}.
     *
    ...
    /**
     * Returns the units of the offset X value. See
     * {@link #setWidth(String, double, String)} for a description of the
     * recognized values.
     *
    ...
    /**
     * Returns the units of the offset Y value. See
     * {@link #setHeight(String, double, String)} for a description of the
     * recognized values.
     *
     ...
     *
     * @return Unscaled width. The units of this value depend on the current
     * height units.
     *
     * @see #getWidthMode()
     * @see #getWidthUnits()
     ...
     *
     * @return Unscaled height. The units of this value depend on the current
     * height units.
     *
     * @see #getHeightMode()
     * @see #getHeightUnits()
    ...
    /**
     * Computes the width and height of a rectangle within a container
     * rectangle.
     *
     ...
     * @param units One of {@link AVKey#PIXELS} or {@link AVKey#FRACTION}. If
     * the {@code units} value is not one of the expected options,
     * {@link AVKey#PIXELS} is used as the default.
     ...
     * <code>restorableSupport</code>. If <code>context</code> is not
     ...
     *
     * @param restorableSupport the <code>RestorableSupport</code> that receives
     * the size's state.
     * @param context the <code>StateObject</code> the state is appended to, if
     * not <code>null</code>.
     *
     * @throws IllegalArgumentException if <code>restorableSupport</code> is
     * <code>null</code>.
     */
     ...
     * <code>null</code> it's searched for state values, otherwise the
     * <code>RestorableSupport</code> root is searched.
     *
     * @param restorableSupport the <code>RestorableSupport</code> that contains
     * the size's state.
     * @param context the <code>StateObject</code> to search for state values,
    ...
    /**
     * Converts a legacy size mode <code>string</code> ("NativeDimension",
     * "MaintainAspectRatio", "ExplicitDimension"), into one of the mode
     * constants (<code>NATIVE_DIMENSION</code>,
     * <code>MAINTAIN_ASPECT_RATIO</code>, or <code>EXPLICIT_DIMENSION</code>).
     * Returns the input string unmodified if the input does not match a legacy
     * size mode.
     *
     * @param string the legacy size mode <code>String</code> to convert to a
     * size mode.
     *
     * @return a size mode constant, or the input string if <code>string</code>
