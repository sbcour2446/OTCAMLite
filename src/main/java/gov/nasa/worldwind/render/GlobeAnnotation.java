...
/**
 * Represent a text label attached to a Position on the globe and its rendering
 * attributes.
 *
...
public class GlobeAnnotation extends AbstractAnnotation implements Locatable, Movable, Draggable {
    ...
    /**
     * Creates a <code>GlobeAnnotation</code> with the given text, at the given
     * globe <code>Position</code>.
     *
    ...
    /**
     * Creates a <code>GlobeAnnotation</code> with the given text, at the given
     * globe <code>Position</code>. Specify the <code>Font</code> to be used.
     *
     * @param text the annotation text.
     * @param position the annotation <code>Position</code>.
    ...
    /**
     * Creates a <code>GlobeAnnotation</code> with the given text, at the given
     * globe <code>Position</code>. Specify the <code>Font</code> and text
     * <code>Color</code> to be used.
     *
     * @param text the annotation text.
     * @param position the annotation <code>Position</code>.
     * @param font the <code>Font</code> to use.
    ...
    /**
     * Creates a <code>GlobeAnnotation</code> with the given text, at the given
     * globe <code>Position</code>. Specify the default
    ...
    /**
     * Get the annotation's altitude mode. The altitude mode may be null,
     * indicating that the legacy altitude mode described below will be used.
     * <p>
     * <b>Legacy altitude mode</b><br>If the annotation Position elevation is
     * lower then the highest elevation on the globe, the annotation will be
     * drawn above the ground using its elevation as an offset, scaled by the
     * current vertical exaggeration. Otherwise, the original elevation will be
     * used. This functionality is supported for backward compatibility. New
     * code that uses Globe Annotation should specify an altitude mode.
     *
     ...
     * {@code null} indicates that the legacy altitude mode should be used. See
    ...
    /**
     * Returns the real world height of the annotation frame in meter. If this
     * dimension is greater then zero, the annotation will be scaled so as to
     * maintain this fixed dimension, which makes it appear as part of the
     * surrounding terrain. This overrides min and max distance scaling -
     * however min distance opacity is still accounted for.
     * <p>
     * If this dimension is zero, the annotation always maintains the same
     * apparent size with possible scaling relative to the viewport center point
     * if min and max distance scale factors are not one.
     *
    ...
    /**
     * Set the real world height of the annotation frame in meter. If this
     * dimension is greater then zero, the annotation will be scaled so as to
     * maintain this fixed dimension, which makes it appear as part of the
     * surrounding terrain. This overrides min and max distance scaling -
     * however min distance opacity is still accounted for.
    ...
    /**
     * Get the final Vec4 point at which an annotation will be drawn. The
     * altitude mode will be used to determine the annotation point, if an
     * altitude mode has been set. If the altitude mode is null, then the legacy
     * altitude mode described in {@link #getAltitudeMode()} will be used to
     * determine the point.
     *
    ...
    /**
     * Compute the draw point using the legacy altitude mode. See
     * {@link #getAltitudeMode()} for details on the legacy mode.
     *
     * @param dc the current DrawContext.
     *
     * @return the annotation draw Cartesian point
