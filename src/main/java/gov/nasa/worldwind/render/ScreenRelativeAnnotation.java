...
/**
 * Provides a screen annotation positioned relatively to the window rather than
 * absolutely. The annotation's position is specified as fractional locations
 * along the window's X and Y axes. When the window is resized the annotation is
 * repositioned according to its relative coordinates.
 * <p>
 * The annotation can be forced to remain fully visible even if its relative
 * position would place a portion of it outside the window. X and Y margins can
 * be specified to ensure distance between the annotation's edges and the window
 * edges.
 *
 * @author tag
 * @version $Id: ScreenRelativeAnnotation.java 1171 2013-02-11 21:45:02Z
 * dcollins $
 */
public class ScreenRelativeAnnotation extends ScreenAnnotation {
     ...
     * @param xFraction the relative X position of the annotation. A value of 0
     * indicates the window's left edge, a value of 1 indicates its right edge.
     * The annotation is centered horizontally on this position prior to
     * applying any X offest specified in the annotation's attributes.
     * @param yFraction the relative Y position of the annotation. A value of 0
     * indicates the window's bottom edge, a value of 1 indicates the window's
     * top edge. The annotation's lower edge is justified to this position prior
     * to applying any Y offset specified in the annotation's attributes.
     *
    ...
    /**
     * Specifies whether to adjust the annotation's position so that it is
     * always fully visible when the window has sufficient area to display it,
     * even if it would be fully or partially obscured when placed according to
     * its relative coordinates.
     *
     * @param keepFullyVisible true to keep the annotation fully visible,
     * otherwise false.
     */
     ...
     *
     * @return the annotation's relative X position, as specified to the
     * constructor or {@link #setXFraction(double)}.
     */
    ...
    /**
     * Specifies the annotation's relative X position. A value of 0 indicates
     * the window's left edge, a value of 1 indicates its right edge.
     *
     ...
     *
     * @return the annotation's relative Y position, as specified to the
     * constructor or {@link #setYFraction(double)}.
     */
    ...
    /**
     * Specifies the annotation's relative Y position. A value of 0 indicates
     * the window's lower edge, a value of 1 indicates its top edge.
     *
    ...
    /**
     * Specifies the annotation's X margin, the minimum distance to maintain
     * between the annotation's leading and trailing edges and the respective
     * window edge. Used only when the <code>keepFullyVisible</code> flag is
     * true.
     *
    ...
    /**
     * Specifies the annotation's Y margin, the minimum distance to maintain
     * between the annotation's top and bottom edges and the respective window
     * edge. Used only when the <code>keepFullyVisible</code> flag is true.
     *
    ...
    /**
     * Computes and returns the screen point in pixels corresponding to the
     * annotation's relative position coordinates and the current window size.
     *
     * @return the pixel coordinates corresponding to the annotation's relative
     * coordinates. The pixel coordinate origin is the lower left of the window.
     */
    @Override
