 ...
 * {@link gov.nasa.worldwind.render.Renderable}s. Implements common attribute
 * handling and rendering flow for outlined shapes. Provides common defaults and
 * common export code.
 * <p>
...
public abstract class AbstractShape extends WWObjectImpl
    ...
    /**
     * Compute the intersections of a specified line with this shape. If the
     * shape's altitude mode is other than {@link
     * WorldWind#ABSOLUTE}, the shape's geometry is created relative to the
     * specified terrain rather than the terrain used during rendering, which
     * may be at lower level of detail than required for accurate intersection
     * determination.
     *
     * @param line the line to intersect.
     * @param terrain the {@link Terrain} to use when computing the shape's
     * geometry.
     *
     * @return a list of intersections identifying where the line intersects the
     * shape, or null if the line does not intersect the shape.
     *
    ...
    /**
     * Called during construction to establish any subclass-specific state such
     * as different default values than those set by this class.
     */
    abstract protected void initialize();

    /**
     * Indicates whether texture should be applied to this shape. Called during
     * rendering to determine whether texture state should be established during
     * preparation for interior drawing.
     * <p>
    ...
    /**
     * Produces the geometry and other state necessary to represent this shape
     * as an ordered renderable. Places this shape on the draw context's ordered
     * renderable list for subsequent rendering. This method is called during {@link
     * #pick(DrawContext, java.awt.Point)} and {@link #render(DrawContext)} when
     * it's been determined that the shape is likely to be visible.
     *
     * @param dc the current draw context.
     *
     * @return true if the ordered renderable state was successfully computed,
     * otherwise false, in which case the current pick or render pass is
     * terminated for this shape. Subclasses should return false if it is not
    ...
    /**
     * Determines whether this shape's ordered renderable state is valid and can
     * be rendered. Called by {@link
     * #makeOrderedRenderable(DrawContext)}just prior to adding the shape to the
     * ordered renderable list.
     *
     * @param dc the current draw context.
     *
     * @return true if this shape is ready to be rendered as an ordered
     * renderable.
     */
     ...
     * ShapeAttributes, ShapeAttributes)}, which establishes OpenGL state for
     * lighting, blending, pick color and line attributes. Subclasses should
     * execute the drawing commands specific to the type of shape.
     * <p>
     ...
     * ShapeAttributes, ShapeAttributes)}, which establishes OpenGL state for
     * lighting, blending, pick color and interior attributes. Subclasses should
     * execute the drawing commands specific to the type of shape.
     * <p>
     * A {@link gov.nasa.worldwind.render.AbstractShape.AbstractShapeData} must
     * be current when this method is called.
    ...
    /**
     * Fill this shape's vertex buffer objects. If the vertex buffer object
     * resource IDs don't yet exist, create them.
     * <p>
     * A {@link gov.nasa.worldwind.render.AbstractShape.AbstractShapeData} must
     * be current when this method is called.
     *
     ...
     * destination.
     * @throws XMLStreamException if an exception occurs converting this shape's
     * fields to XML.
     */
    ...
    /**
     * The attributes active for a particular pick and render pass. These are
     * determined according to the highlighting mode.
     */
    ...
    /**
     * Holds globe-dependent computed data. One entry per globe encountered
     * during {@link #render(DrawContext)}.
     */
     ...
     * {@link #render(DrawContext)}. The active data is drawn from this shape's
     * data cache at the beginning of the <code>render</code> method.
     */
    ...
    protected static class AbstractShapeData extends ShapeDataCache.ShapeDataCacheEntry {
        ...
        /**
         * A quick-to-compute metric to determine eye distance changes that
         * invalidate this entry's geometry.
         */
        protected Double referenceDistance;
        /**
         * The GPU-resource cache key to use for this entry's VBOs, if VBOs are
         * used.
         */
        protected Object vboCacheKey = new Object();

        /**
         * Constructs a data cache entry and initializes its globe-dependent
         * state key for the globe in the specified draw context and capture the
         * current vertical exaggeration. The entry becomes invalid when these
         * values change or when the entry's expiration timer expires.
         *
         * @param dc the current draw context.
         * @param minExpiryTime the minimum number of milliseconds to use this
         * shape before regenerating its geometry.
         * @param maxExpiryTime the maximum number of milliseconds to use this
         * shape before regenerating its geometry.
         */
     ...
     * {@link gov.nasa.worldwind.render.OutlinedShape}s.
     */
    ...
    /**
     * Invokes {@link #initialize()} during construction and sets the data
     * cache's expiration time to a default value.
     */
    ...
    /**
     * Invalidates computed values. Called when this shape's contents or certain
     * attributes change.
     */
     ...
     *
     * @param highlightAttrs the highlight attributes. May be null, in which
     * case default attributes are used.
     */
     ...
     *
     * @param visible true to draw this shape, otherwise false. The default
     * value is true.
     *
    ...
    /**
     * Indicates whether batch rendering is enabled for the concrete shape type
     * of this shape.
     *
    ...
    /**
     * Specifies whether adjacent shapes of this shape's concrete type in the
     * ordered renderable list may be rendered together if they are contained in
     * the same layer. This increases performance. There is seldom a reason to
     * disable it.
     *
     * @param enableBatchRendering true to enable batch rendering, otherwise
     * false.
     */
    ...
    /**
     * Specifies whether adjacent shapes of this shape's concrete type in the
     * ordered renderable list may be pick-tested together if they are contained
     * in the same layer. This increases performance but allows only the
     * top-most of the polygons to be reported in a
     * {@link gov.nasa.worldwind.event.SelectEvent} even if several of the
     * polygons are at the pick position.
     * <p>
     * Batch rendering ({@link #setEnableBatchRendering(boolean)}) must be
     * enabled in order for batch picking to occur.
     *
     * @param enableBatchPicking true to enable batch rendering, otherwise
     * false.
     */
    ...
    /**
     * Indicates the outline line width to use during picking. A larger width
     * than normal typically makes the outline easier to pick.
     *
    ...
    /**
     * Specifies the outline line width to use during picking. A larger width
     * than normal typically makes the outline easier to pick.
     * <p>
     * Note that the size of the pick aperture also affects the precision
     * necessary to pick.
     *
    ...
    /**
     * Indicates whether the filled sides of this shape should be offset towards
     * the viewer to help eliminate artifacts when two or more faces of this or
     * other filled shapes are coincident.
     *
    ...
    /**
     * Specifies whether the filled sides of this shape should be offset towards
     * the viewer to help eliminate artifacts when two or more faces of this or
     * other filled shapes are coincident. See {@link
     * gov.nasa.worldwind.render.Offset}.
     *
     * @param enableDepthOffset true if depth offset is applied, otherwise
     * false.
     */
     ...
     * #setGeometryRegenerationInterval(int)} for the regeneration-interval's
     * description.
     *
    ...
    /**
     * Specifies the maximum length of time between geometry regenerations. The
     * geometry is regenerated when this shape's altitude mode is
     * {@link WorldWind#CLAMP_TO_GROUND} or {@link WorldWind#RELATIVE_TO_GROUND}
     * in order to capture changes to the terrain. (The terrain changes when its
     * resolution changes or when new elevation data is returned from a server.)
     * Decreasing this value causes the geometry to more quickly track terrain
     * changes but at the cost of performance. Increasing this value often does
     * not have much effect because there are limiting factors other than
     * geometry regeneration.
     *
     * @param geometryRegenerationInterval the geometry regeneration interval,
     * in milliseconds. The default is two seconds.
     */
    ...
    /**
     * Specifies the position to use as a reference position for computed
     * geometry. This value should typically left to the default value of the
     * first position in the polygon's outer boundary.
     *
     * @param referencePosition the reference position. May be null, in which
     * case the first position of the outer boundary is the reference position.
     */
    ...
    /**
     * Returns the Cartesian coordinates of this shape's reference position as
     * computed during the most recent rendering.
     *
     * @return the Cartesian coordinates corresponding to this shape's reference
     * position, or null if the point has not been computed.
     */
    public Vec4 getReferencePoint() {
     ...
     * each frame. Places the result in this shape's current active attributes.
     *
    ...
    /**
     * Returns this shape's currently active attributes, as determined during
     * the most recent call to {@link
     * #determineActiveAttributes()}. The active attributes are either the
     * normal or highlight attributes, depending on this shape's highlight flag,
     * and incorporates default attributes for those not specified in the
    ...
    /**
     * Indicates whether this shape's renderable geometry must be recomputed,
     * either as a result of an attribute or property change or the expiration
     * of the geometry regeneration interval.
     * <p>
     * A {@link gov.nasa.worldwind.render.AbstractShape.AbstractShapeData} must
     * be current when this method is called.
     *
     * @param dc the current draw context.
     *
     * @return true if this shape's geometry must be regenerated, otherwise
     * false.
     */
     ...
     * @param dc the current draw context.
     *
     * @return true if this shape should use vertex buffer objects, otherwise
     * false.
     */
    ...
    /**
     * Indicates whether standard lighting must be applied by consulting the
     * current active attributes. Calls {@link
     * #mustApplyLighting(DrawContext, ShapeAttributes)}, specifying null for
     * the activeAttrs.
     *
     * @param dc the current draw context
     *
    ...
    /**
     * Indicates whether standard lighting must be applied by consulting either
     * the specified active attributes or the current active attributes.
     *
     * @param dc the current draw context
     * @param activeAttrs the attribute bundle to consider when determining
     * whether lighting is applied. May be null, in which case the current
     * active attributes are used.
     *
     * @return true if lighting must be applied, otherwise false.
     */
    ...
    /**
     * Indicates whether normal vectors must be computed by consulting the
     * current active attributes. Calls {@link
     * #mustCreateNormals(DrawContext, ShapeAttributes)}, specifying null for
     * the activeAttrs.
     *
     * @param dc the current draw context
     *
    ...
    /**
     * Indicates whether normal vectors must be computed by consulting either
     * the specified active attributes or the current active attributes. Calls
     * {@link #mustApplyLighting(DrawContext, ShapeAttributes)}, passing the
     * specified active attrs.
     *
     * @param dc the current draw context
     * @param activeAttrs the attribute bundle to consider when determining
     * whether normals should be computed. May be null, in which case the
     * current active attributes are used.
     *
     * @return true if normal vectors must be computed, otherwise false.
     */
    ...
    /**
     * Returns a {@link SurfaceShape} that corresponds to this Path and is used
     * for drawing on 2D globes.
     *
    ...
    /**
     * Sets surface shape parameters prior to picking and rendering the 2D shape
     * used to represent this shape on 2D globes. Subclasses should override
     * this method if they need to update more than the highlighted state,
     * visibility state and delegate owner.
     */
    ...
    /**
     * Determines whether to add this shape to the draw context's ordered
     * renderable list. Creates this shapes renderable geometry.
     * <p>
     * A {@link gov.nasa.worldwind.render.AbstractShape.AbstractShapeData} must
     * be current when this method is called.
     *
     ...
     *
     * @return true if this shape's geometry depends on the terrain, otherwise
     * false.
     */
    ...
    /**
     * Indicates whether this shape's terrain-dependent geometry is continually
     * computed as its distance from the eye point changes. This is often
     * necessary to ensure that the shape is updated as the terrain precision
     * changes. But it's often not necessary as well, and can be disabled.
     *
     * @return true if the terrain dependent geometry is updated as the eye
     * distance changes, otherwise false. The default is true.
     */
    ...
    /**
     * Specifies whether this shape's terrain-dependent geometry is continually
     * computed as its distance from the eye point changes. This is often
     * necessary to ensure that the shape is updated as the terrain precision
     * changes. But it's often not necessary as well, and can be disabled.
     *
     * @param viewDistanceExpiration true to enable view distance expiration,
     * otherwise false.
     */
    ...
    /**
     * Determines whether this shape's geometry should be invalidated because
     * the view distance changed, and if so, invalidates the geometry.
     *
     * @param dc the current draw context.
     */
    ...
    /**
     * Draws this ordered renderable and all subsequent Path ordered renderables
     * in the ordered renderable list. If the current pick mode is true, only
     * shapes within the same layer are drawn as a batch.
     * <p>
     * A {@link gov.nasa.worldwind.render.AbstractShape.AbstractShapeData} must
     * be current when this method is called.
     *
    ...
    /**
     * Draw this shape as an ordered renderable. If in picking mode, add it to
     * the picked object list of specified {@link PickSupport}. The
     * <code>PickSupport</code> may not be the one associated with this
     * instance. During batch picking the <code>PickSupport</code> of the
     * instance initiating the batch picking is used so that all shapes rendered
     * in batch are added to the same pick list.
     * <p>
     * A {@link gov.nasa.worldwind.render.AbstractShape.AbstractShapeData} must
     * be current when this method is called.
     *
     * @param dc the current draw context.
     * @param pickCandidates a pick support holding the picked object list to
     * add this shape to.
     */
    ...
    /**
     * Creates a {@link gov.nasa.worldwind.pick.PickedObject} for this shape and
     * the specified unique pick color. The PickedObject returned by this method
     ...
     *
     * @deprecated Use the more general {@link #createPickedObject(int)}
     * instead.
     */
    @Deprecated
    ...
    /**
     * Creates a {@link gov.nasa.worldwind.pick.PickedObject} for this shape and
     * the specified unique pick color code. The PickedObject returned by this
     * method will be added to the pick list to represent the current shape.
     *
     ...
     *
     * @param dc the current draw context.
     * @param attrMask an attribute mask indicating state the caller will set.
     * This base class implementation sets      <code>GL_CURRENT_BIT, GL_LINE_BIT, GL_HINT_BIT, GL_POLYGON_BIT, GL_COLOR_BUFFER_BIT, and
     *                 GL_TRANSFORM_BIT</code>.
     *
     * @return the stack handler used to set the OpenGL state. Callers should
     * use this to set additional state, especially state indicated in the
     * attribute mask argument.
     */
    ...
    /**
     * Establishes OpenGL state for drawing the outline, including setting the
     * color/material, line smoothing, line width and stipple. Disables texture.
     *
     * @param dc the current draw context.
     * @param activeAttrs the attributes indicating the state value to set.
     * @param defaultAttrs the attributes to use if <code>activeAttrs</code>
     * does not contain a necessary value.
     */
    ...
    /**
     * Establishes OpenGL state for drawing the interior, including setting the
     * color/material. Enabling texture is left to the subclass.
     *
     * @param dc the current draw context.
     * @param activeAttrs the attributes indicating the state value to set.
     * @param defaultAttrs the attributes to use if <code>activeAttrs</code>
    ...
    /**
     * Computes a model-coordinate point from a position, applying this shape's
     * altitude mode.
     *
     * @param terrain the terrain to compute a point relative to the globe's
     * surface.
     * @param position the position to compute a point for.
     *
     * @return the model-coordinate point corresponding to the position and this
     * shape's shape type.
     */
    ...
    /**
     * Computes a model-coordinate point from a position, applying this shape's
     * altitude mode, and using <code>CLAMP_TO_GROUND</code> if the current
     * globe is 2D.
     *
     * @param dc the current draw context.
     * @param terrain the terrain to compute a point relative to the globe's
     ...
     * @param verticalExaggeration the vertical exaggeration to apply to
     * computed terrain points.
     * @param positions the positions to compute the extent for.
     *
     * @return the extent, or null if an extent cannot be computed. Null is
     * returned if either <code>globe</code> or <code>positions</code> is null.
     */
     ...
     * @param dc the current draw context.
     *
     * @return an array containing the coordinate vertex buffer ID in the first
     * position and the index vertex buffer ID in the second position.
     */
    ...
    /**
     * Removes from the GPU resource cache the entry for the current data cache
     * entry's VBOs.
     * <p>
     * A {@link gov.nasa.worldwind.render.AbstractShape.AbstractShapeData} must
     * be current when this method is called.
    ...
    /**
     * Export the placemark to KML as a {@code <Placemark>} element. The
