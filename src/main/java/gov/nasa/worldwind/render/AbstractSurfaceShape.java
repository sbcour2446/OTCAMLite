 ...
 * gov.nasa.worldwind.render.SurfaceQuad}, and
 * {@link gov.nasa.worldwind.render.SurfaceSector}.
 * <p>
 * SurfaceShapes have separate attributes for normal display and highlighted
 * display. If no attributes are specified, default attributes are used. See
 * {@link #DEFAULT_INTERIOR_MATERIAL}, {@link #DEFAULT_OUTLINE_MATERIAL}, and {@link
 * #DEFAULT_HIGHLIGHT_MATERIAL}.
 * <p>
 * AbstractSurfaceShape extends from
 * {@link gov.nasa.worldwind.render.AbstractSurfaceObject}, and therefore
 * inherits AbstractSurfaceObject's batch rendering capabilities.
 *
 * @author dcollins
...
public abstract class AbstractSurfaceShape extends AbstractSurfaceObject implements SurfaceShape, Movable, Movable2,
    ...
    /**
     * Constructs a new surface shape with the specified normal (as opposed to
     * highlight) attributes. Modifying the attribute reference after calling
     * this constructor causes this shape's appearance to change accordingly.
     *
     ...
     * {@inheritDoc}
     * <p>
     * The returned state key is constructed the SurfaceShape's unique ID, last
     * modified time, and its active attributes. The returned state key has no
     * dependency on the {@link gov.nasa.worldwind.globes.Globe}. Subclasses
     * that depend on the Globe should return a state key that include the
     * globe's state key.
     */
    @Override
    ...
    /**
     * Computes the bounding sectors for the shape. There will be more than one
     * if the shape crosses the date line, but does not enclose a pole.
     *
     ...
     * {@inheritDoc}
     * <p>
     * Overridden to clear this SurfaceShape's internal sector and geometry
     * caches.
     */
    @Override
    ...
    //**************************************************************//
    /**
     * Overridden to determine the shape's active attributes during
     * preRendering, prior to building the shape's pickable representation and
     * the SceneController's composite representation.
     *
    ...
    /**
     * Indicates whether the shape is a closed polygon that can enclose a pole,
     * or an open path that cannot. This makes a difference when computing the
     * bounding sector for a shape. For example, consider the positions (-100,
     * 85), (0, 80), (100, 80). If these positions are treated as a closed
     * polygon (a triangle over the North Pole) then the bounding sector is 80
     * to 90 lat, -180 to 180 lon. But if they are treated as an open path (a
     * line wrapping partway around the pole) then the bounding sector is 80 to
     * 85 lat, -100 to 100 lon.
     *
     * @return True if the shape is a closed polygon that can contain a pole, or
     * false if it is treated as an open path that cannot contain a pole.
     */
    ...
    /**
     * Determine if a list of geographic locations encloses either the North or
     * South pole. The list is treated as a closed loop. (If the first and last
     * positions are not equal the loop will be closed for purposes of this
     * computation.)
     *
     * @param locations Locations to test.
     *
     * @return AVKey.NORTH if the North Pole is enclosed, AVKey.SOUTH if the
     * South Pole is enclosed, or null if neither pole is enclosed. Always
     * returns null if {@link #canContainPole()} returns false.
     */
    ...
    /**
     * Divide a list of locations that encloses a pole along the international
     * date line. This method determines where the locations cross the date
     * line, and inserts locations to the pole, and then back to the
     * intersection position. This allows the shape to be "unrolled" when
     * projected in a lat-lon projection.
     *
     * @param locations Locations to cut at date line. This list is not
     * modified.
     * @param pole Pole contained by locations, either AVKey.NORTH or
     * AVKey.SOUTH.
     * @param globe Current globe.
     *
     * @return New location list with locations added to correctly handle date
     * line intersection.
     */
    ...
    /**
     * Returns a list containing two copies of the specified list of locations
     * crossing the dateline: one that extends across the -180 longitude
     * boundary and one that extends across the +180 longitude boundary. If the
     * list of locations does not cross the dateline this returns a list
    ...
    /**
     * Restores state values from previous versions of the SurfaceShape state
     * XML. These values are stored or named differently than the current
     * implementation. Those values which have not changed are ignored here, and
     * will restored in {@link #doRestoreState(gov.nasa.worldwind.util.RestorableSupport,
     * gov.nasa.worldwind.util.RestorableSupport.StateObject)}.
     *
     * @param rs RestorableSupport object which contains the state value
     * properties.
     * @param context active context in the RestorableSupport to read state
     * from.
     */
     ...
     * gov.nasa.worldwind.render.AbstractSurfaceObject.SurfaceObjectStateKey} by
     * adding the shape's current {@link
     * gov.nasa.worldwind.render.ShapeAttributes} and the globe's state key.
     * <p>
     * SurfaceShapeStateKey uniquely identifies a surface shapes's current state
     * exactly as SurfaceObjectStateKey does, but also distinguishes the shape's
     * active ShapeAttributes from any previous attributes, and distinguishes
     * between different globes via the globe state key.
     */
    protected static class SurfaceShapeStateKey extends SurfaceObjectStateKey {

        /**
         * The SurfaceShape's attributes. May be null if the shape has no
         * attributes.
         */
        protected final ShapeAttributes attributes;
        /**
         * The Globe's state key. May be null if the shape's state does not
         * depend on the globe.
         */
        protected final Object globeStateKey;

        /**
         * Constructs a new SurfaceShapeStateKey with the specified unique ID,
         * modified time, attributes, and globe state key. The globe state key
         * should be null if the surface shape does not depend on the globe.
         *
         * @param uniqueID the SurfaceShape's unique ID.
         * @param modifiedTime the SurfaceShape's modified time.
         * @param attributes the SurfaceShape's attributes, or null if the shape
         * has no attributes.
         * @param globeStateKey the globe's state key, or null if the shape does
         * not depend on the globe.
         *
        ...
        /**
         * Returns the state key's size in bytes. Overridden to include the
         * attributes and the reference to the globe state key.
         *
     ...
     *
     * @return One of
     * {@link Exportable#FORMAT_SUPPORTED}, {@link Exportable#FORMAT_NOT_SUPPORTED},
     * or {@link
    ...
    /**
     * Export the Polygon. The {@code output} object will receive the exported
     ...
     *
     * @throws java.io.IOException If an exception occurs writing to the output
     * object.
     * @throws UnsupportedOperationException if the format is not supported by
     * this object, or if the {@code output} argument is not of a supported
     * type.
     */
