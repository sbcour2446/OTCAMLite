 ...
 * If A is the radius in the north-south direction, and b is the radius in the
 * east-west direction, and c is the radius in the vertical direction
 * (increasing altitude), then A == B == C defines a unit shape, A == B &gt; C
 * defines a vertically flattened shape (disk-shaped), A == B &lt; C defines a
 * vertically stretched shape.
 *
 * @author ccrick
...
public abstract class RigidShape extends AbstractShape {
    ...
    protected static class ShapeData extends AbstractShapeData {
        ...
        /**
         * The GPU-resource cache keys to use for this entry's VBOs (one for
         * eack LOD), if VBOs are used.
         */
         ...
         * new buffer is used or other reason.
         */
     ...
     * This method returns null if this piece of geometry's image source has not
     * yet been retrieved.
     *
     * @param index the index of the piece of geometry whose texture will be
     * returned.
     *
     ...
     *
     * @param index the index of the piece of geometry for which we are setting
     * the texture.
    ...
    /**
     * Indicates the image source for this shape's optional texture for the
     * #index piece of geometry.
     *
     * @param index the index of the piece of geometry for which we are
    ...
    /**
     * Specifies the single image source for this shape's optional texture, to
     * be placed on every face.
     *
     * @param imageSource the texture image source. May be a
     * {@link java.io.File}, file path, a stream, a URL or a
     ...
     *
     * @param imageSources the list of texture image sources. May be
     * {@link java.io.File}, file path, a stream, a URL or a
     * {@link java.awt.image.BufferedImage}.
     */
    ...
    /**
     * Returns the pair of texture coordinate offsets corresponding to the shape
     * face and texture coordinate specified by the faceIndex and offsetIndex.
     *
     * @param faceIndex the shape face from which to retrieve the offset pair
     * @param offsetIndex the index of the specific texture coordinate on the
     * face whose offsets we wish to retrieve
     *
    ...
    /**
     * Sets the u and v texture coordinate offsets for the specified texture
     * coordinate on the specified shape face.
     *
     * @param faceIndex the shape face on which we would like to set the texture
     * coordinate offsets
     * @param offsetIndex the index of the particular texture coordinate we
     * would like to set offsets for
    ...
    /**
     * Indicates the radius of this shape's axis in the north-south
    ...
    /**
     * Specifies this shape's radius in meters in the north-south (latitudinal)
     * direction. The radius must be greater than 0.
     *
     * @param northSouthRadius the shape radius in the north-south direction.
    ...
    /**
     * Indicates the radius of this shape's axis in the east-west (longitudinal)
     * direction.
     *
    ...
    /**
     * Specifies this shape's radius in meters in the east-west (longitudinal)
     * direction. The radius must be greater than 0.
     *
     * @param eastWestRadius the shape radius in the east-west direction. Must
     * be greater than 0.
     *
     * @throws IllegalArgumentException if the radius is not greater than 0.
     */
    ...
    /**
     * Indicates the radius of this shape's axis in the vertical (altitudinal)
     * direction.
     *
    ...
    /**
     * Specifies this shape's radius in meters in the vertical (altitudinal)
     * direction. The radius must be greater than 0.
     *
     * @param verticalRadius the shape radius in the vertical direction. Must be
     * greater than 0.
     *
     * @throws IllegalArgumentException if the radius is not greater than 0.
     */
     ...
     * North corresponds to an azimuth of 0. Angles are in degrees and positive
     * clockwise.
     *
     ...
     * distance from the eye. Values greater than 0 cause higher resolution
     * tessellation, but at an increased performance cost. Values less than 0
     * decrease the default tessellation resolution at any given distance from
     * the eye. The default value is 0. Values typically range between -0.5 and
     * 0.5.
     *
     * @param detailHint the degree to modify the default tessellation
     * resolution of the shape. Values greater than 1 increase the resolution.
     * Values less than zero decrease the resolution. The default value is 0.
     */
     ...
     *
     * Code within this class assumes that this shape's extent is computed from
     * the shape's fields rather than its computed geometry. If you override
     * this method, be sure that this lack of dependency on computed geometry is
     * adhered to by the overriding method. See doMakeOrderedRenderable above
     * for a description of why this assumption is made. Also see WWJ-482.
     *
     ...
     * @param numVertices the number of distinct vertices in the buffer (assume
     * 3-space)
    ...
    /**
     * Sets the shape's referencePoint, which is essentially its centerPosition
     * in Cartesian coordinates.
     *
     * @param dc the current DrawContext
     *
     * @return the computed reference point relative to the globe associated
     * with the draw context.
     */
     ...
     * @param verticalExaggeration the current vertical exaggeration
     *
     * @return the computed reference point, or null if the point could not be
     * computed.
     */
    ...
    /**
     * Computes the transform to use during rendering to convert the unit sphere
     * geometry representation of this shape to its correct shape location,
    ...
    /**
     * Computes the inverse of the transform to use during rendering to convert
     * the unit geometry representation of this shape to its correct shape
     * location, orientation and scale, essentially bringing the shape back into
    ...
    /**
     * Computes the number of subdivisions necessary to achieve the expected
     * Level of Detail given the shape's relationship to the viewer.
     *
     * @param dc the current drawContext.
    ...
    //*****************************************************************//
    /**
     * Sets the Geometry mesh for this shape, either by pulling it from the
     * geometryCache, or by creating it anew if the appropriate geometry does
     ...
     *
     * @throws IllegalArgumentException if the draw context is null or the
     * element buffer is null
     */
    ...
    /**
     * Renders the shape, using data from the provided buffer and the given
     * parameters.
     *
     * @param dc the current draw context
     ...
     * @param elementBuffer the buffer containing the list of elements to be
     * drawn
     ...
     * @param terrain the {@link Terrain} to use when computing the wedge's
     * geometry.
     *
    ...
    /**
     * Returns intersections of line with the ith face of this shape, Assumes we
     * already know the line intersects the shape somewhere (but perhaps not on
     * this face)
     *
     * @param line the line to intersect.
     ...
     *
     * @return a list of intersections identifying where the line intersects
     * this shape face, or null if the line does not intersect this face.
     *
     * @throws InterruptedException if the operation is interrupted.
     */
