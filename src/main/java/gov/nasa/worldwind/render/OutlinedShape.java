 ...
 * DrawContext#drawOutlinedShape(OutlinedShape, Object)} that renders outlines
 * around filled shapes correctly and resolves depth-buffer fighting between the
 * shape being drawn and those previously drawn. The methods of this interface
 * are called by the draw context during the multi-pass rendering.
 *
 * @author tag
...
public interface OutlinedShape {
    ...
    /**
     * Indicates whether the shape's depth should be adjusted to give its filled
     * faces priority over coincident items previously drawn.
     *
     * @param dc the current draw context.
     * @param shape the shape being drawn.
     *
     ...
     * <p>
     * The amount of depth offset when depth offset is enabled is computed by
     * the formula <i>factor</i> * DZ + r
     *
     * <i>units</i>, where DZ is a measurement of the change in depth relative
     * to the screen area of the shape, and r is the smallest value guaranteed
     * to produce a resolvable offset. <i>units</i> is the value return by {@link
     ...
     * <i>units</i>, where DZ is a measurement of the change in depth relative
     * to the screen area of the shape, and r is the smallest value guaranteed
     * to produce a resolvable offset. <i>factor</i> is the value return by {@link
