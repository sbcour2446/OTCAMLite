...
/**
 * A multi-sided 3D shell formed by a base polygon in latitude and longitude
 * extruded from the terrain to either a specified height or an independent
 * height per location. The base polygon may be complex with multiple internal
 * but not intersecting contours.
 * <p>
 * Extruded polygon boundaries may be specified using either {@link LatLon}
 * locations or {@link Position} positions, but all the shape's boundary
 * vertices must be the same type.
 * <p>
 * Extruded polygons may optionally be textured. Textures may be applied to both
 * the faces of the outer and inner boundaries or just the outer boundaries.
 * Texture can also be applied independently to the cap. Standard lighting is
 * optionally applied. Texture source images are lazily retrieved and loaded.
 * This can cause a brief period in which the texture is not displayed while it
 * is retrieved from disk or network.
 * <p>
 * <code>ExtrudedPolygon</code> side faces and cap have independent attributes
 * for both normal and highlighted drawing.
 * <p>
 * When specifying a single height (altitude mode {@link WorldWind#CONSTANT},
 * the height is relative to a reference location designated by one of the
 * specified polygon locations in the outer boundary. The default reference
 * location is the first one in the polygon's outer boundary. An alternative
 * location may be specified by calling {@link
 * #setReferenceLocation(LatLon)}. The extruded polygon is capped with a plane
 * at the specified height and tangent to the ellipsoid at the reference
 * location. Since locations other than the reference location may resolve to
 * points at elevations other than that at the reference location, the distances
 * from those points to the cap are adjusted so that the adjacent sides
 * precisely meet the cap. When specifying polygons using a single height, only
 * the latitudes and longitudes of polygon boundary positions must be specified.
 * <p>
 * Independent per-location heights may be specified via the <i>altitude</i>
 * field of {@link Position}s defining the polygon's inner and outer boundaries.
 * Depending on the specified altitude mode, the position altitudes may be
 * interpreted as altitudes relative to mean sea level or altitudes above the
 * ground at the associated latitude and longitude locations.
 * <p>
 * Boundaries are required to be closed, their first location must be equal to
 * their last location. Boundaries that are not closed are explicitly closed by
 * this shape when they are specified.
 * <p>
 * Extruded polygons are safe to share among WorldWindows. They should not be
 * shared among layers in the same World Window.
 * <p>
 * In order to support simultaneous use of this shape with multiple globes
 * (windows), this shape maintains a cache of data computed relative to each
 * globe. During rendering, the data for the currently active globe, as
 * indicated in the draw context, is made current. Subsequently called methods
 * rely on the existence of this current data cache entry.
 * <p>
 * When drawn on a 2D globe, this shape uses a {@link SurfacePolygon} to
 * represent itself. Cap texture is not supported in this case.
 *
 * @author tag
...
public class ExtrudedPolygon extends AbstractShape {
    ...
    protected static class ShapeData extends AbstractShapeData implements Iterable<ExtrudedBoundaryInfo> {
        ...
        /**
         * A buffer holding the Cartesian vertices of all the shape's side
         * vertices.
         */
         ...
         * Non-null only when texture is applied.
         */
        ...
        /**
         * The indices identifying the cap vertices in a shape data's vertex
         * buffer. Determined when this shape is tessellated, which occurs only
         * once unless the shape's boundaries are re-specified.
         */
        ...
        /**
         * Indicates whether a tessellation error occurred. No more attempts to
         * tessellate will be made if set to true.
         */
        protected boolean tessellationError = false;

        /**
         * Constructs an instance using the boundaries of a specified extruded
         * polygon.
         *
    ...
    /**
     * Holds globe-specific information for each contour of the polygon. This
     * class is meant only to be used as a way to group per-boundary information
     * in globe-specific <code>ShapeData</code>.
     */
    protected static class ExtrudedBoundaryInfo {

        /**
         * The boundary vertices. This is merely a reference to the paren't
         * shape's boundaries.
         */
        ...
        /**
         * The vertices defining the boundary's base. These are always on the
         * terrain.
         */
        ...
        protected FloatBuffer capVertexBuffer;
        /**
         * A buffer holding the boundary cap's vertex normals. Non-null only
         * when lighting is applied.
         */
        protected FloatBuffer capNormalBuffer;

        /**
         * The indices identifying the boundary's side faces in the side-vertex
         * buffer.
         */
        protected IntBuffer sideIndices;
        /**
         * The indices identifying the boundary's edge indices in the
         * side-vertex buffer.
         */
        ...
        protected FloatBuffer sideVertexBuffer;
        /**
         * A buffer holding per-vertex normals. Non-null only when lighting is
         * applied.
         */
        protected FloatBuffer sideNormalBuffer;
        /**
        ...
        /**
         * The texture coordinates to use when applying side textures, a
         * coordinate pair for each of 4 corners. These are globe-specific
         * because they account for the varying positions of the base on the
         * terrain. (The cap texture coordinates are not globe-specific.)
         */
         ...
         *
         * @param locations the boundary locations. Only this reference is kept;
         * the boundaries are not copied.
         */
    ...
    /**
     * Indicates the number of vertices that must be present in order for VBOs
     * to be used to render this shape.
     */
    protected static final int VBO_THRESHOLD = Configuration.getIntegerValue(AVKey.VBO_THRESHOLD, 30);

    /**
     * The location of each vertex in this shape's boundaries. There is one list
     * per boundary. There is always an entry for the outer boundary, but its
     * list is empty if an outer boundary has not been specified.
     */
    ...
    /**
     * The currently active side attributes, derived from the specified
     * attributes. Current only during rendering.
     */
    ...
    /**
     * The globe state key for the globe used in the most recent intersection
     * calculation.
     */
    ...
    /**
     * Constructs an extruded polygon with an empty outer boundary and a default
     * height of 1 meter.
     */
    ...
    /**
     * Constructs an extruded polygon of a specified height and an empty outer
     * boundary.
     *
     * @param height the shape height, in meters. May be null, in which case a
     * height of 1 is used. The height is used only when the altitude mode is
     * {@link WorldWind#CONSTANT}, which is the default for this shape.
     */
    ...
    /**
     * Constructs an extruded polygon for a specified list of outer boundary
     * locations and a height.
     *
     * @param corners the list of locations defining this extruded polygon's
     * outer boundary.
     * @param height the shape height, in meters. May be null, in which case a
     * height of 1 is used. The height is used only when the altitude mode is
     * {@link WorldWind#CONSTANT}, which is the default for this shape.
     *
     * @throws IllegalArgumentException if the location list is null or the
    ...
    /**
     * Constructs an extruded polygon from an outer boundary, a height, and
     * images for its outer faces.
     *
     * @param corners the list of locations defining this polygon's outer
     * boundary.
     * @param height the shape height, in meters. May be null, in which case a
     * height of 1 is used. The height is used only when the altitude mode is
     * {@link WorldWind#CONSTANT}, which is the default for this shape.
     * @param imageSources images to apply to the polygon's outer faces. One
     * image for each face must be included. May also be null.
     *
     * @throws IllegalArgumentException if the location list is null or the
     * height is specified but less than or equal to zero.
     */
     ...
     *
     * @param corners the list of outer boundary positions -- latitude longitude
     * and altitude. The altitude mode determines whether the positions are
     * considered relative to mean sea level (they are "absolute") or the ground
    ...
    /**
     * Constructs an extruded polygon from an outer boundary specified with
     * position heights.
     *
     * @param corners the list of positions -- latitude longitude and altitude
     * -- defining the polygon's outer boundary. The altitude mode determines
     * whether the positions are considered relative to mean sea level (they are
     * "absolute") or the ground elevation at the associated latitude and
     * longitude.
     *
     * @throws IllegalArgumentException if the position list is null.
     */
    ...
    /**
     * Constructs an extruded polygon from an outer boundary and apply specified
     * textures to its outer faces.
     *
     * @param corners the list of positions -- latitude longitude and altitude
     * -- defining the polygon. The altitude mode determines whether the
     * positions are considered relative to mean sea level (they are "absolute")
     * or the ground elevation at the associated latitude and longitude.
     * @param imageSources textures to apply to the polygon's outer faces. One
     * texture for each face must be included. May also be null.
     *
     * @throws IllegalArgumentException if the position list is null.
     */
    ...
    /**
     * Counts the total number of locations in this polygon's boundaries, not
     * including positions introduced by extrusion.
     *
    ...
    /**
     * Returns the list of locations or positions defining this polygon's outer
     * boundary.
     *
    ...
    /**
     * Indicates whether this shape's outer boundary exists and has more than
     * two points.
     *
    ...
    /**
     * Specifies the latitude and longitude of the locations defining the outer
     * boundary of this polygon. To specify altitudes, pass {@link Position}s
     * rather than {@link LatLon}s. The shape's height is not modified.
     *
     * @param corners the outer boundary locations.
     *
     * @throws IllegalArgumentException if the location list is null or contains
    ...
    /**
     * Specifies the latitudes, longitudes and outer-boundary images for the
     * outer boundary of this polygon. To specify altitudes, pass
     * {@link Position}s rather than {@link LatLon}s.
     *
     * @param corners the polygon locations.
     * @param imageSources images to apply to the outer faces. One image must be
     * specified for each face. May be null.
     *
    ...
    /**
     * Specifies the latitude and longitude of the outer boundary locations
     * defining this polygon, and optionally the extruded polygon's height. To
     * specify altitudes for each boundary location, pass {@link Position}s
     * rather than {@link LatLon}s, but those altitudes are used only when the
     * shape's altitude mode is {@link WorldWind#ABSOLUTE}.
     *
     * @param corners the outer boundary locations.
    ...
    /**
     * Add an inner boundary to this polygon and specify images to apply to each
     * of the boundary's faces. Specify {@link
     * LatLon}s to use the polygon's single height, or {@link Position}s, to
     * include individual altitudes.
     *
     * @param corners the boundary's locations.
     * @param imageSources images to apply to the boundary's faces. One image
     * must be specified for each face. May be null.
     *
     * @throws IllegalArgumentException if the location list is null.
     */
     ...
     *
     * @return the list of texture objects, or null if the
     * <code>imageSources</code> argument is null.
     */
     ...
     *
     * @return the texture image source, or null if no source has been
     * specified.
     */
     ...
     *
     * @param imageSource the image source. May be a {@link String} identifying
     * a file path or URL, a {@link File}, or a {@link java.net.URL}.
     * @param texCoords the (s, t) texture coordinates aligning the image to the
     * polygon. There must be one texture coordinate pair, (s, t), for each
     * location in the polygon's outer boundary.
     * @param texCoordCount the number of texture coordinates, (s, v) pairs,
     * specified.
     *
     * @throws IllegalArgumentException if the image source is not null and
     * either the texture coordinates are null or inconsistent with the
     * specified texture-coordinate count, or there are fewer than three texture
     * coordinate pairs.
     */
     ...
     *
     * @return the texture coordinates, or null if no cap texture coordinates
     * have been specified.
     */
     ...
     *
     * @return The texture, or null if there is no texture or the texture is not
     * yet available.
     */
     ...
     *
     * @throws IllegalArgumentException if the height is less than or equal to
     * zero.
     */
    ...
    /**
     * Each time this polygon is rendered the appropriate attributes for the
     * current mode are determined. This method returns the resolved attributes.
     *
    ...
    /**
     * Indicates the location to use as a reference location for computed
     * geometry.
     *
     * @return the reference location, or null if no reference location has been
     * specified.
     *
    ...
    /**
     * Specifies the location to use as a reference location for computed
     * geometry. This value should typically be left to the default, which is
     * the first location in the extruded polygon's outer boundary.
     *
     * @param referenceLocation the reference location. May be null, in which
     * case the first location of the outer boundary is the reference location.
     *
    ...
    /**
     * Indicates the position used as a reference position for this extruded
     * polygon's computed geometry.
     *
     * @return the reference position, or null if no reference position has been
     * specified.
     */
    ...
    /**
     * Specifies a depth below the terrain at which to place this extruded
     * polygon's base vertices. This value does not affect the height of the
     * extruded polygon nor the position of its cap. It positions the base
     * vertices such that they are the specified distance below the terrain. The
     * default value is zero, therefore the base vertices are position on the
     * terrain.
     *
     * @param baseDepth the depth in meters to position the base vertices below
     * the terrain. Specify positive values to position the base vertices below
     * the terrain. (Negative values position the base vertices above the
     * terrain.)
     */
     ...
     *
     * @return a collection of lists each identifying the image sources for the
     * associated outer or inner polygon boundary.
     */
    ...
    /**
     * Indicates whether side images have been specified for this extruded
     * polygon.
     *
     ...
     * @param refPoint a reference point to which the extent is translated. May
     * be null.
     *
     ...
     * <p>
     * This base implementation draws the outline of the basic polygon.
     * Subclasses should override it to draw their outline or an alternate
     * outline of the basic polygon.
     *
...
//            dc.drawNormals(1000, this.vertexBuffer, this.normalBuffer);
    ...
    /**
     * Computes the information necessary to determine this extruded polygon's
     * extent.
     *
     * @param dc the current draw context.
     * @param shapeData this shape's current globe-specific data.
    ...
    /**
     * Computes and sets this shape's reference point, the Cartesian position
     * corresponding to its geographic location.
     *
     * @param terrain the terrain to use when computing the reference point. The
     * reference point is always on the terrain.
     * @param shapeData the current shape data.
     */
     ...
     * @param skipOuterBoundary true if outer boundaries vertices do not need to
     * be calculated, otherwise false.
     */
     ...
     * @param shapeData the current shape data.
     * @param skipOuterBoundary if true, do not compute the outer boundary's
     * vertices because they have already been computed.
     */
    ...
    /**
     * Compute and set the Cartesian vertices for one specified boundary of this
     * shape.
     *
     * @param terrain the terrain to use when computing the vertices.
     * @param boundary the boundary for which to compute the vertices.
     * @param refPoint the reference point specifying the coordinate origin of
     * the vertices.
     */
    ...
    /**
     * Constructs the Cartesian geometry of this shape's sides and sets it in
     * the specified shape data.
     *
     * @param shapeData the current shape data.
     */
     ...
     * @param nBuf the buffer in which to place the computed normals. Must have
     * enough remaining space to hold the normals.
     *
     * @return the buffer specified as input, with its limit incremented by the
     * number of vertices copied, and its position set to 0.
     */
     ...
     * @param tBuf the buffer in which to place the computed texture
     * coordinates.
     */
     ...
     *
     * @return a buffer of indices that can be passed to OpenGL to draw all the
     * shape's edges.
     */
    ...
    /**
     * Returns the indices defining the vertices of each face of this extruded
     * polygon.
     *
     * @param n the number of positions in this extruded polygon.
     *
     * @return a buffer of indices that can be passed to OpenGL to draw all face
     * of the shape.
     */
    ...
    /**
     * Tessellates this extruded polygon's cap. This method catches
     * {@link OutOfMemoryError} exceptions and if the draw context is not null
     * passes the exception to the rendering exception listener (see {@link
     ...
     * @param normal a unit normal vector for the plane containing the polygon
     * vertices. Even though the the vertices might not be coplanar, only one
     * representative normal is used for tessellation.
     */
    ...
    /**
     * Construct the lists of indices that identify the tessellated shape's
     ...
     * If the polygon's altitude mode is other than {@link WorldWind#ABSOLUTE},
     * the extruded polygon's geometry is created relative to the specified
     * terrain rather than the terrain used during rendering, which may be at
     * lower level of detail than required for accurate intersection
     * determination.
     *
     * @param line the line to intersect.
     * @param terrain the {@link Terrain} to use when computing the extruded
     * polygon's geometry.
     *
     * @return a list of intersections identifying where the line intersects the
     * extruded polygon, or null if the line does not intersect the extruded
     * polygon.
     *
     ...
     *
     * @return the computed intersections, or null if there are no
     * intersections.
     *
     * @throws InterruptedException if the operation is interrupted.
     */
     ...
     * {@inheritDoc}
     * <p>
     * Note that this method overwrites the boundary locations lists, and
     * therefore no longer refer to the originally specified boundary lists.
     *
    ...
    /**
     * Writes the boundary in KML as either a list of lat, lon, altitude tuples
     * or lat, lon tuples, depending on the type originally specified.
     *
