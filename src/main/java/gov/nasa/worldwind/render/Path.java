 ...
 * <p>
 * Altitudes within the path's positions are interpreted according to the path's
 * altitude mode. If the altitude mode is {@link WorldWind#ABSOLUTE}, the
 * altitudes are considered as height above the ellipsoid. If the altitude mode
 * is {@link WorldWind#RELATIVE_TO_GROUND}, the altitudes are added to the
 * elevation of the terrain at the position. If the altitude mode is
 * {@link WorldWind#CLAMP_TO_GROUND} the altitudes are ignored.
 * <p>
 * Between the specified positions the path is drawn along a curve specified by
 * the path's path type, either {@link
 * AVKey#GREAT_CIRCLE}, {@link AVKey#RHUMB_LINE} or {@link AVKey#LINEAR}. (See
 * {@link #setPathType(String)}.)
 * <p>
 * Paths have separate attributes for normal display and highlighted display. If
 * no attributes are specified, default attributes are used. See
 * {@link #DEFAULT_PATH_INTERIOR_MATERIAL}, {@link #DEFAULT_PATH_OUTLINE_MATERIAL},
 * and {@link
 * #DEFAULT_HIGHLIGHT_MATERIAL}.
 * <p>
 * When the path type is <code>LINEAR</code> the path conforms to terrain only
 * if the follow-terrain property is true. Otherwise the path control points
 * will be connected by straight line segments.
 * <p>
 * The terrain conformance of <code>GREAT_CIRCLE</code> or
 * <code>RHUMB_LINE</code> paths is determined by the path's follow-terrain and
 * terrain-conformance properties. When the follow-terrain property is true,
 * terrain conformance adapts as the view moves relative to the path; the
 * terrain-conformance property governs the precision of conformance, and the
 * number of intermediate positions computed varies. See
 * {@link #setFollowTerrain(boolean)} and {@link
 * #setTerrainConformance(double)}. If the follow-terrain property is false, the
 * view position is not considered and the number of intermediate positions
 * between specified positions is the constant value specified by the
 * num-subsegments property (see {@link #setNumSubsegments(int)}). The latter
 * case may produce higher performance than the former.
 * <p>
 * The path positions may be shown by calling {@link #setShowPositions(boolean)}
 * with an argument of <code>true</code>. This causes dots to be drawn at each
 * originally specified path position. Dots are not drawn at tessellated path
 * positions. The size of the dots may be specified via
 * {@link #setShowPositionsScale(double)}. The dots are drawn only when the Path
 * is within a threshold distance from the eye point. The threshold may be
 * specified by calling {@link
 * #setShowPositionsThreshold(double)}. The dots are drawn in the path's outline
 * material colors by default.
 * <p>
 * The path's line and the path's position dots may be drawn in unique RGBA
 * colors by configuring the path with a {@link
 * PositionColors} (see
 * {@link #setPositionColors(gov.nasa.worldwind.render.Path.PositionColors)}).
 * <p>
 * Path picking includes information about which position dots are picked, in
 * addition to the path itself. A position dot under the cursor is returned as
 * an Integer object in the PickedObject's AVList under they key AVKey.ORDINAL.
 * Position dots intersecting the pick rectangle are returned as a List of
 * Integer objects in the PickedObject's AVList under the key
 * AVKey.ORDINAL_LIST.
 * <p>
 * When drawn on a 2D globe, this shape uses a {@link SurfacePolyline} to
 * represent itself. The following features are not provided in this case:
 * display of path positions, extrusion, outline pick width, and identification
 * of path position picked.
 *
 * @author tag
...
public class Path extends AbstractShape {
    ...
    /**
     * The default number of tessellation points between the specified path
     * positions.
     */
    ...
    /**
     * The default distance from the eye beyond which positions dots are not
     * drawn.
     */
    protected static final double DEFAULT_DRAW_POSITIONS_THRESHOLD = 1e6;
    /**
     * The default scale for position dots. The scale is applied to the current
     * outline width to produce the dot size.
     */
    protected static final double DEFAULT_DRAW_POSITIONS_SCALE = 10;

    /**
     * The PositionColors interface defines an RGBA color for each of a path's
     * original positions.
     */
    public static interface PositionColors {

        /**
         * Returns an RGBA color corresponding to the specified position and
         * ordinal. This returns <code>null</code> if a color cannot be
         * determined for the specified position and ordinal. The specified
         * <code>position</code> is guaranteed to be one of the same Position
         * references passed to a path at construction or in a call to {@link
         * Path#setPositions(Iterable)}.
         * <p>
         * The specified <code>ordinal</code> denotes the position's ordinal
         * number as it appears in the position list passed to the path. Ordinal
         * numbers start with 0 and increase by 1 for every originally specified
         * position. For example, the first three path positions have ordinal
         * values 0, 1, 2.
         * <p>
         * The returned color's RGB components must <em>not</em> be
         * premultiplied by its Alpha component.
         *
         * @param position the path position the color corresponds to.
         * @param ordinal the ordinal number of the specified position.
         *
         * @return an RGBA color corresponding to the position and ordinal, or
         * <code>null</code> if a color cannot be determined.
         */
     ...
     * encounters in calls to {@link AbstractShape#render(DrawContext)}. See {@link
     * AbstractShape}.
     */
    protected static class PathData extends AbstractShapeData {
         ...
         * <code>null</code> if the path's <code>positionColors</code> is
         * <code>null</code>.
         */
        ...
        /**
         * Indices to the <code>renderedPath</code> identifying the vertices of
         * the originally specified boundary positions and their corresponding
         * terrain point. This is used to draw vertical lines at those positions
         * when the path is extruded.
         */
        protected IntBuffer polePositions; // identifies original positions and corresponding ground points
        /**
         * Indices to the <code>renderedPath</code> identifying the vertices of
         * the originally specified boundary positions. (Not their terrain
         * points as well, as <code>polePositions</code> does.)
         */
        protected IntBuffer positionPoints; // identifies the original positions in the rendered path.
        /**
         * Indices of tessellated path lines when using 2D globe and the path
         * crosses the dateline.
         */
        protected IntBuffer path2DIndices;
        /**
         * Indices of the tessellated positions at which new lines must be
         * formed rather than continuing the previous line. Used only when the
         * path's positions span the dateline and a 2D globe is being used.
         */
        ...
        /**
         * Indicates the offset in number of floats to the first RGBA color
         * tuple in <code>renderedPath</code>. This is <code>0</code> if
         * <code>renderedPath</code> has no RGBA color tuples.
         */
        protected int colorOffset;
        /**
         * Indicates the stride in number of floats between the first element of
         * consecutive vertices in <code>renderedPath</code>.
         */
        ...
        /**
         * The positions resulting from tessellating this path. If the path's
         * attributes don't cause tessellation, then the positions returned are
         ...
         * <code>tessellatedPositions</code>, or <code>null</code> if the path
         * does not have per-position colors.
         *
         * @return the colors corresponding to each path position, or
         ...
         * <code>tessellatedPositions</code>, or <code>null</code> to specify
         * that the path does not have per-position colors. The entries in the
         * specified list must have a one-to-one correspondence with the entries
         * in <code>tessellatedPositions</code>.
         *
         * @param tessellatedColors the colors corresponding to each path
         * position, or <code>null</code> if the path does not have per-position
         * colors.
         */
        ...
        /**
         * The Cartesian coordinates of the tessellated positions. If path
         * verticals are enabled, this path also contains the ground points
         * corresponding to the path positions.
         *
         ...
         * ({@link #renderedPath} that identify the originally specified
         * positions that remain after tessellation. These positions are those
         * of the position dots, if drawn.
         *
         * @return the path's originally specified positions that survived
         * tessellation.
         */
        ...
        /**
         * Returns a buffer of indices into the rendered path
         * ({@link #renderedPath} that identify the top and bottom vertices of
         * this path's vertical line segments.
         *
        ...
        /**
         * Indicates whether this path is extruded and the extrusion points have
         * been computed.
         *
         * @return true if the path is extruded and the extrusion points are
        ...
        /**
         * Indicates the offset in number of floats to the first RGBA color
         * tuple in <code>renderedPath</code>. This returns <code>0</code> if
         * <code>renderedPath</code> has no RGBA color tuples.
         *
         * @return the offset in number of floats to the first RGBA color tuple
         * in <code>renderedPath</code>.
         */
        ...
        /**
         * Specifies the offset in number of floats to the first RGBA color
         * tuple in <code>renderedPath</code>. Specify 0 if
         * <code>renderedPath</code> has no RGBA color tuples.
         *
         * @param offset the offset in number of floats to the first RGBA color
         * tuple in <code>renderedPath</code>.
         */
         ...
         * consecutive vertices in <code>renderedPath</code>.
         *
         * @return the stride in number of floats between vertices in in
         * <code>renderedPath</code>.
         */
        ...
        /**
         * Specifies the stride in number of floats between the first element of
         * consecutive vertices in <code>renderedPath</code>.
         *
         * @param stride the stride in number of floats between vertices in in
         * <code>renderedPath</code>.
         */
        ...
        /**
         * Specifies the number of vertices in <code>renderedPath</code>.
         * Specify 0 if <code>renderedPath</code> contains no vertices.
         *
    ...
    /**
     * PickablePositions associates a range of pick color codes with a Path. The
     * color codes represent the range of pick colors that the Path's position
     * points are drawn in. The color codes represent ARGB colors packed into a
     * 32-bit integer.
     */
    protected static class PickablePositions {
        ...
        /**
         * The Path who's position points are associated with the specified
         * color code range.
         */
        public final Path path;

        /**
         * Creates a new PickablePositions with the specified color code range
         * and Path. See the PickablePositions class-level documentation for
         * more information.
         *
         * @param minColorCode the minimum color code, inclusive.
         * @param maxColorCode the maximum color code, inclusive.
         * @param path the Path who's position points are associated with the
         * specified color code range.
         */
    ...
    /**
     * Subclass of PickSupport that adds the capability to resolve a Path's
     * picked position point. Path position points are registered with
     * PathPickSupport by calling {@link #addPickablePositions(int, int, Path)}
     * with the minimum and maximum color codes that the Path's position points
     * are drawn in.
     * <p>
     * The resolution of the picked position point is integrated with the
     * resolution of the picked Path. Either an entire Path or one of its
     * position points may be picked. In either case, resolvePick and
     * getTopObject return a PickedObject that specifies the picked Path. If a
     * position point is picked, the PickedObject's AVList contains the position
     * and ordinal number of the picked position.
     */
    protected static class PathPickSupport extends PickSupport {
        ...
        /**
         * The list of Path pickable positions that this PathPickSupport is
         * currently tracking. This list maps a range of color codes to a Path,
         * where the color codes represent the range of pick colors that the
         * Path's position points are drawn in.
         */
        protected List<PickablePositions> pickablePositions = new ArrayList<>();
        /**
         * A map that associates each path with a picked object. Used to during
         * box picking to consolidate the information about what parts of each
         * path are picked into a single picked object. Path's positions are
         * drawn in unique colors and are therefore separately pickable during
         * box picking.
         */
        ...
        /**
         * Indicates the list of Path pickable positions that this
         * PathPickSupport is currently tracking. This list maps a range of
         * color codes to a Path, where the color codes represent the range of
         * pick colors that the Path's position points are drawn in. The
         * returned list is empty if addPickablePositions has not been called
         * since the last call to clearPickList.
         *
        ...
        /**
         * Registers a range of unique pick color codes with a Path,
         * representing the range of pick colors that the Path's position points
         * are drawn in. The color codes represent ARGB colors packed into a
         * 32-bit integer.
         *
         * @param minColorCode the minimum color code, inclusive.
         * @param maxColorCode the maximum color code, inclusive.
        ...
        /**
         * Computes and returns the top object at the specified pick point. This
         * either resolves a pick of an entire Path or one of its position
         * points. In either case, this returns a PickedObject that specifies
         * the picked Path. If a position point is picked, the PickedObject's
         * AVList contains the picked position's geographic position in the key
         * AVKey.POSITION and its ordinal number in the key AVKey.ORDINAL.
         * <p>
         * This returns null if the pickPoint is null, or if there is no Path or
         * Path position point at the specified pick point.
         *
         * @param dc the current draw context.
         * @param pickPoint the screen-coordinate point in question.
         *
         * @return a new picked object instances indicating the Path or Path
         * position point at the specified pick point, or null if no Path is at
         * the specified pick point.
         *
        ...
        /**
         * Adds all picked paths that are registered with this PickSupport and
         * intersect the specified rectangle in AWT screen coordinates (if any)
         * to the draw context's list of picked objects. Each picked object
         * includes the picked path and the ordinal numbers of positions that
         * intersect the specified rectangle, if any. If any positions intersect
         * the rectangle, the picked object's AVList contains the ordinal
         * numbers in the key AVKey.ORDINAL_LIST.
         *
     ...
     *
     * @param positions the path positions. This reference is retained by this
     * shape; the positions are not copied. If any positions in the set change,
     * {@link #setPositions(Iterable)} must be called to inform this shape of
     * the change.
     *
     ...
     *
     * @param coords the path coordinates. This reference is retained by this
     * shape; the positions are not copied. If any positions in the set change,
     * {@link #setPositions(Iterable)} must be called to inform this shape of
     * the change.
     ...
     * {@inheritDoc}
     * <p>
     * Overridden to assign this Path's pickSupport property to a new
     * PathPickSupport instance.
     */
    @Override
    ...
    /**
     * Convenience method for migration from Polyline.setClosed. Simply appends
     * a copy of the starting Position to the end of the position list.
     */
    ...
    /**
     * Indicates the PositionColors that defines the RGBA color for each of this
     * path's positions. A return value of <code>null</code> is valid and
     * indicates that this path's positions are colored according to its
     * ShapeAttributes.
     *
     * @return this Path's PositionColors, or <code>null</code> if this path is
     * colored according to its ShapeAttributes.
     *
    ...
    /**
     * Specifies the position colors used to define an RGBA color for each of
     * this path's positions. When non-<code>null</code>, the specified
     * <code>positionColors</code> is called during rendering to define a color
     * at each of this path's positions specified during construction or in a
     * call to {@link #setPositions(Iterable)}. The returned colors are applied
     * to this path's line and the optional dots drawn at each position when
     * <code>showPositions</code> is true, and override the ShapeAttributes'
     * outline color and outline opacity for both normal display and highlighted
     * display. The specified <code>positionColors</code> do not affect this
     * path's filled interior or vertical drop lines displayed when this path is
     * extruded.
     * <p>
     * If this path is configured to tessellate itself by creating additional
     * positions between the originally specified positions, an interpolated
     * color is assigned to each tessellated position by computing a weighted
     * linear combination of the colors at the originally specified positions.
     * <p>
     * Specify <code>null</code> to disable position colors and draw this path's
     * line and optional position dots according to its ShapeAttributes. This
     * path's position colors reference is <code>null</code> by default.
     *
     * @param positionColors the PositionColors that defines an RGBA color for
     * each of this path's positions, or <code>null</code> to color this path's
     * positions according to its ShapeAttributes.
     *
     ...
     *
     * @param terrainConformance the number of pixels between tessellation
     * points.
     */
     ...
     * {@link AVKey#LINEAR}.
     *
     * @param pathType the current path type. The default value is
     * {@link AVKey#LINEAR}.
     *
     ...
     *
     * @param drawVerticals true to draw the lines, otherwise false. The default
     * value is true.
     */
    ...
    /**
     * Specifies whether to draw dots at the original positions of the Path. The
     * dot color and size are controlled by the Path's outline material and
     * scale attributes.
     *
     * @param showPositions true if dots are drawn at each original (not
     * tessellated) position, otherwise false.
     */
    ...
    /**
     * Indicates the scale factor controlling the size of dots drawn at this
     * path's specified positions. The scale is multiplied by the outline width
     * given in this path's {@link ShapeAttributes} to determine the actual size
     * of the dots, in pixels. See
     * {@link ShapeAttributes#setOutlineWidth(double)}.
     *
    ...
    /**
     * Specifies the scale factor controlling the size of dots drawn at this
     * path's specified positions. The scale is multiplied by the outline width
     * given in this path's {@link ShapeAttributes} to determine the actual size
     * of the dots, in pixels. See
     * {@link ShapeAttributes#setOutlineWidth(double)}.
    ...
    /**
     * Indicates the eye distance from this shape's center beyond which position
     * dots are not drawn.
     *
     * @return the eye distance at which to enable or disable position dot
     * drawing. The default is 1e6 meters, which typically causes the dots to
     * always be drawn.
     */
    ...
    /**
     * Specifies the eye distance from this shape's center beyond which position
     * dots are not drawn.
     *
     * @param showPositionsThreshold the eye distance at which to enable or
     * disable position dot drawing.
     */
     ...
     * between are located on the underlying terrain, and <code>false</code>
     * otherwise.
     */
    ...
    /**
     * Sets whether this Path's defining positions and the positions in between
     * are located on the underlying terrain.
     *
     * If the surfacePath parameter is true this Path's altitude mode is set to
     * <code>WorldWind.CLAMP_TO_GROUND</code> and the follow terrain property is
     * set to <code>true</code>.
     *
     * If the surfacePath parameter is false this Path's altitude mode and
     * follow terrain property are returned to their defaults.
     *
     * @param surfacePath whether or not this Path should be located on the
     * underlying terrain.
     */
     ...
     * <p>
     * Note: when the draw context is in picking mode, this binds the current
     * GL_ARRAY_BUFFER to 0 after using the currently bound GL_ARRAY_BUFFER to
     * specify the vertex pointer. This does not restore GL_ARRAY_BUFFER to the
     * its previous state. If the caller intends to use that buffer after this
     * method returns, the caller must bind the buffer again.
     *
     * @param dc the current draw context.
     ...
     * @param dc the current draw context.
     * @param positions the positions to create a path for.
     * @param altitude if non-null, the height above the terrain to use for all
     * positions. If null, each position's altitude is used as the height above
     * the terrain.
     * @param path a buffer in which to store the computed points. May be null.
     * The buffer is not used if it is null or tool small for the required
     * number of points. A new buffer is created in that case and returned by
     * this method. This method modifies the buffer,s position and limit fields.
     * @param pathData the current globe-specific path data.
     *
     ...
     * @param color an array of length 4 containing the position's corresponding
     * color as RGBA values in the range [0, 1], or <code>null</code> if the
     * position has no associated color.
     * @param path the path to append to. Assumes that the path has adequate
     * capacity.
     * @param pathData the current globe-specific path data.
     */
    ...
    /**
     * Registers this Path's pickable position color codes with the specified
     * pickCandidates. The pickCandidates must be an instance of
     * PathPickSupport. This does nothing if this Path's position points are not
     * drawn.
     *
     * @param dc the current draw context.
     * @param pickCandidates the PickSupport to register with. Must be an
     * instance of PathPickSupport.
     */
    ...
    /**
     * Returns a new PickedObject corresponding to this Path's position point at
     * the specified index. The returned PickedObject's AVList contains the
     * picked position's geographic position in the key AVKey.POSITION and its
     * ordinal number in the key AVKey.ORDINAL.
     *
     * @param colorCode the color code corresponding to the picked position
     * point.
     * @param positionIndex the position point's index.
     *
     * @return a PickedObject corresponding to the position point at the
     * specified index.
     */
    ...
    /**
     * Computes this Path's distance from the eye point, for use in determining
     * when to show positions points. The value returned is only an
     * approximation because the eye distance varies along the path.
     *
     * @param dc the current draw context.
     * @param pathData this path's current shape data.
     *
     * @return the distance of the shape from the eye point. If the eye distance
     * cannot be computed, the eye position's elevation is returned instead.
     */
    ...
    /**
     * Adds a position to this path's <code>tessellatedPositions</code> list. If
     * the specified color is not <code>null</code>, this adds the color to this
     * path's <code>tessellatedColors</code> list. If the specified ordinal is
     * not <code>null</code>, this adds the position's index to the
     * <code>polePositions</code> and <code>positionPoints</code> index buffers.
     *
     * @param pos the position to add.
     * @param color the color corresponding to the position. May be
     * <code>null</code> to indicate that the position has no associated color.
     * @param ordinal the ordinal number corresponding to the position's
     * location in the original position list. May be <code>null</code> to
     * indicate that the position is not one of the originally specified
     * positions.
     * @param pathData the current globe-specific path data.
     */
    ...
    /**
     * Returns the Path position corresponding index. This returns null if the
     * index does not correspond to an original position.
     *
    ...
    /**
     * Returns the ordinal number corresponding to the position. This returns
     * null if the position index does not correspond to an original position.
     *
     * @param positionIndex the position's index.
     *
    ...
    /**
     * Returns an RGBA color corresponding to the specified position from the
     * original position list and its corresponding ordinal number by delegating
     * the call to this path's positionColors. This returns <code>null</code> if
     * this path's positionColors property is <code>null</code>. This returns
     * white if a color cannot be determined for the specified position and
     * ordinal.
     *
     ...
     *
     * @return an RGBA color corresponding to the position and ordinal, or
     * <code>null</code> if this path's positionColors property is
     * <code>null</code>.
     */
     ...
     * @param ptA the model-coordinate point corresponding to the segment's
     * first position.
     * @param ptB the model-coordinate point corresponding to the segment's
     * second position.
     *
     ...
     * @param ptB the model-coordinate point corresponding to the segment's
     * second position.
     * @param colorA the color corresponding to the segment's first position, or
     * <code>null</code> if the first position has no associated color.
     * @param colorB the color corresponding to the segment's second position,
     * or <code>null</code> if the first position has no associated color.
     * @param ordinalA the ordinal number corresponding to the segment's first
     * position in the original position list.
     * @param ordinalB the ordinal number corresponding to the segment's second
     * position in the original position list.
     * @param pathData the current globe-specific path data.
     */
    ...
    /**
     * Computes the approximate model-coordinate, path length between two
     * positions. The length of the path depends on the path type: great circle,
     * rhumb, or linear.
     *
     * @param dc the current draw context.
     * @param posA the first position.
     ...
     * altitude mode is {@link WorldWind#CLAMP_TO_GROUND}. See {@link #setAltitudeMode(int)
     * }.
     *
