...
/**
 * Draws an image parallel to the screen at a specified screen location relative
 * to the WorldWindow. If no image is specified, a filled rectangle is drawn in
 * its place.
 *
 * @author tag
...
public class ScreenImage extends WWObjectImpl implements Renderable, Exportable {
    ...
    /**
     * Indicates the location of this screen image in the viewport (on the
     * screen) in OpenGL coordinates. This property is computed in
     * <code>computeOffsets</code> and used in <code>draw</code> Initially
     * <code>null</code>.
     */
    protected Point screenLocation;
    /**
     * Indicates the location of this screen image in the viewport (on the
     * screen) in AWT coordinates. This property is assigned in
     * <code>setScreenLocation</code> and <code>computeOffsets</code>. In
     * <code>computeOffsets</code>, this is computed by converting the
     * <code>screenLocation</code> from OpenGL coordinates to AWT coordinates.
    ...
    /**
     * Returns the location of the image on the screen. The position is relative
     * to the upper-left corner of the World Window. The point specified by the
     * image offset will be aligned to this point. If the position was specified
     * as an offset, it may change if the viewport size changes. The value
     * returned by this method is the most recently computed screen location.
     * Call {@link #getScreenLocation(DrawContext)} to ensure an accurate result
    ...
    /**
     * Returns the location of the image on the screen. The position is relative
     * to the upper-left corner of the World Window. The image is centered on
     * this position.
     *
     ...
     * The specified <code>screenLocation</code> is relative to the upper-left
     * corner of the WorldWindow, and the image is centered on this location.
     *
     * @param screenLocation the screen location on which to center the image.
     * May be null, in which case the image is not displayed.
     *
    ...
    /**
     * Get the offset of the point on the screen to align with the image offset
     * point.
     *
     * @return Offset of the image point that will be aligned to the image
     * offset point.
     *
     * @see #getImageOffset()
     */
    ...
    /**
     * Set the offset of the image relative to the viewport. The screen point
     * identified by this offset will be aligned to the image point identified
     * by the image offset.
     *
    ...
    /**
     * Get the offset of the point on the image to align with the screen offset
     * point.
     *
     * @return Offset of the image point that will be aligned to the screen
     * offset point.
     *
     * @see #getScreenOffset()
     */
    ...
    /**
     * Set the image offset point. This point will be aligned to the screen
     * point identified by the screen offset.
     *
     * @param imageOffset Offset that identifies a point on the image to align
     * with the screen offset point.
     *
     * @see #setScreenOffset(Offset)
     */
     ...
     *
     * @return Rotation point in image coordinates, or null if there is no
     * rotation point set. The origin of the coordinate system is at the lower
    ...
    /**
     * Set a dynamic dimension to apply to the image. The dimension allows the
     * image to be scaled relative to the viewport size.
     *
     ...
     * java.awt.image.BufferedImage}. If the image is not already in memory, it
     * will be loaded in the background.
     *
     * @param imageSource the image source, either a file path {@link String} or
     * a {@link
    ...
    /**
     * Returns the opacity of the surface. A value of 1 or greater means the
     * surface is fully opaque, a value of 0 means that the surface is fully
     * transparent.
     *
    ...
    /**
     * If no image is set, or if the image is not yet available, a rectangle
    ...
    /**
     * Set the color of the rectangle drawn when the image cannot be drawn. The
     * image may not be drawn because it has not been loaded, or because no
     * image has been set.
     *
    ...
    /**
     * Sets the opacity of the surface. A value of 1 or greater means the
     * surface is fully opaque, a value of 0 means that the surface is fully
     * transparent.
     *
     * @param opacity a positive value indicating the opacity of the surface.
     *
     * @throws IllegalArgumentException if the specified opacity is less than
     * zero.
     */
    ...
    /**
     * Returns the width of the source image after dynamic scaling has been
     * applied. If no image has been specified, but a dimension has been
     * specified, the width of the dimension is returned.
     *
     ...
     * If no image has been specified, but a dimension has been specified, the
     * height of the dimension is returned.
     *
     * @param dc the current draw context.
     *
     ...
     * {@link gov.nasa.worldwind.event.SelectEvent}s when this object is picked.
     *
    ...
    /**
     * Specify the object to identify as the picked object when this shape is
     * picked.
     *
     * @param delegateOwner the object included in
     * {@link gov.nasa.worldwind.event.SelectEvent}s as the picked object.
     */
    ...
    /**
     * Get the texture for this image. The texture is loaded on a background
     * thread. This method will return null until the texture has been loaded.
     *
     * @return The texture or null if the texture is not yet available.
     */
    ...
    /**
     * Export the screen image. The {@code output} object will receive the
     * exported data. The type of this object depends on the export format. The
     * formats and object types supported by this class are:
     * <pre>
    ...
    /**
     * Export the screen image to KML as a {@code <ScreenOverlay>} element. The
     ...
     * <p>
     * The image path can only be exported if the image source is a path or URL.
     * If the image source is a BufferedImage, for example, the image will not
     * be exported and no icon reference will be written into the ScreenOverlay
     * tag.
     *
