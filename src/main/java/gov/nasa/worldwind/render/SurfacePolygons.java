...
/**
 * Renders fast multiple polygons with or without holes in one pass. It relies
 * on a {@link
 * gov.nasa.worldwind.util.CompoundVecBuffer}.
 * <p>
 * Whether a polygon ring is filled or is a hole in another polygon depends on
 * the vertices winding order and the winding rule used - see
 * setWindingRule(String).
 *
 * @author Dave Collins
...
public class SurfacePolygons extends SurfacePolylines // TODO: Review
{
     ...
     * <p>
     * When not null the polygon ring groups array identifies the starting sub
     * buffer index for each polygon. In that case rings from a same group will
     * be tesselated together as part of the same polygon.
     * <p>
     * When <code>null</code> polygon rings that follow the current winding rule
     * are tessellated separatly as different polygons. Rings that are reverse
     * winded are considered holes to be applied to the last straight winded
     * ring polygon.
     *
     ...
     * {@link AVKey#CLOCKWISE} (default) or {@link
     * AVKey#COUNTER_CLOCKWISE}.
     * <p>
     * When set to {@link AVKey#CLOCKWISE} polygons which run clockwise will be
     * filled and those which run counter clockwise will produce 'holes'. The
     * interpretation is reversed when the winding rule is set to {@link
     * AVKey#COUNTER_CLOCKWISE}.
     *
