...
/**
 * A general ellipsoid volume defined by a center position and the three
 * ellipsoid axis radii. If A is the radius in the north-south direction, and b
 * is the radius in the east-west direction, and c is the radius in the vertical
 * direction (increasing altitude), then A == B == C defines a sphere, A == B
 * &gt; C defines a vertically flattened spheroid (disk-shaped), A == B &lt; C
 * defines a vertically stretched spheroid.
 *
 * @author tag
...
public class Ellipsoid extends RigidShape {
    ...
    /**
     * Construct a default ellipsoid with centerPosition ZERO and radii all
     * equal to one.
     */
    public Ellipsoid() {
     ...
     *
     * @throws IllegalArgumentException if the center position is null or any of
     * the radii are not greater than 0.
     */
    ...
    /**
     * Construct an ellipsoid from a specified center position, axes lengths and
     * rotation angles. All angles are specified in degrees and positive angles
     ...
     * @param verticalRadius the ellipsoid's vertical radius, in meters.
     * @param eastWestRadius the ellipsoid's east-west radius, in meters.
     * @param heading the ellipsoid's azimuth, its rotation about its vertical
     * axis.
     * @param tilt the ellipsoids pitch, its rotation about its east-west axis.
     * @param roll the ellipsoid's roll, its rotation about its north-south
     * axis.
     */
    public Ellipsoid(Position centerPosition, double northSouthRadius, double verticalRadius, double eastWestRadius,
    ...
    /**
     * Computes a threshold value, based on the current detailHint, for use in
     * the sufficientDetail() calculation.
     *
    ...
    //**************************************************************//
    /**
     * Sets the Geometry mesh for this Ellipsoid, either by pulling it from the
    ...
    /**
     * Generates a unit sphere geometry, including the vertices, indices,
     * normals and texture coordinates, tessellated with the specified number of
     * divisions.
     *
     * @param subdivisions the number of times to subdivide the unit sphere
     * geometry
    ...
    /**
     * Renders the Ellipsoid, using data from the provided buffer and the given
     * parameters.
     *
    ...
    /**
     * Generates ellipsoidal geometry, including the vertices, indices, normals
     * and texture coordinates, tessellated with the specified number of
     * divisions.
     *
