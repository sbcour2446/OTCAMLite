 ...
 * WWIcon which has the property key {@link
 * gov.nasa.worldwind.avlist.AVKey#FEEDBACK_ENABLED} set to <code>true</code>.
 * IconRenderer does not record any feedback information during the pick pass.
 * When feedback is enabled, IconRenderer puts properties which describe how
 * each WWIcon has been processed in key-value pairs attached to the WWIcon. Any
 * of these properties may be null, indicating that processing of the WWIcon was
 * terminated before this information became available. The feedback properties
 * for WWIcon are as follows: <table> <caption>WWIcon Feedback
 * Properties</caption><tr><th>Key</th><th>Description</th></tr> <tr><td>{@link
 * gov.nasa.worldwind.avlist.AVKey#FEEDBACK_REFERENCE_POINT}</td><td>The icon's
 * reference point in model coordinates.</td></tr>
 * <tr><td>{@link gov.nasa.worldwind.avlist.AVKey#FEEDBACK_SCREEN_BOUNDS}</td><td>The
 * icon's bounding rectangle in screen coordinates.</td></tr> </table>
 *
 * @author tag
...
public class IconRenderer {
     ...
     * clipping. The default is <code>true</code>, view volume clipping is not
     * performed.
     *
