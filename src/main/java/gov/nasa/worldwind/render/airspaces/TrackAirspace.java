...
/**
 * Creates a sequence of potentially disconnected rectangular airspaces
 * specified by a collection of {@link
...
public class TrackAirspace extends AbstractAirspace {
    ...
    /**
     * Denotes the the threshold that defines whether the angle between two
     * adjacent legs is small. Initially 22.5 degrees.
     */
    ...
    /**
     * Desnotes the threshold that defines whether the angle between two
     * adjacent legs is small. This threshold is used to determine the best
     * method for adjusting the vertices of adjacent legs.
     *
     * @return the angle used to determine when the angle between two adjacent
     * legs is small.
     *
    ...
    /**
     * Specifies the threshold that defines whether the angle between two
     * adjacent legs is small. This threshold is used to determine the best
     * method for adjusting the vertices of adjacent legs.
     * <p>
     * When the angle between adjacent legs is small, the standard method of
     * joining the leg's vertices forms a very large peak pointing away from the
     * leg's common point. In this case <code>TrackAirspace</code> uses a method
     * that avoids this peak and produces a seamless transition between the
     * adjacent legs.
     *
     * @param angle the angle to use when determining when the angle between two
     * adjacent legs is small.
     *
     * @throws IllegalArgumentException if <code>angle</code> is
     * <code>null</code>.
     ...
     * <code>leg1</code> must precede <code>leg2</code>. A track's legs must be
     * joined when two adjacent legs share a common location. In this case, the
     * geometry of the two adjacent boxes contains a gap on one side and an
     * intersection on the other. Joining the legs modifies the edges of each
     * leg at their common location to produce a seamless transition from the
     * first leg to the second.
     *
     * @param leg1 the first leg.
     * @param leg2 the second leg.
     *
     * @return <code>true</code> if the legs must be joined, otherwise
     * <code>false</code>.
     */
    ...
    /**
     * Modifies the adjacent edges of the specified adjacent legs to produce a
     * seamless transition from the first leg to the second. <code>leg1</code>
     * must precede <code>leg2</code>, and they must share a common location at
     * the end of <code>leg1</code> and the beginning of <code>leg2</code>.
     * Without joining the adjacent edges, the geometry of the two adjacent
     * boxes contains a gap on one side and an intersection on the other.
     * <p>
