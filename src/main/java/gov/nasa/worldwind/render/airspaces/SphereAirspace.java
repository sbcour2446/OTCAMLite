...
/**
 * A spherical airspace shape defined by a center location and a radius. The
 * sphere's center altitude and terrain conformance attributes are taken from
 * the lower altitude and lower terrain conformance. When terrain conformance is
 * disabled, the sphere's altitude behaves as a height above mean sea level.
 * When terrain conformance is enabled, the sphere's altitude will behave as a
 * height offset above the terrain. Unlike other airspace shapes, the sphere's
...
public class SphereAirspace extends AbstractAirspace {
    ...
    /**
     * Sets the radius of the sphere in meters. This will also set the altitude
     * limits to match the new radius and center elevation.
     *
    ...
    /**
     * Returns this SphereAirspace's {@link gov.nasa.worldwind.geom.Extent} for
     * the specified DrawContext. This overrides
     * {@link gov.nasa.worldwind.render.airspaces.AbstractAirspace#getExtent(gov.nasa.worldwind.render.DrawContext)}
     * in order to bypass the superclass' extent caching. Unlike other
     * Airspace's Extents, SphereAirspace's Extent is a perfect fitting
     * {@link gov.nasa.worldwind.geom.Sphere}, who's center point depends on the
     * current surface geometry. For this reason SphereAirspace's exact bounding
     * volume can be easily computed, and should not be cached.
     *
