...
/**
 * A <code>{@link gov.nasa.worldwind.render.ScreenBalloon}</code> that displays
 * HTML, JavaScript, and Flash content using the system's native browser, and
 * who's origin is located at a point on the screen.
 *
 * @author pabercrombie
...
public class ScreenBrowserBalloon extends AbstractBrowserBalloon implements ScreenBalloon {
    ...
    /**
     * Indicates this balloon's screen location. The screen location's
     * coordinate system has its origin in the upper left corner of the
     * <code>WorldWindow</code>, with the y-axis pointing right and the x-axis
     * pointing down. Initialized to a non-<code>null</code> value at
     * construction.
     */
    protected Point screenLocation;

    /**
     * Constructs a new <code>ScreenBrowserBalloon</code> with the specified
     * text content and screen location.
     *
     * @param text the balloon's initial text content.
     * @param point the balloon's initial screen location, in AWT coordinates
     * (origin at upper left corner of the <code>WorldWindow</code>).
     *
     * @throws IllegalArgumentException if either <code>text</code> or
     * <code>point</code> are <code>null</code>.
     */
    ...
    /**
     * Computes and stores this balloon's screen coordinates. This assigns
     * balloon coordinate properties as follows:
     * <ul> <li><code>screenOffset</code> - the balloon frame's
     * screen-coordinate offset from this balloon's screen location.</li>
     * <li><code>screenRect</code> - the balloon frame's screen-coordinate
     * rectangle.</li>
     * <li><code>screenExtent</code> - this balloon's screen-coordinate bounding
     * rectangle.</li>
