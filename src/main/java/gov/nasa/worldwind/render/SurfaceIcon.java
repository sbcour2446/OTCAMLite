...
public class SurfaceIcon extends AbstractSurfaceRenderable implements Movable, Draggable {
     ...
     * <p>
     * When <code>null</code> the icon will be drawn with it's image center on
     * top of it's reference location - see {@link #setLocation(LatLon)}.
     * Otherwise the icon will be shifted of a distance equivalent to the number
     * of pixels specified as <code>x</code> and <code>y</code> offset values.
     * Positive values will move the icon to the right for <code>x</code> and up
     * for <code>y</code>. Negative values will have the opposite effect.
     *
     * @return the icon displacement in pixels relative to the reference
     * location.
     */
     ...
     * <p>
     * When <code>null</code> the icon will be drawn with it's image center on
     * top of it's refence location - see {@link
     * #setLocation(LatLon)}. Otherwise the icon will be shifted of a distance
     * equivalent to the number of pixels specified as <code>x</code> and
     * <code>y</code> offset values. Positive values will move the icon to the
     * right for <code>x</code> and up for <code>y</code>. Negative values will
     * have the opposite effect.
     *
     * @param locationOffset the icon displacement in pixels relative to the
     * reference location.
     */
    ...
    /**
     * Returns whether the icon will apply mip-map filtering to it's source
     * image. If <code>true</code> the icon image is drawn using mip-maps. If
     * <code>false</code> the icon is drawn without mip-maps, resulting in
     * aliasing if the icon image is drawn smaller than it's native size in
     * pixels.
     *
     * @return <code>true</code> if the icon image is drawn with mip-map
     ...
     * If <code>true</code> the icon image is drawn using mip-maps. If
     * <code>false</code> the icon is drawn without mip-maps, resulting in
     * aliasing if the icon image is drawn smaller than it's native size in
     * pixels.
     *
     * @param useMipMaps <code>true</code> if the icon image should be drawn
     * with mip-map filtering; <code>false</code> otherwise.
     */
    ...
    /**
     * Set the scaling factor to apply to the source image. A value of
     * <code>1</code> will produce no change, a value greater then
     * <code>1</code> will enlarge the image and a value smaller then
     * <code>1</code> will reduce it.
     *
    ...
    /**
     * Get the current heading {@link Angle}, clockwise from North or
     * <code>null</code>.
     *
     * @return the current heading {@link Angle}, clockwise from North or
     * <code>null</code>.
     */
    ...
    /**
     * Set the heading {@link Angle}, clockwise from North. Setting this value
     * to <code>null</code> will have the icon follow the view heading so as to
     * always face the eye. The icon will rotate around it's reference location.
     *
     * @param heading the heading {@link Angle}, clockwise from North or
     * <code>null</code>.
     */
    ...
    /**
     * Determines whether the icon constantly maintains it's apparent size. If
     * <code>true</code> the icon is constantly redrawn at the proper size
     * depending on it's distance from the eye. If <code>false</code> the icon
     * will be drawn only once per level of the underlying tile pyramid. Thus
     * it's apparent size will vary up to twice it's 'normal' dimension in
     * between levels.
     *
     * @return <code>true</code> if the icon constantly maintains it's apparent
     * size.
     */
    ...
    /**
     * Sets whether the icon constantly maintains it's apparent size. If
     * <code>true</code> the icon is constantly redrawn at the proper size
     * depending on it's distance from the eye. If <code>false</code> the icon
     * will be drawn only once per level of the underlying tile pyramid. Thus
     * it's apparent size will vary up to twice it's 'normal' dimension in
     * between levels.
     *
     * @param state <code>true</code> if the icon should constantly maintains
     * it's apparent size.
     */
    ...
    /**
     * Get the minimum size in meter the icon image is allowed to be reduced to
     * once applied to the terrain surface. This limit applies to the source
     * image largest dimension.
     * <p>
     * The icon will try to maintain it's apparent size depending on it's
     * distance from the eye and will extend over a rectangular area which
     * largest dimension is bounded by the values provided with
    ...
    /**
     * Set the minimum size in meter the icon image is allowed to be reduced to
     * once applied to the terrain surface. This limit applies to the source
     * image largest dimension.
     * <p>
     * The icon will try to maintain it's apparent size depending on it's
     * distance from the eye and will extend over a rectangular area which
     * largest dimension is bounded by the values provided with
     * <code>setMinSize(double)</code> and {@link #setMaxSize(double)}.
     *
    ...
    /**
     * Get the maximum size in meter the icon image is allowed to be enlarged to
     * once applied to the terrain surface. This limit applies to the source
     * image largest dimension.
     * <p>
     * The icon will try to maintain it's apparent size depending on it's
     ...
     * distance from the eye and will extend over a rectangular area which
     * largest dimension is bounded by the values provided with
     * {@link #setMinSize(double)} and <code>setMaxSize(double)</code>.
     *
    ...
    /**
     * Set the {@link Color} the source image will be combined with - default to
     * white.
     * <p>
     * A non white color will mostly affect the white portions from the original
     * image. This is mostly useful to alter the appearance of 'colorless' icons
     * - which mainly contain black, white and shades of gray.
     *
     ...
     * {@inheritDoc}
     * <p>
     * Overridden to return a unique state key if the icon is configured to
     * always redraw. SurfaceIcon does not use a cached representation if it's
     * heading is configured to follow the view, or if it's configured to
     * maintain a constant screen size.
     *
     * @see #getHeading()
     * @see #isMaintainSize()
     */
