...
/**
 * Represents a texture derived from a lazily loaded image source such as an
 * image file or a {@link
 * java.awt.image.BufferedImage}.
 * <p>
 * The interface contains a method, {@link #isTextureInitializationFailed()} to
 * determine whether the instance failed to convert an image source to a
 * texture. If such a failure occurs, the method returns true and no further
 * attempts are made to create the texture.
 * <p>
 * This class performs lazy retrieval and loading of an image source, attempting
 * to retrieve and load the image source only when the
 * {@link #bind(DrawContext)} or {@link #applyInternalTransform(DrawContext)}
 * methods are called. If the image source is a {@link BufferedImage} the
 * associated {@link Texture} object is created and available immediately when
 * <code>bind</code> or <code>applyInternalTransform</code> are called. If the
 * image source is a local file or remote stream (URL), retrieval and loading is
 * performed on a separate thread from the EDT.
 *
 * @author tag
...
public class LazilyLoadedTexture extends AVListImpl implements WWTexture {
    ...
    /**
     * The texture's texture coordinates, as determined by JOGL when the texture
     * is created.
     */
    protected TextureCoords texCoords;
    /**
     * The texture data created as the image source is read. It's removed - set
     * to null - once the textures is fully created. This intermediate texture
     * data is necessary because the image source is read in a non-EDT thread.
    ...
    /**
     * Indicates that texture initialization failed. This texture should not be
     * used if true.
     */
    ...
    /**
     * Identifies the {@link gov.nasa.worldwind.cache.FileStore} of the
     * supporting file cache for this model.
     */
    protected FileStore fileStore = WorldWind.getDataFileStore();
    /**
     * Provides a semaphore to synchronize access to the texture file if
     * duplicate request tasks are active.
     */
    protected final Object fileLock = new Object();

    /**
     * The object to notify when an image is eventually loaded in memory. The
     * current layer at the time the image source is requested is assigned to
     * this field.
     */
    protected PropertyChangeListener listener;

    /**
     * Constructs a texture object for a specified image source. Requests that
     * mip-maps be used.
     *
     * @param imageSource the source of the image, either a file path
     * {@link String} or a {@link
     *                    java.awt.image.BufferedImage}.
     *
     ...
     * {@link String} or a {@link
     *                    java.awt.image.BufferedImage}.
     * @param useMipMaps Indicates whether to generate and use mip-maps for the
     * image.
     *
     * @throws IllegalArgumentException if the <code>imageSource</code> is null.
     */
     ...
     *
     * @return true if the image source is a <code>BufferedImage</code>,
     * otherwise false.
     */
    ...
    /**
     * Indicates the texture's width, which is the same as the image source's
     * width. The width is known only after the image source has been retrieved
     * from disk or network and read as <code>TextureData</code>. It's value is
     * -1 until then.
     *
    ...
    /**
     * Indicates the texture's height, which is the same as the image source's
     * height. The height is known only after the image source has been
     * retrieved from disk or network and read as <code>TextureData</code>. It's
     * value is -1 until then.
     *
     ...
     * {@inheritDoc}
     * <p>
     * This method behaves identically to {@link #getWidth()}. The
     * <code>DrawContext</code> argument is not used.
     *
     ...
     * {@inheritDoc}
     * <p>
     * This method behaves identically to {@link #getHeight()}. The
     * <code>DrawContext</code> argument is not used.
     *
     * @param dc this parameter is not used by this class.
     *
    ...
    /**
     * Indicates whether the texture should use mip-maps. If they are not
     * available in the source image they are created.
     *
     ...
     * rendered.
     *
     * @param useAnisotropy true if anisotropy is to be applied, otherwise
     * false.
     */
    ...
    /**
     * Indicates whether an attempt was made to retrieve and read the texture
     * but it failed. If this flag is true, this texture should not be used.
     *
     * @return true if texture retrieval or creation failed, otherwise true,
     * even if the image source has not yet been retrieved.
     */
     ...
     *
     * @return this instance's texture, or null if the texture does not
     * currently exist.
     */
     ...
     * <code>Texture</code> has not yet been created for it.
     * <p>
     * If this object's texture data field is non-null, a new texture is created
     * from the texture data when the tile is next bound or otherwise
     * initialized. This object's texture data field is then set to null.
     *
     * @return the texture data, which may be null indicating that the image
     * source has not been read or that a texture has been created.
     */
    ...
    /**
     * Specifies texture data for the tile. If texture data is non-null, a new
     * texture is created from the texture data when the tile is next bound.
     * <p>
    ...
    /**
     * Binds this instance's {@link Texture} to the <code>GLContext</code> if
     * the texture has been created, otherwise initiates image source retrieval
     * and texture creation in a separate thread.
     *
     * @param dc the current draw context.
     *
     ...
     * and returns the texture, otherwise creates a task in a separate thread to
     * retrieve it from its local or remote location.
     *
     * @param dc the current draw context.
     *
     ...
     * <code>run</code> method retrieves and loads this texture's image source.
     *
     * @return a new request task that retrieves and loads this texture's image
     * source.
     */
    ...
    /**
     * Creates this instance's {@link Texture} if the image source is a
     * <code>BufferedImage</code>.
     *
     * @param dc the current draw context.
     *
     * @return the newly created texture, or null if the texture was not
     * created.
     *
     * @throws IllegalStateException if the image source is null or not a
     * <code>BufferedImage</code>.
     */
    ...
    /**
     * Creates a {@link Texture} from this instance's {@link TextureData} if the
     * <code>TextureData</code> exists.
     *
     * @param dc the current draw context.
     *
     * @return the newly created texture, or null if this instance has no
     * current <code>TextureData</code> or if texture creation failed.
     */
    ...
    /**
     * Attempts to find this texture's image file locally, and if that fails
     * attempts to find it remotely.
     */
    protected static class RequestTask implements Runnable {
         ...
         *
         * @param wwTexture the texture object for which to construct the
         * request task.
         */
    ...
    /**
     * Loads the image from disk into memory. If successful, texture data is
     * created and available via {@link
     * #getTextureData()}.
     *
    ...
    /**
     * Reads and returns a {@link TextureData} for an image from a specified
     * file URL.
     *
