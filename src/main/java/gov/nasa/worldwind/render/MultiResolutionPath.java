...
/**
 * A Version of {@link Path} that provides level-of-detail. Positions in a
 * {@code Path} are filtered based on a "skip count", the number of positions to
 * skip between positions that are drawn. The skip-count algorithm can be
 * replaced by the application. The default algorithm skips up to four
 * positions, depending on the eye distance from the positions. Also, if the
 * segment between any two positions is too small to be distinguished, it is not
 * drawn. See {@link
 * #makePositions(DrawContext, gov.nasa.worldwind.render.Path.PathData)}.
 * <p>
 * NOTE: This shape does not draw correctly on a 2D globe when its positions
 * span the dateline.
 *
 * @author tag
...
public class MultiResolutionPath extends Path {
    ...
    /**
     * This interface provides the means for the application to specify the
     * algorithm used to determine the number of specified positions skipped
     * during path tessellation.
     * <p>
     * This class overrides the method
     * {@link Path#makePositions(DrawContext, PathData)}.
     */
    public interface SkipCountComputer {

        /**
         * Determines the number of positions to skip for the current viewing
         * state. Determines the number of positions to skip for the current
         * viewing state.
         *
         * @param dc the current draw context.
         * @param pathData this shape's current path data.
         *
         * @return the number of positions to skip when computing the
         * tessellated or non-tessellated path.
         */
        public int computeSkipCount(DrawContext dc, PathData pathData);
    }

    /**
     * Subclass of PathData that adds the capability to map which ordinal number
     * corresponds to each rendered position.
     */
    protected static class MultiResolutionPathData extends PathData {

        /**
         * Maps indices of rendered positions to their corresponding ordinal
         * numbers.
         */
        protected IntBuffer positionOrdinals;

        /**
         * Creates a new MultiResolutionPathData with the specified draw context
         * and path.
         *
        ...
        /**
         * Returns a buffer mapping indices of rendered positions to their
        ...
        /**
         * Specifies a buffer that maps indices of rendered positions to their
         * corresponding ordinal numbers.
         *
    ...
    /**
     * The default implementation of <code>SkipCountComputer</code>. This
     * implementation returns a value of 4 when the eye distance to the path is
     * greater than 10e3, a value of 2 when the eye distance is greater than 1e3
     * meters but less then 10e3, and a value of 1 when the eye distance is less
     * than 1e3.
     */
    ...
    /**
     * Creates a path with specified positions. When the path is rendered, only
     * path positions that are visually distinct for the current viewing state
     * are considered. The path adjusts the positions it uses as the view state
     * changes, using more of the specified positions as the eye point comes
     * closer to the shape.
     * <p>
     ...
     * When the path is rendered, only path positions that are visually distinct
     * for the current viewing state are considered. The path adjusts the
     * positions it uses as the view state changes, using more of the specified
     * positions as the eye point comes closer to the shape.
     * <p>
     * Note: If fewer than two positions are specified, no path is drawn.
     *
     * @param positions the path positions. This reference is retained by this
    ...
    /**
     * Indicates the SkipCountComputer that is used to determine the number of
    ...
    /**
     * Specifies the SkipCountComputer that determines the number of specified
     * positions skipped during path tessellation.
     *
     ...
     * {@inheritDoc}
     * <p>
     * Overridden to initialize and build the PathData's positionOrdinals
     * buffer.
     */
    @Override
     ...
     * Positions are skipped first according to this Path's skipCountComputer.
     * The skipCountComputer determines how many positions this path skips
     * between tessellated positions. Any positions remaining after this step
     * are skipped if the segment they are part of is either very small or not
     * visible.
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * Overridden to create a mapping between the current tessellated position
     * and the specified ordinal, if the ordinal is not null.
     */
    @Override
     ...
     * {@inheritDoc}
     * <p>
     * Overridden to use the MultiResolutionPathData's positionOrdinals buffer
     * to map the specified position index to its corresponding ordinal number.
     */
    @Override
