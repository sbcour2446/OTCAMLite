 ...
 * <p>
 * Note: The view input handlers detect the surface image rather than the
 * terrain as the top picked object in {@link
...
public class SurfaceImage extends WWObjectImpl
    ...
    /**
     * A list that contains only a reference to this instance. Used as an
     * argument to the surface tile renderer to prevent its having to create a
     * list every time this surface image is rendered.
     */
    protected List<SurfaceImage> thisList = Arrays.asList(this);

    /**
     * Create a new surface image with no image source. The image will not be
     * rendered until an image source is set.
     */
    public SurfaceImage() {
    }
     ...
     *
     * @param imageSource either the file path to a local image or a
    ...
    /**
     * Specifies whether this surface image should appear on top of other image
     * layers and surface shapes in the scene. If the flag is <code>true</code>,
     * this surface image appears visually above other image layers and surface
     * shapes. Otherwise, this surface image appears interleaved with other
     * image layers according to its relative order in the layer list, and
     * appears beneath all surface shapes. The default is <code>false</code>.
     *
     * @param alwaysOnTop <code>true</code> if the surface image should appear
    ...
    /**
     * Export the Surface Image. The {@code output} object will receive the
    ...
    /**
     * Export the surface image to KML as a {@code <GroundOverlay>} element. The
