...
/**
 * /** A 3D polygon. The polygon may be complex with multiple internal but not
 * intersecting contours.
 * <p>
 * Polygons are safe to share among WorldWindows. They should not be shared
 * among layers in the same WorldWindow.
 * <p>
...
public class Polygon extends AbstractShape {
    ...
    protected static class ShapeData extends AbstractShapeData implements Iterable<BoundaryInfo> {
        ...
        /**
         * The vertex data buffer for this shape data. The first half contains
         * vertex coordinates, the second half contains normals.
         */
        ...
        /**
         * Indicates whether the index buffer needs to be filled because a new
         * buffer is used or some other reason.
         */
        protected boolean refillIndexBuffer = true; // set to true if the index buffer needs to be refilled
        /**
         * Indicates whether the index buffer's VBO needs to be filled because a
         * new buffer is used or some other reason.
         */
        ...
        /**
         * Returns the boundary information for this shape data's outer
         * boundary.
         *
         ...
         *
         * @return this shape data's rotation matrix, or null if there isn't
         * one.
         */
    ...
    /**
     * Holds information for each contour of the polygon. The vertex values are
     * updated at every geometry regeneration.
     */
    ...
    /**
     * This static hash map holds the vertex indices that define the shape's
     * visual outline. The contents depend only on the number of locations in
     * the source polygon, so they can be reused by all shapes with the same
     * location count.
     */
     ...
     * -- defining the polygon. The current altitude mode determines whether the
    ...
    /**
     * Void any computed data. Called when a factor affecting the computed data
     * is changed.
     */
    ...
    /**
     * Counts the total number of positions in this shape, including all
     * positions in all boundaries.
     *
     ...
     *
     * @return this polygon's outer boundary positions. The list may be empty
     ...
     *
     * @return this polygon's outer boundary. The list may be empty but will not
     * be null.
     */
    ...
    /**
     * Specifies the latitude, longitude and altitude of the outer boundary
     * positions defining this polygon.
     *
     * @param corners this polygon's positions. A copy of the list is made and
     * retained, and a duplicate of the first position is appended to the copy
     * if the first and last positions are not identical.
     *
     ...
     * Closes the boundary if it's not already closed.
     *
    ...
    /**
     * Add an inner boundary to this polygon. A duplicate of the first position
     * is appended to the list if the list's last position is not identical to
     * the first.
     *
     * @param corners the new boundary positions. A copy of the list is created
     * and retained, and a duplicate of the first position is added to the list
     * if the first and last positions are not identical.
     *
     * @throws IllegalArgumentException if the location list is null or contains
     * fewer than three locations.
    ...
    /**
     * Get the texture applied to this polygon. The texture is loaded on a
     * background thread. This method will return null until the texture has
     * been loaded.
     *
     ...
     *
     * @return the texture coordinates, or null if no texture coordinates have
     * been specified.
     */
     ...
     *
     * @param imageSource the texture image source. May be a {@link String}
     * identifying a file path or URL, a {@link
     ...
     * polygon location in the polygon's outer boundary.
    ...
    /**
     * Specifies the amount of rotation applied to this polygon. Positive
     * rotation is counter-clockwise.
     *
     * @param rotation the amount of rotation to apply, in degrees, or null to
     * apply no rotation.
     */
     ...
     *
     * @return the boundary's extent. Returns null if the boundary's vertices
...
//            dc.drawNormals(1000, this.boundarySet.coordBuffer, this.boundarySet.normalBuffer);
    ...
    /**
     * Compute enough geometry to determine this polygon's extent, reference
     * point and eye distance.
     * <p>
     ...
     *
     * @param terrain the terrain to use if the altitude mode is relative to the
     * terrain.
     * @param shapeData the current shape data for this shape.
     * @param skipOuterBoundary if true, don't calculate the vertices for the
     * outer boundary. This is used when the outer boundary vertices were
     * computed as minimal geometry.
     */
     ...
     *
     * @param terrain the terrain to use when calculating vertices relative to
     * the ground.
     * @param boundary the boundary to compute vertices for.
     * @param refPoint the reference point. Vertices are computed relative to
     * this point, which is usually the shape's reference point.
     ...
     *
     * @param shapeData the current shape data holding the vertex coordinates
     * and in which the normal vectors are added. The normal vectors are
     * appended to the vertex coordinates in the same buffer. The shape data's
     * coordinate buffer must have sufficient capacity to hold the vertex
     * normals.
     */
     ...
     * <p>
     * This method catches {@link OutOfMemoryError} exceptions and if the draw
     * context is not null passes the exception to the rendering exception
     * listener (see
    ...
    /**
     * Compute the intersections of a specified line with this polygon. If the
     * polygon's altitude mode is other than {@link WorldWind#ABSOLUTE}, the
     * polygon's geometry is created relative to the specified terrain rather
     * than the terrain used during rendering, which may be at lower level of
     * detail than required for accurate intersection determination.
     *
     * @param line the line to intersect.
     * @param terrain the {@link Terrain} to use when computing the polygon's
     * geometry.
     *
     * @return a list of intersections identifying where the line intersects the
     * polygon, or null if the line does not intersect the polygon.
     *
     ...
     * @param position the new position of the shape's reference position.
     *
     * @throws java.lang.IllegalArgumentException if the globe or position is
     * null.
     */
