...
public class PointPlacemarkAttributes implements Exportable {
    ...
    /**
     * Color to be blended with the image. This can be used to render the same
     * image in different colors for different placemarks.
     */
    ...
    /**
     * The image file to use for the placemark's icon if no image file is
     * specified in the placemark attributes.
     */
    ...
    /**
     * The image offset to use if none specified. This value is that required by
     * the default image.
     */
    public static final Offset DEFAULT_IMAGE_OFFSET = new Offset(19d, 8d, AVKey.PIXELS, AVKey.PIXELS);
    /**
     * The image scale to use if none specified. This value is appropriate for
     * the default image.
     */
    ...
    /**
     * The default label offset. This value is appropriate for the default
     * image.
     */
    ...
    /**
     * Constructs an instance with default values for image address, image
     * offset, image scale, label offset, label font and label color.
     */
    public PointPlacemarkAttributes() {
    }

    /**
     * Constructs an instance and initializes it to the values in a specified
     * instance.
     *
     * @param attrs the instance from which to copy the initial attribute values
     * of this. May be null.
     */
    ...
    /**
     * Copies all values, including null values, from another instance to this
     * one.
     *
    ...
    /**
     * Specifies the line width to use when rendering the optional placemark
     * line.
     *
     * @param lineWidth the line width. May be null, in which case a width of 1
    ...
    /**
     * Sets the image color. The image color is blended with the image, allowing
     * the same image to be rendered in different colors for different
     * placemarks.
     *
    ...
    /**
     * Returns the anti-alias hint. See {@link #setAntiAliasHint(int)} for the
     * recognized values.
     *
    ...
    /**
     * Specifies whether and how line anti-aliasing is performed. Recognized
     * values are {@link GL#GL_NEAREST}, {@link
    ...
    /**
     * Specifies the address of the placemark's image. The address may be a file
     * path or a URL.
     *
     * @param address the address of the placemark's image. May be null, in
     * which case a default image is used.
     */
    ...
    /**
     * Specifies a {@link java.awt.image.BufferedImage} for
     * {@link gov.nasa.worldwind.render.PointPlacemark}s associated with this
     * attribute bundle. When this method is called, this attribute bundle's
     * image address is automatically set to a unique identifier for the image.
     *
     * @param image the buffered image to use for the associated point
     * placemarks. May be null, in which case this attribute bundle's image
     * address is set to null by this method.
     */
    ...
    /**
     * Returns the placemark image scale. See [@link #setScale} for its
     * description.
     *
    ...
    /**
     * Specifies the placemark image scale. The scale is applied to the
     * placemark image's width and height during rendering in order to control
     * the rendered size of the image.
     *
     * @param scale the placemark image scale. May be null, in which case no
     * scaling is applied.
     */
    ...
    /**
     * Specifies the placemark image heading, which is used by some placemark
     * implementations to orient the placemark image.
     *
     * @param heading the placemark heading in degrees clockwise from North. May
     * be null, in which case no heading is applied during rendering.
     */
    ...
    /**
     * Specifies the heading reference. If
     * {@link gov.nasa.worldwind.avlist.AVKey#RELATIVE_TO_SCREEN}, the heading
     * is interpreted as relative to the screen and the placemark icon maintains
     * the heading relative to the screen's vertical edges. If
     * {@link gov.nasa.worldwind.avlist.AVKey#RELATIVE_TO_GLOBE}, the heading is
     * interpreted relative to the globe and the placemark icon maintains the
     ...
     *
     * @param headingReference the heading reference. See the description for
     * possible values.
     */
     ...
     *
     * @param pitch the placemark pitch in degrees. May be null, in which case
     * no pitch is applied during rendering.
     */
    ...
    /**
     * Specifies a location within the placemark image to align with the
     * placemark point.
     *
     * @param offset the hot spot controlling the image's placement relative to
     * the placemark point. May be null to indicate that the image's lower left
     * corner is aligned with the placemark point.
     */
    ...
    /**
     * Specifies a location relative to the placemark's image at which to align
     * the label. The label text begins at the point indicated by the offset. A
     * offset of (0, 0) pixels causes the text to start at the lower left corner
     * of the image. An offset of (1, 1) in fraction units causes the text to
     * start at the upper right corner of the image. The text would also start
     * there if the offset is in units of pixels and the X and Y values are the
     * image width and height, respectively.
     * <p>
     * If no offset is specified, the label is placed at the right edge of the
     * image with the top of the text at about the same level as the top of the
     * image. (An offset of (X = 1.0, Y = 0.6, both in fraction units.)
     *
     * @param offset the hot spot controlling the image's placement relative to
     * the placemark point. May be null to indicate the default label offset.
     */
    ...
    /**
     * Returns the placemark's label scale. See [@link #setLabelScale} for its
     * description.
     *
    ...
    /**
     * Specifies the placemark label scale. The scale is applied to the
     * placemark label's size after the base size is determined by the font.
     *
     * @param scale the placemark image scale. May be null, in which case no
     * scaling is applied.
     */
     ...
     *
     * @return true if a point is drawn when the current source image is null,
     * otherwise false.
     *
     ...
     * When drawing the point, the scale of the currently active attributes
     * specifies the diameter of the point in pixels. The point is drawn in the
     * line color of the currently active attributes.
     *
     * @param usePointAsDefaultImage true to draw a point when the current
     * source image is null, otherwise false.
     *
     ...
     *
     * @return <code>true</code> if the image is drawn, otherwise
     * <code>false</code>.
     */
    ...
    /**
     * Specifies whether to draw the placemark's image. When the image is not
     * drawn, the placemark's label, if any, is drawn relative to the
     * placemark's position.
     *
     * @param drawImage <code>true</code> to draw the image, otherwise
     * <code>false</code>.
     */
     ...
     *
     * @return <code>true</code> if the label is drawn, otherwise
     * <code>false</code>.
     */
     ...
     *
     * @param drawLabel <code>true</code> to draw the label, otherwise
     * <code>false</code>.
     */
    ...
    /**
     * Export the placemark attributes to KML as a {@code <Style>} element. The
     * {@code output} object will receive the data. This object must be one of:
     * java.io.Writer<br> java.io.OutputStream<br>
     * javax.xml.stream.XMLStreamWriter
     *
