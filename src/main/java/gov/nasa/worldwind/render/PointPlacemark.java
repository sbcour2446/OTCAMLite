...
/**
 * Represents a point placemark consisting of an image, an optional line linking
 * the image to a corresponding point on the terrain, and an optional label. The
 * image and the label are displayed in the plane of the screen.
 * <p>
 * Point placemarks have separate attributes for normal rendering and
 * highlighted rendering. If highlighting is requested but no highlight
 * attributes are specified, the normal attributes are used. If the normal
 * attributes are not specified, default attributes are used. See
 * {@link #getDefaultAttributes()}.
 * <p>
 * This class implements and extends the functionality of a KML <i>Point</i>.
 * <p>
 * Point placemarks can participate in global text decluttering by setting their
 * decluttering-enabled flag to {@code
 * true}. See {@link #setEnableDecluttering(boolean)}. The default for this flag
 * is {@code false}. When participating in decluttering, only the point
 * placemark's label is considered when determining interference with other
 * text.
 * <p>
 * When the label of a point placemark is picked, the associated
 * {@link gov.nasa.worldwind.pick.PickedObject} contains the key
 * {@link AVKey#LABEL}
 *
 * @author tag
...
public class PointPlacemark extends WWObjectImpl
    ...
    /**
     * An interface to enable application selection of placemark level of
     * detail.
     */
    public interface LODSelector {

        /**
         * Modifies the placemark's attributes and properties to achieve a
         * desired level of detail during rendering. This method is called
         * during rendering in order to provide the application an opportunity
         * to adjust the placemark's attributes and properties to achieve a
         * level of detail based on the placemark's distance from the view's eye
         ...
         * to the placemark's geographic position.
         */
        void selectLOD(DrawContext dc, PointPlacemark placemark, double eyeDistance);
    }

    /**
     * The scale to use when highlighting if no highlight attributes are
     * specified.
     */
    protected static final Double DEFAULT_HIGHLIGHT_SCALE = 1.3;
    /**
     * The label offset to use if none is specified but an image has been
     * specified.
     */
    ...
    /**
     * The address of the transparent image used when attributes.isDrawImage is
     * false.
     */
     ...
     *
     * @param visible true if the placemark is drawn when in view, otherwise
     * false.
     */
    ...
    /**
     * Returns the placemark's altitude mode. See {@link #setAltitudeMode(int)}
    ...
    /**
     * Specifies the placemark's altitude mode. Recognized modes are: <ul>
     * <li><b>@link WorldWind#CLAMP_TO_GROUND}</b>
     * -- the point is placed on the terrain at the latitude and longitude of
     * its position.</li> <li><b>@link WorldWind#RELATIVE_TO_GROUND}</b> -- the
     * point is placed above the terrain at the latitude and longitude of its
     * position and the distance specified by its elevation.</li>
     * <li><b>{@link WorldWind#ABSOLUTE}</b> -- the point is placed at its
     * specified position. </ul>
     *
    ...
    /**
     * Indicates whether a line from the placemark point to the corresponding
     * position on the terrain is drawn.
     *
    ...
    /**
     * Specifies whether a line from the placemark point to the corresponding
     * position on the terrain is drawn.
     *
    ...
    /**
     * Specifies the attributes used when the placemark is drawn normally, not
     * highlighted.
     *
     * @param attrs the attributes to use in normal mode. May be null to
     * indicate use of default attributes.
     */
    ...
    /**
     * Returns the attributes used when the placemark is drawn normally, not
     * highlighted.
     *
     * @return the attributes used in normal mode. May be null to indicate use
    ...
    /**
     * Specifies the attributes used to draw the placemark when it's
     * highlighted.
     *
     * @param attrs the attributes to use in normal mode. May be null to
     * indicate use of the normal attributes.
     */
     ...
     *
     * @param highlighted true if the placemark is drawn highlighted, otherwise
     * false.
     */
    ...
    /**
     * Specifies the placemark's label text that is displayed alongside the
     * placemark.
     *
     * @param labelText the placemark label text. If null, no label is
     * displayed.
     */
    ...
    /**
     * Specifies whether adjacent PointPlacemarks in the ordered renderable list
     * may be rendered together if they are contained in the same layer. This
     * increases performance and there is seldom a reason to disable it.
     *
    ...
    /**
     * Returns the delegate owner of this placemark. If non-null, the returned
     * object replaces the placemark as the pickable object returned during
     * picking. If null, the placemark itself is the pickable object returned
     * during picking.
     *
     * @return the object used as the pickable object returned during picking,
     * or null to indicate the the placemark is returned during picking.
     */
    ...
    /**
     * Specifies the delegate owner of this placemark. If non-null, the delegate
     * owner replaces the placemark as the pickable object returned during
     * picking. If null, the placemark itself is the pickable object returned
     * during picking.
     *
     * @param owner the object to use as the pickable object returned during
     * picking, or null to return the placemark.
     */
    ...
    /**
     * Specifies whether adjacent PointPlacemarks in the ordered renderable list
     * may be pick-tested together if they are contained in the same layer. This
     * increases performance but allows only the top-most of the placemarks to
     * be reported in a {@link gov.nasa.worldwind.event.SelectEvent} even if
     * several of the placemarks are at the pick position.
     * <p>
     ...
     *
     * @return the value of the clip-to-horizon flag. {@code true} if horizon
     * clipping is enabled, otherwise {@code
     ...
     *
     * @param clipToHorizon {@code true} if this placemark should not be shown
     * when beyond the horizon, otherwise {@code
     *                      false}.
     */
    ...
    /**
     * Indicates whether this placemark participates in global text
     * decluttering.
     *
     * @return {@code true} if this placemark participates in global text
     * decluttering, otherwise false. The default value is {@code false}. Only
     * the placemark's label is considered during decluttering.
     */
    public boolean isEnableDecluttering() {
     ...
     *
     * @param enableDecluttering {@code true} if the placemark participates in
     * global text decluttering, otherwise {@code false}. The default value is
     * {@code false}. Only the placemark lable is considered during
     * decluttering.
     */
     ...
     *
     * @return <code>true</code> if this placemark's label is considered during
     * picking, otherwise <code>false</code>.
     */
    ...
    /**
     * Specifies whether this placemark's label should be considered during
     * picking. The default is not to consider the placemark's label during
     * picking.
     *
     * @param enableLabelPicking <code>true</code> to consider the label during
     * picking, otherwise <code>false</code>.
     */
     ...
     *
     * @return <code>true</code> if the always-on-top flag is set, otherwise
     * <code>false</code>.
     */
    ...
    /**
     * Specifies whether this placemark should appear on top of other placemarks
     * and shapes in the scene. If the flag is <code>true</code>, this
     * placemark's eye distance is set to 0 so that it will appear visually
     * above other shapes whose eye distance is greater than 0.
     *
     * @param alwaysOnTop <code>true</code> if the placemark should appear
     * always on top, otherwise <code>false</code>.
     */
     ...
     *
     * @return this placemark's level of detail selector, or null if one has not
     * been specified.
     */
    ...
    /**
     * Indicates whether a point should be drawn when the active texture is
     * null.
     *
     * @param dc the current draw context.
     *
    ...
    /**
     * Draws this ordered renderable and all subsequent PointPlacemark ordered
     * renderables in the ordered renderable list.
     *
     * @param dc the current draw context.
     */
    ...
    /**
     * Draw this placemark as an ordered renderable.If in picking mode, add it
     * to the picked object list of specified {@link PickSupport}. The
...
//            dc.drawUnitQuadOutline(); // for debugging label placement
    ...
    /**
     * Create a {@link PickedObject} for this placemark. The PickedObject
     * returned by this method will be added to the pick list to represent the
     * current placemark.
     *
     ...
     *
     * @return the label bounds, in lower-left origin screen coordinates, or
    ...
    protected void drawLabel(DrawContext dc, PickSupport pickCandidates, OrderedPlacemark opm) {
        ...
        if (dc.isPickingMode()) {
            ...
            try {
                ...
                textRenderer.setColor(color);//changed from backgroundColor because it looked bad
     ...
     *
     * @param dc the current draw context.
     * @param pickCandidates the pick support object to use when adding this as
     * a pick candidate.
    ...
    /**
     * Determines whether the placemark's optional line should be drawn and
     * whether it intersects the view frustum.
     *
     * @param dc the current draw context.
     * @param opm The placemark to check.
     *
     * @return true if the line should be drawn and it intersects the view
     ...
     *
     * @param attrs the attributes specifying the placemark image and
     * properties.
     *
    ...
    /**
     * Load a texture. If the texture source is not available locally, this
     * method requests the texture source and returns null.
     *
     * @param address Path or URL to the image to load into a texture.
     *
     * @return The new texture, or null if the texture could not be created
     * because the resource is not yet available locally.
     */
    ...
    /**
     * Computes and stores the placemark's Cartesian location, the Cartesian
     * location of the corresponding point on the terrain (if the altitude mode
     * requires it), and the screen-space projection of the placemark's
     * point.Applies the placemark's altitude mode when computing the points.
     *
     * @param dc the current draw context.
    ...
    /**
     * Computes the screen coordinate (lower-left origin) location of this
     * placemark's label.
     *
     * @param dc the current draw context.
     * @param opm the ordered renderable for the placemark.
    ...
    /**
     * Export the Placemark. The {@code output} object will receive the exported
     * data. The type of this object depends on the export format. The formats
     * and object types supported by this class are:
     * <pre>
     ...
     * @param output An object that will receive the exported data. The type of
     * this object depends on the export format (see above).
     *
     ...
     * @param styleType The type of style: normal or highlight. Value should
     * match either {@link KMLConstants#NORMAL} or
     * {@link KMLConstants#HIGHLIGHT}
     * @param attributes Attributes to export. The method takes no action if
