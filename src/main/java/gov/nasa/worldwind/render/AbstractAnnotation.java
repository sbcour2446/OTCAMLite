...
/**
 * An {@link Annotation} represent a text label and its rendering attributes.
 * Annotations must be attached either to a globe <code>Position</code>
 * ({@link GlobeAnnotation}) or a viewport <code>Point</code>
 * (ScreenAnnotation).
 * <pre>
 ...
 * </pre>
 * <p>
 * Each Annotation refers to an {@link AnnotationAttributes} object which
 * defines how the text will be rendered.
 * </p> Rendering attributes allow to set: <ul> <li>the size of the bounding
 * rectangle into which the text will be displayed</li> <li>its frame shape,
 * border color, width and stippling pattern</li> <li>the text font, size, style
 * and color</li> <li>the background color or image</li> <li>how much an
 * annotation scales and fades with distance</li>
 * </ul>
 * <pre>
 ...
 * </pre>
 * <p>
 * Annotations are usually handled by an
 * {@link gov.nasa.worldwind.layers.AnnotationLayer}. Although they also
 * implement the {@link Renderable} interface and thus can be handled by a
 * {@link gov.nasa.worldwind.layers.RenderableLayer} too.
 * <pre>
 * AnnotationLayer layer = new AnnotationLayer();
 * layer.addAnnotation(new GlobeAnnotation("Text...", Position.fromDegrees(0, 0, 0)));
 * </pre>
 * <p>
 * Each Annotation starts its life with a fresh attribute set that can be
 * altered to produce the desired effect. However,
 * <code>AnnotationAttributes</code> can be set and shared between annotations
 * allowing to control the rendering attributes of many annotations from a
 * single <code>AnnotationAttributes</code> object.
 * <pre>
 ...
 * </pre>
 * <p>
 * In the above example changing the text color of the attributes set will
 * affect all annotations referring it. However, changing the text color of one
 * of those annotations will also affect all others since it will in fact change
 * the common attributes set.
 * <p>
 * To use an attributes object only as default values for a series of
 * annotations use:
 * </p>
 * <pre>
 ...
 * </pre>
 * <p>
 * Finer control over attributes inheritance can be achieved using default or
 * fallback attributes set.
 * <p>
 * Most attributes can be set to a 'use default' value which is minus one for
 * numeric values and <code>null</code> for attributes referring objects
 * (colors, dimensions, insets..). In such a case the value of an attribute will
 * be that of the default attribute set. New annotations have all their
 * attributes set to use default values. </p>
 * <p>
 * Each <code>AnnotationAttributes</code> object points to a default static
 * attributes set which is the fallback source for attributes with
 * <code>null</code> or <code>-1</code> values. This default attributes set can
 * be set to any attributes object other than the static one.
 * <pre>
 ...
 * </pre>
 * <p>
 * In the above example all geographic features have an ellipse shape, water
 * bodies and mountains use that attributes set has defaults and have their own
 * text colors. They are in turn used as defaults by the two annotations. Mount
 * Saint Helens attributes could be changed without affecting other mountains.
 * However, changes on the geoFeatures attributes would affect all mountains and
 * lakes.
 * <p>
 * Background images are specified by setting the Annotation attribute
 * {@link gov.nasa.worldwind.render.AnnotationAttributes#setImageSource(Object)}.
 * The source can be either a path to a valid image file, or a
 * {@link java.awt.image.BufferedImage}. By default, background images are
 * aligned with the annotation as follows: the image's upper left corner is
 * aligned with the annotation's upper left corner, and the image's lower right
 * corner is aligned with a point <code>(imageWidth,
 * imageHeight)</code> pixels right and down from the annotation's upper left
 * corner. Thus the background image coordinate system has its origin at the
 * annotation's upper left corner, has the +X axis pointing to the right, and
 * has the +Y axis pointing down. Units are in image pixels, where one image
 * pixel corresponds to one screen pixel. The background image may be translated
 * or scaled by setting the attributes
 * {@link gov.nasa.worldwind.render.AnnotationAttributes#setImageOffset(java.awt.Point)}
 * and
 * {@link gov.nasa.worldwind.render.AnnotationAttributes#setImageScale(double)},
 * respectively. The offset attribute defines an offset right and down in
 * background image coordinates. The scale attribute is unitless, and defines
 * the background image's magnification or minification factor relative to the
 * annotation. For example, a scale of <code>0.5</code> indicates the image
 * should be 1/2 its original size relative to the annotation, while a scale of
 * <code>2.0</code> indicates the image should be 2x its original size.
 * <p>
 * <strong>Warning:</strong> For compatibility across the myriad of graphics
 * hardware, background images must have power-of-two dimensions.
 * Non-power-of-two images are handled inconsistently by graphics hardware. Not
 * all hardware supports them, and many that do lack full support for the
 * features available when using power-of-two images. Proper conversion from a
 * non-power-of-two image to a power-of-two image depends on the image's
 * intended use. However, the following two step solution works for most
 * applications: <ol> <li>Create a transparent power-of-two image larger than
 * the original image. The utility method
 * {@link gov.nasa.worldwind.util.WWMath#powerOfTwoCeiling(int)} is useful for
 * computing power-of-two dimensions: <code> <br><br> int newWidth =
 * WWMath.powerOfTwoCeiling(originalWidth);<br> int newHeight =
 * WWMath.powerOfTwoCeiling(originalHeight); <br><br> </code> </li> <li>Copy the
 * original image contents into the empty power-of-two image. Any pixels not
 * covered by the original image are left completely transparent:
 * <code> <br><br> BufferedImage newImage = new BufferedImage(newWidth,
 * newHeight, BufferedImage.TYPE_INT_ARGB);<br>
 * Graphics2D g2d = newImage.createGraphics();<br> try<br> {<br>
 * &nbsp;&nbsp;g2d.drawImage(originalImage, 0, 0, null);<br> }<br> finally<br>
 * {<br> &nbsp;&nbsp;g2d.dispose();<br> }<br><br> </code></li> </ol> When used
 * as an Annotation background image, the new power-of-two image appears
 * identical to its original non-power-of-two counterpart, except that the new
 * image is now compatible with most graphics hardware.
 *
...
public abstract class AbstractAnnotation extends AVListImpl implements Annotation {
    ...
    /**
     * Get the annotation bounding {@link java.awt.Rectangle} using OGL
     * coordinates - bottom-left corner x and y relative to the
     * {@link WorldWindow} bottom-left corner, and the annotation callout width
     * and height.
     * <p>
     * The annotation offset from it's reference point is factored in such that
     * the callout leader shape and reference point are included in the bounding
     * rectangle.
     *
     * @param dc the current DrawContext.
     *
     * @return the annotation bounding {@link java.awt.Rectangle} using OGL
    ...
    /**
     * Draws an annotation with the given dimensions and opacity. Current GL
     * state has ortho identity model view active with origin at the screen
     * point.
     *
     ...
     * @param pickPosition <code>Position</code> that will be associated with
     * any <code>PickedObject</code> produced during picking.
     */
    ...
    /**
     * Transforms texture coordinates from standard GL image coordinates to
     * Annotation background image coordinates. In standard GL image coordinates
     * <code>(0, 0)</code> maps to the image's lower left corner, and
     * <code>(1, 1)</code> maps to the image's upper right corner. In Annotation
     * background image coordinates <code>(0, 0)</code> maps to the image's
     * upper left corner, and <code>(imageWidth, imageHeight)</code> maps to the
     * image's lower right corner. This assumes the current OGL matrix mode is
     * <code>GL_TEXTURE</code>.
     *
     * @param dc the DrawContext to receive the texture coordinate transform.
     * @param texture the texture to transform from standard GL image
     * coordinates to Annotation background image coordinates.
     */
    ...
    /**
     * Transforms texture coordinates from Annotation background image
     * coordinates to Annotation geometry coordinates (in screen pixels), and
     * applies the Annotation's image scale and image offset attributes. In
     * Annotation background image coordinates <code>(0, 0)</code> maps to the
     * image's upper left corner, and <code>(imageWidth,
     * imageHeight)</code> maps to the image's lower right corner. In Annotation
     * geometry coordinates <code>(0,
     * 0)</code> maps to the Annotation geometry's lower left corner (ignoring
     * any leader geometry), and <code>(width,
     * height)</code> maps to the Annotation's upper right corner in window
     * coordinates (screen pixels). This assumes the current OGL matrix mode is
     * <code>GL_TEXTURE</code>.
     *
     * @param dc the DrawContext to receive the texture coordinate transform.
     * @param width the Annotation's width, in window coordinates (screen
     * pixels).
     * @param height the Annotation's height, in window coordinates (screen
     * pixels).
     * @param texture the texture to transform from Annotation background image
     * coordinates to Annotation geometry coordinates.
     */
    @SuppressWarnings({"UnusedDeclaration"})
...
//        mltr.setLineSpacing(-2);
    ...
    //**************************************************************//
    /**
     * Returns an XML state document String describing the public attributes of
     * this AbstractAnnotation.
     *
    ...
    /**
     * Restores publicly settable attribute values found in the specified XML
     * state document String. The document specified by <code>stateInXml</code>
