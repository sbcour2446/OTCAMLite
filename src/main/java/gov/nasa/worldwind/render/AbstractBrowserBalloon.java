...
/**
 * A <code>{@link gov.nasa.worldwind.render.Balloon}</code> that displays HTML,
 * JavaScript, and Flash content using the system's native browser. The
 * balloon's HTML content is specified by calling <code>setText</code> with an
 * HTML formatted string. A browser balloon resolves relative <code>URLs</code>
 * in the HTML content by consulting its resource resolver. The resource
 * resolver converts a relative <code>URL</code> to an absolute <code>URL</code>
 * that the browser can load. The resource resolver is specified by calling
 * <code>setResourceResolver</code>, and may be one of the following: a
 * <code>{@link gov.nasa.worldwind.util.webview.WebResourceResolver}</code>, a <code>{@link
 * java.net.URL}</code>, or a <code>String</code> containing a valid URL
 * description. If a browser balloon's resource resolver is <code>null</code> or
 * is an unrecognized type, the browser interprets relative <code>URLs</code> as
 * unresolved references.
 * <p>
 * <b>Browser Controls</b>
 * <p>
 * Browser balloons display three default browser controls that enable users to
 * navigate the browser's history back and forward, and to close the balloon.
 * When the user selects one of these controls, a <code>SelectEvent</code> is
 * generated with the <code>PickedObject's</code> <code>AVKey.ACTION</code>
 * value set to one of <code>AVKey.CLOSE</code>, <code>AVKey.BACK</code>, or
 * <code>AVKey.FORWARD</code>. These controls may be enabled or disabled by
 * calling <code>setDrawBrowserControls</code> (they are enabled by default),
 * and may be customized by adding or removing controls from the browser
 * balloon. See <code>getBrowserControls</code>, <code>addBrowserControl</code>,
 * and <code>removeBrowserControl</code>.
 * <p>
 * <b>Resize Control</b>
 * <p>
 * Browser balloons provide a default resize control that is activated by
 * dragging the balloon's border. When the user drags the border, a
 * <code>SelectEvent</code> is generated with the PickedObject's
 * <code>AVKey.ACTION</code> value set to <code>AVKey.RESIZE</code>. The
 * <code>PickedObject's</code> <code>AVKey.BOUNDS</code> value holds the
 * Balloon's screen bounds in AWT coordinates (origin at the upper left corner)
 * as a <code>java.awt.Rectangle</code>. The resize control may be enabled or
 * disabled by calling <code>setDrawResizeControl</code> (it is enabled by
 * default).
 * <p>
 * <b>Balloon Size</b>
 * <p>
 * The browser balloon's screen width and height are specified as a
 * <code>{@link gov.nasa.worldwind.render.Size}</code> object in its
 * <code>BalloonAttributes</code>. This size may configured in one of the
 * following three modes: <ul>
 * <li>Explicit size in pixels.</li> <li>Fraction of the
 * <code>WorldWindow</code> size.</li> <li>Fit to the balloon's HTML
 * content.</li> </ul> The balloon's width and height may be configured
 * independently, enabling any combination of these three modes. The balloon's
 * width and height are limited by its maximum size, which is also specified as
 * a <code>Size</code> object in its <code>BalloonAttributes</code>. If the
 * maximum size is <code>null</code>, the balloon's width and height are
 * unlimited. The space provided for the balloon's HTML content equal to the
 * balloon's screen width and height minus the balloon's insets, also specified
 * in its <code>BalloonAttributes</code>
 * <p>
 * The balloon's width or height (or both) may be configured to fit to the
 * balloon's HTML content by configuring its <code>BalloonAttributes</code> with
 * a <code>Size</code> who's width or height mode is
 * <code>Size.NATIVE_DIMENSION</code> or
 * <code>Size.MAINTAIN_ASPECT_RATIO</code>. When configured in this mode, the
 * browser balloon's size always fits the HTML content specified at construction
 * or by calling <code>setText</code>. If a user action causes the balloon to
 * navigate to another page, the balloon continues to fit to its current HTML
 * content.
 * <p>
 * The balloon frame's corner radius and leader width specified in its
 * <code>BalloonAttributes</code> are limited by the balloon's size. The corner
 * radius is first limited by the balloon's width and height, then the leader
 * width is limited by the balloon's width and height minus space taken by
 * rounded corners. For example, if the corner radius is 100 and the width and
 * height are 50 and 100, the actual corner radius used is 25 - half of the
 * rectangle's smallest dimension. Similarly, if the leader is attached to the
 * rectangle's bottom, its width is limited by the rectangle's width minus any
 * space used by the balloon's rounded corners.
 * <p>
 * <b>Hiding the balloon</b>
 * <p>
 * The balloon can be made visible or invisible by calling
 * {@link #setVisible(boolean) setVisible}. The balloon's {@code
 * visibilityAction} determines what happens to the native web browser when the
 * balloon is invisible. The balloon can either release its native web browser,
 * preventing the native browser from consuming system resources while the
 * balloon is invisible, or it can retain the native browser. Possible values of
 * {@code visibilityAction} are: <ul> <li>
 * AVKey.VISIBILITY_ACTION_RELEASE (Default) &mdash; Release the native web
 * browser when this balloon is invisible. The browser will be recreated when
 * the balloon becomes visible again. All browser state (navigation history,
 * page scroll position, etc) will be lost. </li><li>
 * AVKey.VISIBILITY_ACTION_RETAIN &mdash; Do not release the native browser when
 * this balloon is invisible. This action will retain all browser state while
 * this balloon is invisible, but the browser will continue to consume system
 * resources. Dynamic content on the page (such as animations and Flash video)
 * will continue to play while the balloon is invisible.</li></ul>
 *
 * @author dcollins
...
public abstract class AbstractBrowserBalloon extends AbstractBalloon implements HotSpot, Disposable {
    ...
    /**
     * Holds the vertex data and the defining properties of a balloon's frame
     * and leader geometry. The <code>vertexBuffer</code> represents the
     * screen-coordinate vertices of the balloon's frame. The <code>size</code>,
     * <code>offset</code>, <code>balloonShape</code>, <code>leaderShape</code>,
     * <code>leaderWidth</code>, and <code>cornerRadius</code> are the frame
     * geometry's defining properties. These are used to determine when the
     * frame geometry is invalid and must be recomputed.
     */
    ...
    protected class OrderedBrowserBalloon implements OrderedRenderable {
        ...
        /**
         * The location and size of the balloon's content frame in the viewport
         * (on the screen).
         */
        ...
        /**
         * The extend of the balloon's pickable geometry in the viewport (on the
         * screen). Includes this balloon's outline where it exceeds the screen
         * extent.
         */
        protected Rectangle screenPickExtent;
        /**
         * The location and size of the WebView's content frame in the viewport
         * (on the screen).
         */
        ...
        /**
         * Identifies the frame used to calculate the balloon's active
    ...
    /**
     * The browser balloon's default native size: 400x300. This default size is
     * used when a balloon's size is configured to use a native dimension, but
     * the WebView either has not been created or its content size is not known.
     * This default chosen to minimize the popping effect when the balloon's
     * size switches from the default to the WebView's content size.
     */
    protected static final Dimension DEFAULT_NATIVE_SIZE = new Dimension(400, 300);
    /**
     * The default outline pick width in pixels. The default is 10 pixels, the
     * maximum OpenGL line width supported by most graphics cards.
     */
    protected static final int DEFAULT_OUTLINE_PICK_WIDTH = 10;
    /**
     * The class name of the default
     * <code>{@link gov.nasa.worldwind.util.webview.WebViewFactory}</code> used
     * to create the balloon's internal <code>WebView</code>. This factory is
     * used when the configuration does not specify a WebView factory.
     */
    ...
    /**
     * The number of slices used to display each of a rectangular balloon
     * frame's rounded corners: 16.
     */
    protected static final int FRAME_GEOMETRY_RECTANGLE_CORNER_SLICES = 16;

    /**
     * Returns a list containing the browser balloon's three default browser
     * controls, configured as follows:
     * <table> <caption style="font-weight: bold;">Controls</caption>
     * <tr><th>Control</th><th>Action</th><th>Offset</th><th>Size</th><th>Image
     * Source</th></tr>
     * <tr><td>Close</td><td><code>AVKey.CLOSE</code></td><td>(30, 25) pixels
     * inset from the balloon's upper right corner</td><td>Image source's native
     * size in pixels (16x16)</td><td>images/browser-close-16x16.gif</td></tr>
     * <tr><td>Back</td><td><code>AVKey.BACK</code></td><td>(15, 25) pixels
     * inset from the balloon's upper left corner</td><td>Image source's native
     * size in pixels (16x16)</td><td>images/browser-back-16x16.gif</td></tr>
     * <tr><td>Forward</td><td><code>AVKey.FORWARD</code></td><td>(35, 25)
     * pixels inset from the balloon's upper left corner</td><td>Image source's
     * native size in pixels
     * (16x16)</td><td>images/browser-forward-16x16.gif</td></tr>
     * </table>
     *
    ...
    /**
     * Indicates the object used to resolve relative resource paths in this
     * browser balloon's HTML content. May be one of the following:
     * <code>{@link gov.nasa.worldwind.util.webview.WebResourceResolver}</code>, <code>{@link
     * java.net.URL}</code>, <code>{@link String}</code> containing a valid URL
     * description, or <code>null</code> to specify that relative paths should
     * be interpreted as unresolved references. Initially <code>null</code>.
     */
    ...
    /**
     * Denotes whether or not an attempt at WebView creation failed. When
     * <code>true</code> the balloon does not perform subsequent attempts to
     * create the WebView. Initially <code>false</code>.
     */
    protected boolean webViewCreationFailed;
    /**
     * Interface for interacting with the operating system's web browser
     * control. Initially <code>null</code>.
     */
    ...
    /**
     * The location of the balloon's content frame relative to the balloon's
     * screen point in the viewport.
     */
    protected Point screenOffset;
    /**
     * The size of the WebView's HTML content size, in pixels. This is the size
     * that the WebView can be displayed at without the need for scroll bars.
     * May be <code>null</code> or <code>(0, 0)</code>, indicating that the
     * WebView's HTML content size is unknown. Initially <code>null</code>.
     */
    ...
    /**
     * The screen coordinate of the last <code>SelectEvent</code> sent to this
     * balloon's <code>select</code> method.
     */
    protected Point lastPickPoint;
    /**
     * Support for setting up and restoring picking state, and resolving the
     * picked object.
     */
    ...
    /**
     * Computes and stores the balloon's model-coordinate and screen-coordinate
     * points.
     *
     ...
     * <code>{@link gov.nasa.worldwind.util.webview.WebView}</code>. This does
     * nothing if the balloon is already disposed.
     */
    @Override
    public void dispose() {
        this.disposeWebView();
     ...
     * {@inheritDoc}
     * <p>
     * When this balloon is set to invisible, the {@code visibilityAction}
     * determines what happens to the native web browser that backs the balloon.
     * By default, the browser resources are released when the balloon is not
     * visible.
     *
    ...
    /**
     * Indicates the outline line width (in pixels) used during picking. A
     * larger width than normal typically makes the outline easier to pick.
     *
    ...
    /**
     * Specifies the outline line width (in pixels) to use during picking. The
     * specified <code>width</code> must be zero or a positive integer.
     * Specifying a pick width of zero effectively disables the picking of the
     * balloon's outline and its resize control. A larger width than normal
     * typically makes the outline easier to pick.
     * <p>
     * When the the balloon's resize control is enabled, the outline becomes the
     * resize control and is drawn in the specified <code>width</code>.
     * Therefore this value also controls the balloon's resize control width. If
     * the resize control is disabled by calling
     * <code>{@link #setDrawResizeControl(boolean)}</code> with a value of
     * <code>false</code>, this has no effect on the balloon's resize control
     * until it is enabled.
     *
    ...
    /**
     * Indicates the object used to resolve relative resource paths in this
     * browser balloon's HTML content.
     *
     * @return the object used to resolve relative resource paths in HTML
     * content. One of the following: <code>{@link
     *         gov.nasa.worldwind.util.webview.WebResourceResolver}</code>,
     * <code>{@link java.net.URL}</code>, <code>{@link String}</code> containing
     * a valid URL description, or <code>null</code> to indicate that relative
    ...
    /**
     * Specifies a the object to use when resolving relative resource paths in
     * this browser balloon's HTML content. The <code>resourceResolver</code>
     * may be one of the following:
     * <ul> <li>a
     * <code>{@link gov.nasa.worldwind.util.webview.WebResourceResolver}</code></li>
     * <li>a <code>{@link
     * java.net.URL}</code></li> <li>a <code>{@link String}</code> containing a
     * valid URL description</li> </ul>
     * <p>
     * If the <code>resourceResolver</code> is <code>null</code> or is not one
     * of the recognized types, this browser balloon interprets relative
     * resource paths as unresolved references.
     *
     * @param resourceResolver the object to use when resolving relative
     * resource paths in HTML content. May be one of the following:
     * <code>{@link gov.nasa.worldwind.util.webview.WebResourceResolver}</code>,
     * <code>{@link java.net.URL}</code>, <code>{@link String}</code> containing
     * a valid URL description, or <code>null</code> to specify that relative
     * paths should be interpreted as unresolved references.
     *
    ...
    /**
     * Indicates the the action that occurs when the BrowserBalloon is set to
     * invisible. See {@link
     * #setVisibilityAction(String) setVisibilityAction} for a description of
     * the possible actions.
     *
     * @return A string that indicates the action that will occur when the
     * balloon is set to invisible.
     *
     * @see #setVisibilityAction(String)
     ...
     * Possible actions are: <ul> <li>
     * AVKey.VISIBILITY_ACTION_RELEASE (Default) &mdash; Release the native web
     * browser when the balloon is invisible. The browser will be recreated when
     * the balloon becomes visible. Note that all browser state (navigation
     * history, page scroll position, etc) will be lost. </li><li>
     * AVKey.VISIBILITY_ACTION_RETAIN &mdash; Do not release the native browser
     * when this balloon is invisible. This action will retain all browser state
     * while the balloon is invisible, but the browser will continue to consume
     * system resources while is is invisible. Dynamic content on the page (for
     * example, Flash video) will continue to play.</li></ul>
     *
     * @param visibilityAction Either {@link AVKey#VISIBILITY_ACTION_RELEASE} or
     * {@link AVKey#VISIBILITY_ACTION_RETAIN}.
     */
    ...
    /**
     * Navigate the browser to the previous page in the browsing history. Has no
     * effect if there is previous page.
     */
    public void goBack() {
    ...
    /**
     * Navigate the browser to the next page in the browsing history. Has no
     * effect if there is no next page.
     */
     ...
     * {@inheritDoc}
     * <p>
     * Overridden to suppress <code>AVKey.REPAINT</code> property change events
     * sent by the balloon's internal
     * <code>{@link gov.nasa.worldwind.util.webview.WebView}</code> when
     * <code>isVisible</code> returns <code>false</code>.
     */
    @Override
    ...
    /**
     * Updates the balloon's per-frame rendering state, and determines whether
     * to queue an ordered renderable for the balloon. This queues an ordered
     * renderable if the balloon intersects the current viewing frustum, and if
     * the balloon's internal rendering state can be computed. This updates the
     * balloon's rendering state by calling
     * <code>updateRenderStateIfNeeded</code>, and updates its geometry by
     * calling <code>computeGeometry</code>.
     * <p>
     * BrowserBalloon separates render state updates from geometry updates for
     * two reasons: <ul> <li>Geometry may be updated based on different
     * conditions.</li> <li>Rendering state potentially needs to be updated in
     * getBounds.</li> </ul>
     *
     * @param dc the current draw context.
     */
    ...
    /**
     * Update the balloon's active attributes and points, if that hasn't already
     * been done this frame.This updates the balloon's rendering state as
     * follows: <ul> <li>Computes the balloon's active attributes by calling
     * <code>determineActiveAttributes</code> and stores the result in
     * <code>activeAttributes</code>.</li> <li>Computes the balloon's
     * model-coordinate and screen-coordinate points by calling
     * <code>computeBalloonPoints</code>.</li>
     * </ul>
     *
     * @param dc the current draw context.
    ...
    /**
     * Computes the size of this balloon's frame in the viewport (on the
     * screen). If this balloon's maximum size is not <code>null</code>, the
     * returned size is no larger than the maximum size.
     *
     * @param dc the current draw context.
    ...
    /**
     * Indicates whether this balloon's screen-coordinate geometry must be
     * recomputed as a result of a balloon attribute changing.
     *
     * @param obb The balloon to check.
     * @return <code>true</code> if this balloon's geometry must be recomputed,
     ...
     * <code>frameInfo</code> according to the current screen bounds, screen
     * offset, and active attributes.
     *
     * @param obb The balloon to update.
     */
    ...
    /**
     * Creates the balloon's frame vertex buffer according to the active
     * attributes.
     *
     * @param obb The balloon.
     ...
     * @param obb The balloon to check.
     *
     * @return <code>true</code> If the balloon intersects the frustum,
     * otherwise <code>false</code>.
     */
    ...
    /**
     * Draws this browser balloon's interior geometry in screen-coordinates,
     * with the <code>WebView's</code> texture representation applied as an
     * OpenGL decal.OpenGL's texture decal mode uses the texture color where the
     * texture's alpha is 1, and uses the balloon's background color where it's
     * 0. The texture's internal format must be RGBA to work correctly, and this
     * assumes that the WebView's texture format is RGBA.
     * <p>
     * If the <code>WebView's</code> texture cannot be created or cannot be
     * applied for any reason, this displays the balloon's interior geometry in
     * the balloon's background color without applying the
     * <code>WebView's</code> texture.
     * <p>
     * If the specified draw context is in picking mode, this draws the
     * balloon's interior geometry in the current color and does not apply the
     * <code>WebView's</code> texture.
     *
     * @param dc the current draw context.
    ...
    /**
     * Computes the line width to use during picking (in pixels). Returns the
     * larger of this balloon's outline width and its
     * <code>outlinePickWidth</code>.
     *
    ...
    /**
     * Draw pickable regions for the resize controls.A pickable region is drawn
    ...
    /**
     * Forwards the <code>MouseEvent</code> associated with the specified
     * <code>event</code> to the balloon's internal <code>WebView</code>. This
     * does not consume the event, because the <code>{@link
     * gov.nasa.worldwind.event.InputHandler}</code> implements the policy for
     ...
     * <code>{@link gov.nasa.worldwind.util.webview.WebView}</code> and consumes
     * the event. This consumes the event so the
     * <code>{@link gov.nasa.worldwind.View}</code> doesn't respond to it.
     *
     ...
     * <code>{@link gov.nasa.worldwind.util.webview.WebView}</code> and consumes
     * the event. This consumes the event so the
     * <code>{@link gov.nasa.worldwind.View}</code> doesn't respond to it. The
     *
     * @param event The event to forward.
     */
     ...
     * <code>{@link gov.nasa.worldwind.util.webview.WebView}</code>. This does
     * not consume the event, because the
     * <code>{@link gov.nasa.worldwind.event.InputHandler}</code> implements the
     * policy for consuming or forwarding mouse moved events to other objects.
     * <p>
     * Unlike mouse clicked, mouse pressed, and mouse dragged events, mouse move
     * events cannot be forwarded to the WebView via SelectEvents in
     * <code>selected</code>, because mouse movement events are not selection
     * events.
     *
     * @param event The event to forward.
     */
     ...
     * <code>{@link gov.nasa.worldwind.View}</code> doesn't respond to it.
     * <p>
     * Unlike mouse clicked, mouse pressed, and mouse dragged events, mouse
     * wheel events cannot be forwarded to the WebView via SelectEvents in
     * <code>selected</code>, because mouse wheel events are not selection
     * events.
     *
     * @param event The event to forward.
     */
    ...
    /**
     * Returns a <code>null</code> Cursor, indicating the default cursor should
     * be used when the BrowserBalloon is active. The Cursor is set by the
     * <code>{@link gov.nasa.worldwind.util.webview.WebView}</code> in response
     * to mouse moved events.
     *
    ...
    /**
     * Sends the specified <code>SelectEvent</code> to the balloon's internal
     * <code>WebView</code>. The event's <code>pickPoint</code> is converted
     * from AWT coordinates to the WebView's local coordinate system.
     *
    ...
    /**
     * Sends the specified <code>KeyEvent</code> to the balloon's internal
     * <code>WebView</code>.
     * <p>
     * This does nothing if the balloon's internal <code>WebView</code> is
     * uninitialized.
     *
     * @param event the event to send.
     */
    ...
    /**
     * Sends the specified <code>MouseEvent</code> to the balloon's internal
     * <code>WebView</code>. The event's point is converted from AWT coordinates
    ...
    /**
     * Converts the specified screen point from AWT coordinates to local WebView
     * coordinates.
     *
     * @param point The point to convert.
     * @param context the component who's coordinate system the point is in.
     *
     * @return A new <code>Point</code> in the WebView's local coordinate
     * system.
     */
