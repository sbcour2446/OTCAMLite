...
/**
 * Builds a list of {@link gov.nasa.worldwind.render.SurfaceTile} instances
 * who's content is defined by a specified set of
 * {@link gov.nasa.worldwind.render.SurfaceRenderable} instances. It's typically
 * not necessary to use SurfaceObjectTileBuilder directly. WorldWind's default
 * scene controller automatically batches instances of SurfaceRenderable in a
 * single SurfaceObjectTileBuilder. Applications that need to draw basic surface
 * shapes should use or extend {@link gov.nasa.worldwind.render.SurfaceShape}
 * instead of using SurfaceObjectTileBuilder directly.
 * <p>
 * Surface tiles are built by calling {@link #buildTiles(DrawContext, Iterable)}
 * with an iterable of surface renderables. This assembles a set of surface
 * tiles that meet the resolution requirements for the specified draw context,
 * then draws the surface renderables into those offscreen surface tiles by
 * calling render on each instance. This process may temporarily use the
 * framebuffer to perform offscreen rendering, and therefore should be called
 * during the preRender method of a WorldWind layer. See the
 * {@link gov.nasa.worldwind.render.PreRenderable} interface for details. Once
 * built, the surface tiles can be rendered by a
 * {@link gov.nasa.worldwind.render.SurfaceTileRenderer}.
 * <p>
 * By default, SurfaceObjectTileBuilder creates texture tiles with a width and
 * height of 512 pixels, and with internal format <code>GL_RGBA</code>. These
 * parameters are configurable by calling {@link
 * #setTileDimension(java.awt.Dimension)} or {@link #setTileTextureFormat(int)}.
 * <p>
 * The most common usage pattern for SurfaceObjectTileBuilder is to build the
 * surface tiles from a set of surface renderables during the preRender phase,
 * then draw those surface tiles during the render phase. For example, a
 * renderable can use SurfaceObjectTileBuilder to draw a set of surface
 * renderables as follows:
 * <pre>
 * <code>
 ...
 *
 * @author dcollins
 * @version $Id: SurfaceObjectTileBuilder.java 3108 2015-05-26 19:07:06Z
 * dcollins $
 */
public class SurfaceObjectTileBuilder {
    ...
    /**
     * The default split scale. The split scale 2.9 has been empirically
     * determined to render sharp lines and edges with the SurfaceShapes such as
     * SurfacePolyline and SurfacePolygon.
     */
    ...
    /**
     * The default number of levels used to construct a LevelSet. Approximately
     * 0.1 meters per pixel at the Earth's equator.
     */
    protected static final int DEFAULT_NUM_LEVELS = 17;
    /**
     * The next unique ID. This property is shared by all instances of
     ...
     * This map is a class property in order to share LevelSets across all
     * instances of SurfaceObjectTileBuilder.
     */
    protected static Map<Dimension, LevelSet> levelSetMap = new HashMap<Dimension, LevelSet>();

    /**
     * Indicates the desired tile texture width and height, in pixels. Initially
     * set to <code>DEFAULT_TEXTURE_DIMENSION</code>.
     */
    protected Dimension tileDimension = new Dimension(DEFAULT_TEXTURE_DIMENSION, DEFAULT_TEXTURE_DIMENSION);
    /**
     * The surface tile OpenGL texture format. 0 indicates the default format is
     * used.
     */
    protected int tileTextureFormat;
    /**
     * Controls if surface tiles are rendered using a linear filter or a
     ...
     */
    protected boolean forceTileUpdates = true;
    /**
     * Controls the tile resolution as distance changes between the globe's
     * surface and the eye point.
     */
    protected double splitScale = DEFAULT_SPLIT_SCALE;
    /**
     * List of currently assembled surface renderables. Valid only during the
     * execution of {@link
    ...
    /**
     * The currently active TileInfo. Valid only during the execution of
     * {@link #buildTiles(DrawContext, Iterable)}.
     */
     ...
     * <code>512</code>, with the default tile texture format, with linear
     * filtering enabled, and with mip-mapping disabled.
     */
    public SurfaceObjectTileBuilder() {
    }

    /**
     * Constructs a new SurfaceObjectTileBuilder width the specified tile
     * dimension, tile texture format, and flags specifying if linear filtering
     * and mip-mapping are enabled.
     *
     * @param tileTextureDimension the surface tile texture dimension, in
     * pixels.
     * @param tileTextureFormat the surface tile OpenGL texture format, or 0 to
     * use the default format.
     * @param useLinearFilter true to use linear filtering while rendering
     * surface tiles; false to use nearest-neighbor filtering.
     * @param useMipmaps true to generate mip-maps for surface tile textures;
     * false otherwise.
     *
    ...
    /**
     * Specifies the preferred surface tile texture dimension. If the dimension
     * is larger than the viewport dimension, this uses a dimension with width
     * and height set to the largest power of two that is less than or equal to
     * the specified dimension and the viewport dimension.
     *
    ...
    /**
     * Returns the surface tile's OpenGL texture format, or 0 to indicate that
    ...
    /**
     * Specifies the surface tile's OpenGL texture format. A value of 0
     * indicates that the default format should be used. Otherwise, the texture
     * format may be one of the following: <ul> <li>GL_ALPHA</li>
     * <li>GL_ALPHA4</li>
     * <li>GL_ALPHA8</li> <li>GL_ALPHA12</li> <li>GL_ALPHA16</li>
     * <li>GL_COMPRESSED_ALPHA</li>
     * <li>GL_COMPRESSED_LUMINANCE</li> <li>GL_COMPRESSED_LUMINANCE_ALPHA</li>
     * <li>GL_COMPRESSED_INTENSITY</li>
     * <li>GL_COMPRESSED_RGB</li> <li>GL_COMPRESSED_RGBA</li>
     * <li>GL_DEPTH_COMPONENT</li> <li>GL_DEPTH_COMPONENT16</li>
     * <li>GL_DEPTH_COMPONENT24</li> <li>GL_DEPTH_COMPONENT32</li>
     * <li>GL_LUMINANCE</li> <li>GL_LUMINANCE4</li>
     * <li>GL_LUMINANCE8</li> <li>GL_LUMINANCE12</li> <li>GL_LUMINANCE16</li>
     * <li>GL_LUMINANCE_ALPHA</li>
     * <li>GL_LUMINANCE4_ALPHA4</li> <li>GL_LUMINANCE6_ALPHA2</li>
     * <li>GL_LUMINANCE8_ALPHA8</li>
     * <li>GL_LUMINANCE12_ALPHA4</li> <li>GL_LUMINANCE12_ALPHA12</li>
     * <li>GL_LUMINANCE16_ALPHA16</li>
     * <li>GL_INTENSITY</li> <li>GL_INTENSITY4</li> <li>GL_INTENSITY8</li>
     * <li>GL_INTENSITY12</li>
     * <li>GL_INTENSITY16</li> <li>GL_R3_G3_B2</li> <li>GL_RGB</li>
     * <li>GL_RGB4</li> <li>GL_RGB5</li> <li>GL_RGB8</li>
     * <li>GL_RGB10</li> <li>GL_RGB12</li> <li>GL_RGB16</li> <li>GL_RGBA</li>
     * <li>GL_RGBA2</li> <li>GL_RGBA4</li>
     * <li>GL_RGB5_A1</li> <li>GL_RGBA8</li> <li>GL_RGB10_A2</li>
     * <li>GL_RGBA12</li> <li>GL_RGBA16</li>
     * <li>GL_SLUMINANCE</li> <li>GL_SLUMINANCE8</li>
     * <li>GL_SLUMINANCE_ALPHA</li> <li>GL_SLUMINANCE8_ALPHA8</li>
     * <li>GL_SRGB</li> <li>GL_SRGB8</li> <li>GL_SRGB_ALPHA</li>
     * <li>GL_SRGB8_ALPHA8</li> </ul>
     * <p>
     * If the texture format is any of
     * <code>GL_RGB, GL_RGB8, GL_RGBA, or GL_RGBA8</code>, the tile builder
     * attempts to use OpenGL framebuffer objects to render shapes to the
     * texture tiles. Otherwise, this renders shapes to the framebuffer and
     * copies the framebuffer contents to the texture tiles.
     *
     * @param textureFormat the OpenGL texture format, or 0 to use the default
     * format.
     */
     ...
     *
     * @return true if linear filtering is used; false if nearest-neighbor
     * filtering is used.
     */
    ...
    /**
     * Specifies if linear filtering should be used when rendering surface
     * tiles.
     *
     * @param useLinearFilter true to use linear filtering; false to use
     ...
     * {@link #buildTiles(DrawContext, Iterable)}. When true, tile textures
     * always update their contents with the current surface renderables. When
     * false, tile textures only update their contents when the surface
     * renderables change. Initially false.
     *
     * @return true if tile textures always update their contents, false if tile
     * textures only update when the surface renderables change.
     */
     ...
     * renderables change.
     *
     * @param forceTileUpdates true if tile textures should always update their
     * contents, false if tile textures should only update when the surface
     * renderables change.
     */
    ...
    /**
     * Sets the parameter controlling the tile resolution as distance changes
     * between the globe's surface and the eye point. Higher resolution is
     * displayed as the split scale increases from 1.0. Lower resolution is
     * displayed as the split scale decreases from 1.0. The default value is
     * 2.9.
     *
     * @param splitScale a value near 1.0 that controls the tile's surface texel
     * resolution as the distance between the globe's surface and the eye point
     * change. Increasing values select higher resolution, decreasing values
     * select lower resolution. The default value is 2.9.
     */
    ...
    /**
     * Returns the split scale value controlling the tile's surface texel
     * resolution relative to the distance between the globe's surface at the
     * image position and the eye point.
     *
     ...
     *
     * @return a count of SurfaceTiles containing a composite representation of
     * the specified surface renderables.
     *
     ...
     * @param dc the draw context used to build tiles.
     *
     * @return a List of SurfaceTiles containing a composite representation of
     * the specified surface renderables.
     *
     * @throws IllegalArgumentException if the draw context is null.
     */
    ...
    /**
     * Assembles the surface tiles and draws any surface renderables in the
     * iterable into those offscreen tiles. The surface tiles are assembled to
     * meet the necessary resolution of to the draw context's {@link
     * gov.nasa.worldwind.View}. This may temporarily use the framebuffer to
     * perform offscreen rendering, and therefore should be called during the
     * preRender method of a WorldWind {@link gov.nasa.worldwind.layers.Layer}.
     * <p>
     * This does nothing if the specified iterable is null, is empty or contains
     * no surface renderables.
     *
    ...
    /**
     * Removes all entries from the list of SurfaceTiles assembled during the
     * last call to {@link
     * #buildTiles(DrawContext, Iterable)}.
     *
     * @param dc the draw context used to build tiles.
    ...
    /**
     * Returns the list of pickable object candidates associated with the
     * SurfaceTiles assembled during the last call to
     * {@link #buildTiles(DrawContext, Iterable)}.
     *
     * @param dc the draw context used to build tiles.
     *
    ...
    /**
     * Removes all entries from the list of pickable object candidates assembled
     * during the last call to {@link
     * #buildTiles(DrawContext, Iterable)}.
     *
     * @param dc the draw context used to build tiles.
     *
    ...
    //**************************************************************//
    /**
     * Updates each {@link SurfaceObjectTileBuilder.SurfaceObjectTile} in the
     * {@link #currentInfo}. This is typically called after
     * {@link #assembleTiles(DrawContext)} to update the assembled tiles.
     * <p>
    ...
    /**
     * Draws the current list of surface renderables into the specified surface
     * tile. The surface tiles is updated only when necessary. The tile keeps
     * track of the list of surface renderables rendered into it, and the state
     * keys those objects. The tile is updated if the list changes, if any of
     * the state keys change, or if the tile has no texture. Otherwise the tile
     * is left unchanged and the update is skipped.
     *
    ...
    /**
     * Returns a new surface tile texture for use on the specified draw context
     * with the specified width and height.
     * <p>
     * The returned texture's internal format is specified by
     * <code>tilePixelFormat</code>. If <code>tilePixelFormat</code> is zero,
     * this returns a texture with internal format <code>GL_RGBA8</code>.
     * <p>
     * The returned texture's parameters are configured as follows: <table>
     * <caption style="font-weight: bold;">Parameters</caption><tr><th>Parameter
     * Name</th><th>Value</th></tr>
     * <tr><td><code>GL.GL_TEXTURE_MIN_FILTER</code></td><td><code>GL_LINEAR_MIPMAP_LINEAR</code>
     * if <code>useLinearFilter</code> and <code>useMipmaps</code> are both
     * true, <code>GL_LINEAR</code> if <code>useLinearFilter</code> is true and
     * <code>useMipmaps</code> is false, and <code>GL_NEAREST</code> if
     * <code>useLinearFilter</code> is false.</td></tr>
     * <tr><td><code>GL.GL_TEXTURE_MAG_FILTER</code></td><td><code>GL_LINEAR</code>
     * if <code>useLinearFilter</code> is true, <code>GL_NEAREST</code> if
     * <code>useLinearFilter</code> is false.</td></tr>
    ...
    protected Texture createTileTexture(DrawContext dc, int width, int height) {
        ...
        {
            /**
             * Overridden to return a non-zero size. TextureData does not
             * compute an estimated memory size if the buffer is null. Therefore
             * we override getEstimatedMemorySize() to return the appropriate
             * size in bytes of a texture with the common pixel formats.
             */
            @Override
    ...
    /**
     * Returns a new Object representing the drawing context for the specified
     * tile. The returned object should represent the tile's sector and it's
     * corresponding viewport in pixels.
     *
    ...
    //**************************************************************//
    /**
     * Adds any SurfaceRenderables in the specified Iterable to the tile
     * builder's {@link #currentSurfaceObjects} list.
     *
    ...
    //**************************************************************//
    /**
     * Returns a shared <code>LevelSet</code> for the specified
     * <code>tileDimension</code>. All instances of
     * <code>SurfaceObjectTileBuilder</code> share common LevelSets to determine
     * which tiles are visible, but create unique tile instances and uses a
     * unique tile cache name. Since all instances use the same tile structure
     * to determine visible tiles, this saves memory while ensuring that each
     * instance stores its own tiles in the cache.
     * <p>
     * The returned LevelSet's cache name and dataset name are dummy values, and
     * should not be used. Use this tile builder's cache name for the specified
     * <code>tileDimension</code> instead.
     * <p>
     * In practice, there are at most 10 dimensions we use: 512, 256, 128, 64,
     * 32, 16, 8, 4, 2, 1. Therefore keeping the <code>LevelSet</code>s in a map
     * requires little memory overhead, and ensures each <code>LevelSet</code>
     * is retained once constructed. Retaining references to the
     * <code>LevelSet</code>s means we're able to re-use the texture resources
     * associated with each <code>LevelSet</code> in the
     * <code>DrawContext</code>'s texture cache.
     * <p>
     * Subsequent calls are guaranteed to return the same <code>LevelSet</code>
     * for the same <code>tileDimension</code>.
     *
    ...
    /**
     * Returns a new LevelSet with the specified tile width and height. The
     * LevelSet overs the full sphere, has a level zero tile delta of
     * {@link #DEFAULT_LEVEL_ZERO_TILE_DELTA}, has number of levels equal to {@link
     * #DEFAULT_NUM_LEVELS} (with no empty levels). The LevelSets' cache name
     * and dataset name dummy values, and should not be used.
     *
    ...
    //**************************************************************//
    /**
     * Returns true if the draw context's viewport width and height are greater
     * than zero.
     *
     * @param dc the DrawContext to test.
     *
     * @return true if the DrawContext's has a non-zero viewport; false
     * otherwise.
     */
    ...
    /**
     * Assembles a set of surface tiles that are visible in the specified
     * DrawContext and meet the tile builder's resolution criteria. Tiles are
     * culled against the current surface renderable list, against the
     * DrawContext's view frustum during rendering mode, and against the
     * DrawContext's pick frustums during picking mode. If a tile does not meet
     * the tile builder's resolution criteria, it's split into four sub-tiles
     * and the process recursively repeated on the sub-tiles. Visible leaf tiles
     * are added to the {@link #currentInfo}.
     * <p>
     * During assembly each surface renderable in {@link #currentSurfaceObjects}
     * is sorted into the tiles they intersect. The top level tiles are used as
     * an index to quickly determine which tiles each renderable intersects.
     * Surface renderables are sorted into sub-tiles by simple intersection
     * tests, and are added to each tile's surface renderable list at most once.
     * See {@link SurfaceObjectTileBuilder.SurfaceObjectTile#addSurfaceObject(SurfaceRenderable,
     * gov.nasa.worldwind.geom.Sector)}. Tiles that don't intersect any surface
     * renderables are discarded.
     *
    ...
    /**
     * Potentially adds the specified tile or its descendants to the tile
     * builder's {@link #currentInfo}. The tile and its descendants are
     * discarded if the tile is not visible or does not intersect any surface
     * renderables in the parent's surface renderable list. See
     * {@link SurfaceObjectTileBuilder.SurfaceObjectTile#getObjectList()}.
     * <p>
     * If the tile meet the tile builder's resolution criteria it's added to the
     * tile builder's <code>currentTiles</code> list. Otherwise, it's split into
     * four sub-tiles and each tile is recursively processed. See
     * {@link #meetsRenderCriteria(DrawContext, gov.nasa.worldwind.util.LevelSet, gov.nasa.worldwind.util.Tile)}.
     *
    ...
    /**
     * Adds surface renderables from the parent's object list to the specified
     * tile's object list. If the tile's sector does not intersect the sector
     * bounding the parent's object list, this does nothing. Otherwise, this
     * adds any of the parent's surface renderables that intersect the tile's
     * sector to the tile's object list.
     *
     * @param dc the current DrawContext.
    ...
    /**
     * Adds the specified tile to this tile builder's {@link #currentInfo} and
     * the TextureTile memory cache.
     *
     * @param tile the tile to add.
     */
    ...
    /**
     * Test if the tile intersects the specified draw context's frustum. During
     * picking mode, this tests intersection against all of the draw context's
     * pick frustums. During rendering mode, this tests intersection against the
     * draw context's viewing frustum.
     *
     * @param dc the draw context the surface renderable is related to.
     * @param tile the tile to test for intersection.
     *
     * @return true if the tile intersects the draw context's frustum; false
     * otherwise.
     */
     ...
     * This returns false if the draw context's visible sector is null.
     *
     * @param dc the current draw context.
     * @param tile the tile to test for intersection.
     *
     * @return true if the tile intersects the draw context's visible sector;
     * false otherwise.
     */
    ...
    /**
     * Tests if the specified tile meets the rendering criteria on the specified
     * draw context. This returns true if the tile is from the level set's final
     * level, or if the tile achieves the desired resolution on the draw
     * context.
     *
     * @param dc the current draw context.
    ...
    /**
     * Tests if the specified tile must be split to meets the desired resolution
     * on the specified draw context. This compares the distance form the eye
     * point to the tile to determine if the tile meets the desired resolution
     * for the {@link gov.nasa.worldwind.View} attached to the draw context.
     *
     * @param dc the current draw context.
     * @param tile the tile to test.
     *
    ...
    //**************************************************************//
    /**
     * Creates a key to address the tile information associated with the
     * specified draw context. Each key is unique to this instance, the tile
     * dimensions that fit in the draw context's viewport, and the globe offset
     * when a 2D globe is in use. Using a unique set of tile information ensures
     * that
     * <p>
     * In practices, there are at most 10 dimensions we'll use (512, 256, 128,
     * 64, 32, 16, 8, 4, 2, 1) and 3 globe offsets (-1, 0, 1). Therefore there
     * are at most 30 sets of tile information for each instance of
     * SurfaceObjectTileBuilder.
     *
    ...
    /**
     * Returns the tile dimension used to create the tile textures for the
     * specified <code>DrawContext</code>. This attempts to use this tile
     * builder's {@link #tileDimension}, but always returns a dimension that is
     * is a power of two, is square, and fits in the <code>DrawContext</code>'s
     * viewport.
     *
     * @param dc the <code>DrawContext</code> to compute a texture tile
     * dimension for.
     *
     * @return a texture tile dimension appropriate for the specified
     ...
     * {@code sector}, {@code level}, {@code row}, {@code
     * column}, and {@code cacheName}.
     *
     * @param sector The tile's Sector.
     * @param level The tile's Level in a {@link LevelSet}.
     * @param row The tile's row in the Level, starting from 0 and increasing to
     * the right.
     * @param column The tile's column in the Level, starting from 0 and
     * increasing upward.
     ...
     * {@code level}, {@code row}, {@code column}, and {@code cacheName}.
     *
     * @param level The tile's Level in a {@link LevelSet}.
     * @param row The tile's row in the Level, starting from 0 and increasing to
     * the right.
    ...
    /**
     * Represents a {@link gov.nasa.worldwind.layers.TextureTile} who's contents
     * is constructed by a set of surface objects. The tile maintains a
     * collection of surface renderables that intersect the tile, and provides
     * methods for to modify and retrieve that collection. Additionally, the
     * method {@link #getStateKey(DrawContext)} provides a mechanism to uniquely
     * identify the tile's current state, including the state of each
     * intersecting surface object.
     */
    protected static class SurfaceObjectTile extends TextureTile {
        ...
        /**
         * Constructs a tile for a given sector, level, row and column of the
         * tile's containing tile set.
         *
         * @param sector The sector corresponding with the tile.
         * @param level The tile's level within a containing level set.
         * @param row The row index (0 origin) of the tile within the indicated
         * level.
         * @param column The column index (0 origin) of the tile within the
         * indicated level.
         * @param cacheName The tile's cache name. Overrides the Level's cache
         * name to associates the tile with it's tile builder in a global cache.
         *
         * @throws IllegalArgumentException if any of the {@code sector},
         * {@code level}, or {@code cacheName } are {@code null}.
         */
        ...
        /**
         * Returns the tile's size in bytes. Overridden to append the size of
         * the {@link #lastUpdateStateKey} to the superclass' computed size.
         *
        ...
        /**
         * Returns an object that uniquely identifies the tile's state on the
         * specified draw context. This object is guaranteed to be globally
         * unique; an equality test with a state key from another always returns
         * false.
         *
        ...
        /**
         * Returns a sector that bounds the surface renderables intersecting the
         * tile. This returns null if no surface objects intersect the tile.
         *
        ...
        /**
         * Returns whether list of surface renderables intersecting this tile
         * has elements.
         *
         * @return {@code true} if the list of surface renderables intersecting
         * this tile has elements, and {@code
         * false} otherwise.
         */
         ...
         * {@link #getObjectSector()} returns null after calling this method.
         */
        ...
        /**
         * Adds the specified surface renderable to the tile's list of
        ...
        /**
         * Adds the specified collection of surface renderables to the tile's
         ...
         * {@inheritDoc}
         * <p>
         * Overridden to return a new SurfaceObjectTile. The returned tile is
         * created with the same cache name as this tile.
         */
        @Override
    ...
    /**
     * Represents a surface renderable tile's current state. TileStateKey
     * distinguishes the tile's state by comparing the individual state keys of
     * the surface renderables intersecting the tile. This does not retain any
     * references to the surface renderables themselves. Should the tile state
     * key live longer than the surface renderables, the state key does not
     * prevent those objects from being reclaimed by the garbage collector.
     */
    protected static class SurfaceObjectTileStateKey implements Cacheable {
        ...
        /**
         * Construsts a tile state key for the specified surface renderable
         * tile.
         *
        ...
        /**
         * Returns the tile state key's size in bytes. The total size of the
         * intersecting object keys, plus the size of the array itself. The
         * tileKey is owned by the SurfaceObjectTile, so we don't include it in
         * the state key's size.
         *
