...
public abstract class URLRetriever extends WWObjectImpl implements Retriever {
    ...
    /**
     * Applications never need to use this constant. It provides compatibility
     * with very old WorldWind tile servers that deliver zipped content without
     * identifying the content type as other than application/zip. In these
     * cases, the object requesting the content must know the content type to
     * expect, and also requires that the zip file be opened and only its first
     * entry returned.
     */
     ...
     *
     * @return a retriever for the protocol specified in the url, or null if no
     * retriever exists for the protocol.
     *
     ...
     * @param postProcessor the retrieval post-processor to invoke when the
     * resource is retrieved. May be null.
     *
    ...
    /**
     * {@inheritDoc} Expiration time is determined by either the Expires header,
     * or the max-age directive of the Cache-Control header. Cache-Control has
     * priority if both headers are specified (see section 14.9.3 of the <a
     * href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html" target="_blank">HTTP
     * Specification</a>).
     */
     ...
     *
     * @throws Exception if <code>connection</code> is null or an exception
...
//        System.out.println(this.contentLength + " bytes to read");
     ...
     *
     * @throws java.io.IOException if the stream does not refer to a zip
     * resource or an exception occurs during reading.
    ...
    /**
     * Indicates the expiration time specified by either the Expires header or
     * the max-age directive of the Cache-Control header. If both are present,
     * then Cache-Control is given priority (See section 14.9.3 of the HTTP
     * Specification: http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html).
     * <p>
     * If both the Expires and Date headers are present then the expiration time
     * is calculated as current time + (expires - date). This helps guard
     * against clock skew between the client and server.
     *
     * @param connection Connection for which to get expiration time.
     *
     * @return The expiration time, in milliseconds since the Epoch, specified
     * by the HTTP headers, or zero if there is no expiration time.
     */
