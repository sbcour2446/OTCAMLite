...
/**
 * Abstract base class for retrieval post-processors. Verifies the retrieval
 * operation and dispatches the content to the a subclasses content handlers.
 * <p>
 * Subclasses are expected to override the methods necessary to handle their
 * particular post-processing operations.
 *
 * @author Tom Gaskins
 * @version $Id: AbstractRetrievalPostProcessor.java 1171 2013-02-11 21:45:02Z
 * dcollins $
 */
public abstract class AbstractRetrievalPostProcessor implements RetrievalPostProcessor {
    ...
    /**
     * The retriever associated with the post-processor. Only non-null after
     * {@link #run(Retriever)} is called.
     */
    protected Retriever retriever;

    /**
     * Abstract method that subclasses must provide to identify the output file
     * for the post-processor's retrieval content.
     *
    ...
    /**
     * Create a post-processor and pass it attributes that can be examined
     * during content handling.
     *
     * @param avList an attribute-value list with values that might be used
     ...
     *
     * @return a buffer containing the downloaded data, perhaps converted during
     * content handling. null is returned if a fatal problem occurred during
     * post-processing.
     *
     ...
     *
     * @return the retriever associated with the post-processor, or null if no
     ...
     * {@link Retriever#RETRIEVER_STATE_SUCCESSFUL}. Can be overridden by
     * subclasses to handle special error cases. The default implementation
     * calls {@link #markResourceAbsent()} if the retrieval state is
     * {@link Retriever#RETRIEVER_STATE_ERROR}.
     */
     ...
     *
     * @return true if the response code is the OK value for the protocol, e.g.,
     * ({@link HttpURLConnection#HTTP_OK} for HTTP protocol), otherwise false.
     */
    ...
    /**
     * Checks the retrieval's HTTP response code. Must only be called when the
     * retriever is a subclass of {@link
     * gov.nasa.worldwind.retrieve.HTTPRetriever}.
     *
     * @return true if the response code is {@link HttpURLConnection#HTTP_OK},
     * otherwise false.
     */
    ...
    /**
     * Handle the case of an invalid response code. Subclasses can override this
     * method to handle special cases. The default implementation calls
     * {@link #markResourceAbsent()} and logs the contents of the retrieval
     * buffer if it contains content of type "text".
     */
    ...
    /**
     * Marks the retrieval target absent. Subclasses should override this method
     * if they keep track of absent-resources. The default implementation does
     * nothing.
     */
    ...
    /**
     * Saves the retrieved and possibly transformed data. The data may have been
     * transformed during content handling.
     * <p>
     * The default implementation of this method simply calls
     * {@link #saveBuffer(java.nio.ByteBuffer)} with an argument of null.
     *
     * @return true if the buffer was saved, false if the output file could not
     * be determined or already exists and not overwritten.
     *
     * @throws IOException if an IO error occurs while attempting to save the
     * buffer.
     */
    ...
    /**
     * Saves the retrieved and possibly transformed data. The data may have been
     * transformed during content handling. The data is not saved if the output
     * file already exists unless {@link #overwriteExistingFile()} returns true.
     *
     ...
     * be determined or already exists and not overwritten.
     *
     * @throws IOException if an IO error occurred when attempting to save the
     * buffer.
     */
    ...
    /**
     * Indicates whether the retrieved data should be written to the output file
     * if a file of the same name already exists. The default implementation of
     * this method returns false (files are not overwritten).
     *
    ...
    /**
     * Indicates whether the output file should have its delete-on-exit flag set
     * so that it's deleted when the JVM terminates.
     *
     * @param outFile the output file.
     *
     * @return true if the output file's delete-on-exit flag should be set,
     * otherwise false.
     */
    ...
    /**
     * Returns an object that can be used to synchronize writing to the output
     * file. Superclasses should override this method and return the object used
     * as a lock by other objects that read or otherwise interact with the
     * output file.
     *
     * @return an object to use for read/write synchronization, or null if no
     * lock is needed.
     */
    ...
    /**
     * Process the retrieved data. Dispatches content handling to content-type
     * specific handlers: {@link
     * #handleZipContent()} for content types containing "zip",
     * {@link #handleTextContent()} for content types starting with "text", and
     * {@link #handleImageContent()} for contents types starting with "image".
     *
     * @return a buffer containing the retrieved data, which may have been
     * transformed during content handling.
     *
    ...
    /**
     * Reacts to exceptions occurring during content handling. Subclasses may
     * override this method to perform special exception handling. The default
     * implementation logs a message specific to the exception.
     *
     ...
     * Subclasses may override this method to handle such cases. The default
     * implementation logs an error message and returns null.
     *
     * @return null if no further processing should occur, otherwise the
     * retrieved data, perhaps transformed.
     */
    ...
    /**
     * Handles Text content. If the content type is text/xml,
     * {@link #handleXMLContent()} is called. If the content type is text/html,
     * {@link #handleHTMLContent()} is called. For all other sub-types the
     * content is logged as a message with level
    ...
    /**
     * Handles XML content. The default implementation only calls
     * {@link #logTextBuffer(java.nio.ByteBuffer)} and returns.
     *
    ...
    /**
     * Handles HTML content. The default implementation only calls
     * {@link #logTextBuffer(java.nio.ByteBuffer)} and returns.
     *
    ...
    /**
     * Log the content of a buffer as a String. If the buffer is null or empty,
     * nothing is logged. Only the first 2,048 characters of the buffer are
     * included in the log message.
     *
     * @param buffer the content to log. The content is assumed to be of type
     * "text".
     */
    ...
    /**
     * Handles zipped content. The default implementation saves the data to the
     * retriever's output file without unzipping it.
     *
    ...
    /**
     * Handles application content. The default implementation saves the
     * retrieved data without modification via {@link
     * #saveBuffer()} without.
     *
     * @return a buffer containing the retrieved data.
     *
    ...
    /**
     * Handles image content. The default implementation simply saves the
     * retrieved data via {@link #saveBuffer()}, first converting it to DDS if
     * the suffix of the output file is .dds.
     * <p>
     * The default implementation of this method returns immediately if the
     * output file cannot be determined or it exists and
     * {@link #overwriteExistingFile()} returns false.
     *
     * @return a buffer containing the retrieved data.
     *
     * @throws IOException if an IO error occurs while processing the data.
    ...
    /**
     * Transform the retrieved data in some purpose-specific way. May be
     * overridden by subclasses to perform special transformations. The default
     * implementation calls {@link ImageUtil#mapTransparencyColors(java.awt.image.BufferedImage,
     * int[])} if the attribute-value list specified at construction contains
     * transparency colors (includes the {@link
     * AVKey#TRANSPARENCY_COLORS} key).
     *
     * @return returns the transformed data if a transform is performed,
    ...
    /**
     * Saves a DDS image file after first converting any other image format to
     * DDS.
     *
     * @return the converted image data if a conversion is performed, otherwise
     * the original image data.
     *
     * @throws IOException if an IO error occurs while converting or saving the
     * image.
     */
    ...
    /**
     * Converts an image to DDS. If the image format is not originally DDS,
     * calls {@link #transformPixels()} to perform any defined image transform.
     *
     * @return the converted image data if a conversion is performed, otherwise
     * the original image data.
     *
