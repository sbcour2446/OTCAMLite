...
/**
 * Implements an elevation model for a local file or collection of files
 * containing elevation data.
 * <p>
 * Note: Unless the amount of data associated with the local elevation models is
 * small, it's best to construct and add elevations to a local elevation model
 * on a thread other than the event dispatch thread in order to avoid freezing
 * the user interface.
 *
 * @author tag
...
public class LocalElevationModel extends AbstractElevationModel {
    ...
    /**
     * Performs the lookup and assembly of elevations for a list of specified
     * locations. This method is provided to enable subclasses to override this
     * operation.
     *
     ...
     * @param targetResolution the desired maximum horizontal resolution of the
     * elevation data to draw from.
     * @param buffer a buffer in which to return the elevations. Must be at
     * least as large as the list of locations.
     * @param mapMissingData indicates whether to replace any elevations that
     * match this elevation model's missing data signal to this model's missing
     ...
     * or buffer is null, or the buffer size is not at least as large as the
     * location list.
     ...
     *
     * @throws IllegalArgumentException if the filename is null or the file does
     * not exist, or if the file does not contain elevations in a recognizable
     * format.
     * @throws IOException if an exception occurs while opening or reading the
     * specified file.
     * @throws WWRuntimeException if an error occurs attempting to read and
     * interpret the elevation data, or if necessary information is missing from
     * the data file or its auxiliary files.
     */
     ...
     * if the file does not contain elevations in a recognizable format, or if
     * the specified file does not contain required information such as the
     * data's location and data type.
     * @throws IOException if an exception occurs while opening or reading the
     * specified file.
     * @throws WWRuntimeException if an error occurs attempting to read and
     * interpret the elevation data, or if an error occurs attempting to read
     ...
     * @param filename the filename associated with the raster. Used only for
     * error reporting.
     *
     * @throws IllegalArgumentException if necessary information is missing from
     * the raster.
     * @throws IOException if an exception occurs while opening or reading the
     * specified file.
     * @throws WWRuntimeException if an error occurs attempting to read and
     * interpret the elevation data.
     */
    ...
    /**
     * Adds new elevations to this elevation model. The elevations are specified
     * as a rectangular array arranged in row-major order in a linear buffer.
     *
     * @param byteBuffer the elevations to add.
     * @param sector the sector containing the elevations.
     * @param width the number of elevation values in a row of the array. Each
     * consecutive group of {@code width} elevation values is considered one row
     * of the array, with the first element of the buffer starting the row of
     * elevations at the sector's maximum latitude.
     * @param height the number of rows in the array.
     * @param parameters a list of key-value pairs indicating information about
     * the raster. A value for {@code
     *                   AVKey.DATA_TYPE} is required. Values recognized but not required are {@code
     *                   AVKey.MISSING_DATA_SIGNAL}, {@code AVKey.BYTE_ORDER},
     * {@code AVKey.ELEVATION_MIN} and {@code
     *                   AVKey.ELEVATION_MAX}.
     *
     * @throws IllegalArgumentException if either the buffer, sector or
     * parameters list is null, or if a key-value pair for
     * {@code AVKey.DATA_TYPE} is not specified in the parameters.
     */
    ...
    /**
     * Updates the min and max elevations for this elevation model to account
     ...
     *
     * @return the elevation if the specified location is contained in this
     * elevation model, null if it's not, or this elevation model's missing data
     * flag if that's the value at the specified location.
     */
    ...
    /**
     * Finds the tile in this elevation model that contains a specified
     * location.
     *
     * @param latRadians the location's latitude, in radians.
     * @param lonRadians the location's longitude, in radians.
     *
     * @return the tile that contains the location, or null if no tile contains
     * it.
     */
    ...
    /**
     * An internal class that represents one elevation raster in the elevation
     * model.
     */
    protected static class LocalTile {
         ...
         * @param tileWidth the number of elevation values in a row of this
         * tile's elevation raster.
         * @param tileHeight the number of rows in this tile's elevation raster.
         * @param elevations the elevations.
         * @param minEl the minimum elevation of this tile. May be null, in
         * which case the minimum is determined.
         * @param maxEl the maximum elevation of this tile. May be null, in
         * which case the maximum is determined.
         */
        ...
        /**
         * Determines whether a value signifies missing data -- voids. This
         * method not only tests the value againsy the tile's missing data flag,
         * it also tests for its being within range of the tile's minimum and
         * maximum values. The values also tested for the magic value -32767,
         * which for some reason is commonly used but not always identified in
         * GeoTiff files.
         *
         * @param value the value to test.
         *
         * @return true if the value is this tile's missing data signal, or if
         * it is outside the min/max range of the tile, or if it's equal to
         * -32767.
         */
