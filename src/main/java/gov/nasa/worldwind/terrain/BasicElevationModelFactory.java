 ...
 *
 * @author tag
 * @version $Id: BasicElevationModelFactory.java 2347 2014-09-24 23:37:03Z
 * dcollins $
 */
public class BasicElevationModelFactory extends BasicFactory {

    /**
     * Creates an elevation model from a general configuration source. The
     * source can be one of the following: <ul>
     * <li>a {@link java.net.URL}</li> <li>a {@link java.io.File}</li> <li>a
     * {@link java.io.InputStream}</li> <li> an {@link org.w3c.dom.Element}</li>
     * <li>a {@link String} holding a file name, a name of a resource on the
     * classpath, or a string representation of a URL</li> </ul>
     * <p>
     * For non-compound models, this method maps the <code>serviceName</code>
     * attribute of the <code>ElevationModel/Service</code> element of the XML
     * configuration document to the appropriate elevation-model type. Service
     * types recognized are:" <ul> <li>"WMS" for elevation models that draw
     * their data from a WMS web service.</li> <li>"WWTileService" for elevation
     * models that draw their data from a WorldWind tile service.</li>
     * <li>"Offline" for elevation models that draw their data only from the
     * local cache.</li> </ul>
     *
     * @param configSource the configuration source. See above for supported
     * types.
     * @param params properties to associate with the elevation model during
     * creation.
     *
     ...
     * or an empty string.
     * @throws WWUnrecognizedException if the source type is unrecognized or the
     * requested elevation-model type is unrecognized.
     * @throws WWRuntimeException if object creation fails for other reasons.
     * The exception identifying the source of the failure is included as the
    ...
    /**
     * Creates an elevation model from an XML description. An "href" link to an
     * external elevation model description is followed if it exists.
     *
     * @param domElement an XML element containing the elevation model
     * description.
     * @param params any parameters to apply when creating the elevation models.
     *
     * @return the requested elevation model, or null if the specified element
     * does not describe an elevation model.
     *
    ...
    /**
     * Creates a compound elevation model and populates it with a specified list
     * of elevation models.
     * <p>
     * Any exceptions occurring during creation of the elevation models are
     * logged and not re-thrown. The elevation models associated with the
     * exceptions are not included in the returned compound model.
     *
     * @param elements the XML elements describing the models in the new
     * elevation model.
     * @param params any parameters to apply when creating the elevation models.
     *
     * @return a compound elevation model populated with the specified elevation
     * models. The compound model will contain no elevation models if none were
     * specified or exceptions occurred for all that were specified.
     *
     * @see #createNonCompoundModel(org.w3c.dom.Element,
     * gov.nasa.worldwind.avlist.AVList)
     */
     ...
     *
     * @param domElement the XML element describing the elevation model to
     * create. The element must inculde a service name identifying the type of
     * service to use to retrieve elevation data. Recognized service types are
