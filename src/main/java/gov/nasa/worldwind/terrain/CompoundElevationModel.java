...
public class CompoundElevationModel extends AbstractElevationModel {
    ...
    /**
     * Returns true if this CompoundElevationModel contains the specified
     * ElevationModel, and false otherwise.
     *
     * @param em the ElevationModel to test.
     *
     * @return true if the ElevationModel is in this CompoundElevationModel;
     * false otherwise.
     *
    ...
    /**
     * Adds an elevation to this compound elevation model. The list of elevation
     * models for this class is sorted from lowest resolution to highest. This
     * method inserts the specified elevation model at the appropriate position
     * in the list, and as a side effect resorts the entire list.
     *
     * @param em The elevation model to add.
     *
     * @throws IllegalArgumentException if the specified elevation model is
     * null.
     */
    ...
    /**
     * Adds a specified elevation model to a specified position in this compound
     * elevation model's elevation model list. It's expected that this class'
     * elevation model list is sorted from lowest resolution to highest. The
     * method
     * {@link #addElevationModel(gov.nasa.worldwind.globes.ElevationModel)}
     * inserts added elevation models at the appropriate place in the list. This
     * method, however, inserts the elevation model at the specified position in
     * the list. For proper operation of this compound elevation model, the
     * caller should ensure that the specified position is the appropriate one
     * for the inserted elevation model's resolution.
     *
     * @param index The position at which to insert the specified model, zero
     * origin. Existing models are shifted to the right.
    ...
    /**
     * Recursively specifies the time of the most recent dataset update for this
     * compound model, and for each elevation model contained within this
     * compound model. Any cached data older than this time is invalid.
     *
     ...
     * {@inheritDoc}
     * <p>
     * NOTE: This method returns only unmapped elevations if the compound model
     * contains more than one elevation model. This enables the compound model's
     * lower resolution elevation models to specify missing data values for the
     * higher resolution elevation models.
     *
     ...
     * at a specified location if the source file for that level and the
     * specified location exists in the local elevation cache on disk. Note that
     * this method consults only those elevation models whose type is
     * {@link BasicElevationModel}.
     *
