...
/**
 * Downloads elevation data not currently available in the WorldWind file cache
 * or a specified {@link FileStore}. The class derives from {@link Thread} and
 * is meant to operate in its own thread.
 * <p>
 * The sector and resolution associated with the downloader are specified during
 * construction and are final.
 *
 * @author tag
 * @version $Id: BasicElevationModelBulkDownloader.java 1171 2013-02-11
 * 21:45:02Z dcollins $
 */
public class BasicElevationModelBulkDownloader extends BulkRetrievalThread {
    ...
    /**
     * Constructs a downloader to retrieve elevations not currently available in
     * the WorldWind file cache.
     * <p>
     * The thread returned is not started during construction, the caller must
     * start the thread.
     *
     * @param elevationModel the elevation model for which to download
     * elevations.
     ...
     * per texel. This value is final.
     * @param listener an optional retrieval listener. May be null.
     *
     * @throws IllegalArgumentException if either the elevation model or sector
     * are null, or the resolution is less than zero.
     */
     ...
     * @param listener an optional retrieval listener. May be null.
     *
     * @throws IllegalArgumentException if either the elevation model, the
     * sector or file store are null, or the resolution is less than zero.
     */
    public BasicElevationModelBulkDownloader(BasicElevationModel elevationModel, Sector sector, double resolution,
