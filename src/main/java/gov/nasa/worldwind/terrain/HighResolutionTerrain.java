...
/**
 * Provides operations on the best available terrain. Operations such as
 * line/terrain intersection and surface point computation use the highest
 * resolution terrain data available from the globe's elevation model. Because
 * the best available data may not be available when the operations are
 * performed, the operations block while they retrieve the required data from
 * either the local disk cache or a remote server. A timeout may be specified to
 * limit the amount of time allowed for retrieving data. Operations fail if the
 * timeout is exceeded.
 *
 * @author tag
...
public class HighResolutionTerrain extends WWObjectImpl implements Terrain {
    ...
    /**
     * Holds a tile's geometry. It's heavyweight so cached when created to
     * enable re-use.
     */
...
//            System.out.println(2 * 4 + 4 + this.vertices.length * 3 * 4);
    ...
    /**
     * Defines an internal terrain tile. This class is meant to be small and
     * quickly constructed so that many can exist simultaneously and can be
     * created anew for each operation. The geometry they refer to is cached and
     * is independent of a particular RectTile instance. Current and future
     * RectTile instances use the same cached geometry instance.
     */
     ...
     * @param targetResolution the target terrain resolution, in meters, or null
     * to use the globe's highest resolution.
     *
     ...
     *
     * @param globe the terrain's globe.
     * @param sector the desired range for the terrain. Only locations within
     * this sector may be used in operations. If null, the sector spans the
     * entire globe.
     * @param targetResolution the target terrain resolution, in meters, or null
     * to use the globe's highest resolution.
     * @param verticalExaggeration the vertical exaggeration to apply to terrain
     * elevations. Null indicates no exaggeration.
     *
     * @throws IllegalArgumentException if the globe is null.
     */
...
//        });
     ...
     *
     * @return the fraction of the cache currently used: a number between 0 and
     * 1.
     */
     ...
     *
     * @return the object's sector, either the sector specified at construction
     * or the default sector if no sector was specified at construction.
     */
    ...
    /**
     * Indicates the vertical exaggeration used when performing terrain
     * operations.
     *
    ...
    /**
     * Indicates the current timeout for operations requiring terrain data
     * retrieval.
     *
    ...
    /**
     * Specifies the maximum amount of time allowed for retrieval of all terrain
     * data necessary to satisfy an operation. Operations that retrieve data
     * throw a {@link gov.nasa.worldwind.exception.WWTimeoutException} if the
     * specified timeout is exceeded.
     *
     * @param timeout the number of milliseconds to wait. May be null, to
     * indicate that operations have unlimited amount of time to operate.
     */
    ...
    /**
     * Specifies the number of intervals within a single terrain tile. Density
     * does not affect precision, it just determines how many sample points to
     * include with each internal terrain tile. The precision -- the distance
     * between each sample point -- is governed by the terrain tolerance
     ...
     *
     * @param size the cache capacity, in bytes. Values less than 1 MB are
     * clamped to 1 MB.
     */
     ...
     * <p>
     * Note: This method produces a result only if the line is below the globe's
     * horizon, i.e., it intersects the globe's ellipsoid. If the line is above
     * the horizon, null is returned even if there is terrain in the path of the
     * line.
     *
     * @param line the line to intersect
     *
     * @return an array of intersections with the terrain, or null if there are
     * no intersections or the line is above the globe's horizon.
     *
     * @deprecated
     */
    @Deprecated
    ...
    public interface IntersectionCallback {
        ...
        /**
         * Called with the computed intersections for a line. This method is
         * called only for lines along which intersections occur.
         *
    ...
    /**
     * Intersects a specified list of geographic two-position lines with the
     * terrain.
     *
     * @param positions The positions to intersect, with the line segments
     * formed by each pair of positions, e.g. the first line in formed by
     * positions[0] and positions[1], the second by positions[2] and
     * positions[3], etc.
     * @param callback An object to call in order to return the computed
     * intersections.
     *
    ...
    /**
     * Cause the tiles used by subsequent intersection calculations to be cached
     * so that they are available immediately to those subsequent calculations.
     * <p>
     * Pre-caching is unnecessary and is useful only when it can occur before
     * the intersection calculations are needed. It will incur extra overhead
     * otherwise. The normal intersection calculations cause the same caching.
     *
     ...
     * @throws InterruptedException if the operation is interrupted. if the
     * current timeout is exceeded while retrieving terrain data.
     */
     ...
     *
     * @return the tile for the specified row and column, or null if the row or
     * column are invalid.
     */
     ...
     *
     * @return the tile for the sector, or null if the sector is outside this
     * instance's sector.
     */
     ...
     *
     * @param range the reference sector, typically that of this terrain
     * instance.
     ...
     * @param line the line to intersect
     *
     * @return a list of tiles that likely intersect the line. Some returned
     * tiles may not intersect the line but will only be near it.
     */
     ...
     *
     * @throws InterruptedException if the operation is interrupted.
     * @throws gov.nasa.worldwind.exception.WWTimeoutException if terrain data
     * retrieval exceeds the current timeout.
     */
    ...
    /**
     * Indicates whether cached elevations are used exclusively. When this flag
     * is true this high resolution terrain instance uses
     * {@link ElevationModel#getUnmappedLocalSourceElevation(Angle, Angle)} to
     * retrieve elevations. This assumes that the highest-resolution elevations
     * for the elevation model are cached locally.
     */
    ...
    /**
     * Computes the tile's cell locations, determined by the tile's density and
     * sector.
     *
    ...
    /**
     * Computes the Cartesian, model-coordinate point of a location within a
     * terrain tile.
     * <p>
     * This operation fails with a
     * {@link gov.nasa.worldwind.exception.WWTimeoutException} if a timeout has
     * been specified and it is exceeded during the operation.
     *
     ...
     *
     * @return the Cartesian, model-coordinate point of a the specified
     * location, or null if the specified location does not exist within this
     * instance's sector or if the operation is interrupted.
     *
     * @throws IllegalArgumentException if the latitude or longitude are null.
     * @throws gov.nasa.worldwind.exception.WWTimeoutException if the current
     * timeout is exceeded while retrieving terrain data.
     * @throws InterruptedException if the operation is interrupted.
     * @see #setTimeout(Long)
     */
     ...
     *
     * @return a list of the intersection points at which the triangle
     * intersects the tile, or null if there are no intersections. If there are
     * intersections, each entry in the returned list contains a two-element
     * array holding the Cartesian coordinates of the intersection point with
     * one terrain triangle. In the cases of co-planar triangles, all three
     * vertices of the terrain triangle are returned, in a three-element array.
     *
     * @throws InterruptedException if the operation is interrupted before it
     * completes.
     */
...
//                    intersections.add(new Vec4[] {triB[0], triB[1], triB[2], triB[0]});
     ...
     * @param intersectPositionsOut A list in which to place the intersection
     * positions. May not be null.
     *
     * @throws InterruptedException if the operation is interrupted before it
     * completes.
     */
    ...
    /**
     * Determines the minimum and maximum elevations and their locations within
     * a specified {@link Sector}.
     *
     * @param sector The sector in question.
     *
     * @return a two-element array containing the minimum and maximum elevations
     * and their locations in the sector. The minimum as at index 0 in the
     * array, the maximum is at index 1. If either cannot be determined, null is
    ...
    /**
     * Determines the minimum and maximum elevations and their locations within
     * a specified geographic quadrilateral.
     *
     ...
     *
     * @return a two-element array containing the minimum and maximum elevations
     * and their locations in the quadrilateral. The minimum as at index 0 in
     * the array, the maximum is at index 1. If either cannot be determined,
