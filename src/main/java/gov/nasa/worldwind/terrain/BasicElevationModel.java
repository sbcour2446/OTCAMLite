...
public class BasicElevationModel extends AbstractElevationModel implements BulkRetrievable {
     ...
     * beyond which cached data is invalid. If greater than zero, the model
     * ignores and eliminates any in-memory or on-disk cached data older than
     * the time specified, and requests new information from the data source. If
     * zero, the default, the model applies any expiry times associated with its
     ...
     * the model's individual levels if the value specified to the method is
     * greater than zero.
     *
...
//**************************************************************//
    ...
    // *** Bulk download ***
    /**
     * Start a new {@link BulkRetrievalThread} that downloads all elevations for
     * a given sector and resolution to the current WorldWind file cache,
     * without downloading imagery already in the cache.
     * <p>
     * This method creates and starts a thread to perform the download. A
     * reference to the thread is returned. To create a downloader that has not
     * been started, construct a {@link BasicElevationModelBulkDownloader}.
     * <p>
     * Note that the target resolution must be provided in radians of latitude
     * per texel, which is the resolution in meters divided by the globe radius.
     *
     * @param sector the sector to download data for.
     * @param resolution the target resolution, provided in radians of latitude
     * per texel.
     * @param listener an optional retrieval listener. May be null.
     *
     * @return the {@link BulkRetrievalThread} executing the retrieval or
     * <code>null</code> if the specified sector does not intersect the
     * elevation model bounding sector.
     *
     * @throws IllegalArgumentException if the sector is null or the resolution
     * is less than zero.
    ...
    /**
     * Start a new {@link BulkRetrievalThread} that downloads all elevations for
     * a given sector and resolution to a specified file store, without
     * downloading imagery already in the file store.
     * <p>
     * This method creates and starts a thread to perform the download. A
     * reference to the thread is returned. To create a downloader that has not
     * been started, construct a {@link BasicElevationModelBulkDownloader}.
     ...
     * @param resolution the target resolution, provided in radians of latitude
     * per texel.
     * @param fileStore the file store in which to place the downloaded
     * elevations. If null the current WorldWind file cache is used.
     * @param listener an optional retrieval listener. May be null.
     *
     * @return the {@link BulkRetrievalThread} executing the retrieval or
     * <code>null</code> if the specified sector does not intersect the
    ...
    /**
     * Get the estimated size in bytes of the elevations not in the WorldWind
     * file cache for the given sector and resolution.
     * <p>
     * Note that the target resolution must be provided in radians of latitude
     * per texel, which is the resolution in meters divided by the globe radius.
     *
    ...
    /**
     * Get the estimated size in bytes of the elevations not in a specified file
     * store for the given sector and resolution.
     * <p>
     * Note that the target resolution must be provided in radians of latitude
     * per texel, which is the resolution in meters divided by the globe radius.
     *
     ...
     * @param resolution the target resolution, provided in radians of latitude
     * per texel.
     * @param fileStore the file store to examine. If null the current WorldWind
     * file cache is used.
     *
     * @return the estimated size in bytes of the missing elevations.
     *
...
//        }
    ...
    /**
     * Internal class to hold collections of elevation tiles that provide
     * elevations for a specific sector.
     */
...
//    }
     ...
     *
     * @return
     * {@link gov.nasa.worldwind.avlist.AVKey#RETRIEVAL_STATE_SUCCESSFUL} if the
     * retrieval succeeded, {@link
     * gov.nasa.worldwind.avlist.AVKey#RETRIEVAL_STATE_ERROR} if the retrieval
     * failed with errors, and <code>null</code> if the retrieval state is
     * unknown.
     */
     ...
     * method calls in {@link
     * SwingUtilities#invokeLater(Runnable)}.
     *
     * @param caps the WMS Capabilities document retrieved from this
     * ElevationModel's WMS server.
     * @param params the parameter list describing the WMS layer names
     * associated with this ElevationModel.
     *
     * @throws IllegalArgumentException if either the Capabilities or the
    ...
    /**
     * Starts retrieving non-tile resources associated with this model in a
     ...
     * AVKey#ELEVATION_MIN}</td><td>ExtremeElevations/@min</td><td>Double</td></tr>
     * </table> This also writes common elevation model and LevelSet
     * configuration parameters by invoking {@link gov.nasa.worldwind.terrain.AbstractElevationModel#createElevationModelConfigElements(gov.nasa.worldwind.avlist.AVList,
     * org.w3c.dom.Element)} and {@link DataConfigurationUtils#createLevelSetConfigElements(gov.nasa.worldwind.avlist.AVList,
     * org.w3c.dom.Element)}.
     *
     * @param params the key-value pairs which define the BasicElevationModel
     * configuration parameters.
     * @param context the XML document root on which to append
     * BasicElevationModel configuration elements.
     *
     ...
     * key and parameter names are: <table>
     * <caption style="font-weight: bold;">Parameters</caption>
     * <tr><th>Parameter</th><th>Element Path</th><th>Type</th></tr>
     * <tr><td>{@link
     ...
     * AVKey#ELEVATION_MAX}</td><td>ExtremeElevations/@max</td><td>Double</td></tr> <tr><td>{@link
     * AVKey#ELEVATION_MIN}</td><td>ExtremeElevations/@min</td><td>Double</td></tr>
     * </table> This also parses common elevation model and LevelSet
     * configuration parameters by invoking {@link gov.nasa.worldwind.terrain.AbstractElevationModel#getElevationModelConfigParams(org.w3c.dom.Element,
     ...
     *
     * @param domElement the XML document root to parse for BasicElevationModel
     * configuration parameters.
     * @param params the output key-value pairs which recieve the
     * BasicElevationModel configuration parameters. A null reference is
     * permitted.
     *
