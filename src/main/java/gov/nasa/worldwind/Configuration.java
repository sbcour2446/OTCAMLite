...
/**
 * This class manages the initial WorldWind configuration. It reads WorldWind
 * configuration files and registers their contents. Configurations files
 * contain the names of classes to create at run-time, the initial model
 * definition, including the globe, elevation model and layers, and various
 * control quantities such as cache sizes and data retrieval timeouts.
 * <p>
 * The Configuration class is a singleton, but its instance is not exposed
 * publicly. It is addressed only via static methods of the class. It is
 * constructed upon first use of any of its static methods.
 * <p>
 * When the Configuration class is first instantiated it reads the XML document
 * <code>config/worldwind.xml</code> and registers all the information there.
 * The information can subsequently be retrieved via the class' various
 * <code>getValue</code> methods. Many WorldWind start-up objects query this
 * information to determine the classes to create. For example, the first
 * WorldWind object created by an application is typically a {@link
 * gov.nasa.worldwind.awt.WorldWindowGLCanvas}. During construction that class
 * causes WorldWind's internal classes to be constructed, using the names of
 * those classes drawn from the Configuration singleton, this class.
 * <p>
 * The default WorldWind configuration document is
 * <code>config/worldwind.xml</code>. This can be changed by setting the Java
 * property <code>gov.nasa.worldwind.config.file</code> to a different file name
 * or a valid URL prior to creating any WorldWind object or invoking any static
 * methods of WorldWind classes, including the Configuration class. When an
 * application specifies a different configuration location it typically does so
 * in its main method prior to using WorldWind. If a file is specified its
 * location must be on the classpath. (The contents of application and WorldWind
 * jar files are typically on the classpath, in which case the configuration
 * file may be in the jar file.)
 * <p>
 * Additionally, an application may set another Java property,
 * <code>gov.nasa.worldwind.app.config.document</code>, to a file name or URL
 * whose contents contain configuration values to override those of the primary
 * configuration document. WorldWind overrides only those values in this
 * application document, it leaves all others to the value specified in the
 * primary document. Applications usually specify an override document in order
 * to specify the initial layers in the model.
 * <p>
 * See <code>config/worldwind.xml</code> for documentation on setting
 * configuration values.
 * <p>
 * Configuration values can also be set programatically via
 * {@link Configuration#setValue(String, Object)}, but they are not retroactive
 * so affect only Configuration queries made subsequent to setting the value.
 * <p>
 * <em>Note:</em> Prior to September of 2009, configuration properties were read
 * from the file <code>config/worldwind.properties</code>. An alternate file
 * could be specified via the <code>gov.nasa.worldwind.config.file</code> Java
 * property. These mechanisms remain available but are deprecated. WorldWind no
 * longer contains a <code>worldwind.properties</code> file. If
 * <code>worldwind.properties</code> or its replacement as specified through the
 * Java property exists at run-time and can be found via the classpath,
 * configuration values specified by that mechanism are given precedence over
 * values specified by the new mechanism.
 *
 * @author Tom Gaskins
...
//                this.loadConfigProperties(doc);
     ...
     *
     * @return the value associated with the key, or the specified default value
     ...
     *
     * @return the value associated with the key, or the specified default value
     * if the key does not exist or is not an Integer or string representation
     * of an Integer.
     */
     ...
     *
     * @return the value associated with the key, or null if the key does not
     * exist or is not an Integer or string representation of an Integer.
     */
     ...
     *
     * @return the value associated with the key, or the specified default value
     * if the key does not exist or is not a Long or string representation of a
     * Long.
     */
     ...
     *
     * @return the value associated with the key, or null if the key does not
     * exist or is not a Long or string representation of a Long.
     */
     ...
     *
     * @return the value associated with the key, or the specified default value
     * if the key does not exist or is not an Double or string representation of
     * an Double.
     */
     ...
     *
     * @return the value associated with the key, or null if the key does not
     * exist or is not an Double or string representation of an Double.
     */
     ...
     * <p>
     * Valid values for true are '1' or anything that starts with 't' or 'T'.
     * ie. 'true', 'True', 't' Valid values for false are '0' or anything that
     * starts with 'f' or 'F'. ie. 'false', 'False', 'f'
     *
     * @param key the key for the desired value.
     * @param defaultValue the value to return if the key does not exist.
     *
     * @return the value associated with the key, or the specified default value
     * if the key does not exist or is not a Boolean or string representation of
     * an Boolean.
     */
     ...
     *
     * @return the value associated with the key, or null if the key does not
     * exist or is not a Boolean or string representation of an Boolean.
     */
    ...
    /**
     * Removes a key and its value from the configuration if the configuration
     * contains the key.
     *
     * @param key the key of interest.
     */
    ...
    /**
     * Adds a key and value to the configuration, or changes the value
     * associated with the key if the key is already in the configuration.
     *
    ...
    /**
     * Returns the path to the current user's application data directory. The
     * path returned depends on the operating system on which the Java Virtual
     * Machine is running. The following table provides the path for all
     * supported operating systems:
     * <table><caption style="font-weight: bold;">Mapping</caption>
     * <tr><th>Operating System</th><th>Path</th></tr> <tr><td>Mac OS
     * X</td><td>~/Library/Application Support</td></tr>
     * <tr><td>Windows</td><td>~\\Application Data</td></tr> <tr><td>Linux,
     * Unix, Solaris</td><td>~/</td></tr> </table>
     *
     * @return the absolute path to the current user's application data
     * directory.
     */
     ...
     *
     * @return true if the operating system is a Mac operating system, otherwise
     * false.
     */
     ...
     *
     * @return true if the operating system is a Windows operating system,
     * otherwise false.
     */
     ...
     *
     * @return true if the operating system is a Windows XP operating system,
     * otherwise false.
     */
    ...
    /**
     * Determines whether the operating system is Windows Vista operating
     * system.
     *
     * @return true if the operating system is a Windows Vista operating system,
     * otherwise false.
     */
     ...
     *
     * @return true if the operating system is a Linux operating system,
     * otherwise false.
     */
     ...
     *
     * @return true if the operating system is a Unix operating system,
     * otherwise false.
     */
     ...
     *
     * @return true if the operating system is a Solaris operating system,
     * otherwise false.
     */
    ...
    /**
     * Returns the highest OpenGL profile available on the current graphics
     * device that is compatible with WorldWind. The returned profile favors
     * hardware acceleration over software acceleration. With JOGL version 2.0,
     * this returns the highest available profile from the following list:
     * <ul> <li>OpenGL compatibility profile 4.x</li> <li>OpenGL compatibility
     * profile 3.x</li> <li>OpenGL profile 1.x up to 3.0</li> </ul>
     *
    ...
    /**
     * Returns a {@link com.jogamp.opengl.GLCapabilities} identifying graphics
     * features required by WorldWind. The capabilities instance returned
     * requests the maximum OpenGL profile supporting GL fixed function
     * operations, a frame buffer with 8 bits each of red, green, blue and
     * alpha, a 24-bit depth buffer, double buffering, and if the Java property
     * "gov.nasa.worldwind.stereo.mode" is set to "device", device supported
     * stereo.
     *
     * @return a new capabilities instance identifying required graphics
     * features.
     */
     ...
     *
     * @param xpathExpression an XPath expression identifying the element of
     * interest.
     *
     * @return the element of interest if the XPath expression is valid and the
     * element exists, otherwise null.
     *
