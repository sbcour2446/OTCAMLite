...
public class LatLon {
    ...
    /**
     * A near zero threshold used in some of the rhumb line calculations where
     * floating point calculations cause errors.
     */
    ...
    /**
     * Factor method for obtaining a new <code>LatLon</code> from two angles
     * expressed in radians.
     *
     * @param latitude in radians
     * @param longitude in radians
     *
     * @return a new <code>LatLon</code> from the given angles, which are
    ...
    /**
     * Factory method for obtaining a new <code>LatLon</code> from two angles
    ...
    /**
     * Constructs a new  <code>LatLon</code> from two angles. Neither angle may
     * be null.
     *
     * @param latitude latitude
     * @param longitude longitude
     *
     * @throws IllegalArgumentException if <code>latitude</code> or
     * <code>longitude</code> is null
     */
     ...
     * type is {@link AVKey#GREAT_CIRCLE} this returns an interpolated value on
     * the great arc that spans the two locations (see
     * {@link #interpolateGreatCircle(double, LatLon, LatLon)}). If the path
     * type is {@link AVKey#RHUMB_LINE} or {@link AVKey#LOXODROME} this returns
     * an interpolated value on the rhumb line that spans the two locations (see
     * {@link #interpolateRhumb(double, LatLon, LatLon)}. Otherwise, this
     * returns the linear interpolation of the two locations (see
     * {@link #interpolate(double, LatLon, LatLon)}.
     *
     * @param pathType the path type used to interpolate between geographic
     * locations.
     ...
     *
     * @throws IllegalArgumentException if the path type or either location is
     * null.
     */
     ...
     * corresponding to the specified interpolation factor. This method uses a
     ...
     * the specified interpolation factor. This method uses a spherical model,
     * not elliptical.
     *
     * @param amount the interpolation factor
     * @param value1 the first location.
    ...
    /**
     * Returns the length of the path between <code>value1</code> and
     * <code>value2</code>, according to the specified path type. If the path
     * type is {@link AVKey#GREAT_CIRCLE} this returns the length of the great
     * arc that spans the two locations (see
     * {@link #greatCircleDistance(LatLon, LatLon)}). If the path type is
     * {@link AVKey#RHUMB_LINE} or {@link AVKey#LOXODROME} this returns the
     * length of the rhumb line that spans the two locations (see {@link
     * #rhumbDistance(LatLon, LatLon)}). Otherwise, this returns the linear
     * distance between the two locations (see
     ...
     * @param value2 the second location.
     *
     * @return an length of the path between <code>value1</code> and
     * <code>value2</code>, according to the specified path type.
     *
     * @throws IllegalArgumentException if the path type or either location is
     * null.
     ...
     * globe. This method uses a spherical model, not elliptical.
     *
     ...
     * location, and passes through the second location. This method uses a
     * spherical model, not elliptical.
     *
     * @param p1 LatLon of the first location
     * @param p2 LatLon of the second location
     ...
     * location, azimuth, and arc distance. This method uses a spherical model,
     * not elliptical.
     *
     * @param p LatLon of the starting location
     * @param greatCircleAzimuth great circle azimuth angle (clockwise from
     * North)
     ...
     *
     * @param p LatLon of the starting location
     * @param greatCircleAzimuthRadians great circle azimuth angle (clockwise
     ...
     * with the given starting location and azimuth. This method uses a
     * spherical model, not elliptical.
     *
    ...
    /**
     * Returns two locations with the most extreme latitudes on the great circle
     * arc defined by, and limited to, the two locations. This method uses a
     * spherical model, not elliptical.
     *
     ...
     * arc.
     *
     * @throws IllegalArgumentException if either <code>begin</code> or
     ...
     * iterable. This method uses a spherical model, not elliptical.
     *
     * @param locations the pairs of locations defining a sequence of great
     * circle arcs.
     *
     * @return two locations with the most extreme latitudes on the great circle
     * arcs.
     ...
     * of the globe. This method uses a spherical model, not elliptical.
     *
     * @param p1 LatLon of the first location
     * @param p2 LatLon of the second location
     *
     ...
     * of constant heading) between two locations. This method uses a spherical
     * model, not elliptical.
     *
     * @param p1 LatLon of the first location
     * @param p2 LatLon of the second location
     *
     ...
     * rhumb azimuth, and arc distance along the line. This method uses a
     * spherical model, not elliptical.
     *
     * @param p LatLon of the starting location
     ...
     *
     * @param p LatLon of the starting location
     * @param rhumbAzimuthRadians rhumb azimuth angle (clockwise from North), in
     * radians
     * @param pathLengthRadians arc distance to travel, in radians
     *
     * @return LatLon location on the rhumb line.
    ...
    /**
     * Computes the average distance between a specified center point and a list
     * of locations.
     *
     ...
     *
     * @throws java.lang.IllegalArgumentException if any of the specified globe,
     * center or locations are null.
     */
     ...
     *
     * @throws java.lang.IllegalArgumentException if the specified locations is
     * null.
     */
     ...
     * @return the average of the locations.
     *
     * @throws java.lang.IllegalArgumentException if either the specified globe
    ...
    /**
     * Determines if a sequence of geographic locations encloses either the
     * North or South pole. The sequence is treated as a closed loop. (If the
     * first and last positions are not equal the loop will be closed for
     * purposes of this computation.)
     *
     * @param locations The locations to test.
     *
     * @return AVKey.NORTH if the North Pole is enclosed, AVKey.SOUTH if the
     * South Pole is enclosed, or null if neither pole is enclosed.
     *
    ...
    /**
     * Returns a list containing two copies of a sequence of geographic
     * locations that cross the dateline: one that extends across the -180
     * longitude boundary and one that extends across the +180 longitude
     * boundary. If the sequence does not cross the dateline this returns a list
     * containing a copy of the original list.
     *
     * @param locations The locations to repeat.
     *
     * @return A list containing two new location lists, one copy for either
    ...
    /**
     * Divides a sequence of geographic locations that encloses a pole along the
     * international dateline. This method determines where the locations cross
     * the dateline, and inserts locations to the pole, and then back to the
     * intersection position. This allows the shape to be "unrolled" when
     * projected in a lat-lon projection.
     *
     * @param locations Locations to cut at dateline. This list is not modified.
     * @param pole Pole contained by locations, either AVKey.NORTH or
     * AVKey.SOUTH.
     * @param globe Current globe, or null to treat geographic coordinates as
     * linear for the purpose of computing the dateline intersection.
     *
     * @return New location list with locations added to correctly handle
     * dateline intersection.
     *
     * @throws java.lang.IllegalArgumentException if the locations are null or
     * if the pole is null.
     */
     ...
     * The intersection test is performed by intersecting a line in Cartesian
     * space between the two positions with a plane through the meridian. Thus,
     * it is most suitable for working with positions that are fairly close
     * together as the calculation does not take into account great circle or
     * rhumb paths.
     *
     * @param p1 The first location.
     * @param p2 The second location.
     * @param meridian The line of constant longitude to intersect with.
     * @param globe Globe used to compute intersection, or null to treat
     * geographic coordinates as linear for the purpose of computing the
     * intersection.
     *
     * @return The intersection location along the meridian.
     *
     * @throws java.lang.IllegalArgumentException if either location is null, or
     * if the meridian is null.
     */
    ...
    /**
     * Determine where a line between two locations crosses a given meridian.
     * The intersection test is performed by treating geographic coordinates as
     * linear and computing the intersection of the linear segment with the
     * vertical line indicated by the meridian. This computation correctly
    ...
    /**
     * Parses a string containing latitude and longitude coordinates in either
     * Degrees-minutes-seconds or decimal degrees. The latitude must precede the
     * longitude and the angles must be separated by a comma.
     *
     * @param latLonString a string containing the comma separated latitude and
     * longitude in either DMS or decimal degrees.
     *
     ...
     * @throws NumberFormatException if the string does not form a latitude,
     * longitude pair.
     */
     ...
     * <p>
     * The algorithm used is iterative and will iterate only 10 times if it does
     * not converge.
     * </p>
     *
     * @param p1 first position
    ...
    /**
     * Computes a new set of locations translated from a specified location to a
     * new location.
     *
     ...
     *
     * @return the translated locations, or null if the locations could not be
     * translated.
     *
