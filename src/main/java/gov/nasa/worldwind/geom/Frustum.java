...
/**
 * Represents a view frustum composed of six planes: left, right, bottom, top,
 * near far.
 * <p>
...
public class Frustum {
    ...
    /**
     * Holds all six frustum planes in an array in the order left, right,
     * bottom, top, near, far.
     */
    protected final Plane[] allPlanes;

    /**
     * Constructs a frustum two meters wide centered at the origin. Primarily
     * used for testing.
     */
    public Frustum() {
        this(
    ...
    /**
     * Create a frustum from six {@link gov.nasa.worldwind.geom.Plane}s defining
     * the frustum boundaries.
     * <p>
     ...
     *
     * @return an array of the frustum planes, in the order left, right, bottom,
     * top, near, far.
     */
    ...
    // ============== Factory Functions ======================= //
    /**
     * Creates a frustum by extracting the six frustum planes from a projection
     * matrix.
     *
     * @param projectionMatrix the projection matrix to extract the frustum
     * planes from.
     *
    ...
    /**
     * Creates a <code>Frustum</code> from a horizontal field-of-view, viewport
     * aspect ratio and distance to near and far depth clipping planes. The near
     * plane must be closer than the far plane, and both near and far values
     * must be positive.
     *
     * @param horizontalFieldOfView horizontal field-of-view angle in the range
     * (0, 180)
     ...
     *
     * @throws IllegalArgumentException if fov is not in the range (0, 180), if
     * either near or far are negative, or near is greater than or equal to far
     */
    ...
    /**
     * Creates a <code>Frustum</code> from three sets of parallel clipping
     * planes (a parallel projectionMatrix). In this case, the near and far
     * depth clipping planes may be negative.
     *
     * @param near distance to the near depth clipping plane
     * @param far distance to far depth clipping plane
     ...
     *
     * @throws IllegalArgumentException if the difference of any plane set
     * (lright - left, top - bottom, far - near) is less than or equal to zero.
     */
    ...
    /**
     * Creates a <code>Frustum</code> from four edge vectors, viewport aspect
     * ratio and distance to near and far planes. The edge vectors connect the
     * near corners of the frustum to the far corners. The near plane must be
     * closer than the far plane, and both planes must be positive.
     *
     ...
     *
     * @throws IllegalArgumentException if any of the vectors are null, if
     * either near or far are negative, or near is greater than or equal to far
     */
     ...
     *
     * @return true if the segment intersects or is contained in the frustum,
     * otherwise false.
     *
    ...
    /**
     * Indicates whether a specified {@link Extent} is fully within this
     * frustum.
     *
     * @param extent the Extent to test.
     *
