...
/**
 * An arbitrarily oriented box, typically used as a oriented bounding volume for
 * a collection of points or shapes. A <code>Box</code> is defined by three
 * orthogonal axes and two positions along each of those axes. Each of the
 * positions specifies the location of a box side along the respective axis. The
 * three axes are named by convention "R", "S" and "T", and are ordered by
 * decreasing length -- R is the longest axis, followed by S and then T.
 * <p>
 * The static class field, <code>ProjectionHullTable</code>, defines a table of
 * all possible vertex combinations representing a <code>Box's</code> 2D convex
 * hull in screen coordinates. The index to this table is a 6-bit code, where
 * each bit denotes whether one of the <code>Box's</code> six planes faces the
 * <code>View</code>. This code is organized as follows:
 * <table border='1'> <caption style="font-weight: bold;">Bit Mapping</caption>
 * <tr><td><strong>Bit</strong></td><td >5</td><td>4</td><td >3</td><td >2</td><td >1</td><td
 * >0</td></tr>
 * <tr><td><strong>Code</strong></td><td >left</td><td>right</td><td >back</td><td
 * >front</td><td>bottom</td><td>top</td></tr>
 * </table>
 * <p>
 * Since at most three of a <code>Box's</code> planes can be visible at one
 * time, there are a total of 26 unique vertex combinations that define a
 * <code>Box's</code> 2D convex hull in the viewport. Index codes that represent
 * a valid combination of planes facing the <code>View</code> result in an array
 * of 4 or 6 integers (depending on whether one, two or three planes face the
 * <code>View</code>), where each element in the array is an index for one of
 * the <code>Box's</code> eight vertices as follows:
 * <table border='1'> <caption style="font-weight: bold;">Index
 * Mapping</caption>
 ...
 * </table>
 * <p>
 * The vertices are organized so that they appear in counter-clockwise order on
 * the screen. Index codes that represent an invalid combination of planes
 * facing the <code>View</code> map to <code>null</code>.
 *
 * @author tag
...
public class Box implements Extent, Renderable {
    ...
    /**
     * Construct a box from three specified unit axes and the locations of the
     * box faces relative to those axes. The box faces are specified by two
     * scalar locations along each axis, each location indicating a face. The
     * non-unit length of an axis is the distance between its respective two
     * locations. The longest side is specified first, followed by the second
     * longest side and then the shortest side.
     * <p>
     * The axes are normally principal axes computed from a collection of points
     * in order to form an oriented bounding volume. See
     * {@link WWMath#computePrincipalAxes(Iterable)}.
     * <p>
     * Note: No check is made to ensure the order of the face locations.
     *
     * @param axes the unit-length axes.
     * @param rMin the location along the first axis corresponding to the
     * left-most box side relative to the axis.
     * @param rMax the location along the first axis corresponding to the
     * right-most box side relative to the axis.
     * @param sMin the location along the second axis corresponding to the
     * left-most box side relative to the axis.
     * @param sMax the location along the second axis corresponding to the
     * right-most box side relative to the axis.
     * @param tMin the location along the third axis corresponding to the
     * left-most box side relative to the axis.
     * @param tMax the location along the third axis corresponding to the
     * right-most box side relative to the axis.
     *
     * @throws IllegalArgumentException if the axes array or one of its entries
     * is null.
     */
    ...
    /**
     * Returns the point corresponding to the center of the box side left-most
     * along the R (first) axis.
     *
    ...
    /**
     * Returns the point corresponding to the center of the box side right-most
     * along the R (first) axis.
     *
    ...
    /**
     * Returns the R (first) axis. The axis length is the distance between the
     * box sides perpendicular to the axis.
     *
     * @return the R axis.
     */
    ...
    /**
     * Returns the S (second) axis. The axis length is the distance between the
     * box sides perpendicular to the axis.
     *
     * @return the S axis.
     */
    ...
    /**
     * Returns the T (third) axis. The axis length is the distance between the
     * box sides perpendicular to the axis.
     *
     * @return the T axis.
     */
     ...
     *
     * @return the eight box corners in the order bottom-lower-left,
     * bottom-lower-right, bottom-upper-right, bottom-upper-left,
     * top-lower-left, top-lower-right, top-upper-right, top-upper-left.
     */
    ...
    /**
     * Returns the six planes of the box. The plane normals are directed
     * outwards from the box.
     *
     * @return the six box planes in the order R-min, R-max, S-min, S-max,
     * T-min, T-max.
     */
    ...
    /**
     * Returns the effective diameter of the box as if it were a sphere. The
     * length returned is the square root of the sum of the squares of axis
     * lengths.
     *
    ...
    /**
     * Returns the effective radius of the box as if it were a sphere. The
     * length returned is half the square root of the sum of the squares of axis
     * lengths.
     *
    ...
    /**
     * Compute a <code>Box</code> that bounds a specified list of points.
     * Principal axes are computed for the points and used to form a
     * <code>Box</code>.
     *
     * @param points the points for which to compute a bounding volume.
     *
     * @return the bounding volume, with axes lengths consistent with the
     * conventions described in the <code>Box</code> class overview.
     *
    ...
    /**
     * Computes a <code>Box</code> that bounds a specified buffer of points.
     * Principal axes are computed for the points and used to form a
     * <code>Box</code>.
     * <p>
     ...
     * conventions described in the <code>Box</code> class overview.
     *
     * @throws IllegalArgumentException if the buffer is null or empty, or if
     * the stride is less than three.
     */
    ...
    /**
     * Computes a <code>Box</code> that represents the union of one or more
     * <code>Boxes</code>. If the iterable has two or more non-<code>null</code>
     * <code>Boxes</code>, the returned encloses the <code>Boxes</code>. In this
     * case axes and center point of the returned <code>Box</code> may differ
     * from the <code>Boxes</code>. If the iterable has only one
     * non-<code>null</code> <code>Box</code>, this returns that sole
     * <code>Box</code>. This returns <code>null</code> if the iterable is empty
     * or contains only <code>null</code> references.
     *
     * @param iterable an iterable of <code>Boxes</code> to enclose.
     *
     * @return a new <code>Box</code> that encloses the specified iterable of
     * <code>Boxes</code>.
     *
    ...
    /**
     * Returns the effective radius of this box relative to a specified plane,
     * using only this box's S and T axes. This is an optimization available
     * when using the effective radius to test the distance from a plane to the
     * line segment along this box's R axis, as is done in this class'
     * {@link #intersects(Frustum)} method. See Lengyel, 2 Ed, Section 7.2.4.
     *
     * @param plane the plane in question.
     *
     * @return the effective radius of this box relative to the specified plane,
     * using only this box's S and T axes to determine the effective radius.
     */
    ...
    /**
     * Computes an index into the <code>ProjectionHullTable</code> for this
     * <code>Box</code> given the specified <code>view</code>. The returned
     * integer is a 6-bit code, where each bit denotes whether one of this
     * <code>Box's</code> six planes faces the <code>View</code>. See the
     * documentation for <code>{@link
     * #ProjectionHullTable}</code> for details.
     * <p>
     * If the <code>view</code> is inside this <code>Box</code>, this returns 0
     * indicating that none of this <code>Box's</code> planes face the
     * <code>view</code>.
     *
     * @param view the <code>View</code> to compute a lookup index for.
     *
     * @return an integer who's first 6 bits define an index into the
     * <code>ProjectionHullTable</code>.
     */
