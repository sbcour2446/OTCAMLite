...
public class BasicDataFileStore extends AbstractFileStore {
    ...
    /**
     * The number of milliseconds to wait before a retrieval request for the
     * same file can be reissued.
     */
    ...
    /**
     * Absent-resource list to keep track of resources that were requested by
     * requestFile but failed. The default list holds a maximum of 2000 entries,
     * allows 3 attempts separated by 500 milliseconds before marking a resource
     * semi-permanently absent, and allows additional attempts after 60 seconds.
     * The {@link #getAbsentResourceList()} method may be overridden by
     * subclasses if they wish to provide an alternatively configured
     * absent-resource list.
     */
     ...
     * <code>requestFile</code>. If a URL is requested that does not have a
     * format suffix, <code>requestFile</code> appends a suffix appropriate for
     * the content type returned by the server. Subsequent calls to
     * <code>requestFile</code> use the content types in this list to find the
     * content type matching the cached file.
     * <p>
     * This is initialized to the following list of default content types
     * typically used in WorldWind applications:
     * <ul> <li>application/vnd.google-earth.kml+xml</li>
     * <li>application/vnd.google-earth.kmz</li>
     * <li>model/collada+xml</li> <li>image/dds</li> <li>image/gif</li>
     * <li>image/jpeg</li> <li>image/jpg</li>
     * <li>image/png</li> </ul>
     * <p>
     * This list may be overridden by specifying a comma-delimited list of
     * content types in the WorldWind configuration parameter
     * <code>gov.nasa.worldwind.avkey.CacheContentTypes</code>.
     */
     ...
     * Create an instance.
     *
     * @throws IllegalStateException if the configuration file name cannot be
     * determined from {@link Configuration} or the configuration file cannot be
     * found.
     */
    ...
    /**
     * Returns a file from the cache, the local file system or the classpath if
     * the file exists. The specified address may be a jar URL. See
     * {@link java.net.JarURLConnection} for a description of jar URLs. If
     * <code>searchLocalCache</code> is <code>true</code> this looks for the
     * file in the WorldWind cache, otherwise this only looks for the file in
     * the local file system and the classpath.
     *
     * @param address the name used to identify the cached file.
     * @param retrievalUrl the URL to obtain the file if it is not in the cache.
     * Used only to determine a location to search in the local cache. May be
     * null.
     * @param searchLocalCache <code>true</code> to look for the file in the
     * WorldWind cache, otherwise <code>false</code>.
     *
    ...
    /**
     * Returns the length of the resource referred to by a jar URL. Can be used
     * to test whether the resource exists.
     * <p>
     * Note: This method causes the URL to open a connection and retrieve
     * content length.
     *
     * @param jarUrl the jar URL.
     *
     * @return the jar file's content length, or -1 if a connection to the URL
     * can't be formed or queried.
     */
    ...
    /**
     * Retrieves a specified file and either adds it to the cache or saves it in
     * a temporary file, depending on the value of
     * <code>saveInLocalCache</code>.
     *
     * @param address the name used to identify the cached file.
     * @param url the URL to obtain the file.
     * @param saveInLocalCache <code>true</code> to add the file to the cache,
     * or <code>false</code> to save it in a temporary location.
     */
    ...
    /**
     * Updates a cache entry with information available once the file is
     * retrieved.
     *
     * @param address the name used to identify the file in the cache.
     * @param localFileUrl the path to the local copy of the file.
     * @param expiration time (in milliseconds since the Epoch) at which this
     * entry expires, or zero to indicate that there is no expiration time.
     */
    ...
    /**
     * Makes a path to the file in the cache from the file's URL and content
     * type.
     *
     * @param url the URL to obtain the file.
    ...
    /**
     * Makes a path to the file in the cache from the file's generic URL and
     * content type. If the URL has a non-empty query string, then this returns
     * a path name formatted as follows:
     * <p>
     ...
     * <p>
     * Where <code>host</code> is the name of the host machine,
     * <code>hashCode</code> is a four digit hash code computed from the string
     * "path" or "path_query" (if the URL has a query string), <code>path</code>
     * is the URL's path part, <code>query</code> is the URL's query string, and
     * <code>suffix</code> is either the path's suffix or a suffix created from
     * the specified content type. The <code>hashCode</code> folder is used to
     * limit the number of files that appear beneath the host folder. This is
     * necessary to avoiding the operating system's maximum file limit should a
     * large number of files be requested from the same host. If two URLs have
     * the same hash code, then both URLs are stored under the same
     * <code>hashCode</code> folder in the cache and are differentiated by their
     * <code>path</code> and <code>query</code> parts.
     * <p>
     * This removes any private parameters from the query string to ensure that
     * those parameters are not written to the file store as part of the cache
     * name. For example, the "CONNECTID" query parameter typically encodes a
     * user's unique connection id, and must not be shared. Writing this
     * parameter to the cache would expose that parameter to anyone using the
     * same machine. If the query string is empty after removing any private
     * parameters, it is ignored and only the path part of the URL is used as
     * the filename.
     *
     * @param url the URL to obtain the file.
     * @param contentType the mime type of the file's contents.
    ...
    /**
     * Makes a path to the file in the cache from the file's JAR URL and content
     * type. This returns a path name formatted as follows:
     * <p>
     * <code>host/path.suffix</code>
     * <p>
     * Where <code>host</code> is the path to the JAR file, <code>path</code> is
     * the file's path within the JAR archive, and <code>suffix</code> is either
     * the path's suffix or a suffix created from the specified content type.
     *
     * @param jarURL the URL to obtain the file. This URL is assumed to have the
     * "jar" protocol.
     * @param contentType the mime type of the file's contents.
     *
     * @return a path name.
     ...
     * Since the file store intentionally does not persist a mapping of
     * retrieved URLs to temp files, this deletes the returned temp file when
     * the current Java Virtual Machine terminates.
     *
     * @param url the URL to be associated with the temp file. Used only to
     * determine an appropriate suffix.
     * @param contentType the mime type of the file contents. Used to determine
     * the file's suffix.
     *
    ...
    /**
     * Determines an appropriate suffix for a cached file. If the specified path
     * already has a suffix that matches the specified content type, then this
     * method returns null. Otherwise the method determines and returns a suffix
     ...
     *
     * @return a suffix appropriate to the content type, or null if the
     * specified path already has an appropriate suffix.
     */
    ...
    /**
     * Removes any private parameters from the query string to ensure that those
     * parameters are not written to the file store as part of the cache name.
     * For example, the "CONNECTID" query parameter typically encodes a user's
     * unique connection id, and must not be shared. Writing this parameter to
     * the cache would expose that parameter to anyone using the same machine.
     * <p>
     * This removes the key, the value, and any trailing parameter delimiter of
     * all private parameters in the specified query string. Recognized private
     * query parameters are as follows:
     * <ul> <li>CONNECTID</li> </ul>
     *
     * @param queryString the query string to examine.
     *
     * @return a new string with the private query parameters removed. This
     * string is empty if the query string is empty, or if the query string
     * contains only private parameters.
     */
