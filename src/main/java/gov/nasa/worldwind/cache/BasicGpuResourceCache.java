...
/**
 * Provides the interface for caching of OpenGL resources that are stored on or
 * registered with a GL context. This cache maintains a map of resources that
 * fit within a specifiable memory capacity. If adding a resource would exceed
 * this cache's capacity, existing but least recently used resources are removed
 * from the cache to make room. The cache is reduced to the "low water" size in
 * this case (see {@link #setLowWater(long)}.
 * <p>
 * When a resource is removed from the cache, and if it is a recognized OpenGL
 * resource -- a texture, a list of vertex buffer IDs, a list of display list
 * IDs, etc. -- and there is a current Open GL context, the appropriate glDelete
 * function is called to de-register the resource with the GPU. If there is no
 * current OpenGL context the resource is not deleted and will likely remain
 * allocated on the GPU until the GL context is destroyed.
 *
 * @author tag
...
public class BasicGpuResourceCache implements GpuResourceCache {
    ...
    /**
     * Sets the new capacity (in bytes) for the cache. When decreasing cache
     * size, it is recommended to check that the lowWater variable is suitable.
     * If the capacity infringes on items stored in the cache, these items are
     * removed. Setting a new low water is up to the user, that is, it remains
     * unchanged and may be higher than the maximum capacity. When the low water
     * level is higher than or equal to the maximum capacity, it is ignored,
     * which can lead to poor performance when adding entries.
     *
    ...
    /**
     * Sets the new low water level in bytes, which controls how aggresively the
     * cache discards items.
     * <p>
    ...
    /**
     * Returns the low water level in bytes. When the cache fills, it removes
     * items until it reaches the low water level.
     *
