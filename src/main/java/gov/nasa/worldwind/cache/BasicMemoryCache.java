...
public class BasicMemoryCache implements MemoryCache {
    ...
    /**
     * Constructs a new cache using <code>capacity</code> for maximum size, and
     * <code>loWater</code> for the low water.
     *
    ...
    /**
     * Adds a cache listener, MemoryCache listeners are used to notify classes
    ...
    /**
     * Removes a cache listener, objects using this listener will no longer
     * receive notification of cache events.
     *
    ...
    /**
     * Sets the new capacity for the cache. When decreasing cache size, it is
     * recommended to check that the lowWater variable is suitable. If the
     * capacity infringes on items stored in the cache, these items are removed.
     * Setting a new low water is up to the user, that is, it remains unchanged
     * and may be higher than the maximum capacity. When the low water level is
     * higher than or equal to the maximum capacity, it is ignored, which can
     * lead to poor performance when adding entries.
     *
...
//        this.makeSpace(this.capacity - newCapacity);
    ...
    /**
     * Sets the new low water level in cache units, which controls how
     * aggresively the cache discards items.
     * <p>
     * When the cache fills, it removes items until it reaches the low water
     * level.
     * <p>
     * Setting a high loWater level will increase cache misses, but decrease
     * average add time, but setting a low loWater will do the opposite.
     *
    ...
    /**
     * Returns the low water level in cache units. When the cache fills, it
     * removes items until it reaches the low water level.
     *
    ...
    /**
     * Returns true if the cache contains the item referenced by key. No
     * guarantee is made as to whether or not the item will remain in the cache
     * for any period of time.
     * <p>
     * This function does not cause the object referenced by the key to be
     * marked as accessed. <code>getObject()</code> should be used for that
     * purpose.
     *
    ...
    /**
     * Adds an object to the cache. The add fails if the object or key is null,
     * or if the size is zero, negative or greater than the maximmum capacity.
     *
    ...
    /**
     * Remove the object reference by key from the cache. If no object with the
     * corresponding key is found, this method returns immediately.
     *
    ...
    /**
     * Obtain the object referenced by key without removing it. Apart from
     * adding an object, this is the only way to mark an object as recently
     * used.
     *
    ...
    /**
     * Removes <code>entry</code> from the cache. To remove an entry using its
     * key, use <code>remove()</code>.
     *
    ...
    /**
     * Makes at least <code>spaceRequired</code> space in the cache. If
     * spaceRequired is less than (capacity-lowWater), makes more space. Does
     * nothing if capacity is less than spaceRequired.
     *
    ...
    /**
     * a <code>String</code> representation of this object is returned.&nbsp;
     * This representation consists of maximum size, current used capacity and
