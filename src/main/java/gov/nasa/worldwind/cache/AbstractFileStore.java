...
public abstract class AbstractFileStore extends WWObjectImpl implements FileStore {
     ...
     * @param checkClassPath if <code>true</code>, the class path is first
     * searched for the file, otherwise the class path is not searched unless
     * it's one of the explicit paths in the cache search directories
     *
     ...
     * added to the file store, otherwise null
     *
     * @throws IllegalArgumentException if <code>fileName</code> is null
     */
     ...
     * Only files in the writable WorldWind disk cache or temp file directory
