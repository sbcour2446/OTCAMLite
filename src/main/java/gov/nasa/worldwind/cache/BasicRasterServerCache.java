 ...
 */
/**
 * The <code>BasicRasterServerCache</code> is an implementation of the memory
 * cache that is specific to store maximum possible cacheable items, until the
 * heap size allows. Once the memory limit is hit, it will drop ALL cached
 * items. Also, BasicRasterServerCache creates a pruner thread that removes all
 * cached items which were not used for 20 seconds or more. The least recent use
 * timeout is configurable via the <code>setLeastRecentUseTimeout()</code>
 * method. In addition, the <code>BasicRasterServerCache</code> allocates 100MB
 * memory and keeps only a phantom reference to the allocated 100M memory. Once
 * any part of the application needs more memory the phantom referenced memory
 * will be immediately released and the phantom reference will be added to the
 * internal reference queue, which is monitored by an internal
 * <code>MemoryMonitorThread</code>. Once the phantom reference is added to the
 * reference queue, the entire cached content will be released. This approach
 * allows to use almost entire available heap memory to cache rasters and
...
public class BasicRasterServerCache extends BasicMemoryCache {
    ...
    /**
     * Constructs a new cache which uses entire memory, but will immediately
     * drop all cached entries ones there is a need for more memory by anyone
     * else.
     */
