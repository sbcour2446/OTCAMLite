...
/**
 * This class signals that an object or terrain is under the cursor and
 * identifies that object and the operation that caused the signal. See the
 * <em>Field Summary</em> for a description of the possible operations. When a
 * SelectEvent occurs, all select event listeners registered with the associated
 * {@link gov.nasa.worldwind.WorldWindow} are called. Select event listeners are
 * registered by calling
 * {@link gov.nasa.worldwind.WorldWindow#addSelectListener(SelectListener)}.
 * <p>
 * A <code>ROLLOVER</code> SelectEvent is generated every frame when the cursor
 * is over a visible object either because the user moved it there or because
 * the WorldWindow was repainted and a visible object was found to be under the
 * cursor. A <code>ROLLOVER</code> SelectEvent is also generated when there are
 * no longer any objects under the cursor. Select events generated for objects
 * under the cursor have a non-null pickPoint, and contain the top-most visible
 * object of all objects at the cursor position.
 * <p>
 * A <code>BOX_ROLLOVER</code> SelectEvent is generated every frame when the
 * selection box intersects a visible object either because the user moved or
 * expanded it or because the WorldWindow was repainted and a visible object was
 * found to intersect the box. A <code>BOX_ROLLOVER</code> SelectEvent is also
 * generated when there are no longer any objects intersecting the selection
 * box. Select events generated for objects intersecting the selection box have
 * a non-null pickRectangle, and contain all top-most visible objects of all
 * objects intersecting the selection box.
 * <p>
 * If a select listener performs some action in response to a select event, it
 * should call the event's {@link
 * #consume()} method in order to indicate to subsequently called listeners that
 * the event has been responded to and no further action should be taken. Left
 * press select events should not be consumed unless it is necessary to do so.
 * Consuming left press events prevents the WorldWindow from gaining focus,
 * thereby preventing it from receiving key events.
 * <p>
 * If no object is under the cursor but the cursor is over terrain, the select
 * event will identify the terrain as the picked object and will include the
 * corresponding geographic position. See {@link
...
public class SelectEvent extends WWEvent {
    ...
    /**
     * The user clicked the left mouse button while the cursor was over picked
     * object.
     */
    ...
    /**
     * The user has selected one or more of objects using a selection box. A box
     * rollover event is generated every frame if one or more objects intersect
     * the box, in which case the event's pickedObjects list contain the
     * selected objects. A box rollover event is generated once when the
     * selection becomes empty, in which case the event's pickedObjects is
     * <code>null</code>. In either case, the event's pickRect contains the
     * selection box bounds in AWT screen coordinates.
     */
    ...
    /**
     * Returns a list of all picked objects in this event's picked object list
     * who's onTop flag is set to true. This returns <code>null</code> if this
     * event's picked object list is empty, or does not contain any picked
     * objects marked as on top.
     *
     * @return a new list of the picked objects marked as on top, or
     * <code>null</code> if nothing is marked as on top.
     */
    ...
    /**
     * Returns a list of all objects associated with a picked object in this
     * event's picked object list who's onTop flag is set to true. This returns
     * <code>null</code> if this event's picked object list is empty, or does
     * not contain any picked objects marked as on top.
     *
     * @return a new list of the objects associated with a picked object marked
     * as on top, or <code>null</code> if nothing is marked as on top.
     */
