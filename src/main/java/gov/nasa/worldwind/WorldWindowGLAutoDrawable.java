...
/**
 * A non-platform specific {@link WorldWindow} class. This class can be
 * aggregated into platform-specific classes to provide the core functionality
 * of WorldWind.
 *
 * @author Tom Gaskins
 * @version $Id: WorldWindowGLAutoDrawable.java 2047 2014-06-06 22:48:33Z
 * tgaskins $
 */
public class WorldWindowGLAutoDrawable extends WorldWindowImpl implements WorldWindowGLDrawable, GLEventListener {
     ...
     * {@link View#VIEW_STOPPED} message is sent.
     */
    ...
    /**
     * Schedule task to send the {@link View#VIEW_STOPPED} message after the
     * view stop time elapses.
     */
    protected ScheduledFuture viewRefreshTask;
    protected boolean enableGpuCacheReinitialization = true;

    /**
     * Construct a new <code>WorldWindowGLCanvas</code> for a specified
     * {@link GLDrawable}.
     */
    ...
    /**
     * Indicates the amount of time, in milliseconds, that the View must remain
     * unchanged before a {@link
     * View#VIEW_STOPPED} event is triggered.
     *
     * @return Time in milliseconds that the View must must remain unchanged
     * before the view stopped event is triggered.
     */
    ...
    /**
     * Specifies the amount of time, in milliseconds, that the View must remain
     * unchanged before a {@link
     * View#VIEW_STOPPED} event is triggered.
     *
     * @param time Time in milliseconds that the View must must remain unchanged
     * before the view stopped event is triggered.
     */
...
//        this.drawable.setGL(new DebugGL(this.drawable.getGL())); // uncomment to use the debug drawable
     ...
     * See {@link GLEventListener#init(GLAutoDrawable)}.
     * <p>
     * GLEventListener's dispose method indicates that the GL context has been
     * released, and provides the listener an opportunity to clean up any
     * resources. Dispose does not imply that the component's lifecycle has
     * ended or that the application is closing. There are three cases in which
     * dispose may be called:
     * <ul> <li>The WorldWindow is removed from its parent component.</li>
     * <li>The WorldWindow's parent frame is closed.</li> <li>The application
     * calls either GLCanvas.dispose or GLJPanel.dispose.</li> </ul>
     * <p>
     * This implementation is left empty. In the case when a WorldWindow or a
     * WorldWind application has reached its end of life, its resources should
     * be released by calling {@link gov.nasa.worldwind.WorldWindow#shutdown()}
     * or {@link
     * gov.nasa.worldwind.WorldWind#shutDown()}, respectively. In the case when
     * a WorldWindow is removed from its parent frame or that frame is closed
     * without a call to shutdown, it is assumed that the application intends to
     * reuse the WorldWindow. Resources associated with the previous GL context
     * are released in the subsequent call to init.
     *
     * @param glAutoDrawable the drawable
     */
    @Override
     ...
     * @param glAutoDrawable the drawable
     *
     * @throws IllegalStateException if no {@link SceneController} exists for
     * this canvas
     */
    ...
    /**
     * Determine if the view has changed since the previous frame. If the view
     * has changed, schedule a task that will send a {@link View#VIEW_STOPPED}
     * to the Model if the view does not change for {@link #viewStopTime}
     * milliseconds.
     *
     * @see #getViewStopTime()
     */
    ...
    /**
     * Performs the actual repaint. Provided so that subclasses may override the
     * repaint steps.
     *
    ...
    /**
     * Performs the actual buffer swap. Provided so that subclasses may override
     * the swap steps.
     *
    ...
    /**
     * Schedule a task that will send a {@link View#VIEW_STOPPED} message to the
     * Model when the task executes. If the task runs (is not cancelled), then
     * the view is considered stopped. Only one view stop task is scheduled at a
     * time. If this method is called again before the task executes, the task
     * will be cancelled and a new task scheduled.
     *
