...
/**
 * TODO: This file needs to be updated to implement "correct" stereo, as
 * described at: http://www.orthostereo.com/geometryopengl.html
 * <p>
 * This scene controller draws in stereo, either red-blue anaglyph or device
 * supported if the display device provides stereo directly. It can also draw
 * without applying stereo. To select stereo, prior to calling this class'
 * constructor set the Java VM property
 * <code>gov.nasa.worldwind.stereo.mode</code> to "device" for device supported
 * stereo (if provided by the device) or "redblue" for red-blue anaglyph stereo.
 * If the property is not set or is any other value, this class does not draw in
 * stereo.
 * <p>
 * The {@link WorldWindow} instance must support stereo in order to use
 * device-supported stereo. A stereo <code>WorldWindow</code> is selected by
 * specifying the Java VM property described above prior to creating it. See
 * {@link gov.nasa.worldwind.awt.WorldWindowGLCanvas} for further details.
 * <p>
 * Note: The logic and much of the code here was contributed by Xander Enzmann
 * of Mitre Corporation.
 *
 * @author tag
 * @version $Id: StereoOptionSceneController.java 1171 2013-02-11 21:45:02Z
 * dcollins $
 */
public class StereoOptionSceneController extends BasicSceneController implements StereoSceneController {

    /**
     * The default focus angle. May be specified in the WorldWind configuration
     * file as the <code>gov.nasa.worldwind.StereoFocusAngle</code> property.
     * The default if not specified in the configuration is 1.6 degrees.
     */
    protected static final double DEFAULT_FOCUS_ANGLE = Configuration.getDoubleValue(AVKey.STEREO_FOCUS_ANGLE, 1.6);

    /**
     * The current stereo mode. May not be set to null; use
     * {@link AVKey#STEREO_MODE_NONE} instead.
     */
    ...
    /**
     * Indicates the GL drawable capabilities. Non-null only after this scene
     * controller draws once.
     */
    protected GLCapabilitiesImmutable capabilities;
    /**
     * Indicates whether hardware device stereo is available. Valid only after
     * this scene controller draws once.
     */
    protected boolean hardwareStereo = false;
    /**
     * Indicates whether stereo is being applied, either because a stereo device
     * is being used or a stereo mode is in effect. This field is included
     * because the question is asked every frame, and tracking the answer via a
     * boolean avoids the overhead of more complicated logic that determines the
     * stereo-drawing implementation to call.
     */
     ...
     *
     * @param a the left-right eye direction difference. If null, the angle is
     * set to 0.
     */
     ...
     * {@inheritDoc}
     * <p>
     * If the display device is providing stereo -- {@link #isHardwareStereo()}
     * is <code>true</code> -- this method returns true even if the stereo mode
     * is {@link AVKey#STEREO_MODE_NONE}. In this case, individual stereo images
     ...
     * <p>
     * Note that this method draws the image twice, once to each of the left and
     * right eye buffers, even when stereo is not in effect. This is to prevent
     * the stereo device from drawing blurred scenes.
     *
    ...
    /**
     * Implement stereo using the stereo-enabled graphics device. The mode has
     * an effect only if the display device implements stereo.
     *
     * @param dc the current draw context.
     */
