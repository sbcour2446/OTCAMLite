...
public class ViewUtil {
    ...
    /**
     * Computes a View's vertical field-of-view given a View's horizontal
     * field-of-view and the viewport window dimensions.
     *
     * @param horizontalFieldOfView the angle between the view frustum's left
     * and right clipping planes.
     * @param viewport the viewport dimensions, in window coordinates (screen
     * pixels).
     *
     * @return the angle between the view frustum's bottom and top clipping
     * planes.
     *
     * @throws IllegalArgumentException if the horitontal-field-of-view is null,
     * or if the viewport rectangle is null.
     */
     ...
     * avoids clipping an object at a given distance from the eye point. The
     * given distance should specify the smallest distance between the eye and
     * the object being viewed, but may be an approximation if an exact clip
     * distance is not required.
     *
     * @param fieldOfView The viewport rectangle, in OpenGL screen coordinates.
     * @param distanceToObject The distance from the perspective eye point to
     * the nearest object, in model coordinates.
     *
     * @return The maximum near clip distance, in model coordinates.
     *
     * @throws IllegalArgumentException if the field of view is null, or if the
    ...
    /**
     * Computes the near clip distance that corresponds to a specified far clip
     * distance and a resolution at the far clip distance. This returns zero if
     * either the distance or the resolution are zero.
     *
     * @param farDistance The far clip distance, in model coordinates.
     * @param farResolution The depth resolution at the far clip plane, in model
     * coordinates.
     * @param depthBits The number of bitplanes in the depth buffer. This is
     * typically 16, 24, or 32 for OpenGL depth buffers.
     *
     * @return The near clip distance, in model coordinates.
     *
     * @throws IllegalArgumentException if either the distance or the resolution
     * are negative, or if the depthBits is less than one.
     */
     ...
     * The returned x and y coordinates are relative to the screen's lower left
     * hand screen corner, and the returned z coordinate defines the point's
     * depth in screen coordinates (in the range [0, 1]). This returns null if
     * the specified combination of modelview matrix, projection matrix, and
     * viewport cannot produce a transformation.
     *
     * @param modelPoint the point in model coordinates to transform into window
     * coordinates.
     * @param modelview the modelview matrix.
     * @param projection the projection matrix.
     * @param viewport the viewport rectangle.
     *
     * @return the point in window coordinates, or null if the point cannot be
     * transformed.
     *
     * @throws IllegalArgumentException if any of the model point, modelview
     * matrix, projection matrix, or viewport rectangle are null.
     */
     ...
     * The input x and y are relative to the screen's lower left hand screen
     * corner, while the z-value denotes the point's depth in screen coordinates
     * (in the range [0, 1]). This returns null if the specified combination of
     * modelview matrix, projection matrix, and viewport cannot produce a
     * transformation.
     *
     * @param windowPoint the point in screen coordinates to transform into
     * model coordinates.
     * @param modelview the modelview matrix.
     * @param projection the projection matrix.
     * @param viewport the viewport rectangle.
     *
     * @return the point in model coordinates, or null if the point cannot be
     * transformed.
     *
     * @throws IllegalArgumentException if any of the model point, modelview
     * matrix, projection matrix, or viewport rectangle are null.
