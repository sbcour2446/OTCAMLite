...
public class BasicLayerFactory extends BasicFactory {
    ...
    /**
     * Creates a layer or layer list from a general configuration source. The
     * source can be one of the following: <ul>
     * <li>a {@link java.net.URL}</li> <li>a {@link java.io.File}</li> <li>a
     * {@link java.io.InputStream}</li> <li>{@link
     * Element}</li> <li>a {@link String} holding a file name, a name of a
     * resource on the classpath, or a string representation of a URL</li> </ul>
     * <p>
     * For tiled image layers, this maps the <code>serviceName</code> attribute
     * of the <code>Layer/Service</code> element of the XML configuration file
     * to the appropriate base tiled image layer type. Service types recognized
     * are: <ul> <li>"WMS" for layers that draw their data from a WMS web
     * service.</li> <li>"WWTileService" for layers that draw their data from a
     * WorldWind tile service.</li> <li>"Offline" for layers that draw their
     * data only from the local cache.</li> </ul>
     *
     ...
     * or an empty string.
     * @throws WWUnrecognizedException if the layer service type is
     * unrecognized.
    ...
    /**
     * Create the objects described in an XML element containing layer and/or
     * layer-list descriptions. Nested layer lists and included layers are
     * created recursively.
     *
     * @param domElement an XML element describing the layers and/or layer
     * lists.
     * @param params any properties to apply when creating the included layers.
     *
     * @return a <code>Layer</code>, <code>LayerList</code> or array of
     * <code>LayerList</code>s, as described by the specified description.
     *
     * @throws Exception if an exception occurs during creation. Exceptions
     * occurring during creation of internal layers or layer lists are not
     * re-thrown but are logged. The layer or layer list associated with the
    ...
    /**
     * Create a collection of layer lists and their included layers described by
     * an array of XML layer-list description elements.
     * <p>
     * Any exceptions occurring during creation of the layer lists or their
     * included layers are logged and not re-thrown. The layers associated with
    ...
    /**
     * Create a list of layers described by an array of XML layer description
     * elements.
     * <p>
     * Any exceptions occurring during creation of the layers are logged and not
     * re-thrown. The layers associated with the exceptions are not included in
     * the returned layer list.
     *
     ...
     * @return a new layer
     *
     * @throws WWUnrecognizedException if the layer type or service type given
     * in the describing element is unrecognized.
     * @see #createTiledImageLayer(org.w3c.dom.Element,
    ...
    /**
     * Create a {@link TiledImageLayer} layer described by an XML layer
     * description.
     *
     * @param domElement the XML element describing the layer to create. The
     * element must inculde a service name identifying the type of service to
     * use to retrieve layer data. Recognized service types are "Offline",
     * "WWTileService" and "OGC:WMS".
     * @param params any parameters to apply when creating the layer.
     *
     * @return a new layer
     *
     * @throws WWUnrecognizedException if the service type given in the
     * describing element is unrecognized.
     */
...
//        System.out.println(name);
    ...
    /**
     * Creates a shapefile layer described by an XML layer description. This
     * delegates layer construction to the factory class associated with the
     * configuration key "gov.nasa.worldwind.avkey.ShapefileLayerFactory".
     *
     * @param domElement the XML element describing the layer to create. The
     * element must contain the shapefile location, and may contain elements
     * specifying shapefile attribute mappings, shape attributes to assign to
     * created shapes, and layer properties.
     * @param params any parameters to apply when creating the layer.
     *
     * @return a new layer
     */
