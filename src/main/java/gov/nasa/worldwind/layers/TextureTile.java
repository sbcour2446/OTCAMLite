...
/**
 * This class manages the conversion and timing of image data to a JOGL Texture,
 * and provides an interface for binding the texture and applying any texture
 * transforms to align the texture and texture coordinates.
 * <p>
 *
 * @author tag
...
public class TextureTile extends Tile implements SurfaceTile {
    ...
    /**
     * Splits this texture tile into four tiles; one for each sub quadrant of
     * this texture tile. This attempts to retrieve each sub tile from the
     * texture tile cache. This calls {@link #createSubTile(gov.nasa.worldwind.geom.Sector,
     * gov.nasa.worldwind.util.Level, int, int)} to create sub tiles not found
     * in the cache.
     *
     ...
     * {@link #createSubTiles(gov.nasa.worldwind.util.Level)}, to construct a
     * sub tile for each quadrant of this tile. Subclasses must override this
     * method to return an instance of the derived version.
     *
     ...
     * {@link gov.nasa.worldwind.util.Level}, row, and column. This is called by
     * {@link #createSubTiles(gov.nasa.worldwind.util.Level)}, to create a sub
     * tile key for each quadrant of this tile.
     *
     * @param level the sub tile's level.
     * @param row the sub tile's row.
     * @param col the sub tile's column.
