 ...
 * <p>
 * Note: The view input handlers detect surface images rather than the terrain
 * as the top picked object in {@link
 * gov.nasa.worldwind.event.SelectEvent}s and will not respond to the user's
 * attempts at navigation when the cursor is over the image. If this is not the
 * desired behavior, disable picking for the layer containing the surface image.
 *
 * @author tag
...
public class SurfaceImageLayer extends RenderableLayer {
    ...
    /**
     * Add an image to the collection, reprojecting it to geographic (latitude
     * and longitude) coordinates if necessary. The image's location is
     * determined from metadata files co-located with the image file. The
     * number, names and contents of these files are governed by the type of the
     * specified image. Location metadata must be available.
     * <p>
     * If projection information is available and reprojection of the image's
     * projection type is supported, the image will be reprojected to geographic
     * coordinates. If projection information is not available then it's assumed
     * that the image is already in geographic projection.
     * <p>
     ...
     * @throws IllegalStateException if an error occurs while reprojecting or
     * otherwise processing the image.
    ...
    /**
     * Add an image to the collection and specify its coverage. The image is
     * assumed to be in geographic projection (latitude and longitude).
     *
     * @param imagePath the path to the image file.
    ...
    /**
     * Add a {@link BufferedImage} to the collection at an explicitly specified
     * location. The image is assumed to be in geographic projection (latitude
     * and longitude).
     *
     * @param name a unique name to associate with the image so that it can be
     * subsequently referred to without having to keep a reference to the image
     * itself. Use this name in calls to {@link #removeImage(String)}.
    ...
    /**
     * Add an image to the collection at an explicitly specified location. The
     * image is assumed to be in geographic projection (latitude and longitude).
     *
     * @param imagePath the path to the image file.
     * @param corners the geographic location of the image's corners, specified
     * in order of lower-left, lower-right, upper-right, upper-left.
     *
     * @throws IllegalArgumentException if the image path or sector is null.
     * @throws IOException if an error occurs reading the image file.
     * @throws WWRuntimeException if the image type is unsupported.
     ...
     * @param name a unique name to associate with the image so that it can be
     * subsequently referred to without having to keep a reference to the image
     * itself. Use this name in calls to {@link
     *                #removeImage(String)}.
     * @param image the image to add.
     * @param corners the geographic location of the image's corners, specified
     * in order of lower-left, lower-right, upper-right, upper-left.
     *
     * @throws IllegalArgumentException if the image path is null, the corners
     * list is null, contains null values or fewer than four locations.
     * @throws WWRuntimeException if the image type is unsupported.
     */
     ...
     * image. If null, a new buffered image of type
     * {@link BufferedImage#TYPE_INT_RGB} is created. The image must be the
     * width and height specified in the <code>canvasWidth</code> and
     * <code>canvasHeight</code> arguments.
     *
     * @return image the assembelled image, of size indicated by the
     ...
     *
     * @see ImageUtil#mergeImage(Sector, Sector, double, BufferedImage,
     * BufferedImage)
     */
    @SuppressWarnings({"UnusedDeclaration"})
