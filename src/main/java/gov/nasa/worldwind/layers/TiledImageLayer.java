...
public abstract class TiledImageLayer extends AbstractLayer {
     ...
     * that specifies the approximate size of discernible lengths in the image
     ...
     * discernible for the altitude. Such scales significantly decrease
     * performance.
     *
     * @param detailHint the degree to modify the default relationship of image
     * resolution to screen resolution with changing view altitudes. Values
     * greater than 1 increase the resolution. Values less than zero decrease
     ...
     * store images in their native format. Supported texture formats are as
     * follows: <ul> <li><code>image/dds</code> - Stores images in the
     * compressed DDS format. If the image is already in DDS format it's stored
     * as-is.</li> </ul>
     *
     * @param textureFormat the texture image format; null to store images in
     * their native format.
     */
     ...
     * number of milliseconds beyond the epoch. The default expiry time is zero.
     *
...
//                }
     ...
     * AVKey#AVAILABLE_IMAGE_FORMATS}</td><td>AvailableImageFormats/ImageFormat</td><td>String
     * array</td></tr>
     ...
     * AVKey#URL_CONNECT_TIMEOUT}</td><td>RetrievalTimeouts/ConnectTimeout/Time</td><td>Integer
     * milliseconds</td></tr>
     * <tr><td>{@link AVKey#URL_READ_TIMEOUT}</td><td>RetrievalTimeouts/ReadTimeout/Time</td><td>Integer
     * milliseconds</td></tr>
     * <tr><td>{@link AVKey#RETRIEVAL_QUEUE_STALE_REQUEST_LIMIT}</td>
     * <td>RetrievalTimeouts/StaleRequestLimit/Time</td><td>Integer
     * milliseconds</td></tr> </table> This also writes common layer and
     * LevelSet configuration parameters by invoking {@link gov.nasa.worldwind.layers.AbstractLayer#createLayerConfigElements(gov.nasa.worldwind.avlist.AVList,
     * org.w3c.dom.Element)} and {@link DataConfigurationUtils#createLevelSetConfigElements(gov.nasa.worldwind.avlist.AVList,
     * org.w3c.dom.Element)}.
     *
     * @param params the key-value pairs which define the TiledImageLayer
     * configuration parameters.
     * @param context the XML document root on which to append TiledImageLayer
     ...
     * <tr><td>{@link AVKey#RETRIEVAL_QUEUE_STALE_REQUEST_LIMIT}</td>
     * <td>RetrievalTimeouts/StaleRequestLimit/Time</td><td>Integer
     * milliseconds</td></tr> </table> This also parses common layer and
     * LevelSet configuration parameters by invoking {@link gov.nasa.worldwind.layers.AbstractLayer#getLayerConfigParams(org.w3c.dom.Element,
     ...
     *
     * @param domElement the XML document root to parse for TiledImageLayer
     * configuration parameters.
     * @param params the output key-value pairs which recieve the
     * TiledImageLayer configuration parameters. A null reference is permitted.
     *
     ...
     * <caption style="font-weight: bold;">Supported Names</caption>
     * <tr><th>Parameter</th><th>Element Path</th><th>Type</th></tr>
     * <tr><td>{@link AVKey#TEXTURE_FORMAT}</td><td>CompressTextures</td><td>"image/dds"
     * if CompressTextures is "true"; null otherwise</td></tr> </table>
     *
     * @param domElement the XML document root to parse for legacy
     * TiledImageLayer configuration parameters.
     * @param params the output key-value pairs which recieve the
     * TiledImageLayer configuration parameters. A null reference is permitted.
     *
     * @return a reference to params, or a new AVList if params is null.
    ...
    /**
     * Create an image for the portion of this layer lying within a specified
     * sector.The image is created at a specified aspect ratio within a canvas
     * of a specified size. This returns the specified image if this layer has
     * no content in the specified sector.
     *
     * @param sector the sector of interest.
     * @param canvasWidth the width of the canvas.
     * @param canvasHeight the height of the canvas.
     * @param aspectRatio the aspect ratio, width/height, of the window. If the
     * aspect ratio is greater or equal to one, the full width of the canvas is
     * used for the image; the height used is proportional to the inverse of the
     * aspect ratio. If the aspect ratio is less than one, the full height of
     * the canvas is used, and the width used is proportional to the aspect
     * ratio.
     * @param levelNumber the target level of the tiled image layer.
     * @param mimeType the type of image to create, e.g., "png" and "jpg".
     * @param abortOnError indicates whether to stop assembling the image if an
     * error occurs. If false, processing continues until all portions of the
     * layer that intersect the specified sector have been added to the image.
     * Portions for which an error occurs will be blank.
     * @param image if non-null, a {@link BufferedImage} in which to place the
     * image. If null, a new buffered image is created. The image must be the
     * width and height specified in the <code>canvasWidth</code> and
     * <code>canvasHeight</code> arguments.
     * @param timeout The amount of time to allow for reading the image from the
     * server.
     *
     * @return image the assembled image, of size indicated by the
     * <code>canvasWidth</code> and <code>canvasHeight</code>. If the specified
     * aspect ratio is one, all pixels contain values. If the aspect ratio is
     * greater than one, a full-width segment along the top of the canvas is
     * blank. If the aspect ratio is less than one, a full-height segment along
     * the right side of the canvase is blank. If the <code>image</code>
     ...
     * @see ImageUtil#mergeImage(gov.nasa.worldwind.geom.Sector,
     * gov.nasa.worldwind.geom.Sector, double, java.awt.image.BufferedImage,
