...
public class BasicTiledImageLayer extends TiledImageLayer implements BulkRetrievable {
    ...
    /**
     * Reads and returns the texture data at the specified URL, optionally
     * converting it to the specified format and generating mip-maps. If
     * <code>textureFormat</code> is a recognized mime type, this returns the
     * texture data in the specified format. Otherwise, this returns the texture
     * data in its native format. If <code>useMipMaps</code> is true, this
     * generates mip maps for any non-DDS texture data, and uses any mip-maps
     * contained in DDS texture data.
     * <p>
     * Supported texture formats are as follows: <ul> <li><code>image/dds</code>
     * - Returns DDS texture data, converting the data to DDS if necessary. If
     * the data is already in DDS format it's returned as-is.</li> </ul>
     *
     * @param url the URL referencing the texture data to read.
     * @param textureFormat the texture data format to return.
     * @param useMipMaps true to generate mip-maps for the texture data or use
     * mip maps already in the texture data, and false to read the texture data
     * without generating or using mip-maps.
     *
     * @return TextureData the texture data from the specified URL, in the
     * specified format and with mip-maps.
     */
    ...
    // *** Bulk download ***
    /**
     * Start a new {@link BulkRetrievalThread} that downloads all imagery for a
     * given sector and resolution to the current WorldWind file cache, without
     * downloading imagery that is already in the cache.
     * <p>
     ...
     * been started, construct a {@link BasicTiledImageLayerBulkDownloader}.
     * <p>
     ...
     * <code>null</code> if the specified sector does not intersect the layer
     * bounding sector.
     *
    ...
    /**
     * Start a new {@link BulkRetrievalThread} that downloads all imagery for a
     * given sector and resolution to a specified {@link FileStore}, without
     * downloading imagery that is already in the file store.
     * <p>
     * This method creates and starts a thread to perform the download. A
     * reference to the thread is returned. To create a downloader that has not
     * been started, construct a {@link BasicTiledImageLayerBulkDownloader}.
     ...
     * @param resolution the target resolution, provided in radians of latitude
     * per texel.
     * @param fileStore the file store in which to place the downloaded imagery.
     * If null the current WorldWind file cache is used.
     * @param listener an optional retrieval listener. May be null.
     *
     * @return the {@link BulkRetrievalThread} executing the retrieval or
     * <code>null</code> if the specified sector does not intersect the layer
    ...
    /**
     * Get the estimated size in bytes of the imagery not in the WorldWind file
     * cache for the given sector and resolution.
     * <p>
     * Note that the target resolution must be provided in radians of latitude
     * per texel, which is the resolution in meters divided by the globe radius.
     *
    ...
    /**
     * Get the estimated size in bytes of the imagery not in a specified file
     * store for a specified sector and resolution.
     * <p>
     * Note that the target resolution must be provided in radians of latitude
     * per texel, which is the resolution in meters divided by the globe radius.
     *
     ...
     * online or in the local filesystem, and initializes properties of this
     * Layer using those resources. This returns a key indicating the retrieval
     * state: {@link gov.nasa.worldwind.avlist.AVKey#RETRIEVAL_STATE_SUCCESSFUL}
     * indicates the retrieval succeeded, {@link
     * gov.nasa.worldwind.avlist.AVKey#RETRIEVAL_STATE_ERROR} indicates the
     * retrieval failed with errors, and <code>null</code> indicates the
    ...
    /**
     * Initializes this Layer's expiry time property from the specified WMS
     * Capabilities document and parameter list describing the WMS layer names
     * associated with this Layer. This method is thread safe; it synchronizes
     * changes to this Layer by wrapping the appropriate method calls in
     * {@link SwingUtilities#invokeLater(Runnable)}.
     *
     * @param caps the WMS Capabilities document retrieved from this Layer's WMS
     * server.
    ...
    /**
     * Returns a boolean value indicating if this Layer should retrieve any
     * non-tile resources, either online or in the local filesystem, and
     * initialize itself using those resources.
     *
     * @return <code>true</code> if this Layer should retrieve any non-tile
    ...
    /**
     * Starts retrieving non-tile resources associated with this Layer in a
