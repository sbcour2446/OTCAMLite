...
/**
 * Downloads imagery not currently available in the WorldWind file cache or a
 * specified file store. The class derives from {@link Thread} and is meant to
 * operate in its own thread.
 * <p>
 * The sector and resolution associated with the downloader are specified during
 * construction and are final.
 *
 * @author tag
 * @version $Id: BasicTiledImageLayerBulkDownloader.java 1171 2013-02-11
 * 21:45:02Z dcollins $
 */
public class BasicTiledImageLayerBulkDownloader extends BulkRetrievalThread {
     ...
     *
     * @throws IllegalArgumentException if either the layer or sector are null,
     * or the resolution is less than zero.
     */
     ...
     * @param listener an optional retrieval listener. May be null.
     *
     * @throws IllegalArgumentException if either the layer, the sector or file
     * store are null, or the resolution is less than zero.
     */
    public BasicTiledImageLayerBulkDownloader(BasicTiledImageLayer layer, Sector sector, double resolution,
...
//    }
     ...
     * {@link Sector}, resolution and file store. Note that the target
     * resolution must be provided in radian latitude per texel - which is the
