 ...
 * <p>
 * Usage: do setEventSource(wwd) to have the graph activated and updated with
 * position changes. See public properties for options: keepProportions, follow,
 * unit, start and end latlon... </p>
 *
...
public class TerrainProfileLayer extends AbstractLayer implements PositionListener, SelectListener {
     ...
     * <p>
     * Note that the graph can be both minimized and maximized at the same time.
     * The minimized state will take precedence and the graph will display as an
     * icon. When 'un-minimized' it will display as maximized.</p>
     *
    ...
    /**
     * Get whether the profile graph is maximized - displays over the whole
     * viewport.
     *
    ...
    /**
     * Set whether the profile graph should be maximized - displays over the
     * whole viewport.
     *
    ...
    /**
     * Sets the layer opacity, which is applied to the layer's chart. The
     * opacity is modified slightly for various elements of the chart to
    ...
    /**
     * Returns the layer's opacity value. The opacity is modified slightly for
     * various elements of the chart to distinguish among them.
     *
     ...
     * displayed size of the graphic. This scale factor is used only when the
     * layer's resize behavior is {@link AVKey#RESIZE_STRETCH} or {@link
     * AVKey#RESIZE_SHRINK_ONLY}. The graphic's width is adjusted to occupy the
     * proportion of the viewport's width indicated by this factor. The
     * graphic's height is adjusted to maintain the graphic's Dimension aspect
     * ratio.
     *
     ...
     * AVKey#NORTHWEST}, {@link AVKey#SOUTHEAST}, or {@link AVKey#SOUTHWEST}
     * (the default). These indicate the corner of the viewport.
     *
    ...
    /**
     * Returns the current location offset. See {@link #setLocationOffset} for a
     ...
     *
     * @param locationOffset the number of pixels to shift the layer image from
     * its specified screen position. A positive X value shifts the image to the
     * right. A positive Y value shifts the image up. If null, no offset is
    ...
    /**
     * Sets the behavior the layer uses to size the graphic when the viewport
     * size changes, typically when the World Wind window is resized. If the
     * value is {@link AVKey#RESIZE_KEEP_FIXED_SIZE}, the graphic size is kept
     * to the size specified in its Dimension scaled by the layer's current icon
     * scale. If the value is {@link
     * AVKey#RESIZE_STRETCH}, the graphic is resized to have a constant size
     * relative to the current viewport size. If the viewport shrinks the
     * graphic size decreases; if it expands then the graphic enlarges. If the
     * value is {@link
     * AVKey#RESIZE_SHRINK_ONLY} (the default), graphic sizing behaves as for
     * {@link AVKey#RESIZE_STRETCH} but it will not grow larger than the size
     ...
     *
     * @param borderWidth the number of pixels to offset the graphic from the
    ...
    /**
     * Sets the unit the graphic uses to display distances and elevations. Can
     * be one of {@link #UNIT_METRIC} (the default), or {@link #UNIT_IMPERIAL}.
     *
     ...
     *
     * @param state true if the graph should maintains distance/elevation
     * proportions.
     */
    ...
    /**
     * Set the graph center point placement behavior. Can be one of
     * {@link #FOLLOW_VIEW} (the default), {@link
     * #FOLLOW_CURSOR}, {@link #FOLLOW_EYE}, {@link #FOLLOW_OBJECT}, {@link #FOLLOW_NONE}
     * or {@link #FOLLOW_PATH}. If {@link #FOLLOW_NONE} the profile will be
     * computed between startLatLon and endLatLon. If {@link #FOLLOW_PATH} the
     * profile will be computed along the path defined with
     * {@link #setPathPositions(java.util.ArrayList)}.
     *
     ...
     * {@link #FOLLOW_EYE} or {@link #FOLLOW_OBJECT}.
     *
     ...
     * {@link #FOLLOW_EYE} or {@link
     * #FOLLOW_OBJECT}.
     *
     * @param state if true the eye or object position should be shown on the
     * graph.
     */
    ...
    /**
     * Set the profile length factor - has no effect if {@link #FOLLOW_NONE} or
     * {@link #FOLLOW_PATH}
     *
    ...
    /**
     * Set whether the profile graph should include sea level. True is the
     * default.
     *
     ...
     * {@link #FOLLOW_OBJECT}. The profile graph will be computed perpendicular
     * to the object heading direction.
     *
    ...
    /**
     * Sets the type of path to follow, one of {@link AVKey#GREAT_CIRCLE}, which
     * computes each segment of the path as a great circle, or
     * {@link AVKey#RHUMB_LINE}, which computes each segment of the path as a
     * line of constant heading.
     *
     ...
     *
     * @return the <code>Path</code> used to render the profile line on the
     * ground.
     */
    @SuppressWarnings({"UnusedDeclaration"})
    ...
    /**
     * Get the <code>Path</code> used to render the picked position on the
     * terrain.
     *
     * @return the <code>Path</code> used to render the picked position on the
     * terrain.
     */
    @SuppressWarnings({"UnusedDeclaration"})
     ...
     *
     * @param state true if the profile line should be displayed over the
     * terrain.
     */
...
//            this.expirySupport.restart(dc);
    ...
    /**
     * Computes the Position of the pickPoint over the graph and updates
     * pickedSample indice
     *
     * @param dc the current DrawContext
     * @param locationSW the screen location of the bottom left corner of the
     * graph
...
//            this.wwd.getModel().getGlobe().getElevationModel().addPropertyChangeListener(this);
     ...
     * #FOLLOW_OBJECT}, collects terrain profile data along a great circle line
     * centered at the current position (view, eye, cursor or object) and
     * perpendicular to the view heading - or object heading if
     * {@link #FOLLOW_OBJECT}.
     * <p>
     * If {@link #FOLLOW_NONE} the profile is computed in between start and end
     * latlon.
     * <p>
     * If {@link #FOLLOW_PATH} the profile is computed along the path provided
     * with {@link #setPathPositions(ArrayList)}</p>
     *
