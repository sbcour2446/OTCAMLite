...
/**
 * Displays a world map overlay with a current-position crosshair in a screen
 * corner.
 * <p>
 * A {@link gov.nasa.worldwindx.examples.ClickAndGoSelectListener} can be used
 * in conjunction with this layer to move the view to a selected location when
 * that location is clicked within the layer's map. Specify
 * <code>WorldMapLayer.class</code> when constructing the
 * <code>ClickAndGoSelectListener</code>.
 * <p>
...
public class WorldMapLayer extends AbstractLayer {
    ...
    /**
     * Displays a world map overlay with a current position crosshair in a
     * screen corner
     */
    ...
    /**
     * Sets the world map icon's image location. The layer first searches for
     * this location in the current Java classpath. If not found then the
     * specified path is assumed to refer to the local file system. found there
     * then the
     *
     ...
     * displayed size of the world map icon. This scale factor is used only when
     * the layer's resize behavior is AVKey.RESIZE_STRETCH or
     * AVKey.RESIZE_SHRINK_ONLY. The icon's width is adjusted to occupy the
     * proportion of the viewport's width indicated by this factor. The icon's
     ...
     * resize behavior is other than AVKey.RESIZE_KEEP_FIXED_SIZE, the icon's
    ...
    /**
     * Sets the behavior the layer uses to size the world map icon when the
     * viewport size changes, typically when the WorldWind window is resized. If
     * the value is AVKey.RESIZE_KEEP_FIXED_SIZE, the icon size is kept to the
     * size specified in its image file scaled by the layer's current icon
     * scale. If the value is AVKey.RESIZE_STRETCH, the icon is resized to have
     * a constant size relative to the current viewport size. If the viewport
     * shrinks the icon size decreases; if it expands then the icon file
     * enlarges. The relative size is determined by the current world
     * map-to-viewport scale and by the icon's image file size scaled by the
     * current icon scale. If the value is AVKey.RESIZE_SHRINK_ONLY (the
     * default), icon sizing behaves as for AVKey.RESIZE_STRETCH but the icon
     * will not grow larger than the size specified in its image file scaled by
     ...
     *
     * @param borderWidth the number of pixels to offset the world map icon from
     * the borders indicated by {@link
     *                    #setPosition(String)}.
     */
    ...
    /**
     * Sets the relative viewport location to display the world map icon. Can be
     * one of AVKey.NORTHEAST, AVKey.NORTHWEST (the default), AVKey.SOUTHEAST,
     * or SOUTHWEST. These indicate the corner of the viewport to place the
     * icon.
     *
    ...
    /**
     * Specifies the screen location of the worldmap image, relative to the
     * image's center. May be null. If this value is non-null, it overrides the
     ...
     * The final image location will be affected by the currently specified
     ...
     *
     * @param locationOffset the number of pixels to shift the worldmap image
     * from its specified screen position. A positive X value shifts the image
     * to the right. A positive Y value shifts the image up. If null, no offset
    ...
    /**
     * Get the current view footprint position list. May be null if no footprint
     * is displayed or none has been computed.
     *
