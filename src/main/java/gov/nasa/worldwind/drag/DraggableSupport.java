...
/**
 * Utility functions which support dragging operations on objects implementing
 * the {@link Movable} or {@link Movable2} interface.
 */
public class DraggableSupport {

    /**
     * The
     * {@link DraggableSupport#computeRelativePoint(Line, Globe, SceneController, double)}
     * method uses a numeric search method to determine the coordinates of the
     * desired position. The numeric solver will stop at a defined threshold or
     * step limit. The default threshold is provided here. This value is
     * utilized when using the static drag functions provided by this class.
     */
     ...
     * method uses a numeric search method to determine the coordinates of the
     * desired position. The numeric solver will stop at a defined threshold or
     * step limit. The default step limit is provided here. This value is
     * utilized when using the static drag functions provided by this class.
     */
    public static final int DEFAULT_STEP_LIMIT = 20;
    /**
     * Initial drag operation offset in x and y screen coordinates, between the
     * object reference position and the screen point. Used for screen size
     * constant drag operations.
     */
    protected Vec4 initialScreenPointOffset = null;
    /**
     * Initial drag operation cartesian coordinates of the objects reference
     * position. Used for globe size constant drag operations.
     */
    ...
    /**
     * This instances step limit when using the solver to determine the position
     * of objects using the {@link WorldWind#RELATIVE_TO_GROUND} altitude mode.
     * Increasing this value will increase solver runtime and may cause the
     * event loop to hang. If the solver exceeds the number of steps specified
     * here, it will fall back to using a position calculated by intersection
     * with the ellipsoid.
     */
    protected int stepLimit = DEFAULT_STEP_LIMIT;
    /**
     * This instances convergence threshold for the solver used to determine the
     * position of objects using the {@link WorldWind#RELATIVE_TO_GROUND}
     * altitude mode. Decreasing this value will increase solver runtime and may
     * cause the event loop to hang. If the solver does not converge to the
     * threshold specified here, it will fall back to using a position
     * calculated by intersection with the ellipsoid.
     */
    protected double convergenceThreshold = DEFAULT_CONVERGENCE_THRESHOLD;
    /**
     * The object that will be subject to the drag operations. This object
     * should implement {@link Movable} or {@link
     * Movable2}.
     */
    ...
    /**
     * Provides persistence of initial values of a drag operation to increase
     * dragging precision and provide better dragging behavior.
     *
     ...
     * Uses the information provided by the {@link DragContext} object and
     * attempts to move the object using the {@link Movable2} or {@link Movable}
     * interface. This method maintains a constant screen offset from the cursor
     * to the reference point of the object being dragged. It is suited for
     * objects maintaining a constant screen size presentation like,
     ...
     * Uses the information provided by the {@link DragContext} object and
     * attempts to move the object using the {@link Movable2} or {@link Movable}
     * interface. This method maintains a constant geographic distance between
     * the cursor and the reference point of the object being dragged. It is
     * suited for objects which maintain a constant model space or geographic
     * size.
     *
     * @param dragContext the current {@link DragContext} for this object.
     *
    ...
    /**
     * Returns the step limit used by the position solver method for objects
     * using a screen size constant drag approach and a
     * {@link WorldWind#RELATIVE_TO_GROUND} altitude mode. If the solver exceeds
     * this value it will utilize an intersection with the ellipsoid at the
     * specified altitude.
     *
    ...
    /**
     * Sets the step limit to use for the position solver. The position solver
     * is only used for screen size constant objects with a
     * {@link WorldWind#RELATIVE_TO_GROUND} altitude mode. The step limit is the
     * maximum steps the solver will attempt before using an intersection with
     * the ellipsoid at the specified altitude. Set this value in coordination
    ...
    /**
     * Returns the convergence threshold used by the solver when an a screen
     * size constant object using a {@link WorldWind#RELATIVE_TO_GROUND}
     * altitude mode needs to determine a position. When the solver finds an
     * altitude within the convergence threshold to the desired altitude, the
     * solver will stop and return the cartesian position.
     *
    ...
    /**
     * Sets the convergence threshold for the screen size constant object using
     * a {@link WorldWind#RELATIVE_TO_GROUND} altitude mode. The solver will
     * test each iterations solution altitude with the desired altitude and if
     * it is found to be within the convergence threshold, the cartesian
     * solution will be returned. Set this value in coordination with the step
     * limit.
     *
     * @param convergenceThreshold the convergence threshold to use for the
     * solver.
     */
    ...
    /**
     * Returns the current altitude mode being used by the dragging
     * calculations.
     *
    ...
    /**
     * Determines the cartesian coordinate of a screen point given the altitude
     * mode.
     *
     * @param dragContext the current {@link DragContext} of the dragging event.
     * @param screenPoint the {@link Point} of the screen to determine the
     * position.
     * @param altitude the altitude in meters.
     * @param utilizeSearchMethod if the altitude mode is
     * {@link WorldWind#RELATIVE_TO_GROUND}, this determines if the search
     * method will be used to determine the position, please see the {@link
     *                            DraggableSupport#computeRelativePoint(Line, Globe, SceneController, double)}
     * for more information.
     *
     * @return the cartesian coordinates using an ellipsoidal globe, or null if
    ...
    /**
     * Determines the offset in screen coordinates from the previous screen
     * point ({@link DragContext#getInitialPoint()} and the objects
     * {@link Movable#getReferencePosition()} or
     * {@link Movable2#getReferencePosition()} methods. If the object doesn't
     * implement either of the interfaces, or there is an error determining the
     * offset, this function will return null.
     *
     * @param dragObjectReferencePosition the {@link Movable} or
     * {@link Movable2} reference position {@link Position}.
     * @param dragContext the current {@link DragContext} of this drag event.
     *
     * @return a {@link Vec4} containing the x and y offsets in screen
     * coordinates from the reference position and the previous screen point.
     */
    ...
    /**
     * Extracts the reference {@link Position} from objects implementing
     * {@link Movable} or {@link Movable2}. For objects implementing both, it
     * utilizes the {@link Movable2} interface method. If the object does not
     * implement the interfaces, null is returned.
     *
     * @return the reference {@link Movable#getReferencePosition()} or
     * {@link Movable2#getReferencePosition()}, or null if the object didn't
    ...
    /**
     * Executes the {@link Movable2#moveTo(Globe, Position)} or
     * {@link Movable#moveTo(Position)} methods with the provided data. If the
     * object implements both interfaces, the {@link Movable2} is used. If the
     * object doesn't implement either, it is ignored.
     *
     * @param movePosition the {@link Position} to provide to the
     * {@link Movable2#moveTo} method.
     * @param globe the globe reference, may be null if the object only
    ...
    /**
     * Computes the intersection of the provided {@link Line} with the
     * {@link Globe} while accounting for the altitude mode. If a
     * {@link Globe2D} is specified, then the intersection is calculated using
     * the globe objects method.
     *
     * @param ray the {@link Line} to calculate the intersection of the
     * {@link Globe}.
     * @param altitude the altitude mode for the intersection calculation.
     * @param useSearchMethod if the altitude mode is
     * {@link WorldWind#RELATIVE_TO_GROUND}, this determines if the search
     * method will be used to determine the position, please see the {@link
     *                        DraggableSupport#computeRelativePoint(Line, Globe, SceneController, double)}
     * for more information.
     * @param globe the {@link Globe} to intersect.
     * @param sceneController if an altitude mode other than
     * {@link WorldWind#ABSOLUTE} is specified, the {@link
     *                        SceneController} which will provide terrain information.
     *
     * @return the cartesian coordinates of the intersection based on the
     * {@link Globe}s coordinate system or null if the intersection couldn't be
     * calculated.
     */
    protected Vec4
    ...
    /**
     * Attempts to find a position with the altitude specified for objects which
     * are {@link
     * WorldWind#RELATIVE_TO_GROUND}. Using the provided {@link Line}, conducts
     * a bisectional search along the {@link
     * Line} for a {@link Position} which is within the
     * {@code convergenceThreshold} of the requested {@code altitude} provided.
     * The {@code stepLimit} limits the number of bisections the function will
     * attempt. If the search does not find a position within the
     * {@code stepLimit} or within the {@code convergenceThreshold} it will
     * provide an intersection with the ellipsoid at the provided altitude.
     *
     * @param ray the {@link Line} from the eye point and direction in globe
     * coordinates.
