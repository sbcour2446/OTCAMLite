 ...
 */
/**
 * BasicRasterServer maintains a list of data sources and their properties in
 * the BasicRasterServerCache and is used to compose (mosaic) a data raster of
 * the given region of interest from data sources.
 */
public class BasicRasterServer extends WWObjectImpl implements RasterServer {
    ...
    /**
     * BasicRasterServer constructor reads a list of data raster sources from
     * *.RasterServer.xml (the file that accompanies layer description XML
     * file), reads sector of each source and maintains a list of data sources,
     * their properties,
     *
     * @param o the RasterServer.xml source to read. May by a
     * {@link java.io.File}, a file path, a URL or an
     * {@link org.w3c.dom.Element}
     * @param params optional metadata associated with the data source that
     * might be useful to the BasicRasterServer.
     */
    ...
    /**
     * Returns an instance of the MemoryCache that contains DataRasters and
     * their properties
     *
     * @return an instance of the MemoryCache that contains DataRasters and
     * their properties
     */
...
//        }
    ...
    /**
     * Returns DataSet's pixel format (AVKey.IMAGE or AVKey.ELEVATION), or empty
     * string if not set
     *
     * @return DataSet's pixel format (AVKey.IMAGE or AVKey.ELEVATION), or empty
     * string if not set
     */
    ...
    /**
     * Composes a DataRaster of the given width and height for the specific
     ...
     * <p>
     * Optional keys are: AVKey.PIXEL_FORMAT (AVKey.ELEVATION | AVKey.IMAGE)
     * AVKey.DATA_TYPE AVKey.BYTE_ORDER (AVKey.BIG_ENDIAN | AVKey.LITTLE_ENDIAN
     * )
     *
     * @return a DataRaster for the requested ROI
     *
     * @throws gov.nasa.worldwind.exception.WWRuntimeException if there is no
     * intersection of the source rasters with the requested ROI or the source
     * format is unknown or not supported by currently loaded drivers
     * @throws IllegalArgumentException if any of the required parameters or
     * values are missing
     */
    ...
    /**
     * Composes a DataRaster of the given width and height for the specific
     * geographic region of interest (ROI), in the requested file format
     * (AVKey.IMAGE_FORMAT) and returns as a ByteBuffer
     *
     * @param params This is a required parameter, must not be null or empty;
     * Must contain AVKey.WIDTH, AVKey.HEIGHT, AVKey.SECTOR, and
     * AVKey.IMAGE_FORMAT (mime type) values. Supported mime types are:
