...
public class GDALDataRaster extends AbstractDataRaster implements Cacheable {
    ...
    /**
     * Set a new extent to the data raster. This operation is mostly required
     * for rasters that does not have a georeferenced information. A new
     * geo-transform matrix will be created. The coordinate system is set to
     * Geographic.
     *
...
//                throw new WWRuntimeException( message );
    ...
    /**
     * The purpose of this method is to create the best suited dataset for the
     * requested area. The dataset may contain overviews, so instead of
     * retrieving raster from the highest resolution source, we will compose a
     * temporary dataset from an overview, and/or we may clip only the requested
     * area. This will accelerate reprojection (if needed), because the
     * reporjection will be done on much smaller dataset.
     *
...
//        Double nodata = this.calcNoDataForDestinationRaster(destParams);
     ...
     * <p>
     * AVKey.BAND_ORDER as array of integers, examples: for RGBA image: new
     * int[] { 0, 1, 2, 3 }, or for ARGB image: new int[] { 3, 0, 1, 2 } , or if
     * you want only RGB bands of the RGBA image: new int[] {0, 1, 2 }, or only
     * Intensity (4th) band of the specific aerial image: new int[] { 3 }
     *
     * @return A writable data raster: BufferedImageRaster (if the source
     * dataset is imagery) or ByteBufferRaster (if the source dataset is
     * elevations)
     */
    @Override
