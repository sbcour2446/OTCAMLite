...
public abstract class TiledRasterProducer extends AbstractDataStoreProducer {
     ...
     * AVKey.TILED_RASTER_PRODUCER_LIMIT_MAX_LEVEL. This method requires
     * <code>maxNumOfLevels</code> - the actual maximum numbers of levels.
     * <p>
     * The AVKey.TILED_RASTER_PRODUCER_LIMIT_MAX_LEVEL could specify multiple
     * things:
     * <p>
     * If the value of the AVKey.TILED_RASTER_PRODUCER_LIMIT_MAX_LEVEL is "Auto"
     * (as String), the calculated limit of levels will be 70% of the actual
     * maximum numbers of levels <code>maxNumOfLevels</code>.
     * <p>
     * If the type of the value of the
     * AVKey.TILED_RASTER_PRODUCER_LIMIT_MAX_LEVEL is Integer, it should contain
     * an integer number between 0 (for level 0 only) and the actual maximum
     * numbers of levels <code>maxNumOfLevels</code>.
     * <p>
     * It is also possible to specify the limit as percents, in this case the
     * type of the AVKey.TILED_RASTER_PRODUCER_LIMIT_MAX_LEVEL value must be
     * "String", have a numeric value as text and the "%" percent sign in the
     * end. Examples: "100%", "25%", "50%", etc.
     * <p>
     * Value of AVKey.TILED_RASTER_PRODUCER_LIMIT_MAX_LEVEL could be a numeric
     * string (for example, "3"), or Integer. The value will be correctly
     * extracted and compared with the <code>maxNumOfLevels</code>. Valid values
     * must be smaller or equal to <code>maxNumOfLevels</code>.
     *
     * @param params AVList that may contain
     * AVKey.TILED_RASTER_PRODUCER_LIMIT_MAX_LEVEL property
    ...
    //**************************************************************//
    /**
     * Returns a configuration document which describes the tiled data produced
     * by this TiledRasterProducer. The document's contents are derived from the
     * specified parameter list, and depend on the concrete subclass'
     * implementation. This returns null if the parameter list is null, or if
     * the configuration document cannot be created for any reason.
     *
     * @param params the parameters which describe the configuration document's
     * contents.
     *
    ...
    /**
     * Installs the configuration file which describes the tiled data produced
     * by this TiledRasterProducer. The install location, configuration
     * filename, and configuration file contents are derived from the specified
     * parameter list. This throws an exception if the configuration file cannot
     * be installed for any reason.
     * <p>
     * The parameter list must contain <strong>at least</strong> the following
     * keys: <table>
     * <caption style="font-weight: bold;">Required
     * Keys</caption><tr><th>Key</th></tr>
     ...
     *
     * @param params the parameters which describe the install location, the
     * configuration filename, and the configuration file contents.
     *
     * @throws Exception if the configuration file cannot be installed for any
     * reason.
    ...
    /**
     * Returns the location of the configuration file which describes the tiled
     * data produced by this TiledRasterProducer. The install location is
     * derived from the specified parameter list. This returns null if the
     * parameter list is null, or if it does not contain any of the following
     * keys: <table>
     * <caption style="font-weight: bold;">Required
     * Keys</caption><tr><th>Key</th></tr>
     * <tr><td>{@link gov.nasa.worldwind.avlist.AVKey#FILE_STORE_LOCATION}</td><td></td></tr> <tr><td>{@link
     ...
     *
     * @return the configuration file install location, or null if the parameter
     * list is null or does not contain the required parameters.
     */
