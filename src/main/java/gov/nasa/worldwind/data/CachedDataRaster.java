...
/**
 * The <code>CachedDataRaster</code> is used to hold data raster's source and
 * metadata, while the actual data raster may not be loaded in to the memory.
 * This is mostly used together with a memory caches.
 * <code>CachedDataRaster</code> actually implements all interfaces of the
 * <code>DataRaster</code>, and acts as a proxy, that loads a real data raster
...
public class CachedDataRaster extends AVListImpl implements DataRaster {
     ...
     * @param params metadata as AVList, it is expected to next parameters:
     * AVKey.WIDTH, AVKey.HEIGHT, AVKey.SECTOR, AVKey.PIXEL_FORMAT.
     * <p>
     * If any of these keys is missing, there will be an attempt made to
     * retrieve missign metadata from the source using the reader.
     * @param reader A reference to a DataRasterReader instance
     * @param cache A reference to a MemoryCache instance
     *
     * @throws java.io.IOException thrown if there is an error to read metadata
     * from the source
     * @throws IllegalArgumentException thrown when a source or a reader are
     * null
     */
     ...
     * @param throwException specifies weather to throw exception when a
     * key/value is missing, or just return false.
     *
     * @return TRUE, if all required keys are present in the params list, or
     * both params and required keys are empty, otherwise returns FALSE (if
     * throwException is false)
     *
     * @throws IllegalArgumentException If a key/value is missing and
