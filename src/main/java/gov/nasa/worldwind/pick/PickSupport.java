...
public class PickSupport {
    ...
    /**
     * The picked objects currently registered with this PickSupport,
     * represented as a map of color codes to picked objects. This maps provides
     * constant time access to a picked object when its color code is known.
     */
    protected Map<Integer, PickedObject> pickableObjects = new HashMap<Integer, PickedObject>();
    /**
     * The picked object color code ranges currently registered with this
     * PickSupport, represented as a map of color code ranges to picked object
     * factories. PickSupport uses these factories to delay PickedObject
     * construction until a matching pick color is identified.
     */
    protected Map<Range, PickedObjectFactory> pickableObjectRanges = new HashMap<Range, PickedObjectFactory>();
    /**
     * Indicates the minimum and maximum color code associated with the picked
     * objects in the pickableObjects map. Initially <code>null</code>,
     ...
     * PickSupport uses the factory to delay PickedObject construction until a
     * matching pick color is identified by getTopObject or resolvePick. This
     * eliminates the overhead of creating and managing a large collection of
     * PickedObject instances when only a few may actually be picked.
     *
     * @param colorCode the first color code associated with the range of
     * sequential color codes.
     * @param count the number of sequential color codes in the range of
     * sequential color codes.
     * @param factory the PickedObjectFactory to use when creating a
     * PickedObject for a color in the specified range.
     */
    ...
    /**
     * Adds picked object registered with this PickSupport that are drawn at the
     * specified pick point or intersect the draw context's pick rectangle to
     * the draw context's list of picked objects. This clears any registered
     * picked objects upon returning.
     * <p>
     * If this pick point is <code>null</code>, this ignores the pick point and
     * does not attempt to determine which picked objects are drawn there. If
     * the draw context's pick rectangle is <code>null</code>, this ignores the
     * pick rectangle and does not attempt to determine which picked objects
     * intersect it. This does nothing if no picked objects are currently
     * registered with this PickSupport.
     *
     ...
     *
     * @return the picked object added to the draw context, or <code>null</code>
     * if no picked object is drawn at the specified point.
     */
    ...
    /**
     * Adds a picked object registered with this PickSupport that is drawn at
     * the specified point in AWT screen coordinates (if one exists) to the draw
     * context's list of picked objects.
     *
     * @param dc the draw context which receives the picked object.
     * @param pickPoint the point in AWT screen coordinates.
     * @param layer the layer associated with the picked object.
    ...
    /**
     * Adds all picked objects that are registered with this PickSupport and
     * intersect the specified rectangle in AWT screen coordinates (if any) to
     * the draw context's list of picked objects.
     *
     ...
     * used to create the pick colors.
     * <p>
     * This returns 0 if the point is <code>null</code>, if the point contains
     * the clear color, or if the point is outside the draw context's drawable
     * area.
     *
     * @param dc the draw context to return a color for.
     * @param pickPoint the point to return a color for, in AWT screen
     * coordinates.
     *
    ...
    /**
     * Adjust this PickSupport's minimum and maximum color codes by decreasing
     * and increasing each extreme, respectively, according to the specified
     * code. If the minimum and maximum color codes are unknown, both the
     ...
     *
     * @return true if both objects are not null and they refer to the same user
