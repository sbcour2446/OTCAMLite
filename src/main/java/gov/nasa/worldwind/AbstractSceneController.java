...
public abstract class AbstractSceneController extends WWObjectImpl implements SceneController {
    ...
    /**
     * The list of picked objects at the current pick point. This list is
     * computed during each call to repaint. Initially <code>null</code>.
     */
    protected PickedObjectList lastPickedObjects;
    /**
     * The list of picked objects that intersect the current pick rectangle.
     * This list is computed during each call to repaint. Initially
     * <code>null</code>.
     */
    protected PickedObjectList lastObjectsInPickRect;
    /**
     * Map of integer color codes to picked objects used to quickly resolve the
     * top picked objects in {@link
     * #doResolveTopPick(gov.nasa.worldwind.render.DrawContext, java.awt.Rectangle)}.
     * This map is used only when a pick rectangle is specified. Initialized to
     * a new HashMap.
     */
    ...
    /**
     * The pick point in AWT screen coordinates, or <code>null</code> if the
     * pick point is disabled. Initially <code>null</code>.
     */
    protected Point pickPoint = null;
    /**
     * The pick rectangle in AWT screen coordinates, or <code>null</code> if the
     * pick rectangle is disabled. Initially <code>null</code>.
     */
    ...
    /**
     * Support class used to build the composite representation of surface
     * objects as a list of SurfaceTiles. We keep a reference to the tile
     * builder instance used to build tiles because it acts as a cache key to
     * the tiles and determines when the tiles must be updated. The tile builder
     * does not retain any references the SurfaceObjects, so keeping a reference
     * to it does not leak memory.
     */
...
//            dc.applyGroupingFilters();
    ...
    /**
     * Called to check for openGL errors. This method includes a "round-trip"
     * between the application and renderer, which is slow. Therefore, this
     * method is excluded from the "normal" render pass. It is here as a matter
     * of convenience to developers, and is not part of the API.
     *
     ...
     * {@link gov.nasa.worldwind.render.SurfaceObject} instances in the draw
     * context's ordered surface renderable queue. While building the composite
     * representation this invokes {@link
     * gov.nasa.worldwind.render.SurfaceObject#render(gov.nasa.worldwind.render.DrawContext)}
     * in ordered rendering mode. This does nothing if the ordered surface
     * renderable queue is empty, or if it does not contain any SurfaceObjects.
     * <p>
     * This method is called during the preRender phase, and is therefore free
     * to modify the framebuffer contents to create the composite
     * representation.
     *
     * @param dc The drawing context containing the surface objects to build a
     * composite representation for.
     *
     ...
     * gov.nasa.worldwind.render.SurfaceObject} instances in the draw context's
     * ordered surface renderable queue. This representation was built during
     * the preRender phase. This does nothing if the ordered surface renderable
     * queue is empty, or if it does not contain any SurfaceObjects.
     *
     * @param dc The drawing context containing the surface objects who's
     * composite representation is drawn.
     */
