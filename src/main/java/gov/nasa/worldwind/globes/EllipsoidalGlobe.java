...
/**
 * Defines a globe modeled as an
 * <a href="http://mathworld.wolfram.com/Ellipsoid.html" target="_blank">ellipsoid</a>.
 * This globe uses a Cartesian coordinate system in which the Y axis points to
 * the north pole. The Z axis points to the intersection of the prime meridian
 * and the equator, in the equatorial plane. The X axis completes a right-handed
 * coordinate system, and is 90 degrees east of the Z axis and also in the
 * equatorial plane. Sea level is at z = zero. By default the origin of the
 * coordinate system lies at the center of the globe, but can be set to a
 * different point when the globe is constructed.
 *
 * @author Tom Gaskins
...
public class EllipsoidalGlobe extends WWObjectImpl implements Globe {
    ...
    /**
     * Create a new globe. The globe's center point will be (0, 0, 0). The globe
     * will be tessellated using tessellator defined by the
     * {@link AVKey#TESSELLATOR_CLASS_NAME} configuration parameter.
     *
    ...
    /**
     * Create a new globe, and set the position of the globe's center. The globe
     * will be tessellated using tessellator defined by the
     * {@link AVKey#TESSELLATOR_CLASS_NAME} configuration parameter.
     *
     * @param equatorialRadius Radius of the globe at the equator.
    ...
    /**
     * Maps a position to world Cartesian coordinates. The Y axis points to the
     * north pole. The Z axis points to the intersection of the prime meridian
     * and the equator, in the equatorial plane. The X axis completes a
     * right-handed coordinate system, and is 90 degrees east of the Z axis and
     * also in the equatorial plane. Sea level is at z = zero.
     *
     * @param latitude the latitude of the position.
     * @param longitude the longitude of the position.
    ...
    /**
     * Maps a position to ellipsoidal coordinates. The Y axis points to the
     * north pole. The Z axis points to the intersection of the prime meridian
     * and the equator, in the equatorial plane. The X axis completes a
     * right-handed coordinate system, and is 90 degrees east of the Z axis and
     * also in the equatorial plane. Sea level is at z = zero.
    ...
    /**
     * Maps a grid of geographic positions to Cartesian coordinates. The Y axis
     * points to the north pole. The Z axis points to the intersection of the
     * prime meridian and the equator, in the equatorial plane. The X axis
     * completes a right-handed coordinate system, and is 90 degrees east of the
     * Z axis and also in the equatorial plane. Sea level is at z = zero.
     * <p>
...
//    }
     ...
     *
     * @see #geodeticToCartesian(gov.nasa.worldwind.geom.Angle,
     ...
     * @return The geographic position of {@code cart}.
     *
     * @see #geodeticToEllipsoidal(gov.nasa.worldwind.geom.Angle,
     * gov.nasa.worldwind.geom.Angle, double)
     */
...
//    }
    ...
    /**
     * Construct an elevation model given a key for a configuration source and
     * the source's default value.
     *
     * @param key the key identifying the configuration property in
     * {@link Configuration}.
     * @param defaultValue the default value of the property to use if it's not
     * found in {@link Configuration}.
     *
     * @return a new elevation model configured according to the configuration
     * source.
     */
