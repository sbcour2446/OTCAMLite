...
/**
 * Parses a COLLADA document and provides access to its contents. Instructions
 * for parsing COLLADA files and streams are given in the Description section of
 * {@link gov.nasa.worldwind.ogc.collada}.
 *
 * @author pabercrombie
...
public class ColladaRoot extends ColladaAbstractObject implements ColladaRenderable, Highlightable {
    ...
    /**
     * Create a new <code>ColladaRoot</code> for a {@link ColladaDoc} instance.
     * A ColladaDoc represents COLLADA files from either files or input streams.
     *
     * @param docSource the ColladaDoc instance representing the COLLADA
     * document.
     *
    ...
    /**
     * Indicates this shape's pitch -- often referred to as tilt -- the angle to
     * rotate this shape's model about its X axis.
     *
     * @return this shape's pitch, or null if no pitch has been specified.
     * Positive values are clockwise as observed looking along the model's X
     * axis toward the model's origin.
     */
    ...
    /**
     * Specifies this shape's pitch -- often referred to as tilt -- the angle to
     * rotate this shape's model about its X axis.
     *
     * @param pitch this shape's pitch. Positive values are clockwise as
     * observed looking along the model's X axis toward the model's origin. May
     * be null.
     */
    ...
    /**
     * Indicates this shape's roll, the angle to rotate this shape's model about
     * its Y axis.
     *
     * @return this shape's roll, or null if no roll has been specified.
     * Positive values are clockwise as observed looking along the model's Y
    ...
    /**
     * Specifies this shape's roll, the angle to rotate this shape's model about
     * its Y axis.
     *
     * @param roll this shape's roll. May be null. Positive values are clockwise
     * as observed looking along the model's Y axis toward the origin.
     */
    ...
    /**
     * Resolves a reference to a local or remote file or element. If the link
     * refers to an element in the current document, this method returns that
     * element. If the link refers to a remote document, this method will
     * initiate asynchronous retrieval of the document, and return a URL of the
     * downloaded document in the file cache, if it is available locally. If the
     * link identifies a COLLADA document, the document will be returned as a
     * parsed ColladaRoot.
     *
     * @param link the address of the document or element to resolve. This may
     * be a full URL, a URL fragment that identifies an element in the current
     * document ("#myElement"), or a URL and a fragment identifier
     ...
     * <p>
     * If {@code linkBase} refers to a local COLLADA file and {@code linkRef} is
     * non-null, the return value is the element identified by {@code linkRef}.
     * If {@code linkRef} is null, the return value is a parsed {@link
     * ColladaRoot} for the COLLADA file identified by {@code linkBase}.
     * Otherwise, {@code linkBase} is returned.
     *
     ...
     * FileStore. If {@code linkBase} refers to a COLLADA file and
     * {@code linkRef} is non-null, the return value is the element identified
     * by {@code linkRef}. If {@code linkBase} refers to a COLLADA file and
     * {@code linkRef} is null, the return value is a parsed {@link
     * ColladaRoot} for the COLLADA file identified by {@code linkBase}.
     * Otherwise the return value is a {@link URL} to the file in the file
     * cache.
     *
     * @param linkBase the address of the document containing the requested
     * element.
     * @param linkRef the element's identifier.
     *
     * @return URL to the requested file, parsed ColladaRoot, or COLLADA
     * element. Returns null if the document is not yet available in the
     * FileStore.
     *
    ...
    /**
     * Determines if a MIME type can be parsed as COLLADA. Parsable types are
     * the COLLADA MIME type, as well as "text/plain" and "text/xml".
     *
     ...
     * be any of those supported by {@link
    ...
    /**
     * Indicates the scale defined by the asset/unit element. This scale
     * converts the document's units to meters.
     *
    ...
    /**
     * Determines the path of a supporting file (such an image). If a resource
     * resolver has been specified, the resource resolver will be invoked to
     * determine the file path. Otherwise, the path will be resolved relative to
     * the COLLADA document's file path or URL.
     *
     * @param link Relative path to resolve.
     *
     * @return Absolute path of the resource, or null if the resource cannot be
     * resolved.
     *
     * @throws IOException If an error occurs while attempting to resolve the
     * resource.
     */
