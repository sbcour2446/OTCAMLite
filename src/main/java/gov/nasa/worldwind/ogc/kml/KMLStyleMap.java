...
/**
 * Represents the KML <i>StyleMap</i> element and provides access to its
 * contents.
 *
 * @author tag
...
public class KMLStyleMap extends KMLAbstractStyleSelector {
     ...
     *
     * @param styleState the style key, either {@link KMLConstants#NORMAL} or
     * {@link KMLConstants#HIGHLIGHT}. If null, {@link KMLConstants#NORMAL} is
     * used.
     *
    ...
    /**
     * Obtains the map's effective style for a specified style type
     * (<i>IconStyle</i>, <i>ListStyle</i>, etc.) and state (<i>normal</i> or
     * <i>highlight</i>). The returned style is the result of merging values
     * from the map's style selectors and style URL for the indicated sub-style
     * type, with precedence given to style selectors.
     * <p>
    ...
    /**
     * Merge a list of incoming pairs with the current list. If an incoming pair
     * has the same ID as an existing one, replace the existing one, otherwise
     * just add the incoming one.
     *
