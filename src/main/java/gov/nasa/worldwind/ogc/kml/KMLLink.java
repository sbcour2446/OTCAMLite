 ...
 * The Link maintains a timestamp that indicates when the resource was last
 * updated. The timestamp is updated based on the link's refresh mode. Code that
 * uses a resource loaded from a Link should keep track of when the resource was
 * retrieved, and periodically check that timestamp against the timestamp
 * returned by {@link #getUpdateTime()}.
 *
 * @author tag
...
public class KMLLink extends KMLAbstractObject {
    ...
    /**
     * The time, in milliseconds since the Epoch, at which the linked content
     * was most recently updated.
     */
    protected AtomicLong updateTime = new AtomicLong();

    /**
     * If this link's href does not need to be modified with a query string,
     * this value is the resource address.
     */
    protected String finalHref;
    /**
     * The {@link URL} for the raw href. Will be null if the href is not a
     * remote URL or this link has a query string
     */
    protected URL hrefURL;

    /**
     * Scheduled task that will update the when it runs. Used to implement
     * {@code onInterval} refresh mode.
     */
    ...
    /**
     * {@inheritDoc} Overridden to mark {@code onChange} links as updated when a
     * field set.
     */
    @Override
    ...
    /**
     * Returns the time at which the linked resource was last updated. This
     * method is safe to call from any thread.
     *
     * @return The time at which the linked content was most recently updated.
     * See {@link System#currentTimeMillis()} for its numerical meaning of this
     * timestamp.
     */
    ...
    /**
     * Specifies the time at which the linked resource was last updated. This
     * method is safe to call from any thread.
     *
     * @param updateTime The time at which the linked content was most recently
     * updated. See {@link
    ...
    /**
     * Specifies the time at which the linked resource expires. If the link's
     * update mode is onExpire, the link will mark itself updated at this time.
     *
     * @param time Time, in milliseconds since the Epoch, at which the link
     * expires. Zero indicates no expiration.
     */
    ...
    /**
     * Schedule a task to refresh the link if the refresh mode requires it. In
     * the case of an {@code onInterval} and {@code onExpire} refresh modes,
     * this method schedules a task to update the link after the refresh
     * interval elapses, but only if such a task has not already been scheduled
     * (only one refresh task is active at a time).
     */
    ...
    /**
     * Schedule the link to refresh when an {@link View#VIEW_STOPPED} message is
     * received, and the link's view refresh mode is {@code onStop}.
     *
    ...
    /**
     * Schedule a task to mark a link as updated after a delay. The task only
     * executes once.
     *
     * @param task Task to schedule.
     * @param delay Delay to wait before executing the task. The time unit is
     * determined by {code timeUnit}.
    ...
    /**
     * {@inheritDoc} Overridden to set a default refresh mode of
     * {@code onChange} if the refresh mode is not specified.
     */
    @Override
    ...
    /**
     * Returns the address of the resource specified by this KML link. If the
     * resource specified in this link's <code>href</code> is a local resource,
     * this returns only the <code>href</code>, and ignores the
     * <code>viewFormat</code> and <code>httpQuery</code>. Otherwise, this
     * returns the concatenation of the <code>href</code>, the
     * <code>viewFormat</code> and the <code>httpQuery</code> for form an
     * absolute URL string. If the the <code>href</code> contains a query
     * string, the <code>viewFormat</code> and <code>httpQuery</code> are
     * appended to that string. If necessary, this inserts the
     * <code>&amp;</code> character between the <code>href</code>'s query
     * string, the <code>viewFormat</code>, and the <code>httpQuery</code>.
     * <p>
     * This substitutes the following parameters in <code>viewFormat</code> and
     * <code>httpQuery</code>: <ul>
     * <li><code>[bboxWest],[bboxSouth],[bboxEast],[bboxNorth]</code> - visible
     * bounds of the globe, or 0 if the globe is not visible. The visible bounds
     * are scaled from their centroid by this link's
     * <code>viewBoundScale</code>.</li>
     * <li><code>[lookatLon], [lookatLat]</code> - longitude and latitude of the
     * position on the globe the view is looking at, or 0 if the view is not
     * looking at the globe.</li>
     * <li><code>[lookatRange]</code> - distance between view's eye position and
     * the point on the globe the view is looking at.</li>
     * <li><code>[lookatTilt], [lookatHeading]</code> - view's tilt and
     * heading.</li>
     * <li><code>[lookatTerrainLon], [lookatTerrainLat], [lookatTerrainAlt]</code>
     * - terrain position the view is looking at, or 0 if the view is not
     * looking at the terrain.</li> <li><code>[cameraLon], [cameraLat],
     * [cameraAlt]</code> - view's eye position.</li>
     * <li><code>[horizFov], [vertFov]</code> - view's horizontal and vertical
     * field of view.</li> <li><code>[horizPixels], [vertPixels]</code> - width
     * and height of the viewport.</li> <li><code>[terrainEnabled]</code> -
     * always <code>true</code></li> <li><code>[clientVersion]</code> -
     * WorldWind client version.</li> <li><code>[clientName]</code> - WorldWind
     * client name.</li>
     * <li><code>[kmlVersion]</code> - KML version supported by WorldWind.</li>
     * <li><code>[language]</code> - current locale's language.</li> </ul> If
     * the <code>viewFormat</code> is unspecified, and the
     * <code>viewRefreshMode</code> is one of <code>onRequest</code>,
     * <code>onStop</code> or <code>onRegion</code>, this automatically appends
     * the following information to the query string:
     * <code>BBOX=[bboxWest],[bboxSouth],[bboxEast],[bboxNorth]</code>. The
     * <code>[clientName]</code> and <code>[clientVersion]</code> parameters of
     * the <code>httpQuery</code> may be specified in the configuration file
     * using the keys <code>{@link gov.nasa.worldwind.avlist.AVKey#NAME}</code>
     * and <code>{@link gov.nasa.worldwind.avlist.AVKey#VERSION}</code>. If not
     * specified, this uses default values of
     * <code>{@link gov.nasa.worldwind.Version#getVersionName()}</code> and <code>{@link
     * gov.nasa.worldwind.Version#getVersion()}</code> for
     * <code>[clientName]</code> and <code>[clientVersion]</code>, respectively.
     *
     * @param dc the <code>DrawContext</code> used to determine the current view
     * parameters.
     *
    ...
    /**
     * Returns whether the resource specified by the <code>url</code> is a local
     * resource.
     *
     * @param url the URL to test.
     *
     * @return <code>true</code> if the <code>url</code> specifies a local
    ...
    /**
     * This returns the concatenation of the query part of <code>href</code> (if
     * any), the <code>viewFormat</code> and the <code>httpQuery</code> to form
     * the link URL's query part. This returns <code>null</code> if this link's
     * <code>href</code> does not specify a URL. This substitutes parameters in
     * <code>viewFormat</code> according to the specified
     * <code>DrawContext</code>'s current viewing parameters, and substitutes
     * parameters in <code>httpQuery</code> according to the current
     * <code>{@link gov.nasa.worldwind.Configuration}</code> parameters.
     *
     * @param dc the <code>DrawContext</code> used to determine the current view
     * parameters.
     *
     * @return the query part of this KML link's address, or <code>null</code>
     * if this link does not specify a URL.
     */
    ...
    /**
     * Returns a <code>Sector</code> that specifies the current visible bounds
     * on the globe. If this link specifies a <code>viewBoundScale</code>, this
     * scales the visible bounds from its centroid by that factor, but limits
     * the bounds to [-90,90] latitude and [-180,180] longitude. This returns <code>{@link
     * gov.nasa.worldwind.geom.Sector#EMPTY_SECTOR}</code> if the globe is not
     * visible.
     *
     * @param dc the <code>DrawContext</code> for which to compute the visible
     * bounds.
     *
     * @return the current visible bounds on the specified
