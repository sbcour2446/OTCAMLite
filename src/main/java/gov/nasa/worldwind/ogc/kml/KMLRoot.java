 ...
 * Instructions for parsing KML/KMZ files and streams are given in the
 * Description section of {@link gov.nasa.worldwind.ogc.kml}.
 *
 * @author tag
...
public class KMLRoot extends KMLAbstractObject implements KMLRenderable {
    ...
    /**
     * The <code>PropertyChangeSupport</code> that receives property change
     * events this KMLRoot listens for, and sends property change events to this
     * KMLRoot's listeners. Lazily initialized in <code>getChangeSupport</code>.
     * Initially <code>null</code>.
     */
    protected PropertyChangeSupport propertyChangeSupport;
    /**
     * Indicates this KML root's detail hint. Modifies the default relationship
     * of KML scene resolution to screen resolution as viewing distance changes.
     * Values greater than 0 increase the resolution. Values less than 0
    ...
    /**
     * Flag to indicate that the network link control element has been fetched
     * from the hash map.
     */
     ...
     * {@link File}, a {@link URL}, a {@link
     * InputStream}, or a {@link String} identifying either a file path or a
     * URL. For all types other than <code>InputStream</code> an attempt is made
     * to determine whether the source is KML or KMZ; KML is assumed if the test
     * is not definitive. Null is returned if the source type is not recognized.
     *
     * @param docSource either a {@link File}, a {@link URL}, or an
     * {@link InputStream}, or a {@link String} identifying a file path or URL.
     *
     ...
     * @param docSource either a {@link File}, a {@link URL}, or an
     * {@link InputStream}, or a {@link String} identifying a file path or URL.
     * @param namespaceAware specifies whether to use a namespace-aware XML
     * parser. <code>true</code> if so, <code>false</code> if not.
     *
     * @return a new {@link KMLRoot} for the specified source, or null if the
     * source type is not supported.
     *
     ...
     * @throws IllegalArgumentException if the source is null.
     * @throws IOException if an error occurs while reading the source.
     * @throws javax.xml.stream.XMLStreamException if the KML file has severe
     * errors.
     */
     ...
     * @param contentType the content type of the stream data. Specify
     * {@link KMLConstants#KML_MIME_TYPE} for plain KML and
     * {@link KMLConstants#KMZ_MIME_TYPE} for KMZ. The content is treated as KML
     * for any other value or a value of null.
     *
     * @throws IllegalArgumentException if the document source is null.
     * @throws IOException if an error occurs while reading the KML document.
     */
     ...
     * @param contentType the content type of the data. Specify
     * {@link KMLConstants#KML_MIME_TYPE} for plain KML and
     * {@link KMLConstants#KMZ_MIME_TYPE} for KMZ. Any other non-null value
     * causes the content to be treated as plain KML. If null is specified the
     * content type is read from the server or other end point of the URL. When
     * a content type is specified, the content type returned by the URL's end
     * point is ignored. You can therefore force the content to be treated as
     * KML or KMZ regardless of what a server declares it to be.
     *
     * @throws IllegalArgumentException if the document source is null.
     ...
     * namespace is defined by {@link
     * gov.nasa.worldwind.ogc.kml.KMLConstants#KML_NAMESPACE}).
     *
     * @param namespaceURI the default namespace URI.
     * @param docSource the KML source specified via a {@link KMLDoc} instance.
     * A KMLDoc represents KML and KMZ files from either files or input streams.
     *
     * @throws IllegalArgumentException if the document source is null.
     ...
     * sure to invoke <code>super.initialize(boolean)</code>.
     *
     * @param namespaceAware specifies whether to use a namespace-aware XML
     * parser. <code>true</code> if so, <code>false</code> if not.
     *
     ...
     *
     * @param docSource the document source to create a reader for. The type can
     * be any of those supported by {@link WWXML#openEventReader(Object)}.
     * @param namespaceAware specifies whether to use a namespace-aware XML
     * parser. <code>true</code> if so, <code>false</code> if not.
     *
     * @return a new event reader, or null if the source type cannot be
     * determined.
     */
    ...
    /**
     * Invoked during {@link #initialize(boolean)} to create the parser context.
     * The parser context is created by the global
    ...
    /**
     * Specifies the object to receive notifications of important occurrences
     * during parsing, such as exceptions and the occurrence of unrecognized
     * element types.
     * <p>
     * The default notification listener writes a message to the log, and
     * otherwise does nothing.
     *
     * @param listener the listener to receive notifications. Specify null to
     * indicate no listener.
     *
     ...
     *
     * @return the element requested, or null if there is no corresponding
    ...
    /**
     * Resolves a reference to a remote or local element of the form
     * address#identifier, where "address" identifies a local or remote
     * document, including the current document, and and "identifier" is the id
     * of the desired element.
     * <p>
     * If the address part identifies the current document, the document is
     * searched for the specified identifier. Otherwise the document is
     * retrieved, opened and searched for the identifier. If the address refers
     * to a remote document and the document has not previously been retrieved
     * and cached locally, retrieval is initiated and this method returns
     * <code>null</code>. Once the document is successfully retrieved,
     * subsequent calls to this method return the identified element, if it
     * exists.
     * <p>
     * If the link does not contain an identifier part, this initiates a
     * retrieval for document referenced by the address part and returns
     * <code>null</code>. Once the document is retrieved this opens the the
     * document as a <code>KMLRoot</code>. Subsequent calls to this method
     * return the opened document, if it exists.
     *
     * @param link the document address in the form address#identifier.
     *
     * @return the requested document, the requested or element within a
     * document, or <code>null</code> if the document or the element are not
     * found.
     *
     * @throws IllegalArgumentException if the <code>link</code> is
     * <code>null</code>.
     */
     ...
     * return the opened document, if it exists.
     * <p>
     * The <code>cacheRemoteFile</code> parameter specifies whether to store a
     * retrieved remote document in the World Wind cache or in a temporary
     * location. This parameter has no effect if the document exists locally.
     * The temporary location for a retrieved document does not persist between
     * runtime sessions, and subsequent invocations of this method may not
     * return the same temporary location.
     *
     * @param link the document address in the form address#identifier.
     * @param cacheRemoteFile <code>true</code> to store remote documents in the
     * WorldWind cache, or <code>false</code> to store remote documents in a
     * temporary location. Has no effect if the address is a local document.
     *
     * @return the requested document, the requested or element within a
     * document, or <code>null</code> if the document or the element are not
     * found.
    ...
    /**
     * Resolves a reference to a local element identified by address and
     * identifier, where {@code linkBase} identifies a document, including the
     * current document, and {@code linkRef} is the id of the desired element.
     * <p>
     * If {@code linkBase} refers to a local KML or KMZ file and {@code linkRef}
     * is non-null, the return value is the element identified by
     * {@code linkRef}. If {@code linkRef} is null, the return value is a parsed
     * {@link KMLRoot} for the KML file identified by {@code linkBase}.
     * <p>
     * If {@code linkBase} refers a local file that is not a KML or KMZ file
     * then {@code linkBase} is returned. If {@code linkBase} cannot be resolved
     * to a local file then null is returned.
     *
     * @param linkBase the address of the document containing the requested
     * element.
    ...
    /**
     * Resolves a reference to a remote element identified by address and
     * identifier, where {@code linkBase} identifies a remote document, and
     * {@code linkRef} is the id of the desired element. This method retrieves
     * resources asynchronously using the
     * {@link gov.nasa.worldwind.cache.FileStore}.
     * <p>
     * The return value is null if the file is not yet available in the
     * FileStore. If {@code linkBase} refers to a KML or KMZ file and
     * {@code linkRef} is non-null, the return value is the element identified
     * by {@code linkRef}. If {@code linkBase} refers to a KML or KMZ and
     * {@code linkRef} is null, the return value is a parsed {@link KMLRoot} for
     * the KML file identified by {@code linkBase}. Otherwise the return value
     * is a {@link URL} to the file in the file cache.
     *
     * @param linkBase the address of the document containing the requested
     * element.
     * @param linkRef the element's identifier.
     *
     * @return URL to the requested file, parsed KMLRoot, or KML feature.
     * Returns null if the document is not yet available in the FileStore.
     *
     ...
     * {@code linkRef} is null, the return value is a parsed {@link KMLRoot} for
     * the KML file identified by {@code linkBase}. Otherwise the return value
     * is a {@link URL} to the file in the file cache or a temporary location,
     * depending on the value of <code>cacheRemoteFile</code>.
     * <p>
     * The <code>cacheRemoteFile</code> parameter specifies whether to store a
     * retrieved remote file in the WorldWind cache or in a temporary location.
     * This parameter has no effect if the file exists locally. The temporary
     * location for a retrieved file does not persist between runtime sessions,
     * and subsequent invocations of this method may not return the same
     * temporary location.
     *
     * @param linkBase the address of the document containing the requested
     * element.
     * @param linkRef the element's identifier.
     * @param cacheRemoteFile <code>true</code> to store remote files in the
     * WorldWind cache, or <code>false</code> to store remote files in a
     * temporary location. Has no effect if the address is a local file.
     *
     * @return URL to the requested file, parsed KMLRoot, or KML feature.
     * Returns null if the document is not yet available in the FileStore.
     *
    ...
    /**
     * Resolves a NetworkLink to a local or remote KML document. This method
     * retrieves remote resources asynchronously using the
     * {@link gov.nasa.worldwind.cache.FileStore}.
     * <p>
     * The return value is a parsed KMLRoot representing the linked document.
     * The return value is null if the linked file is not a KML file, or is not
     ...
     * WorldWind cache, or <code>false</code> to store remote files in a
     * temporary location. Has no effect if the address is a local file.
     * @param updateTime the time at which the link was last updated. If a
     * cached file exists for the specified resource, the file must have been
     * retrieved after the link update time. Otherwise, the cache entry is
     * considered invalid, and the file is deleted and retrieved again.
     *
     * @return URL to the requested file, parsed KMLRoot, or KML feature.
     * Returns null if the document is not yet available in the FileStore.
     *
    ...
    /**
     * Check a cached resource for expiration. If the resource is expired, evict
     * it from the cache.
     *
     * @param link Link that identifies the resource to check for expiration.
     * This is the same link that was passed to resolveReference to retrieve the
     * resource.
     * @param expirationTime Time at which the resource expires, in milliseconds
     * since the Epoch. If the current system time is greater than the
     * expiration time, then the resource will be evicted.
     */
     ...
     * {@link #resolveReference(String) resolveReference} or {@link
     * #resolveNetworkLink(String, boolean, long) resolveNetworkLink}.
     *
     * @param link the address of the file (the same address as was previously
     * passed to resolveReference). If null, zero is returned.
     *
     * @return The expiration time of the file, in milliseconds since the Epoch
     * (January 1, 1970, 00:00:00 GMT). Zero indicates that there is no
     * expiration time. Returns zero if te resource identified by {@code link}
    ...
    /**
     * Determines if a MIME type can be parsed as KML or KMZ. Parsable types are
     * the KML and KMZ MIME types, as well as "text/plain" and "text/xml".
     *
     ...
     *
     * @param url the URL of the file to open, expressed as a URL with a scheme
     * of "file".
     * @param linkBase the original address of the document if the file is a
     * retrieved and cached file.
     * @param contentType the mime type of the file's content, either a KML or
     * KMZ mime type.
     * @param namespaceAware specifies whether to use a namespace aware event
     * reader.
     *
    ...
    /**
     * Starts document parsing. This method initiates parsing of the KML
     ...
     * attempting to read the event stream.
     */
    ...
    /**
     * Returns the <code>hint</code> attribute of the <code>KML</code> element
     * (the document root).
     *
    ...
    /**
     * Returns the {@link gov.nasa.worldwind.ogc.kml.KMLNetworkLinkControl}
     * element if the document root contains it.
     *
    ...
    /**
     * Returns the KML <code>Feature</code> element contained in the document
     * root.
     *
     * @return the feature element if it is specified in the document, otherwise
     * null.
     */
     ...
     * <code>{@link #setDetailHint(double)}</code>.
     *
    ...
    /**
     * Specifies this KML root's detail hint. The detail hint modifies the
     * default relationship of KML scene resolution to screen resolution as the
     * viewing distance changes. Values greater than 0 cause KML elements with a
     * level of detail to appear at higher resolution at greater distances than
     * normal, but at an increased performance cost. Values less than 0 decrease
     * the default resolution at any given distance. The default value is 0.
     * Values typically range between -0.5 and 0.5.
     * <p>
     * The top level KML root's detail hint is inherited by all KML elements
     * beneath that root, including any descendant KML roots loaded by network
     * links. If this KML root has been loaded by a network link, its detail
     * hint is ignored.
     *
     * @param detailHint the degree to modify the default relationship of KML
     * scene resolution to screen resolution as viewing distance changes. Values
     * greater than 0 increase the resolution. Values less than 0 decrease the
    ...
    /**
     * Adds the specified property change listener that will be called for all
     * list changes.
     *
    ...
    /**
     * Get the PropertyChangeSupport object for this KML object. The support
