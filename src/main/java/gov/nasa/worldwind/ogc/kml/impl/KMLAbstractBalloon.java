...
/**
 * An implementation of {@link Balloon} that applies a {@link KMLBalloonStyle}
 * to the balloon. Rather than fully implementing the Balloon interface, this
 * class provides a thin wrapper around another Balloon implementation and adds
 * the logic for styling the Balloon according to the KML style. All Balloon
 * methods on this class pass through to the contained Balloon.
 * <p>
 * To use KML Balloon, first create a Balloon of the desired type, and then
 * create the KML Balloon. For example:
 * <pre>
 * <code>
...
public abstract class KMLAbstractBalloon implements Balloon, WebResourceResolver, PropertyChangeListener {
    ...
    /**
     * Indicates that the balloon has default text loaded, rather than text
     * supplied by the BalloonStyle.
     */
    ...
    /**
     * Render the balloon. This method will attempt to resolve the balloon
     * style, if it has not already been resolved.
     *
    ...
    /**
     * Determine the balloon text for this frame, depending on the balloon
     * highlight state.
     */
     ...
     *
     * @param attrType Type of attributes to update. Either
     * {@link KMLConstants#NORMAL} or {@link
    ...
    /**
     * Build a default balloon text string for the feature's extended data. This
     * implementation builds a simple data table.
     *
    ...
    /**
     * Build a default balloon text string for the feature's schema data. This
     * implementation builds a simple data table.
     *
     * @param sb Extended data string will be appended to this StringBuilder.
     ...
     * The Google KML specification states the GE will add hyperlinks to balloon
     * text that does not contain HTML formatting. This method searches for a
     * &lt;html&gt; tag in the content to determine if the content is HTML or
     * plain text.
     *
     * @param text Balloon text to process.
     *
     * @return True if URLs should be converted links. Returns true if a
     * &lt;html&gt; tag is found in the text.
     */
    ...
    /**
     * Add hyperlink tags to URLs in the balloon text. The text may include some
     * simple HTML markup. This method attempts to identify URLs in the text
     * while not altering URLs that are already linked.
     * <p>
     * This method is conservative about what is identified as a URL, in order
     * to avoid adding links to text that the user did not intend to be linked.
     * Only HTTP and HTTPS URLs are recognised, as well as text that begins with
     * www. (in which case a http:// prefix will be prepended). Some punctuation
     * characters that are valid URL characters (such as parentheses) are not
     * treated as URL characters here because users may expect the punctuation
     * to separate the URL from text.
     *
     * @param text Text to process. Each URL in the text will be replaced with
    ...
    /**
     * Get the default attributes applied to the balloon. These attributes will
     * be modified by {@link
     * #assembleBalloonAttributes(gov.nasa.worldwind.ogc.kml.KMLBalloonStyle, gov.nasa.worldwind.render.BalloonAttributes)
     * assembleBalloonAttributes} to reflect the settings in the KML
     * <i>BalloonStyle</i>.
     *
    ...
    /**
     * Get the balloon display mode, either {@link #DISPLAY_MODE_DEFAULT} or
     * {@link #DISPLAY_MODE_HIDE}.
     *
    ...
    /**
     * Set the balloon's display mode, either {@link #DISPLAY_MODE_DEFAULT} or
     * {@link #DISPLAY_MODE_HIDE}. When the mode is {@link #DISPLAY_MODE_HIDE},
     * the balloon will not be drawn.
     *
     ...
     * gov.nasa.worldwind.ogc.kml.io.KMLDoc#getSupportFilePath(String)}</code>
     * on the parent <code>KMLAbstractFeature's</code> <code>KMLDoc</code>. This
     * is necessary to correctly resolve relative references in a KMZ archive.
     * <p>
     * This returns <code>null</code> if the specified <code>address</code> is
     * <code>null</code>.
     */
