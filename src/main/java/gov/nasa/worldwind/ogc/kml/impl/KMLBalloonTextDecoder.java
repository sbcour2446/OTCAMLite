...
public class KMLBalloonTextDecoder extends BasicTextDecoder {
    ...
    /**
     * True if there are entities in the balloon text which may refer to
     * unresolved schema. False if all entities have been resolved, or are known
     * to be unresolvable (because the data they refer to does not exist).
     */
    protected boolean isUnresolved;

    /**
     * Keep a cache of entities that have been resolved so that we don't have to
     * re-resolve them every time the decoded text is requested.
     */
    ...
    /**
     * Create a decoder to generate balloon text for a feature. The feature
     * becomes the context of the entity replacements (for example "$[name]"
     * will be replaced with the feature's name.
     *
    ...
    /**
     * Get the balloon text after entity substitutions (for example, $[name],
     * $[description], etc) have been made. See the KML specification for
     * details on entity replacement. If some entities could not be resolved
     * because they refer to unresolved schema, the decoder will try to decode
     * the string again the next time this method is called.
     *
     * @return String after replacements have been made. Returns null if the
     * input text is null.
     */
    @Override
     ...
     *
     * @return Return the replacement string for the entity, or null if no
    ...
    /**
     * Get the text used to replace the $[geDirections] entity. This
     * implementation returns an empty string. Subclasses can override this
     * method to provide directions text.
     *
