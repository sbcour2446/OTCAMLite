...
public interface KMLConstants {
    ...
    /**
     * The KML view refresh mode <code>never</code>. Indicates that a resource
     * referenced by a <code>KMLLink</code> should ignore changes in the
     * geographic view, and ignore the link's <code>viewFormat</code> property.
     */
    final String NEVER = "never";
    /**
     * The KML view refresh mode <code>onRequest</code>. Indicates that a
     * resource referenced by a <code>KMLLink</code> should refresh only when
     * the user explicitly requests it.
     */
    final String ON_REQUEST = "onRequest";
    /**
     * The KML view refresh mode <code>onStop</code>. Indicates that a resource
     * referenced by a <code>KMLLink</code> should refresh at an elapsed time
     * after view movement stops. The time is specified by the link's
     * <code>viewRefreshTime</code> property.
     */
    final String ON_STOP = "onStop";
    /**
     * The KML view refresh mode <code>onRegion</code>. Indicates that a
     * resource referenced by a <code>KMLLink</code> should refresh if a certain
     * <code>KMLRegion</code> becomes active. The <code>KMLRegion</code> is
     * specified by an ancestor of the <code>KMLLink</code>.
     */
    final String ON_REGION = "onRegion";
    /**
     * The KML link refresh mode <code>onInterval</code>. Indicates that a
     * resource referenced by a <code>KMLLink</code> should refresh
     * periodically.
     */
    final String ON_INTERVAL = "onInterval";
    /**
     * The KML view refresh mode <code>onExpire</code>. Indicates that a
     * resource referenced by a <code>KMLLink</code> should refresh when the
     * resource expires. The expiration time can be set by a HTTP header, or by
     * a {@link
     * KMLNetworkLinkControl} element.
     */
    final String ON_EXPIRE = "onExpire";
    /**
     * The KML view refresh mode <code>onChange</code>. Indicates that a
     * resource referenced by a <code>KMLLink</code> should refresh when the
     * file containing the link is loaded, or when the link parameters change.
     */
