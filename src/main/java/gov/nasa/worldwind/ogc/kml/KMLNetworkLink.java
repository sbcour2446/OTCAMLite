...
/**
 * Represents the KML <i>NetworkLink</i> element and provides access to its
 * contents.
 * <p>
 * During rendering, <code>KMLNetworkLink</code> retrieves and loads its network
 * resource whenever necessary. Upon a successful retrieval,
 * <code>KMLNetworkLink</code> sends an
 * <code>AVKey.RETRIEVAL_STATE_SUCCESSFUL</code> property change event to this
 * link's property change listeners. Once retrieved and loaded,
 * <code>KMLNetworkLink</code> stores its network resource by calling
 * <code>setNetworkResource</code>, draws its network resource during
 * preRendering and rendering, and forwards property change events from the
 * network resource to its property change listeners.
 * <p>
 * During retrieval, <code>KMLNetworkLink</code> attempts to use either the
 * <code>Link</code> or the <code>Url</code>. The <code>Link</code> is the
 * preferred method for encoding a KML NetworkLink's address since KML version
 * 2.1, therefore we give it priority over <code>Url</code>.
 *
 * @author tag
...
public class KMLNetworkLink extends KMLAbstractContainer implements PropertyChangeListener {
     ...
     * <code>KMLNetworkLink</code>. Initially <code>null</code>.
     */
     ...
     * <code>KMLNetworkLink's</code> network resource was last retrieved.
     * Initially <code>-1</code>, indicating that the network resource has not
     * been retrieved.
     */
    ...
    /**
     * Flag that is raised if the link retrieves a files that is not a KML
     * document. In this case, do not attempt to retrieve the resource again
     * until the link changes. If the link changes then target file may also
     * have changed.
     */
    protected boolean invalidTarget;

    /**
     * Cache the root of this network link. Accessing the root by climbing up a
     * deep network link tree can be a performance bottleneck.
     */
    ...
    /**
     * Forward property change events from the inner {@link KMLRoot} to this
     * object's parent KMLRoot.
     *
    ...
    /**
     * {@inheritDoc} Overridden to cache the root instead of climbing the parse
     * tree each time.
     */
    @Override
    ...
    /**
     * Returns this <code>NetworkLink's</code> <code>Link</code> element or its
     * <code>Url</code> element, depending on which is available.
     * <code>Link</code> is the preferred method for encoding a
     * <code>NetworkLink</code> since KML version 2.1, therefore we give it
     * priority. If a <code>Link</code> is not specified, the deprecated
     * <code>Url</code> element is returned.
     *
     * @return this <code>NetworkLink's</code> <code>Link</code> element, if one
     * is specified. Otherwise, this <code>NetworkLink's</code> <code>Url</code>
     * element. Returns <code>null</code> if neither <code>Link</code> or a
    ...
    /**
     * Indicates the network resource referenced by this
     * <code>KMLNetworkLink</code>. This returns <code>null</code> if this link
     * has no resource.
     *
     * @return this link's network resource, or <code>null</code> to indicate
     * that this link has no resource.
     *
    ...
    /**
     * {@inheritDoc} Overridden to apply region culling according to the same
     * rules as in KMLAbstractFeature. This prevents retrieving network links in
     * inactive regions.
     */
    @Override
     ...
     * <code>KMLNetworkLink</code>, or <code>null</code> if this link has no
     * resource. If the specified <code>kmlRoot</code> is not <code>null</code>,
     * this link draws the <code>kmlRoot</code> during preRendering and
     * rendering, and forwards property change events from the
     * <code>kmlRoot</code> to this link's property change listeners.
     *
     * @param kmlRoot the network resource referenced by this
     * <code>KMLNetworkLink</code>. May be <code>null</code>.
     *
     ...
     * <code>KMLNetworkLink</code>. If this link must retrieve its network
     * resource, this initiates a retrieval and does nothing until the resource
     * is retrieved and loaded. Once the network resource is retrieved and
     * loaded, this calls <code>{@link #setNetworkResource(KMLRoot)}</code> to
     * specify this link's new network resource, and sends an
     * <code>{@link gov.nasa.worldwind.avlist.AVKey#RETRIEVAL_STATE_SUCCESSFUL}</code>
     ...
     * <code>KMLNetworkLink</code>. This does nothing if this link has no
     * network resource.
     *
     * @param tc the current KML traversal context.
     * @param dc the current draw context.
    ...
    /**
     * Returns whether this <code>KMLNetworkLink</code> must retrieve its
     * network resource. This always returns <code>false</code> if this
     * <code>KMLNetworkLink</code> has no <code>KMLLink</code>.
     *
     * @return <code>true</code> if this <code>KMLNetworkLink</code> must
     * retrieve its network resource, otherwise <code>false</code>.
     */
    ...
    /**
     * Thread's off a task to determine whether the resource is local or remote
     * and then retrieves it either from disk cache or a remote server.
     *
     * @param dc the current draw context.
     */
     ...
     * <code>KMLNetworkLink</code>. Once the network resource is retrieved and
     * loaded, this calls <code>{@link #setNetworkResource(KMLRoot)}</code> to
     * specify this link's new network resource, and sends an
     * <code>{@link gov.nasa.worldwind.avlist.AVKey#RETRIEVAL_STATE_SUCCESSFUL}</code>
     * property change event to this link's property change listeners.
     * <p>
     * This does nothing if this <code>KMLNetworkLink</code> has no
     * <code>KMLLink</code>.
     *
    ...
    /**
     * Indicates the expiration time of a linked resource. The expiration time
     * is specified by (in order of priority): a NetworkLinkControl/expires
     * element in the target document, a HTTP Cache-Control header, or an HTTP
     * Expires header.
     *
     * @param root Root of target resource.
     * @param address Address of linked resource.
     *
     * @return The expiration time of the resource, in milliseconds since the
     * Epoch. Zero indicates that there is no expiration time.
     */
     ...
     * <code>KMLNetworkLink</code> should be retrieved to the WorldWind cache or
     * to a temporary location. This returns <code>true</code> if all of the
     * following conditions are met, and <code>false</code> otherwise:
     * <ul> <li>This network link has either a <code>Link</code> or a
     * <code>Url</code> element.</li> <li>The Link or Url element's
     * <code>refreshMode</code> is not <code>onInterval</code> or
     * <code>onExpire</code>.</li> <li>The Link or Url element's
     * <code>viewRefreshMode</code> is not <code>onStop</code>.</li> </ul>
     *
     * @return <code>true</code> if this link's network resource can should be
     * stored in a cache, or <code>false</code> if it should be stored in a
     * temporary location.
     */
    ...
    /**
     * Attempts to find this network link resource file locally, and if that
     * fails attempts to find it remotely.
     */
