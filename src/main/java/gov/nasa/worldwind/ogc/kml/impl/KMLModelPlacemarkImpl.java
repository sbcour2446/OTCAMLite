...
public class KMLModelPlacemarkImpl extends WWObjectImpl implements KMLRenderable, ColladaResourceResolver {
    ...
    /**
     * Time, in milliseconds since the Epoch, at which this placemark's model
     * resource was last retrieved. Initially <code>-1</code>, indicating that
     * the resource has not been retrieved.
     */
    protected AtomicLong resourceRetrievalTime = new AtomicLong(-1);

    /**
     * Map specified by the KML Model's ResourceMap element. The map relates
     * relative references within the COLLADA file to paths relative to the KML
     * document.
     */
     ...
     * @param placemark the <i>Placemark</i> element containing the
     * <i>Point</i>.
     * @param geom the {@link gov.nasa.worldwind.ogc.kml.KMLPoint} geometry.
     *
     * @throws NullPointerException if the geometry is null.
     * @throws IllegalArgumentException if the parent placemark or the traversal
     * context is null.
     */
     ...
     *
     * @return Map that relates relative paths in the COLLADA document to paths
     * relative to the KML document.
     */
     ...
     * <code>null</code> if this placemark has no resource.
     *
     * @param root the Collada resource referenced by this placemark. May be
     * <code>null</code>.
     */
    ...
    /**
     * Indicates the Collada resource referenced by this placemark. This returns
     * <code>null</code> if this placemark has no resource.
     *
     * @return this placemark's Collada resource, or <code>null</code> to
     * indicate that this placemark has no resource.
     *
    ...
    /**
     * Resolve COLLADA references relative to the COLLADA document. If the
     * reference is relative then it will resolved relative to the .dae file,
     * not the kml file. If the COLLADA document may be contained in a KMZ
     * archive the resources will be resolved relative to the .dae file within
     * the archive. Normally references in a KMZ are resolved relative to the
     * root of the archive, but Model references are an exception. See
    ...
    /**
     * Returns whether this placemark must retrieve its model resource. This
     * always returns <code>false</code> if this placemark has no
     * <code>KMLLink</code>.
     *
     * @return <code>true</code> if this placemark must retrieve its model
    ...
    /**
     * Initiates a retrieval of the model referenced by this placemark.Once the
     * resource is retrieved and loaded, this calls
     * <code>{@link #setColladaRoot(ColladaRoot)}</code> to specify this link's
     * new network resource, and sends an
     * <code>{@link gov.nasa.worldwind.avlist.AVKey#RETRIEVAL_STATE_SUCCESSFUL}</code>
     * property change event to this link's property change listeners.<p>
    ...
    /**
     * Attempts to find this model link resource file locally, and if that fails
    ...
    protected static class RequestTask implements Runnable {
         ...
         *
         * @param placemark the placemark for which to construct the request
         * task.
