...
/**
 * Represents the KML <i>Container</i> element and provides access to its
 * contents.
 *
 * @author tag
...
public class KMLAbstractContainer extends KMLAbstractFeature {
    ...
    /**
     * Indicates whether this <code>KMLAbstractContainer</code> is active and
     * should be rendered on the specified <code>DrawContext</code>. This
     * returns <code>true</code> if this container's <code>visibility</code> is
     ...
     * features can override the container's Region with its own. Since a
     * descendant Region may be larger or have a less restrictive LOD range than
     * this container, we cannot determine the visibility of the entire tree
     * based on this container's Region. A container's Region is inherited by
     * any descendants that do not specify a Region, and Region visibility is
     * determined at the leaf nodes.
     *
     * @param tc the current KML traversal context. This parameter is ignored.
     * @param dc the draw context. This parameter is ignored.
     *
     * @return <code>true</code> if this container should be rendered, otherwise
     * <code>false</code>.
     */
    @Override
     ...
     * <code>KMLAbstractContainer</code>.
     * <p>
     * Pushes this container's Region on the KML traversal context's region
     * stack before rendering the features, and pops the Region off the stack
     * afterward. Descendant features use the KML traversal context's region
     * stack to inherit Regions from parent containers.
     *
    ...
    /**
     * Prepares this KML container for rendering. This pushes this container's
     * Region on the KML traversal context's region stack. Descendant features
     * use the KML traversal context's region stack to inherit Regions from
     * parent containers. This must be followed by a call to
     * <code>endRendering</code>.
     *
     * @param tc the current KML traversal context.
     * @param dc the current draw context.
     */
    ...
    /**
     * Restores any rendering state changed during rendering. This pops this
     * container's Region off the KML traversal context's region stack to
     * restore the previous container's Region (if any). This must be preceded
     * by a call to <code>beginRendering</code>.
     *
     * @param tc the current KML traversal context.
     * @param dc the current draw context.
     */
    ...
    /**
     * PreRenders this KML container's list of KML features, in the order they
     * appear in the list. This does nothing if the list of features is empty.
     *
     * @param tc the current KML traversal context.
     * @param dc the current draw context.
     */
    ...
    /**
     * Draws this KML container's list of KML features, in the order they appear
     * in the list. This does nothing if the list of features is empty.
     *
     * @param tc the current KML traversal context.
     * @param dc the current draw context.
     */
    ...
    /**
     * Merge a list of incoming features with the current list. If an incoming
     * feature has the same ID as an existing one, replace the existing one,
     * otherwise add the incoming one.
     *
