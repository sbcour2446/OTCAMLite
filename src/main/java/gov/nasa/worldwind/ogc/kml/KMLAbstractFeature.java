...
/**
 * Represents the KML <i>Feature</i> element and provides access to its
 * contents.
 * <p>
 * <code>KMLAbstractFeature</code> implements the <code>KMLRenderable</code>
 * interface, but does not actually render anything. Subclasses should override
 * the methods <code>{@link #doPreRender(gov.nasa.worldwind.ogc.kml.impl.KMLTraversalContext,
 * gov.nasa.worldwind.render.DrawContext)}</code> and <code>{@link #doRender(gov.nasa.worldwind.ogc.kml.impl.KMLTraversalContext,
 * gov.nasa.worldwind.render.DrawContext)}</code> to render their contents. If
 * the <code>visibility</code> property is set to <code>false</code>, this does
 * not call <code>doPreRender</code> and <code>doRender</code> during rendering.
 *
 * @author tag
...
public abstract class KMLAbstractFeature extends KMLAbstractObject implements KMLRenderable {
    ...
    /**
     * The style selectors specified in the KML Feature element. Is empty if no
     * selectors were specified.
     */
    protected List<KMLAbstractStyleSelector> styleSelectors = new ArrayList<KMLAbstractStyleSelector>();
    /**
     * The visibility flag for the feature. This field is determined from the
     * visibility element of the KML feature initially, but the client may set
     * it directly, in which case it may then differ from the visibility field
     * in the <code>fields</code> table.
     */
    protected Boolean visibility; // may be different from the visibility field if application has set it explicitly
    /**
     * The region specified in the KML Feature element. Is null if no region was
     * specified.
     */
    protected KMLRegion region;
    /**
     * A balloon explicitly associated with this feature by the client. This is
     * not a KML field of the Feature element.
     */
    ...
    /**
     * Indicates whether this <code>KMLAbstractFeature</code> is enabled for
     * rendering. This returns <code>null</code> if no visibility is specified.
     * This indicates the default visibility of <code>true</code> should be
     * used.
     *
     * @return <code>true</code> or <code>null</code> to draw feature shape,
     * otherwise <code>false</code>. The default value is <code>true</code>.
     *
    ...
    /**
     * Specifies whether this <code>KMLAbstractFeature</code> is enabled for
     * rendering.
     *
     * @param visibility <code>true</code> or <code>null</code> to draw this
     * feature, otherwise <code>false</code>. The default value is
     * <code>true</code>.
     *
     * @see #getVisibility()
     */
     ...
     * <p>
     * Note: Balloon is not a field in the KML Feature element. It's a direct
     * field of this class and enables the client to associate a balloon with
     * the feature.
     *
     ...
     *
     * @return The balloon associated with the feature. Returns null if there is
     * no associated balloon.
     */
    ...
    /**
     * Indicates whether this <code>KMLAbstractFeature</code> is active and
     * should be rendered on the specified <code>DrawContext</code>. This
     * returns <code>true</code> if the following conditions are all
     * <code>true</code>:
     * <ul> <li>This feature's <code>visibility</code> is unspecified
     * (<code>null</code>) or is set to <code>true</code>.</li> <li>This feature
     * as no Region and does not inherit a Region from an ancestor, or its
     * Region is active for the specified <code>DrawContext</code>.</li> </ul>
     * <p>
     * If this feature has no Region, this inherits the Region of its nearest
     * ancestor by using the Region on the top of the KML traversal context's
     * region stack (if any). If there is no ancestor Region this feature is
     * assumed to be the <code>DrawContext's</code> view and is rendered
     * according to its <code>visibility</code> flag. A Region is considered
     * active if it is visible, and the <code>DrawContext</code> meets the
     * Region's level of detail criteria.
     *
     * @param tc the current KML traversal context. Specifies an inherited
     * Region (if any) and a detail hint.
     * @param dc the current draw context. Used to determine whether this
     * feature's Region is active.
     *
     * @return <code>true</code> if this feature should be rendered, otherwise
     * <code>false</code>.
     */
    ...
    /**
     * Called from <code>preRender</code> if this
     * <code>KMLAbstractFeature</code>'s <code>visibility</code> is not set to
     * <code>false</code>. Subclasses should override this method to pre-render
     * their content.
     *
    ...
    /**
     * Called from <code>render</code> if this <code>KMLAbstractFeature</code>'s
     * <code>visibility</code> is not set to <code>false</code>. Subclasses
     * should override this method to render their content.
     *
     * @param tc the current KML traversal context.
     * @param dc the current draw context.
     */
    ...
    /**
     * Draws the <code>{@link gov.nasa.worldwind.render.Balloon}</code>
     * associated with this KML feature. This does nothing if there is no
    ...
    /**
     * Obtains the effective values for a specified sub-style (<i>IconStyle</i>,
     * <i>ListStyle</i>, etc.) and state (<i>normal</i> or <i>highlight</i>).
     * The returned style is the result of merging values from this feature
     * instance's style selectors and its styleUrl, if any, with precedence
     * given to style selectors.
     * <p>
     * A remote <i>styleUrl</i> that has not yet been resolved is not included
     * in the result. In this case the returned sub-style is marked with the
     * value {@link gov.nasa.worldwind.avlist.AVKey#UNRESOLVED}. The same is
     * true when a StyleMap style selector contains a reference to an external
     * Style and that reference has not been resolved.
     *
     * @param styleState the style mode, either \"normal\" or \"highlight\".
     * @param subStyle an instance of the sub-style desired, such as
     * {@link gov.nasa.worldwind.ogc.kml.KMLIconStyle}. The effective sub-style
     * values are accumulated and merged into this instance. The instance should
     * not be one from within the KML document because its values are overridden
     * and augmented; it's just an independent variable in which to return the
     * merged attribute values. For convenience, the instance specified is
     * returned as the return value of this method.
     *
     * @return the sub-style values for the specified type and state. The
     * reference returned is the one passed in as the <code>subStyle</code>
     * argument.
     */
    ...
    /**
     * Merge a list of incoming style selectors with the current list. If an
     * incoming selector has the same ID as an existing one, replace the
     * existing one, otherwise just add the incoming one.
     *
