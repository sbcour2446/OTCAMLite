...
public class KMZInputStream implements KMLDoc {
    ...
    /**
     * The next entry to read from the input stream. This field refers to the
     * first entry in the stream when the constructor returns, and after each
     * entry is copied to the temp dir it is updated to point to the subsequent
     * entry. If null, the stream contains no more entries.
     */
    protected ZipEntry currentEntry;

    /**
     * A mapping of the files in the KMZ stream to their location in the
     * temporary directory.
     */
    protected Map<String, File> files;

    /**
     * The directory to hold files copied from the stream. Both the directory
     * and the files copied there are temporary.
     */
    protected File tempDir;

    /**
     * Constructs a KMZInputStream instance. Upon return, the new instance's
     * current-entry field refers to the first entry in the stream.
     *
    ...
    /**
     * Moves this stream to its next zip entry and updates this stream's
     * current-entry field.
     *
     * @throws IOException if an exception occurs while attempting to more the
     * stream to the next entry.
     */
    ...
    /**
     * Returns an {@link java.io.InputStream} to the first KML file within the
     * stream.
     *
     * @return an input stream positioned to the first KML file in the stream,
     * or null if the stream does not contain a KML file.
     *
    ...
    /**
     * Returns an {@link InputStream} to a specified file within the KMZ stream.
     * The file's path is resolved relative to the internal root of the KMZ file
     * represented by the stream.
     * <p>
     * Note: Since relative references to files outside the stream have no
     * meaning, this class does not resolve relative references to files in
     * other KMZ archives. For example, it does not resolve references like
     * this:
     * <i>../other.kmz/file.png</i>.
     *
     * @param path the path of the requested file.
     *
     * @return an input stream positioned to the start of the requested file, or
     * null if the file does not exist or the specified path is absolute.
     *
     * @throws IllegalArgumentException if the path is null.
     * @throws IOException if an error occurs while attempting to read the input
     * stream.
     */
    ...
    /**
     * Returns an absolute path to a specified file within the KMZ stream. The
     * file's path is resolved relative to the internal root of the KMZ file
     * represented by the stream.
     * <p>
     * Note: Since relative references to files outside the stream have no
     * meaning, this class does not resolve relative references to files in
     ...
     * @param path the path of the requested file.
     *
     * @return an absolute path for the requested file, or null if the file does
     * not exist or the specified path is absolute.
     *
     * @throws IllegalArgumentException if the path is null.
     * @throws IOException if an error occurs while attempting to create a
     * temporary file.
     */
    ...
    /**
     * Copies a file from the input stream to the temporary area created to
     * represent the KMZ contents. If that area does not yet exists, it is
     * created.
     * <p>
     * Note: This method should be called only by another synchronized method of
     * this instance.
     *
