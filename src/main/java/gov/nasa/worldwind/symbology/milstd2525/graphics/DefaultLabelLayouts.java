...
/**
 * Object to provide default label layouts for MIL-STD-2525C tactical point
 * graphics. The layout is used to arrange text modifiers around the icon.
 *
 * @author pabercrombie
...
public class DefaultLabelLayouts {
     ...
     *
     * @return Map that represents the label layout. The keys indicate the
     * modifier key (unique designation, additional info, etc.). The values are
     * lists of LabelLayout. Most modifiers will only specify a single layout,
     * but some graphics support multiple instances of the same modifier, in
     * which case the list will contain multiple layouts.
     */
     ...
     * @param hotspot Offset within the label to align with the label point in
     * the image.
     *
     * @return New map, populated with one entry for the key/value pair
     ...
     * @param key Modifier key.
     * @param offsets List of offsets from which to create one or more
     * LabelLayout objects. The offsets are specified in pairs: first the image
     * offset and then the label offset. If multiple pairs are provided, then
