...
/**
 * Retriever to fetch icons for MIL-STD-2525C point graphics. The retriever can
 * fetch images from either local or remote locations. See
 * <a href="https://worldwind.arc.nasa.gov/java/tutorials/tactical-symbols/#offline-use">Offline
 * Use</a> for information on how to set the icon retrieval location.
 * <p>
 * The retriever base URL must identify a location on a local or remote file
 * system (including zip and jar files) that holds the icon files in an expected
 * directory structure. Each icon URL is constructed from three parts:
 * [base]/icons/[scheme]/[sidc].png. Parts of the SIDC that do not identify a
 * type of graphic (such as echelon, status, standard identity, order of battle,
 * etc.) are replaced with hyphens. For example, the Underwater Datum graphic
 * (2.X.2.1.1.1.1.1) will be retrieved from this URL:
 * [base]/icons/tacgrp/g-g-gpuud------.png
 * <p>
 * An application should only use this class directly if it needs to access
 * point graphics independent of the {@link
 * TacticalGraphic} system (for example, to populate a UI independent of the
 * globe).
 *
 * @author pabercrombie
 * @version $Id: MilStd2525PointGraphicRetriever.java 526 2012-04-13 21:07:09Z
 * pabercrombie $
 */
public class MilStd2525PointGraphicRetriever extends AbstractIconRetriever {
    ...
    /**
     * Create an icon for a MIL-STD-2525C point graphic. Point graphics are
     * defined in Appendixes B (Tactical Graphics), C (Meteorological and
     * Oceanographic), and G (Emergency Management).
     *
     * @param sidc SIDC identifier for the symbol.
     * @param params Parameters that affect icon retrieval. This retriever
     * accepts only one parameter: AVKey.COLOR, which determines the color of
     * the image. By default the color will be determined from the standard
     * identity.
     *
     * @return An BufferedImage containing the icon for the requested graphic,
     * or null if the icon cannot be retrieved.
     */
     ...
     *
     * @return A new image with the {@code srcImg} drawn over the appropriate
     * fill.
     */
     ...
     *
     * @return True if the graphic has a fill image. False if not. Only three
     * graphics in MIL-STD-2525C Appendix B use a fill pattern: Nuclear
     * Detonation Ground Zero (2.X.3.4.2), Biological Release Event
     * (2.X.3.4.7.1), and Chemical Release Event (2.X.3.4.7.2).
     */
    ...
    /**
     * Indicates the color to apply to a graphic based on the graphic's standard
     * identity.
     *
     * @param code Symbol code that identifies the graphic.
     *
     * @return Color to apply based on the standard identity. (Red for hostile
     * entities, black for friendly, etc.)
     */
     ...
     *
     * @return The file name of the image file that corresponds to the specified
     * graphic, or null if the graphic's scheme is not recognized.
     */
    ...
    /**
     * Indicates the filename of a graphic in the Tactical Graphics scheme
     * (MIL-STD-2525C Appendix B).
     *
     * @param code Code that identifies a graphic in the Tactical Graphics
     * scheme.
    ...
    /**
     * Indicates the filename of a graphic in the Meteorological and
     * Oceanographic scheme (MIL-STD-2525C Appendix C).
     *
    ...
    /**
     * Indicates the filename of a graphic in the Emergency Management scheme
     * (MIL-STD-2525C Appendix G).
     *
