...
/**
 * Base class for axis of advance arrow graphics. These arrows are specified by
 * control points along the center line of the arrow. The final control point
 * determines either the width of the arrowhead (default) or the width of the
 * route. MIL-STD-2525C (pg. 517) specifies that the final control point
 * determines the width of the arrowhead, but some applications may prefer to
 * use this control point to set the width of the route precisely. The {@link
 * #isFinalPointWidthOfRoute() finalPointWidthOfRoute} field controls this
 * behavior.
 *
 * @author pabercrombie
...
public abstract class AbstractAxisArrow extends AbstractMilStd2525TacticalGraphic {
    ...
    /**
     * Indicates whether the final control point marks the width of the route or
     * the width of the arrowhead. Default is the width of the arrowhead.
     */
    ...
    /**
     * Create a new arrow graphic made up of more than one path. This
     * constructor allocates the {@link #paths}, and creates the requested
     * number of paths. The first element in the array is the main path that
     * outlines the arrow. Subclasses are responsible for configuring the other
     * paths.
     *
     * @param sidc Symbol code the identifies the graphic.
    ...
    /**
     * Indicates whether or not the final control point marks the width of the
     * arrowhead or the width of the route. MIL-STD-2525C specifies that the
     * final control point marks the edge of the arrow (pg. 517), and this is
     * the default. However, some applications may prefer to specify the width
     * of the route rather than the width of the arrowhead. In the diagram
     * below, the default behavior is for the final control point to specify
     * point A. When {@code finalPointWidthOfRoute} is true the final control
     * point specifies point B instead.
     * <pre>
     *                 A
     *                 |\
     *                 | \
     ...
     * </pre>
     *
     * @return True if the final control point determines the width of the route
     * instead of the width of the arrow head (point B in the diagram above).
     */
    ...
    /**
     * Specifies whether or not the final control point marks the edge of the
     * arrow head or the width of the route. See
     * {@link #isFinalPointWidthOfRoute()} for details.
     *
     * @param finalPointIsWidthOfRoute True if the final control point
     * determines the width of the route instead of the width of the arrow head.
     */
     ...
     * <p>
     * The arrow head is defined by the first two control points, and the last
     * point. Pt. 1' is the point on the center line at the base of the arrow
     * head, and Pt. N' is the reflection of Pt. N about the center line.
     * <pre>
     *                 Pt N
     *                 |\
     * Left line       | \
     ...
     * </pre>
     *
     * @param leftPositions List to collect positions on the left arrow line.
     * This list receives the position where the left line meets the arrow head.
     * @param rightPositions List to collect positions on the right arrow line.
     * This list receives the position where the right line meets the arrow
     * head.
     * @param arrowHeadPositions List to collect positions that make up the
     * arrow head. This list receives positions for Pt. N, Pt. 1, and Pt. N', in
     * that order.
     ...
     * @param halfWidth Distance from the center line to the left or right
     * lines. Half the width of the arrow's double lines.
     * @param globe Current globe.
     */
