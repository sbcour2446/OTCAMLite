...
public class TacticalGraphicUtil {
    ...
    /**
     * Get the date range from a graphic's modifiers. This method looks at the
     * value of the <code>AVKey.DATE_TIME</code> modifier, and returns the
     * results as a two element array. If the value of the modifier is an
     * <code>Iterable</code>, then this method returns the first two values of
     * the iteration. If the value of the modifier is a single object, this
     * method returns an array containing that object and <code>null</code>.
     *
     * @param graphic Graphic from which to retrieve dates.
     *
     * @return A two element array containing the altitude modifiers. One or
     * both elements may be null.
     */
    ...
    /**
     * Get the altitude range from the graphic's modifiers. This method looks at
     * the value of the <code>AVKey.ALTITUDE</code> modifier, and returns the
     * results as a two element array. If the value of the modifier is an
     * <code>Iterable</code>, then this method returns the first two values of
     * the iteration. If the value of the modifier is a single object, this
     * method returns an array containing that object and <code>null</code>.
    ...
    /**
     * Position one or two labels some distance along the path. Top and bottom
     * labels are often positioned above and below the same point, so this
     * method supports positioning a pair of labels at the same point. The label
     * offsets determine how the labels draw in relation to the line.
     *
     ...
     * The first and last points should mark the start and end of the curve.
     * <p>
     * This function implements the Bezier curve equation from "Mathematics for
     * 3D Game Programming and Computer Graphics, Second Edition" by Eric
     * Lengyel (equation 15.16, pg. 458).
     * <p>
     ...
     * @param coefficients Array to store binomial coefficients between
     * invocations of this function. On the first invocation, pass an int[] with
     * length equal to the controlPoints array. bezierCurve will populate the
     * array on the first invocation, and reuse the computed values on
    ...
    /**
     * Compute binomial coefficients for a polynomial of order n. Stated another
     * way, computes the nth row of Pascal's triangle.
     *
     * @param n Order of polynomial for which to calculate coefficients.
     * @param coefficients Array to receive coefficients. The length of this
     * array must be n + 1.
     */
