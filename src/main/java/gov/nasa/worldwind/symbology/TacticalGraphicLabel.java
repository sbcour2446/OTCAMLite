...
/**
 * A label drawn as part of a tactical graphic. The label is drawn at constant
 * screen size. The label can include multiple lines of text, and can optionally
 * be kept aligned with features on the globe. To align a label with the globe
 * specify an
 * {@link #setOrientationPosition(gov.nasa.worldwind.geom.Position) orientationPosition}
 * for the label. The label will be drawn along a line connecting the label's
...
public class TacticalGraphicLabel {
    ...
    protected class OrderedLabel implements OrderedRenderable {
        ...
        /**
         * Location of the upper left corner of the text measured from the lower
         * left corner of the viewport. This point in OGL coordinates.
         */
        protected Point screenPoint;
        /**
         * Rotation applied to the label. This is computed each frame based on
    ...
    /**
     * Default offset. The default offset aligns the label horizontal with the
     * text alignment position, and centers the label vertically. For example,
     * if the text alignment is <code>AVKey.LEFT</code>, then the left edge of
     * the text will be aligned with the geographic position, and the label will
    ...
    /**
     * The label is drawn along a line from the label position to the
    ...
    /**
     * Effect applied to the text. May be {@link AVKey#TEXT_EFFECT_SHADOW} or
     * {@link AVKey#TEXT_EFFECT_NONE}.
     */
    protected String effect = DEFAULT_TEXT_EFFECT;
    /**
     * Insets that separate the text from its frame. Only applies when the text
     * interior is rendered.
     */
     ...
     * {@link #computeBoundsIfNeeded(gov.nasa.worldwind.render.DrawContext)}.
     */
    ...
    /**
     * Specifies the text of this label. The text may include multiple lines,
    ...
    /**
     * Indicates the label's position. The label is drawn at an offset from this
     * position.
     *
    ...
    /**
     * Indicates the label's geographic position. The label is drawn at an
    ...
    /**
     * Indicates the current text alignment. Can be one of {@link AVKey#LEFT}
     * (default), {@link AVKey#CENTER} or {@link
     * AVKey#RIGHT}.
     *
    ...
    /**
     * Specifies the text alignment. Can be one of {@link AVKey#LEFT} (default),
    ...
    /**
     * Indicates the offset from the geographic position at which to draw the
     * label. See {@link
     * #setOffset(gov.nasa.worldwind.render.Offset) setOffset} for more
     * information on how the offset is interpreted.
     *
    ...
    /**
     * Specifies the offset from the geographic position at which to draw the
     * label. The default offset aligns the label horizontal with the text
     * alignment position, and centers the label vertically. For example, if the
     * text alignment is <code>AVKey.LEFT</code>., then the left edge of the
     * text will be aligned with the geographic position, and the label will be
     * centered vertically.
     * <p>
     * When the text is rotated a horizontal offset moves the text along the
     * orientation line, and a vertical offset moves the text perpendicular to
     ...
     *
     * @return <code>true</code> if the label's interior is drawn, otherwise
     * <code>false</code>.
     *
     ...
     *
     * @param drawInterior <code>true</code> if the label's interior is drawn,
    ...
    /**
     * Indicates the opacity of the text as a floating-point value in the range
     * 0.0 to 1.0. A value of 1.0 specifies a completely opaque text, and 0.0
     * specifies a completely transparent text. Values in between specify a
     * partially transparent text.
     *
     * @return the opacity of the text as a floating-point value from 0.0 to
     * 1.0.
     */
    ...
    /**
     * Specifies the opacity of the text as a floating-point value in the range
     * 0.0 to 1.0. A value of 1.0 specifies a completely opaque text, and 0.0
     * specifies a completely transparent text. Values in between specify a
     * partially transparent text.
     *
     * @param opacity the opacity of text as a floating-point value from 0.0 to
     * 1.0.
     *
    ...
    /**
     * Indicates the opacity of label's interior as a floating-point value in
     * the range 0.0 to 1.0. A value of 1.0 specifies a completely opaque
     * interior, and 0.0 specifies a completely transparent interior. Values in
     * between specify a partially transparent interior.
     *
     * @return the opacity of the interior as a floating-point value from 0.0 to
     * 1.0.
     */
    ...
    /**
     * Specifies the opacity of the label's interior as a floating-point value
     * in the range 0.0 to 1.0. A value of 1.0 specifies a completely opaque
     * interior, and 0.0 specifies a completely transparent interior. Values in
     * between specify a partially transparent interior.
     *
     * @param interiorOpacity the opacity of label's interior as a
    ...
    /**
     * Indicates the orientation position. The label oriented on a line drawn
     * from the label's position to the orientation position.
     *
    ...
    /**
     * Specifies the orientation position. The label is oriented on a line drawn
     * from the label's position to the orientation position. If the orientation
     * position is null then the label is drawn with no rotation.
     *
    ...
    /**
     * Indicates the amount of space between the label's content and its frame,
     * in pixels.
     *
    ...
    /**
     * Specifies the amount of space (in pixels) between the label's content and
     * the edges of the label's frame.
     *
     * @param insets the desired padding between the label's content and its
     * frame, in pixels.
     *
    ...
    /**
     * Indicates an effect used to decorate the text. Can be one of
     * {@link AVKey#TEXT_EFFECT_SHADOW} (default), or
     * {@link AVKey#TEXT_EFFECT_NONE}.
     *
    ...
    /**
     * Specifies an effect used to decorate the text. Can be one of
     * {@link AVKey#TEXT_EFFECT_SHADOW} (default), or
     * {@link AVKey#TEXT_EFFECT_NONE}.
     *
    ...
    /**
     * Returns the delegate owner of this label. If non-null, the returned
     * object replaces the label as the pickable object returned during picking.
     * If null, the label itself is the pickable object returned during picking.
     *
     * @return the object used as the pickable object returned during picking,
     * or null to indicate the the label is returned during picking.
     */
    ...
    /**
     * Specifies the delegate owner of this label. If non-null, the delegate
     * owner replaces the label as the pickable object returned during picking.
     * If null, the label itself is the pickable object returned during picking.
     *
     * @param owner the object to use as the pickable object returned during
     * picking, or null to return the label.
     */
    ...
    /**
     * Specifies whether adjacent Labels in the ordered renderable list may be
     * pick-tested together if they are contained in the same layer. This
     * increases performance but allows only the top-most of the label to be
     * reported in a {@link gov.nasa.worldwind.event.SelectEvent} even if
     * several of the labels are at the pick position.
     * <p>
    ...
    /**
     * Specifies whether adjacent Labels in the ordered renderable list may be
     * rendered together if they are contained in the same layer. This increases
     * performance and there is seldom a reason to disable it.
     *
    ...
    /**
     * Get the label bounding {@link java.awt.Rectangle} using OGL coordinates -
     * bottom-left corner x and y relative to the
     * {@link gov.nasa.worldwind.WorldWindow} bottom-left corner. If the label
     * is rotated then the returned rectangle is the bounding rectangle of the
     * rotated label.
     *
     * @param dc the current DrawContext.
     *
     * @return the label bounding {@link java.awt.Rectangle} using OGL viewport
     * coordinates.
     *
     ...
     *
     * @return True if this label intersects the active frustum (view or pick).
     * Otherwise false.
     */
    ...
    /**
     * Compute the amount of rotation to apply to a label in order to keep it
     * oriented toward its orientation position.
     *
     * @param screenPoint Geographic position of the text, projected onto the
     * screen.
     * @param orientationScreenPoint Orientation position, projected onto the
     * screen.
     *
    ...
    /**
     * Causes this <code>Renderable</code> to render itself using the provided
     * draw context.
     *
    ...
    /**
     * Draw the label's text. This method sets up the text renderer, and then
     * calls {@link #doDrawText(TextRenderer,
     * gov.nasa.worldwind.symbology.TacticalGraphicLabel.OrderedLabel) doDrawText}
    ...
    /**
     * Draw the label's text. This method assumes that the text renderer context
    ...
    /**
     * Draws this ordered renderable and all subsequent Label ordered
     * renderables in the ordered renderable list. This method differs from {@link #drawBatchedText(gov.nasa.worldwind.render.DrawContext, TextRenderer,
     * gov.nasa.worldwind.symbology.TacticalGraphicLabel.OrderedLabel) drawBatchedText}
     * in that this method re-initializes the text renderer to draw the next
     * label, while {@code drawBatchedText} re-uses the active text renderer
     * context. That is, {@code drawBatchedText} attempts to draw as many labels
     * as possible that share same text renderer configuration as this label,
     * and this method attempts to draw as many labels as possible regardless of
     * the text renderer configuration of the subsequent labels.
     *
     * @param dc the current draw context.
    ...
    /**
     * Draws text for subsequent Label ordered renderables in the ordered
     * renderable list. This method is called after the text renderer has been
     * set up (after beginRendering has been called), so this method can only
     * draw text for subsequent labels that use the same font and rotation as
     * this label. This method differs from {@link
     * #drawBatched(gov.nasa.worldwind.render.DrawContext, gov.nasa.worldwind.symbology.TacticalGraphicLabel.OrderedLabel)
     * drawBatched} in that this method reuses the active text renderer context
     * to draw as many labels as possible without switching text renderer state.
     *
     * @param dc the current draw context.
     ...
     *
     * @return If a delegate owner is set, returns the delegate owner. Otherwise
     * returns this label.
     */
    ...
    /**
     * Determine the screen rectangle covered by a label. The input coordinate
     * identifies either the top left, top center, or top right corner of the
     * label, depending on the text alignment. If the label is rotated to align
     * with features on the surface then the extent will be the smallest screen
     * rectangle that completely encloses the rotated label.
     *
     ...
     *
     * @return The rectangle, in OGL screen coordinates (origin at bottom left
     * corner), that is covered by the label.
     */
     ...
     *
     * @return The smallest rectangle that completely contains {@code rect} when
