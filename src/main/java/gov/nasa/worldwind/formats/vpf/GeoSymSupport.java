...
public class GeoSymSupport {
    ...
    /**
     * From MIL-HDBK-857A, section 6.4.1.6.1: Default Symbology Although all
     * efforts have been made to symbolize all features according to the
     * appropriate symbology specifications, should the application software be
     * unable to determine at least one "true" condition for a particular
     * instance of a feature in the VPF dataset, there are default CGMs provided
     * in GeoSym for displaying that feature. The following table identifies
     * which symbol should be used to symbolize an "unknown" point, line, or
     * area feature.
     * <p>
     * Note that the default symbols should only be placed under two conditions:
     * 1. There is no row in the *sym.txt file for the feature (fcode) 2. After
     * evaluating all rows in the *sym.txt file for that fcode, there is no row
     * that results in an evaluation of "true".
     *
     * @return a single element list containing a reference to the unknown
     * symbol key {@link
