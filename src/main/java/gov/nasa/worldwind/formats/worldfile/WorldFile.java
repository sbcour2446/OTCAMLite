...
/**
 * Static methods to handle world files typically distributed with georeferenced
 * rasters.
 *
 * @author tag
...
public class WorldFile {
    ...
    /**
     * Retrieve the metadata files corresponding to a specified image or
     * elevation file.
     *
     * @param dataFile the image or elevation file for which to retrieve the
     * metadata files.
     *
     * @return the metadata files that exist in the same directory as the data
     * file, otherwise null.
     *
     ...
     *
     * @return an attribute-value list containing the values from the world
     * files.
     *
     * @throws IllegalArgumentException if the file is null.
     * @throws FileNotFoundException if the file does not exist.
     * @throws IllegalStateException if the file cannot be parsed as a world
     * file.
     */
     ...
     *
     * @return an array containing the six values in the order they are found in
     * the world file.
     *
     * @throws IllegalArgumentException if the file is null.
     * @throws FileNotFoundException if the file does not exist.
    ...
    /**
     * Indicates whether world file values appear to be in latitude and
     * longitude.
     *
     * @param values the six values to examine.
     *
     * @return true if the values are between the normal limits of latitude,
    ...
    /**
     * Decodes the six values of a world file in the lat/lon coordinate system
     * to a Sector. The rotation values are ignored.
     *
     * @param values the values to parse
     * @param imageWidth the width of the image associated with the world file.
     * @param imageHeight the height of the image associated with the world
     * file.
     *
     * @return a sector computed from the world file values.
     *
     * @throws IllegalArgumentException if the values array is null or has a
     * length less than six, or the image width and height are less than zero.
     * @throws IllegalStateException if the decoded values are not within the
     * normal range of latituded and longitude.
     ...
     * @param imageHeight the height of the image associated with the world
     * file.
     * @param zone the UTM zone number (1 to 60), can be zero if expected units
     ...
     *
     * @return the corresponding <code>Sector</code> or <code>null</code> if the
     * sector could not be computed.
     *
     * @throws IllegalArgumentException if the values array is null or has a
     * length less than six, , the image width and height are less than zero, or
     * the hemisphere indicator is not {@link
    ...
    /**
     * Pattern matching the geographic coordinate system keyword in an OGC
     * coordinate system well-known text.
     */
    protected static final Pattern GEOGCS_WKT_PATTERN = Pattern.compile("\\{*GEOGCS[\\[\\(](.*)[\\]\\)]\\}*");
    /**
     * Pattern matching the projected coordinate system keyword in an OGC
     * coordinate system well-known text.
     */
    protected static final Pattern PROJCS_WKT_PATTERN = Pattern.compile("\\{*PROJCS[\\[\\(](.*)[\\]\\)]\\}*");
    /**
     * Pattern matching the UTM name in an projected coordinate system's
     * well-known text.
     */
    protected static final Pattern UTM_NAME_WKT_PATTERN = Pattern.compile(".*UTM.*ZONE.*?(\\d+).*?([\\w\\s]+).*?");

    /**
     * Retrieves the coordinate system and its parameters from an OGC coordinate
     * system encoded as well-known text. For details, see to the OGC Coordinate
     * Transform Service (CT) specification at <a
     * href="http://www.opengeospatial.org/standards/ct">http://www.opengeospatial.org/standards/ct</a>.
     * This recognizes Geographic and UTM coordinate systems. This configures
     * the parameter list according to the coordinate system as follows: <ul>
     * <li>Geographic -
     * {@link gov.nasa.worldwind.avlist.AVKey#COORDINATE_SYSTEM} set to {@link
     * gov.nasa.worldwind.avlist.AVKey#COORDINATE_SYSTEM_GEOGRAPHIC}.</li>
     * <li>Projected coordinate system: Universal Transverse Mercator (UTM) -
     * {@link gov.nasa.worldwind.avlist.AVKey#COORDINATE_SYSTEM} set to {@link
     ...
     * {@link gov.nasa.worldwind.avlist.AVKey#PROJECTION_HEMISPHERE} set to
     * either {@link gov.nasa.worldwind.avlist.AVKey#NORTH} or
     * {@link gov.nasa.worldwind.avlist.AVKey#SOUTH}.
     * {@link gov.nasa.worldwind.avlist.AVKey#PROJECTION_ZONE} set to an integer
     * in the range 1-60</li> <li>Projected coordinate system: unknown
     * projection - {@link gov.nasa.worldwind.avlist.AVKey#COORDINATE_SYSTEM}
     * set to {@link
     * gov.nasa.worldwind.avlist.AVKey#COORDINATE_SYSTEM_PROJECTED} and
     * {@link gov.nasa.worldwind.avlist.AVKey#PROJECTION_NAME} set to
     * {@link gov.nasa.worldwind.avlist.AVKey#PROJECTION_UNKNOWN}.</li>
     * <li>Unknown coordinate system - {@link
     * gov.nasa.worldwind.avlist.AVKey#COORDINATE_SYSTEM} set to
     * {@link gov.nasa.worldwind.avlist.AVKey#COORDINATE_SYSTEM_UNKNOWN}.
     * </ul> If an exception occurs while parsing the coordinate system text,
     * the parameter list is left unchanged.
     *
     * @param text a String containing an OGC coordinate system in well-known
     * text format.
     * @param params the coordinate system parameter list, or null to indicate a
     * parameter list should be created.
     *
     ...
     * <ul><li>{@link java.net.URL}</li> <li>{@link java.net.URI}</li>
     * <li>{@link java.io.File}</li> <li>{@link String} containing a valid URL
     * description, a valid URI description, or a valid path to a local
     * file.</li> </ul>
     *
     * @param source the source to local file path.
     *
     * @return TRUE if there is a world file for the source, otherwise returns
     * FALSE
     */
