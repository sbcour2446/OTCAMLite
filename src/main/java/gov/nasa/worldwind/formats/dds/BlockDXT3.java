...
/**
 * <code>BlockDXT3</code> is a data structure representing the compressed alpha
 * and color values in a single DXT2/DXT3 block. The DXT3 block contains a 64
 * bit alpha block, and a 64 bit color block, stored here as the properties
 * <code>alphaBlock</code> and <code>colorBlock</code>. The 64 bit alpha block
 * contains 4x4 alpha values quantized into 4 bits. The 64 bit color block is
 * formatted exactly like the DXT1 color block, except that the color block
 * always represents four colors, regardless of the color ordering in the DXT1
 * block.
 *
 * @author dcollins
...
public class BlockDXT3 {
    ...
    /**
     * Creates a new DXT2/DXT3 alpha block with all alpha and color values set
     * to 0.
     */
    public BlockDXT3() {
