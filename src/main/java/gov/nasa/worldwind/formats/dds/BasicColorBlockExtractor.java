...
/**
 * Provides access to 4x4 blocks of pixel data from a <code>BufferedImage</code>
 * via the <code>ColorBlockExtractor</code> interface. This class is not thread
 * safe. Unsynchronized access will result in unpredictable behavior. Acces to
 * methods of this class must be synchronized by the caller.
 *
 * @see java.awt.image.BufferedImage
 *
 * @author dcollins
 * @version $Id: BasicColorBlockExtractor.java 1171 2013-02-11 21:45:02Z
 * dcollins $
 */
public class BasicColorBlockExtractor implements ColorBlockExtractor {
    ...
    /**
     * Creates a <code>BasicColorBlockExtrator</code> which will draw its data
     * from the <code>BufferedImage</code>. The <code>BufferedImage</code> may
     * be of any type, so long as a call to <code>image.getRGB()</code> will
     * succeed.
     *
    ...
    /**
     * Returns the image this <code>BasicColorBlockExtrator</code> will draw its
     * data from.
     *
    ...
    /**
     * Extracts a 4x4 block of pixel data at the specified coordinate
     * <code>(x, y)</code>, and places the data in the specified
     * <code>colorBlock</code>. If the coordinate <code>(x, y)</code> with the
     * image, but the entire 4x4 block is not, this will either truncate the
     * block to fit the image, or copy nearby pixels to fill the block. If the
     * <code>attributes</code> specify that color components should be
     * premultiplied by alpha, this extactor will perform the premultiplication
     * operation on the incoming colors.
     * <p>
     * Access to this method must be synchronized by the caller. This method is
     * frequenty invoked by the DXT compressor, so in order to reduce garbage
     * each instance of this class has unsynchronized properties that are reused
     * during each call.
     *
     * @param attributes the DXT compression attributes which may affect how
     * colors are accessed.
     ...
     *
     * @throws IllegalArgumentException if either <code>attributes</code> or
     * <code>colorBlock</code> is null.
     */
