...
/**
 * Compressor for DXT1 color blocks. This class is not thread safe.
 * Unsynchronized access will result in unpredictable behavior. Access to
 * methods of this class must be synchronized by the caller.
 * <p>
 * Documentation on the DXT1 format is available at
 * http://msdn.microsoft.com/en-us/library/bb694531.aspx under the name "BC1".
 *
 * @author dcollins
...
public class BlockDXT1Compressor {
    ...
    /**
     * Compress the 4x4 color block into a DXT1 block using four colors. This
     * method ignores transparency and guarantees that the DXT1 block will use
     * four colors.
     * <p>
     * Access to this method must be synchronized by the caller. This method is
     * frequently invoked by the DXT compressor, so in order to reduce garbage
     ...
     *
     * @throws IllegalArgumentException if either <code>colorBlock</code> or
     * <code>dxtBlock</code> are null.
     */
    ...
    /**
     * Compress the 4x4 color block into a DXT1 block with three colors. This
     * method will consider a color transparent if its alpha value is less than
     * <code>alphaThreshold</code>.
     * <p>
     * This method is frequently invoked by the DXT compressor. In order to
     * reduce garbage each instance of this class has buffer that is used during
     * each call. Access to this method must be synchronized by the caller.
     *
     * @param colorBlock the 4x4 color block to compress.
     * @param attributes attributes that will control the compression.
     * @param dxtBlock the DXT1 block that will receive the compressed data.
