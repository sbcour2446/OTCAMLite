...
/**
 * A class to bundle the GeoTiff structures found in a GeoTiff file, and to
 * assist in the coding/decoding of those structures.
 *
 * @author brownrigg
...
class GeoCodec {
    ...
    /**
     * Adds ModelTiePoints from an array. Recall that by definition, a tie point
     * is a 6-tuple of <i,j,k><x,y,z> values.
     *
    ...
    /**
     * Sets the ModelTransformation matrix. This is logically a 4x4 matrix of
     * doubles by definition.
     *
    ...
    /**
     * Returns the bounding box of an image that is width X height pixels, as
     * determined by this GeoCodec. Returns UnsupportedOperationException if the
     * transformation can not be determined (see getXYZAtPixel()). The bounding
     * Box is returned as an array of double of length 4: [0] is x coordinate of
     * upper-left corner [1] is y coordinate of upper-left corner [2] is x
     * coordinate of lower-right corner [3] is y coordinate of lower-right
     * corner Note that coordinate units are those of the underlying modeling
     * transformation, and are not guaranteed to be in lon/lat.
     *
     ...
     * @throws UnsupportedOperationException if georeferencing can not be
     * computed.
     */
    ...
    /**
     * Returns the geocoordinates for a given pixel, as determined by the
     * modeling coordinate tranformation embodied in the GeoCodec.
     * <p>
     * TODO: Also throws UnsupportedOperationException if this is anything other
     * than a "simple" georeferenced mapping, meaning that there's a single
     * tie-point known about the point 0,0, we know the inter-pixel spacing, and
     * there's no rotation of the image required. Geo referencing may also be
     * specified via a general 4x4 matrix, or by a list if tie-points, implying
     * a rubbersheeting transformation. These two cases remain to be
     * implemented.
     * <p>
     *
     * @param row pixel-row index
     * @param col pixel-column index
     * @return double[2] containing x,y coordinate of pixel in modelling
     * coordinate units.
     ...
     * <p>
     * While this method handles the general case of multiple ints associated
     * with a key, typically there will be only a single value.
     *
     * @param key GeoKey value
     * @return Array of int values associated with the key, or null if the key
     * was not found.
     * @throws IllegalArgumentException Thrown if the key does not embody
     * integer values.
     */
