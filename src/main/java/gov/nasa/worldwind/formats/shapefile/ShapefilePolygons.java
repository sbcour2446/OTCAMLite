...
public class ShapefilePolygons extends ShapefileRenderable implements OrderedRenderable, PreRenderable, Combinable {
    ...
    /**
     * Creates a new ShapefilePolygons with the specified shapefile. The normal
     * attributes and the highlight attributes for each
     * ShapefileRenderable.Record are assigned default values. In order to
     * modify ShapefileRenderable.Record shape attributes or key-value
     * attributes during construction, use {@link #ShapefilePolygons(gov.nasa.worldwind.formats.shapefile.Shapefile,
     ...
     * during creation of each ShapefileRenderable.Record. See {@link
     * gov.nasa.worldwind.formats.shapefile.ShapefileRenderable.AttributeDelegate}
    ...
    /**
     * Indicates the object's detail hint, which is described in
    ...
    /**
     * Modifies the default relationship of shape resolution to screen
     * resolution as the viewing altitude changes. Values greater than 0 cause
     * shape detail to appear at higher resolution at greater altitudes than
     * normal, but at an increased performance cost. Values less than 0 decrease
     * the default resolution at any given altitude. The default value is 0.
     * Values typically range between -0.5 and 0.5.
     * <p>
     * Note: The resolution-to-height relationship is defined by a scale factor
     * that specifies the approximate size of discernible lengths in the shape
     * relative to eye distance. The scale is specified as a power of 10. A
     * value of 3, for example, specifies that a length of 1 meter on the shape
     * should be distinguishable from an altitude of 10^3 meters (1000 meters).
     * The default scale is 1/10^2.8, (1 over 10 raised to the power 2.8). The
     ...
     * performance.
     *
     * @param detailHint the degree to modify the default relationship of shape
