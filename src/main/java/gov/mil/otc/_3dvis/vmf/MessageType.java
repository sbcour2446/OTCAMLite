...
public enum MessageType {
     ...
     * Constructor
     *
     * @param vmfId the VMF ID
     * @param name the VMF Message Printable Name
     */
    MessageType(String vmfId, String name) {
        this.vmfId = vmfId;
        this.name = name;
    }

    /**
     * Create a VMF Message Type
     *
     * @param umf UMF {link16,binary,vmf,nitfs,rdm,usmtf,doi103,xml-mtf,xml-vmf} Indicates the format of the
     *            message contained in the user data field.
     * @param fad FAD {netcon,infoex,firesupport,airops,int,land,navy,css,special,jtfops,airdef} Identifies the
     *            functional area of a specific VMF message using code words.
     * @param number Message Number Represents the number that identifies a specific VMF message within a
     *               functional area.
     * @return The Message Type
     */
    public static MessageType create(int umf, int fad, int number) {
        switch (umf) {
            case 1:
                return FILE;
            case 2:
                try {
                    String id = String.format("K%02d_%02d", fad, number);
                    return valueOf(id);
                } catch (Exception e) {
                    return OTHER;
                }
